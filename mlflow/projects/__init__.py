...
def run(
    ...
    Args:
        uri: URI of project to run. A local filesystem path or a Git repository URI
        ...
        parameters: Parameters for the entry point command.
        docker_args: Arguments for the docker command.
        experiment_name: Name of experiment under which to launch the run.
        experiment_id: ID of experiment under which to launch the run.
        backend: Execution backend for the run. If running against Databricks, will run against a
            Databricks workspace determined as follows: if a Databricks tracking URI of the form
            ``databricks://profile`` has been set, will run against the workspace specified by
            <profile>. Otherwise, runs against the workspace specified by the default Databricks
            CLI profile.
        backend_config: A dictionary, or a path to a JSON file, which will be passed as config to
            the backend.
        ...
        run_id: Note: this argument is used internally by the MLflow project APIs and should
            not be specified. If specified, the run ID will be used instead of creating a new run.
        ...
        env_manager: Specify an environment manager to create a new environment for the run and
            ...
            If unspecified, MLflow automatically determines the environment manager to use by
            inspecting files in the project directory.
