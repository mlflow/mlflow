...
def _collect_run_params(experiment_name, experiment_id, uri, version, git_username, git_password, entry_point, run_id, mode, parameters):
    if experiment_name:
        exp_id = tracking.MlflowClient().get_experiment_by_name(experiment_name).experiment_id
    else:
    ...
    return exp_id, active_run, project, work_dir


def _run(uri, exp_id, active_run, work_dir, project, entry_point="main", parameters=None, mode=None, cluster_spec=None, use_conda=True, storage_dir=None, block=True):
    """
...
def run(uri, entry_point="main", version=None, parameters=None,
    ...
    exp_id, active_run, project, work_dir = _collect_run_params(experiment_name, experiment_id, uri, version, git_username, git_password, entry_point, run_id, mode, parameters)

    submitted_run_obj = _run(
        uri=uri, exp_id=exp_id, active_run=active_run, work_dir=work_dir, project=project, entry_point=entry_point, parameters=parameters,
        mode=mode, cluster_spec=cluster_spec_dict, use_conda=use_conda,
        storage_dir=storage_dir, block=block)
    if block:
