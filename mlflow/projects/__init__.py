...
from mlflow.projects.utils import (
    _is_valid_branch_name, _get_git_repo_url, _expand_uri, _get_storage_dir,
    prepare_project_and_run
)
...
from mlflow.utils.mlflow_tags import (
    MLFLOW_PROJECT_ENV, MLFLOW_DOCKER_IMAGE_URI, MLFLOW_DOCKER_IMAGE_ID,
    MLFLOW_GIT_REPO_URL, MLFLOW_GIT_BRANCH, LEGACY_MLFLOW_GIT_REPO_URL,
    LEGACY_MLFLOW_GIT_BRANCH_NAME, MLFLOW_PROJECT_BACKEND,
)
...
def _run(uri, experiment_id, entry_point="main", version=None, parameters=None,
    ...
    if backend:
        submitted_run = backend.run(run_id=run_id, experiment_id=experiment_id, project_uri=uri,
                                    entry_point=entry_point, params=parameters, version=version,
                                    backend_config=backend_config)
        tracking.MlflowClient().set_tag(submitted_run.run_id,
                                        MLFLOW_PROJECT_BACKEND, backend.name)
        return submitted_run

    fetched_project = prepare_project_and_run(
        uri, experiment_id, run_id, entry_point, parameters, version)
    if backend_name == "local" or backend_name is None:
        tracking.MlflowClient().set_tag(fetched_project.run.info.run_id,
                                        MLFLOW_PROJECT_BACKEND, "local")
        command_args = []
        command_separator = " "
        # If a docker_env attribute is defined in MLproject then it takes precedence over conda yaml
        # environments, so the project will be executed inside a docker container.
        if fetched_project.project.docker_env:
            tracking.MlflowClient().set_tag(fetched_project.run.info.run_id, MLFLOW_PROJECT_ENV,
                                            "docker")
            _validate_docker_env(fetched_project.project)
            _validate_docker_installation()
            image = _build_docker_image(work_dir=fetched_project.work_dir,
                                        repository_uri=fetched_project.project.name,
                                        base_image=fetched_project.project.docker_env.get('image'),
                                        run_id=fetched_project.run.info.run_id)
            user_env_vars = fetched_project.project.docker_env.get("environment")
            command_args += _get_docker_command(image=image, active_run=fetched_project.run,
                                                volumes=fetched_project.project.docker_env.get(
                                                    "volumes"),
        ...
        # to avoid failures due to multiple concurrent attempts to create the same conda env.
            ...
            command_separator = " && "
            conda_env_name = _get_or_create_conda_env(fetched_project.project.conda_env_path)
            command_args += _get_conda_command(conda_env_name)
        # In synchronous mode, run the entry point command in a blocking fashion, sending status
        # updates to the tracking server when finished. Note that the run state may not be
        # persisted to the tracking server if interrupted
        if synchronous:
            command_args += _get_entry_point_command(fetched_project.project,
                                                     entry_point, parameters, storage_dir)
            command_str = command_separator.join(command_args)
            return _run_entry_point(command_str, fetched_project.work_dir, experiment_id,
                                    run_id=fetched_project.run.info.run_id)
        # Otherwise, invoke `mlflow run` in a subprocess
        return _invoke_mlflow_run_subprocess(
            work_dir=fetched_project.work_dir, entry_point=entry_point, parameters=parameters,
            experiment_id=experiment_id,
            use_conda=use_conda, storage_dir=storage_dir, run_id=fetched_project.run.info.run_id)
    elif backend_name == "kubernetes":
        from mlflow.projects import kubernetes as kb
        tracking.MlflowClient().set_tag(fetched_project.run.info.run_id,
                                        MLFLOW_PROJECT_ENV, "docker")
        tracking.MlflowClient().set_tag(fetched_project.run.info.run_id, MLFLOW_PROJECT_BACKEND,
                                        "kubernetes")
        _validate_docker_env(fetched_project.project)
        _validate_docker_installation()
        ...
        submitted_run = kb.run_kubernetes_job(
            fetched_project.project.name,
            fetched_project.run,
            image.tags[0],
            image_digest,
            _get_entry_point_command(fetched_project.project, entry_point, parameters, storage_dir),
            utils.generate_env_vars_to_attach_to_run(fetched_project.run),
