...
def save_model(keras_model, path, conda_env=None, mlflow_model=Model(), custom_objects=None,
    ...
    :param custom_objects: Keras ``custom_objects`` dictionary containing String:Class
    mappings for custom layers in your Keras model. MLflow saves these custom layers using
    CloudPickle and restores them automatically when the model is loaded with
    :py:func:`mlflow.keras.load_model` and :py:func:`mlflow.pyfunc.load_model`.
    :param keras_module: Keras module to be used to save / load the model (keras or tf.keras).
    If not provided, MLflow will attempt to infer the Keras module based on the given model.
