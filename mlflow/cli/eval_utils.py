...
def extract_assessments_from_traces(
    trace_ids: list[str], scorer_names: list[str], evaluation_run_id: str
) -> list[dict[str, Any]]:
    """
    Extract assessments by reading traces back from MLflow.

    After evaluation, assessments are attached to traces. This function
    reads the traces back and extracts assessments by matching the evaluation run_id.
    ...
    for trace_id in trace_ids:
        ...
        if trace and trace.info.assessments:
            # Filter assessments by the evaluation run_id
            # This ensures we only get assessments generated by this specific evaluation
            evaluation_assessments = [
                assessment
                for assessment in trace.info.assessments
                if assessment.run_id == evaluation_run_id
            ]

            # For each assessment from this evaluation run, extract the data
            if evaluation_assessments:
                for assessment in evaluation_assessments:
                    assessment_dict = {
                    ...
                    if assessment.feedback:
                        assessment_dict["result"] = assessment.feedback.value

                    # Check top-level rationale
                    if assessment.rationale:
                        assessment_dict["rationale"] = assessment.rationale

                    # Check for errors
                    if assessment.error:
                        assessment_dict["error"] = str(assessment.error)

                    trace_result["assessments"].append(assessment_dict)
            else:
                # No assessments found for this run_id, create empty entries using scorer names
                for scorer_name in scorer_names:
            ...
            # No assessments at all on the trace, create empty entries
            for scorer_name in scorer_names:
                trace_result["assessments"].append(
                    {
                        "assessment_name": scorer_name,
