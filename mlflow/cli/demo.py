...
_NEW_SERVER_SENTINEL = "_NEW_SERVER_SENTINEL"
...
def _validate_tracking_uri(ctx: Any, param: Any, value: str | None) -> str | None:
    if value is None:
        return None
...
@click.option(
    "--tracking-uri",
    default=None,
    callback=_validate_tracking_uri,
    help="URL of an existing MLflow tracking server to populate with demo data.",
)
@click.option(
    "--no-browser",
    is_flag=True,
...
def demo(
    ...
    By default, creates a temporary environment with SQLite database and file-based
    artifacts, generates demo data, and opens the browser to the demo experiment.
...
def _get_tracking_uri_interactive(port: int | None) -> str:
    click.echo()
...
def _run_with_new_server(port: int | None, no_browser: bool, debug: bool, refresh: bool) -> None:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        experiment = mlflow.get_experiment_by_name(DEMO_EXPERIMENT_NAME)
        experiment_url = (
            f"http://127.0.0.1:{port}/#/experiments/{experiment.experiment_id}/overview"
        )
