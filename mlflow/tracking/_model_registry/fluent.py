    ...
    *,
    ...
    if RunsArtifactRepository.is_runs_uri(model_uri):
        ...
        if runs_artifact_repo._is_directory(artifact_path):
            # First check if run has artifact at artifact_path, if so use the run's artifact location as source
            source = RunsArtifactRepository.get_underlying_uri(model_uri)
        else:
            # Otherwise check if there's a logged model with name artifact_path and source_run_id run_id
            run = client.get_run(run_id)
            logged_models = _get_logged_models_from_run(run, artifact_path)
            if not logged_models:
                raise MlflowException(
                    f"Unable to find a model with artifact_path {artifact_path} under run {run_id}.",
