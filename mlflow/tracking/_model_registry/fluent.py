...
def register_model(
    ...
    *,
    ...
    :param model_uri: URI referring to the MLmodel directory. Use a ``runs:/`` URI if you want to
                      record the run ID with the model in model registry, or pass the local filesystem
                      path of the model if registering a locally-persisted MLflow model saved using
                      ``save_model``.
