...
def clear_experiment_storage_location(
    experiment_id: str | None = None,
    location: UCSchemaLocation | None = None,
) -> None:
    """
    ...
    Args:
        experiment_id: The MLflow experiment ID to clear the storage location for.
        location: The storage location to clear. If None, all storage locations will be unset.
    """
    experiment_id = experiment_id or _get_experiment_id()
    if experiment_id is None:
        raise MlflowException.invalid_parameter_value(
    ...
    TracingClient()._clear_experiment_storage_location(experiment_id, location)
