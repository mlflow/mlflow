...
def aggregate_cost_from_spans(spans: list[LiveSpan]) -> dict[str, float] | None:
    ...
    # Start with LLM costs (legacy 3-key payload)
    ...
    for attr_key, cost_key in span_to_cost_key_mapping.items():
        ...
        if cost is not None:
            ...
            result[CostKey.TOTAL_COST] = result.get(CostKey.TOTAL_COST, 0.0) + total
            result[cost_key] = result.get(cost_key, 0.0) + total
