...
class DatabricksExternalMonitoringSpanExporter(SpanExporter):
    """
    An exporter implementation that logs the traces to Databricks Trace Monitoring.

    Args:
        trace_destination: The destination of the traces. This is model serving model
            name for now but will be replaced by a construct.
    """

    def __init__(self, trace_destination: str):
        self._trace_destination = trace_destination
    ...
    def export(self, root_spans: Sequence[ReadableSpan]):
        ...
        for span in root_spans:
            ...
            self._deploy_client.predict(
                endpoint=self._trace_destination,
