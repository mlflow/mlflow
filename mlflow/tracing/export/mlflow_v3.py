...
class MlflowV3SpanExporter(SpanExporter):
    ...
    using the V3 trace schema and API. Optionally also exports to Databricks Delta
    in parallel.
    """
    ...
    def __init__(
        ...
        Args:
            ...
            trace_server_config: Optional dict with keys 'spans_table_name', 'ingest_url', 
                                'workspace_url', and 'pat' to enable parallel export to Databricks Delta.
        """
        ...
        # Initialize DatabricksDeltaExporter if config is provided
        self._databricks_delta_exporter = None
        if trace_server_config:
            try:
                from mlflow.tracing.export.databricks_delta import DatabricksDeltaExporter
                self._databricks_delta_exporter = DatabricksDeltaExporter(
                ...
                _logger.info("Databricks Delta parallel export enabled")
            except Exception as e:
                _logger.warning(f"Failed to initialize DatabricksDeltaExporter: {e}")
                self._databricks_delta_exporter = None
    ...
    def _log_trace(self, trace: Trace, prompts: Sequence[PromptVersion]):
        ...
        Optionally also exports to Databricks Delta in parallel.
        Steps:
        1. Create the trace in MLflow
        2. Upload the trace data to blob storage using the returned trace info.
        3. Optionally export to Databricks Delta in parallel
        """
        # Export to MLflow V3
        try:
            if trace:
        ...
        # Export to Databricks Delta in parallel if configured
        if self._databricks_delta_exporter and trace:
            try:
                self._databricks_delta_exporter._log_trace(trace)
            except Exception as e:
