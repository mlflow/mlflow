...
def get_completed_trace(request_id: str) -> Optional[Dict[str, Any]]:
    """
    Get the completed trace data from the buffer. This method is used in
    the Databricks model serving so please be careful when modifying it.
    """
    return _TRACE_BUFFER.get(request_id)
