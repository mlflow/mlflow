...
from mlflow.telemetry.constant import (
    GENAI_EVALUATE_FUNCTION,
    GENAI_EVALUATION_PATH,
    GENAI_MODULES,
    GENAI_SCORERS_PATH,
...
class SimulateConversationEvent(Event):
    ...
    def parse(cls, arguments: dict[str, Any]) -> dict[str, Any] | None:
        ...
        for frame_info in inspect.stack()[:10]:
            ...
            if GENAI_EVALUATION_PATH in frame_filename.replace("\\", "/"):
                if frame_function == GENAI_EVALUATE_FUNCTION:
...
class ScorerCallEvent(Event):
    ...
    def parse(cls, arguments: dict[str, Any]) -> dict[str, Any] | None:
        ...
        for frame_info in inspect.stack()[:10]:
            ...
            if GENAI_SCORERS_PATH in frame_filename.replace("\\", "/"):
                if frame_function == SCORER_RUN_FUNCTION:
                    callsite = "genai_evaluate"
                    break
