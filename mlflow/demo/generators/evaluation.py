...
BASELINE_PROFILE = {
    ...
    "version": "1.0",
    "description": "Initial model evaluation with baseline prompts",
    "pass_rates": {
        "relevance": 0.65,
        "correctness": 0.60,
        "groundedness": 0.55,
...
IMPROVED_PROFILE = {
    ...
    "version": "2.0",
    "description": "Evaluation after prompt optimization and RAG improvements",
    "pass_rates": {
        "relevance": 0.90,
        "correctness": 0.85,
        "groundedness": 0.85,
...
class EvaluationDemoGenerator(BaseDemoGenerator):
    ...
    def generate(self) -> DemoResult:
        ...
        v1_expectation_count = self._add_expectations_to_traces(v1_traces)
        v2_expectation_count = self._add_expectations_to_traces(v2_traces)
        expectation_count = v1_expectation_count + v2_expectation_count
        ...
        v1_run_id, v1_feedback = self._create_single_evaluation_run(
            traces=v1_traces_with_expectations,
            experiment_id=experiment_id,
            run_name=BASELINE_PROFILE["name"],
            scorer_version=BASELINE_PROFILE["version"],
            description=BASELINE_PROFILE["description"],
            pass_rates=BASELINE_PROFILE["pass_rates"],
        )
        run_ids.append(v1_run_id)
        total_feedback += v1_feedback

        v2_run_id, v2_feedback = self._create_single_evaluation_run(
            traces=v2_traces_with_expectations,
            experiment_id=experiment_id,
            run_name=IMPROVED_PROFILE["name"],
            scorer_version=IMPROVED_PROFILE["version"],
            description=IMPROVED_PROFILE["description"],
            pass_rates=IMPROVED_PROFILE["pass_rates"],
        )
        run_ids.append(v2_run_id)
        total_feedback += v2_feedback
    ...
    def _add_expectations_to_traces(self, traces: list[Trace]) -> int:
        ...
        for trace in traces:
            ...
            query = inputs.get("query") or inputs.get("message") or ""
    ...
    def _create_single_evaluation_run(
        ...
        feedback_count = len(traces) * len(demo_scorers)

        return result.run_id, feedback_count
