...
def _backfill_readable_mcp_results(
    ...
    *,
    process_item: Callable[[Any], dict[str, Any] | None] | None = None,
) -> str | None:
...
def _filter_search_mcp_servers(username: str, body: bytes, request: StarletteRequest) -> bytes:
    ...
    def _process_server_entity(entity: Any) -> dict[str, Any] | None:
        s = MCPServerResponse.from_entity(entity).model_dump(mode="json")
        return _process_server_dict(s)

    readable = [s for s in data.get("mcp_servers", []) if _process_server_dict(s) is not None]
    ...
    data["next_page_token"] = _backfill_readable_mcp_results(
        ...
        get_name=lambda entity: entity.name,
        to_dict=lambda entity: MCPServerResponse.from_entity(entity).model_dump(mode="json"),
        process_item=_process_server_entity,
    )
...
def add_fastapi_permission_middleware(app: FastAPI) -> None:
    ...
    async def fastapi_permission_middleware(request, call_next):
        ...
        # TEMP FIX: set workspace ContextVar early so validators and response
        # filters can read it (the workspace middleware is inner and clears it
        # before this middleware's post-processing). Remove once #24479 lands.
        ...
        try:
            ...
            set_server_request_workspace(_ws.name if _ws else None)
        except Exception:
            pass
