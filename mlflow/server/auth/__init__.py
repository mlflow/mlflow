...
def _get_permission_from_store_or_default(
    ...
    - If no direct permission exists and workspaces are enabled, callers provide
      ``workspace_fallback`` to check workspace-level permissions for the resource's workspace.
      This fallback should default to ``NO_PERMISSIONS`` to preserve workspace isolation.
    - If workspace permissions are not applicable (e.g. workspaces are disabled and the fallback
...
def _workspace_permission_for_resource(
    ...
    return _workspace_permission(username, resource_type, workspace_name)
...
def _get_permission_from_experiment_id() -> Permission:
    ...
    return _get_permission_from_store_or_default(
        ...
        workspace_fallback=lambda: _workspace_permission_for_experiment(username, experiment_id),
    )
...
def _get_permission_from_experiment_id_artifact_proxy() -> Permission:
    ...
    if MLFLOW_ENABLE_WORKSPACES.get():
        if workspace_name := workspace_context.get_request_workspace():
            permission = _workspace_permission(username, "experiments", workspace_name)
            if permission is not None:
                return permission
        return NO_PERMISSIONS
...
def _get_permission_from_registered_model_name() -> Permission:
    ...
    return _get_permission_from_store_or_default(
        ...
        workspace_fallback=lambda: _workspace_permission_for_registered_model(username, name),
    )
...
def validate_can_create_experiment() -> bool:
    ...
    perm = _workspace_permission(authenticate_request().username, "experiments", workspace_name)
...
def validate_can_create_registered_model() -> bool:
    ...
    perm = _workspace_permission(
        authenticate_request().username, "registered_models", workspace_name
    )
    return perm is not None and perm.can_manage
...
def filter_experiment_ids(experiment_ids: list[str]) -> list[str]:
    ...
    try:
        ...
        workspace_name = workspace_context.get_request_workspace()
        workspace_perm = (
            _workspace_permission(username, "experiments", workspace_name)
            if workspace_name
            else NO_PERMISSIONS
        )
        workspace_can_read = workspace_perm.can_read if workspace_perm else False

        return [exp_id for exp_id in experiment_ids if can_read.get(exp_id, workspace_can_read)]
...
def _find_validator(req: Request) -> Callable[[], bool] | None:
    ...
    # Only workspace permission routes use parameterized path matching here.
    if "/workspaces/" not in req.path:
        return None

    # Regex matching for parameterized workspace paths (e.g., workspace_name).
...
def _validate_workspace_permission_payload(payload: dict[str, Any]) -> tuple[str, str, str]:
    if missing := {"username", "resource_type", "permission"} - payload.keys():
