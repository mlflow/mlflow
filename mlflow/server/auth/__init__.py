...
def _authenticate_fastapi_request(request: StarletteRequest) -> User | None:
    ...
    if scheme.lower() == "basic":
        ...
        if not internal_token:
            return None
        # Credentials must be "token:username". We rely on the internal token
        # not containing ":" so that partition is unambiguous. The token is
        # generated by secrets.token_hex() which produces hex characters only.
