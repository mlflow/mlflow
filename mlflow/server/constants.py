...
# Secrets management
# KEK (Key Encryption Key) passphrase for envelope encryption of secrets stored in database.
...
# Must be high-entropy (32+ characters) from a secrets manager. See mlflow/utils/crypto.py.
#
# KEK Rotation Workflow (for changing the passphrase):
#   1. Shut down the MLflow server
#   2. Set MLFLOW_SECRETS_KEK_PASSPHRASE to the OLD passphrase
#   3. Run: mlflow secrets rotate-kek --new-passphrase "NEW_PASSPHRASE"
#   4. Update MLFLOW_SECRETS_KEK_PASSPHRASE to NEW passphrase in deployment config
#   5. Restart the MLflow server
#
# The rotation is atomic and idempotent - safe to re-run if it fails.
SECRETS_KEK_PASSPHRASE_ENV_VAR = "MLFLOW_SECRETS_KEK_PASSPHRASE"

# KEK version for tracking which KEK encrypted each secret (default 1).
# Automatically tracked during rotation. See `mlflow secrets rotate-kek` for rotation workflow.
SECRETS_KEK_VERSION_ENV_VAR = "MLFLOW_SECRETS_KEK_VERSION"
