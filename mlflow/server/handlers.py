...
import requests
from flask import Response, current_app, request, send_file, jsonify
...
def _get_experiment_by_name():
    ...
    response_message = get_experiment_by_name_impl(request_message)
    response = Response(mimetype="application/json")
    response.set_data(message_to_json(response_message))
    return response

def get_experiment_by_name_impl(request_message):
...
# Sample schema for demonstration purposes. Later this schema will be autogenerated from the proto to graphql codegen script.
...
@catch_mlflow_exception
# handle errors gracefully - might encounter unexpected errors
@_disable_if_artifacts_only
# Might wanna start with this
# Two modes, maybe don't wanna use graphql in one of the modes
# If we wanna use it in both modes, then it's going to be tricky
# Should start with this - most of the endpoints have this decoration.
def _graphql():
    print("in graphql")
    ...
    print("finished executing the query")
    print("result", result)
...
# resolver function
# inside of it you call a real execution that does the actual work
# this code has to be generated
# the real exeuction has to have some sort of pattern so we can codegen it
