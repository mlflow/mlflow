...
# Component identifier, to keep compatible with Databricks in-house implmenetations.
...
class MlflowRunExtension(MlflowRun):
    ...
    metric_history = graphene.List(
        graphene.NonNull(MlflowMetricWithRunId),
        metricKey=graphene.String(required=True),
    )
    ...
    def resolve_metric_history(self, info, metricKey):
        run_id = self.info.run_id
        input_dict = {
            "run_ids": [run_id],
        ...
        parse_dict(input_dict, request_message)
        return mlflow.server.handlers.get_metric_history_bulk_interval_impl(request_message).metrics
