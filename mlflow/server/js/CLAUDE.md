...
### Reusable Components from @databricks/web-shared

**CRITICAL**: Before implementing ANY UI functionality, search for existing implementations in `@databricks/web-shared`:

```bash
# Search for existing components by functionality
grep -r "export.*Button" src/shared/web-shared/
grep -r "export.*Modal" src/shared/web-shared/
grep -r "export.*Copy" src/shared/web-shared/

# Common reusable components to check:
# - src/shared/web-shared/copy/CopyActionButton.tsx - Use for copy-to-clipboard functionality
# - src/shared/web-shared/genai-traces-table/utils/RoutingTestUtils.tsx - Use TestRouter in tests
```

**Examples of components to reuse**:

- **Copy functionality**: Use `CopyActionButton` from `@databricks/web-shared/copy` instead of custom clipboard implementations
- **Routing in tests**: Use `TestRouter` and `testRoute` from routing utils instead of bare `MemoryRouter`
- **Common UI patterns**: Check web-shared before creating custom solutions
...
**ALWAYS define shared constants in appropriate locations** to avoid magic strings across the codebase:

1. **Query Parameters**: Define in `src/experiment-tracking/constants.ts`
   ```typescript
   ...
   searchParams.get(SELECTED_TRACE_ID_QUERY_PARAM);

   // ❌ BAD - Magic string
   searchParams.get('selectedTraceId');
   ```

2. **Route paths**: Use constants from routing utilities
3. **Feature flags**: Define in FeatureUtils.ts files
4. **Component IDs**: Use descriptive constants when used multiple times

**Benefits**:
- Refactoring becomes safer (change in one place)
...
**Use `URLSearchParams` for building URLs with query parameters** instead of manual string concatenation:

```typescript
// ✅ GOOD - URLSearchParams handles encoding automatically
const params = new URLSearchParams({
  [SELECTED_TRACE_ID_QUERY_PARAM]: traceId
});
const url = `${baseUrl}?${params.toString()}`;

// ❌ BAD - Manual encoding is error-prone
const url = `${baseUrl}?selectedTraceId=${encodeURIComponent(traceId)}`;
```

**Why URLSearchParams is better**:
- Handles special characters automatically
- More canonical and maintainable
- Easier to add multiple parameters
- Less likely to have encoding bugs

### Testing Patterns

**Use project-specific test utilities** instead of generic testing library components:

```typescript
// ✅ GOOD - Use TestRouter from project utilities
import { TestRouter, testRoute } from '../utils/RoutingTestUtils';

const { container } = render(
  <TestRouter
    routes={[
      testRoute(<YourComponent {...props} />),
    ]}
  />
);

// ❌ BAD - Using MemoryRouter directly
...
- May include project-specific configuration
- Easier to maintain when routing patterns change
- More discoverable in code reviews
...
Before submitting a PR, verify you've followed these patterns:

- [ ] **Checked for reusable components** in `@databricks/web-shared`
- [ ] **Created constants** for any strings used in multiple places
- [ ] **Used URLSearchParams** for URL construction with query parameters
- [ ] **Used TestRouter** instead of MemoryRouter in test files
- [ ] **Followed existing patterns** in similar files
- [ ] **Ran all tests** and verified they pass
- [ ] **Ran prettier:fix** to format code
- [ ] **No magic strings** - all shared values are exported constants
