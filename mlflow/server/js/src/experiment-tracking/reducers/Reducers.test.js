  ...
  test('search runs and load more apis updates non empty state correctly', () => {
    ...
    function new_action(paramOrTag, action_type) {
      return {
        ...
        payload: {
          runs: [
            { info: mockRunInfo('run01').toJSON(), data: { [paramOrTag]: [val1_2, val3] } },
            { info: mockRunInfo('run03').toJSON(), data: { [paramOrTag]: [val3] } },
            { info: mockRunInfo('run04').toJSON(), data: { [paramOrTag]: undefined } },
          ],
        },
      };
    }
