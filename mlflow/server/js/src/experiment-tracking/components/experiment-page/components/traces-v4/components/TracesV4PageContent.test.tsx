...
describe('TracesV4PageContent', () => {
  ...
  describe('pagination bar', () => {
    ...
    test('shows "N of N" in the pagination bar when a single page is the whole result set', async () => {
      // Single full page (no next token, page 1) → the result set total is known, so the count reads
      // "3 of 3". The count lives in the pagination bar, not the toolbar.
      state.pages = { '': { traces: makeTraces(3), next_page_token: undefined } };
      renderPage();
      await findTraceRow('tr-000');
