...
import {
  ...
  getRandomRunName,
  stableNormalRandom,
} from '../charts/CompareRunsCharts.stories-common';
...
export default {
  title: 'Compare runs charts/Context menu',
  argTypes: {},
  parameters: {
    layout: 'fullscreen',
  },
...
const ContextMenuComponent = ({
  ...
  isHovering,
  closeContextMenu,
}: CompareRunsTooltipBodyProps<{ runs: ReturnType<typeof createMockData> }>) => {
...
const withChartMenuContext =
  ...
  (props: P) => {
    ...
    return (
      <CompareRunsTooltipWrapper component={ContextMenuComponent} contextData={data}>
...
export const ChartContextMenuStory = withChartMenuContext(({ data }: any) => {
  const { setTooltip, resetTooltip } = useCompareRunsTooltip('dummy-chart-id');
