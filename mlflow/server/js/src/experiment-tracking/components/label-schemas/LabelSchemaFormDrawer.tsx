...
const CID = 'mlflow.label-schema-form-drawer';

export interface LabelSchemaFormDrawerProps {
  experimentId: string;
  /** When non-null, the drawer opens in edit mode pre-populated from the schema. */
...
/**
 * Right-hand drawer for creating or editing a review question (label schema):
 * a two-pane form (fields | live preview) over the label-schema create/update
 * mutations. Surfaced from the Review tab's "Review questions" manager; the
 * form itself is the schema authoring UI shared with the rest of the app.
 */
export const LabelSchemaFormDrawer = ({
  experimentId,
  editingSchema,
  visible,
  onClose,
}: LabelSchemaFormDrawerProps) => {
  ...
  // Reset the form whenever the drawer opens or the edited schema changes. The
  // drawer stays mounted across open/close, so without this react-hook-form's
  ...
  const onSubmit = async (form: LabelSchemaFormData) => {
    ...
    try {
      ...
      // Errors surface via `submitError`; keep the drawer open.
      return;
    }
      ...
      <Drawer.Content
        ...
        title={
          isEdit ? (
            ...
            <FormattedMessage defaultMessage="Add question" description="Add review question drawer title" />
          )
        }
      >
        <form
        ...
        >
          ...
          >
            ...
            <Button componentId={`${CID}.cancel`} onClick={handleCancel}>
              <FormattedMessage defaultMessage="Cancel" description="Review question drawer cancel button" />
            </Button>
            <Button
              componentId={`${CID}.submit`}
              type="primary"
              htmlType="submit"
