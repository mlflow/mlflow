...
import { REGISTERED_PROMPT_COMMIT_MESSAGE_TAG_KEY } from '../utils';
...
export const PromptVersionMetadata = ({
  ...
  const commitMessage = useMemo(() => {
    return registeredPromptVersion?.tags?.find((tag) => tag.key === REGISTERED_PROMPT_COMMIT_MESSAGE_TAG_KEY)?.value;
  }, [registeredPromptVersion]);
    ...
    >
      ...
      {commitMessage && (
        <>
          ...
          <Typography.Text>{commitMessage}</Typography.Text>
        </>
      )}
    </div>
  );
