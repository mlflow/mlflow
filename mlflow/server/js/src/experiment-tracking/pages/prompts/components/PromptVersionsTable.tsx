...
export const PromptVersionsTable = ({
  ...
  const columns = useMemo(() => {
    const resultColumns: PromptVersionsTableColumnDef[] = [
      { id: 'version', header: 'Version', accessorKey: 'version', cell: PromptsListTableVersionCell },
    ];
  ...
  }, [mode, intl, aliasesByVersion, showEditAliasesModal, registeredPrompt]);
  ...
  const getEmptyState = () => {
    if (!isLoading && promptVersions?.length === 0) {
      return (
        <Empty
          title={<FormattedMessage defaultMessage="No prompt versions created" description="TODO" />}
          description={
            <FormattedMessage
