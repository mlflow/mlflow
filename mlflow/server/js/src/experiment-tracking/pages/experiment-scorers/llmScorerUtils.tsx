...
export const validateInstructions = (value: string | undefined, scope?: ScorerEvaluationScope): true | string => {
  ...
  for (const match of matches) {
    ...
    if (!validVariables.includes(varName)) {
      if (isSessionLevel) {
        return `Invalid variable: {{ ${varName} }}. Only {{ conversation }} is allowed`;
      }
  ...
  if (foundVariables.size === 0) {
    if (isSessionLevel) {
      return 'Must contain {{ conversation }} variable';
    }
...
export const convertEvaluationResultToAssessment = (
  ...
  // If the result is already an assessment, return it with trace ID attached and assessment ID generated
  if (firstResult && isFeedbackAssessmentInJudgeEvaluationResult(firstResult)) {
    return { ...firstResult, trace_id: traceId, assessment_id: `${Date.now()}` };
  }
