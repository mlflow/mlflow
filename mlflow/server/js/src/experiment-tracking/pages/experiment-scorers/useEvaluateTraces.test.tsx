...
import { shouldUseTracesV4API } from '@databricks/web-shared/model-trace-explorer';
import { fetchOrFail } from '../../../common/utils/FetchUtils';
import { setupServer } from '../../../common/utils/setup-msw';
import { rest } from 'msw';
// Setup test fetch to mock RPC API
...
describe('useEvaluateTraces', () => {
  ...
  describe('V4 API support for UC_SCHEMA traces', () => {
    it('should use V4 API to fetch traces when locations include UC_SCHEMA and feature flag is enabled', async () => {
      jest.mocked(shouldUseTracesV4API).mockReturnValue(true);

      const traceId = 'tr-uc-trace-1';
      const experimentId = 'exp-uc-123';
      const judgeInstructions = 'Evaluate the quality';
      const ucSchemaLocation = {
      ...
      // Setup search traces handler (still uses the shared search endpoint)
      const traces = new Map([[traceId, mockTrace]]);
      setupSearchTracesHandler(server, traces);

      // Setup V4 batch-get handler via MSW
      server.use(
        rest.get('*/ajax-api/4.0/mlflow/traces/catalog.schema/batchGet', (req, res, ctx) => {
          const traceIds = req.url.searchParams.getAll('trace_ids');
          const responseTraces = traceIds
          ...
          return res(ctx.json({ traces: responseTraces }));
        }),
      );

      // Setup chat completions mock via fetchOrFail
      mockedFetchOrFail.mockImplementation((url: RequestInfo | URL) => {
        const urlString = typeof url === 'string' ? url : url.toString();
        if (urlString.includes('ajax-api/2.0/agents/chat-completions')) {
          return Promise.resolve({
            json: () => Promise.resolve(apiResponse),
          } as any);
        }
      ...
      // Verify trace was cached with V4 key
      const cachedTrace = queryClient.getQueryData(['GetMlflowTraceV4', traceId, ucSchemaLocation]);
      expect(cachedTrace).toEqual(mockTrace);

      // Verify V3 cache key was NOT used
      const v3CachedTrace = queryClient.getQueryData(['GetMlflowTraceV3', traceId]);
      expect(v3CachedTrace).toBeUndefined();
