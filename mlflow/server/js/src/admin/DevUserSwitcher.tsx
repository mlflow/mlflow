...
// Static-prefix deployments (e.g. served under ``/mlflow``) carry cookies
// scoped to the prefix path. Setting/clearing only ``path=/`` would create
// duplicate cookies with ambiguous precedence on apply, and miss prefixed
// cookies on clear. Iterate over root, the current app's base path, and
// the trailing-slash-stripped variant — same coverage as the Account
// logout flow.
const candidateCookiePaths = (): string[] => {
...
const applyCredentials = (username: string, password: string) => {
  ...
  for (const path of candidateCookiePaths()) {
...
const clearAuthCookies = () => {
  ...
  for (const path of candidateCookiePaths()) {
    document.cookie = `${AUTH_HEADER_COOKIE}=; ${expiresAttr}; path=${path};`;
    document.cookie = `${MLFLOW_USER_COOKIE}=; ${expiresAttr}; path=${path};`;
  }
};
