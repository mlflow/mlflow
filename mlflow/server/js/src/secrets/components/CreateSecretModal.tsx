...
export const CreateSecretModal = ({ visible, onCancel }: { visible: boolean; onCancel: () => void }) => {
  ...
  }, [createSecret, secretName, envVarKey, secretValue, intl]);
  ...
  const isFormValid = useMemo(() => {
    return secretName.trim().length > 0 && envVarKey.trim().length > 0 && secretValue.length > 0;
  }, [secretName, envVarKey, secretValue]);
  ...
  return (
    <Modal
      ...
      title={<FormattedMessage defaultMessage="Create Global Secret" description="Create global secret modal title" />}
    >
            ...
            <div css={{ display: 'flex', alignItems: 'center', gap: theme.spacing.xs }}>
              ...
              <LegacyTooltip
                title={intl.formatMessage({
                  defaultMessage:
                    'A friendly nickname to identify this global secret (e.g., "anthropic-general-use-dev", "openai-prod-key").',
