...
describe('GatewayApi', () => {
  ...
  describe('LiteLLM Provider Metadata', () => {
    it('should properly return error when listProviders API responds with bare status', async () => {
      server.use(rest.get('/ajax-api/3.0/mlflow/endpoints/litellm/providers', (req, res, ctx) => res(ctx.status(404))));
    ...
    it('should properly return error with message extracted from listModels API', async () => {
      server.use(
        rest.get('/ajax-api/3.0/mlflow/endpoints/litellm/models', (req, res, ctx) =>
          res(
            ctx.status(500),
            ctx.json({
              code: 'INTERNAL_ERROR',
              message: 'Failed to fetch models from LiteLLM',
            }),
          ),
        ),
      );

      await expect(GatewayApi.listModels()).rejects.toThrow('Failed to fetch models from LiteLLM');
    });
  });
