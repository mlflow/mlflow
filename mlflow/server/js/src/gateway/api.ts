...
export const GatewayApi = {
  ...
  listProviders: () => {
    const relativeUrl = 'ajax-api/3.0/mlflow/endpoints/litellm/providers';
  ...
  listModels: (provider?: string) => {
    ...
    const relativeUrl = ['ajax-api/3.0/mlflow/endpoints/litellm/models', params.toString()].join('?');
    return fetchEndpoint({
      relativeUrl,
      error: defaultErrorHandler,
  ...
  getProviderConfig: (provider: string) => {
    const params = new URLSearchParams();
    params.append('provider', provider);
    const relativeUrl = ['ajax-api/3.0/mlflow/endpoints/litellm/provider-config', params.toString()].join('?');
    return fetchEndpoint({
      relativeUrl,
      error: defaultErrorHandler,
  ...
  getSecretsConfig: () => {
    return fetchEndpoint({
      relativeUrl: 'ajax-api/3.0/mlflow/secrets/config',
      error: defaultErrorHandler,
