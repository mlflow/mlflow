...
describe('GatewayLayout', () => {
  ...
  test('shows install guide when gateway dependencies are not available', () => {
    mockGatewayUnavailable();
    jest.mocked(useEndpointsQuery).mockReturnValue({
      data: [],
      isLoading: false,
      error: undefined,
    ...
    expect(screen.getByText('Additional Setup Required')).toBeInTheDocument();
    expect(screen.getByText(/mlflow\[genai\]/)).toBeInTheDocument();
    expect(screen.queryByText('Encryption Required')).not.toBeInTheDocument();
  });
});
