...
export interface ModelDefinitionsFilter {
  providers: string[];
}

interface ModelDefinitionsFilterButtonProps {
  availableProviders: string[];
  filter: ModelDefinitionsFilter;
  onFilterChange: (filter: ModelDefinitionsFilter) => void;
}

export const ModelDefinitionsFilterButton = ({
  availableProviders,
  filter,
  onFilterChange,
}: ModelDefinitionsFilterButtonProps) => {
  ...
  return (
    <Popover.Root
      componentId="mlflow.gateway.model-definitions-list.filter-popover"
      open={isOpen}
      onOpenChange={setIsOpen}
    >
      <Popover.Trigger asChild>
        <Button
          componentId="mlflow.gateway.model-definitions-list.filter-button"
        ...
        <div css={{ display: 'flex', flexDirection: 'column', gap: theme.spacing.sm }}>
          ...
          {sortedProviders.length === 0 ? (
            ...
            sortedProviders.map((provider) => (
              <Checkbox
                key={provider}
                componentId={`mlflow.gateway.model-definitions-list.filter.provider.${provider}`}
