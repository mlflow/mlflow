import { Alert, Typography, useDesignSystemTheme, KeyIcon, GearIcon } from '@databricks/design-system';
...
interface SecretsSetupGuideProps {
  gatewayAvailable?: boolean;
  secretsAvailable?: boolean;
}

export const SecretsSetupGuide = ({ gatewayAvailable, secretsAvailable }: SecretsSetupGuideProps) => {
  const { theme } = useDesignSystemTheme();

  // If gateway is not available, show install instructions
  if (!gatewayAvailable) {
    const installCommand = 'pip install "mlflow[genai]"';
    ...
    return (
      ...
      >
        <div css={{ textAlign: 'center', marginBottom: theme.spacing.lg }}>
          ...
          >
            <GearIcon css={{ fontSize: 28, color: theme.colors.textSecondary }} />
          </div>
          <Typography.Title level={2} css={{ marginBottom: theme.spacing.sm }}>
            <FormattedMessage
              defaultMessage="Additional Setup Required"
              description="Title for gateway install guide"
            />
          </Typography.Title>
          <Typography.Text color="secondary">
            <FormattedMessage
              defaultMessage="AI Gateway requires additional dependencies. Follow the steps below to enable this feature."
              description="Subtitle for gateway install guide"
            />
          </Typography.Text>
        </div>
            ...
            <li>
              <Typography.Text>
                <FormattedMessage
                  ...
                  description="Step 1 in gateway install guide"
                />
              </Typography.Text>
            </li>
            <li>
              <div css={{ display: 'flex', flexDirection: 'column', gap: theme.spacing.xs }}>
                <Typography.Text>
                  <FormattedMessage
                    defaultMessage="Install the genai extras:"
                    description="Step 2 in gateway install guide"
                  />
                </Typography.Text>
              ...
              <div css={{ display: 'flex', flexDirection: 'column', gap: theme.spacing.xs }}>
                <Typography.Text>
                  <FormattedMessage
                    ...
                    description="Step 3 in gateway install guide"
                  />
                </Typography.Text>
  ...
  // Gateway is available but secrets are not configured
