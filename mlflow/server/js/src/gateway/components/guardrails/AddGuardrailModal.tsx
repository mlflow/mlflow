...
// Temporary aliases — the component uses the prototype naming internally.
// These will be refactored in a follow-up to use the backend types natively.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type Guardrail = any;
type GuardrailHook = 'PRE' | 'POST';
type GuardrailOperation = 'VALIDATION' | 'MUTATION';
interface GuardrailScorerConfig {
...
interface TestGuardrailResponse {
  result: { score: string; rationale: string; modified_text?: string };
  guardrail: { guardrail_id: string; scorer_name: string; hook: string; operation: string };
  input_text: string;
  trace_input?: string;
  trace_output?: string;
}
...
function GuardrailsAIIcon({ size = 20 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M12 2L3 7v6c0 5.25 3.83 10.16 9 11.33C17.17 23.16 21 18.25 21 13V7l-9-5z" fill="#15B8A6" />
      <text
        x="12"
        y="16.5"
        textAnchor="middle"
...
function getCompatibleBuiltins(hook: GuardrailHook, operation: GuardrailOperation): BuiltinJudgeInfo[] {
  ...
  return BUILTIN_JUDGES.filter((j) => j.variables.includes(requiredVar) && j.operations.includes(operation));
}
...
function isRegisteredScorerCompatible(scorer: RegisteredScorer, hook: GuardrailHook): boolean {
  try {
    ...
    const instructions: string | undefined =
      parsed?.instructions_judge_pydantic_data?.instructions ?? parsed?.instructions ?? parsed?.judge_prompt;
    ...
    const hasConversation = instructions.includes('{{conversation}}') || instructions.includes('{{ conversation }}');
...
export const AddGuardrailModal = ({
  open,
  onClose,
  onSuccess,
  onDelete,
  ...
  const { mutateAsync: addGuardrail, isLoading: isAdding, error: addError, reset: resetAdd } = useAddGuardrail();
  const {
  ...
  const buildTestRequest = useCallback((): Record<string, unknown> => {
    ...
    if (formData.scorerSource === 'builtin') {
      ...
      config.response_schema =
        formData.operation === 'VALIDATION' ? 'yes_no' : formData.hook === 'PRE' ? 'chat_request' : 'chat_response';
      if (formData.model) {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    if (isEditMode && editingGuardrail) {
      // TODO: align request shape with backend UpdateEndpointGuardrailConfigRequest
      ...
      // TODO: align request shape with backend CreateGatewayGuardrailRequest
      await addGuardrail({
        scorer_name: formData.scorerName,
        hook: formData.hook,
        operation: formData.operation,
          ...
          <>
            ...
            >
              <div css={{ display: 'flex', gap: theme.spacing.lg }}>
                ...
                <div css={{ flex: 1 }}>
                  ...
                  <Typography.Text bold css={{ display: 'block' }}>
                    {editingGuardrail.hook === 'PRE' ? 'Before LLM' : 'After LLM'}
                  </Typography.Text>
                </div>
                <div css={{ flex: 1 }}>
                  <Typography.Text color="secondary" css={{ fontSize: theme.typography.fontSizeSm }}>
                  ...
                  <Typography.Text bold css={{ display: 'block' }}>
                    {editingGuardrail.operation === 'VALIDATION' ? 'Block' : 'Modify'}
                  </Typography.Text>
                </div>
                <div css={{ flex: 1 }}>
                  <Typography.Text color="secondary" css={{ fontSize: theme.typography.fontSizeSm }}>
            ...
            {formData.scorerSource === 'custom' && (
              <div css={{ display: 'flex', flexDirection: 'column', gap: theme.spacing.xs }}>
                ...
                />
                <ModelSelector model={formData.model} onModelChange={(value) => handleFieldChange('model', value)} />
              </div>
            )}
                      ...
                      <GuardrailCard
                        ...
                        tag={
                          <span css={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
                            <GuardrailsAIIcon size={14} /> BUILTIN
                          </span>
                        }
                        onClick={() => {
                ...
                {formData.scorerSource === 'regex' && (
                  ...
                  >
                    ...
                    <Typography.Text color="secondary" css={{ fontSize: theme.typography.fontSizeSm }}>
                      ...
                      · email <code>[\w.-]+@[\w.-]+\.\w+</code> · credit card{' '}
                      <code>
                ...
                <div css={{ display: 'flex', flexDirection: 'column', gap: theme.spacing.sm }}>
                  ...
                  {isLoadingScorers ? (
                    <div css={{ display: 'flex', alignItems: 'center', gap: theme.spacing.sm }}>
                      <Spinner size="small" />
                      <FormattedMessage
                        defaultMessage="Loading registered guardrails..."
                        description="Loading registered guardrails"
                      />
                    </div>
                    ...
                    <div css={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: theme.spacing.sm }}>
                      {filteredRegistered.map((s) => (
                        <GuardrailCard
                          ...
                          selected={
                            formData.scorerSource === 'registered' && formData.registeredScorer === s.scorer_name
                          }
                          tag="REGISTERED"
                          onClick={() => {
                    ...
                    <Typography.Text color="secondary" css={{ fontSize: theme.typography.fontSizeSm }}>
                      No registered guardrails yet. Use <strong>Create New</strong> to build an LLM judge or code-based
                      scorer, then register it.
                    </Typography.Text>
                  )}
                </div>
              </div>
                ...
                {formData.scorerSource === 'custom' && (
                  <>
                    <div css={{ display: 'flex', flexDirection: 'column', gap: theme.spacing.xs }}>
                      <Typography.Text bold>
                        <FormattedMessage
                          defaultMessage="Guardrail name"
                          description="LLM judge guardrail name label"
                        />
                      </Typography.Text>
                      <Input
                ...
                {formData.scorerSource === 'code' && (
                  <div css={{ display: 'flex', flexDirection: 'column', gap: theme.spacing.md }}>
                    ...
                    <div css={{ display: 'flex', flexDirection: 'column', gap: theme.spacing.xs }}>
                      <Typography.Text bold>
                        <FormattedMessage
                          defaultMessage="Example: block requests containing email addresses"
                          description="Code guardrail example header"
                        />
                      </Typography.Text>
                      <CodeSnippet
                        language="python"
                        ...
                        actions={
                          <SnippetCopyAction
                            componentId="mlflow.gateway.guardrail-modal.copy-block-example"
                            copyText={blockExampleCode}
                          />
                        }
                        wrapLongLines
                    ...
                    <div css={{ display: 'flex', flexDirection: 'column', gap: theme.spacing.xs }}>
                      <Typography.Text bold>
                        <FormattedMessage
                          defaultMessage="Example: redact PII before sending to LLM (Modify)"
                          description="Code guardrail modify example header"
                        />
                      </Typography.Text>
                      <CodeSnippet
                        language="python"
                        ...
                        actions={
                          <SnippetCopyAction
                            componentId="mlflow.gateway.guardrail-modal.copy-modify-example"
                            copyText={modifyExampleCode}
                          />
                        }
                        wrapLongLines
                        style={{ borderRadius: theme.borders.borderRadiusMd, fontSize: theme.typography.fontSizeSm }}
        ...
        {(isEditMode || isFormValid) && (
          ...
          >
            ...
            >
              ...
              <span css={{ fontSize: 12, color: theme.colors.textSecondary }}>{showTest ? '\u25B2' : '\u25BC'}</span>
            </div>
                    ...
                    {!tracesLoading && traces.length > 0 && (
                      ...
                      >
                        {traces.map((trace) => {
                          ...
                          const preview = formData.hook === 'PRE' ? trace.request_preview : trace.response_preview;
                          return (
                            <div
                              key={trace.trace_id}
                              css={{
