...
describe('useWorkspaces', () => {
  ...
  it('fetches workspaces from API and returns them when enabled', async () => {
    ...
    expect(result.current.workspaces[0]).toEqual({ name: 'default', description: 'Default workspace' });
    expect(result.current.workspaces[1]).toEqual({ name: 'team-a', description: 'Team A workspace' });
    expect(result.current.workspaces[2]).toEqual({ name: 'team-b', description: null });
    expect(result.current.isError).toBe(false);
  });
