...
import { useServerSideTraceSearch } from './hooks/useServerSideTraceSearch';
...
function applySpanFilter(nodes: ModelTraceSpanNode[], state: SpanFilterState): ModelTraceSpanNode[] {
  ...
  const filter = (node: ModelTraceSpanNode): ModelTraceSpanNode | null => {
    const typeOk =
      !node.type ||
    ...
    const filteredChildren = (node.children ?? []).map(filter).filter(Boolean) as ModelTraceSpanNode[];
  ...
  return nodes.map(filter).filter(Boolean) as ModelTraceSpanNode[];
}
...
export const ModelTraceExplorerDetailView = ({
  ...
  const serverNodes = serverSearch.filteredTreeNodes ?? topLevelNodes;
