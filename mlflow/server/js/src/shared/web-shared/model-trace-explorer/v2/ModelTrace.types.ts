...
export type ModelTraceExplorerRenderMode = 'default' | 'json' | 'yaml';
...
/**
 * Display-friendly form of the AI Gateway `TraceService` proto oneof.
 * `kind` drives the pill label shown next to the service name:
 *   - 'LLM'     : serving endpoint
 *   - 'MCP'     : MCP tool/server invocation
 *   - 'TOOL'    : non-MCP function-calling tool
 *   - 'UNKNOWN' : no service-kind attribute was found on the root span
 *
 * Modeled as a discriminated union on `kind` for symmetry with the proto
 * `TraceService` oneof; today every variant carries only the display `name`.
 */
type TraceServiceInfoBase = {
  // Display name. Empty for UNKNOWN; missing when the backend didn't populate
  // the underlying proto field. For MCP this is the service's UC FQN, which
  // the cell links to the service's Unity Catalog page (see `serviceLink`).
  name?: string;
};

export type TraceServiceInfo =
  // Serving endpoint (workspace/legacy endpoint or UC Model Service). Links to
  // the AI Gateway endpoint detail page. Flattened from the proto `endpoint`
  // oneof variant.
  | (TraceServiceInfoBase & { kind: 'LLM' })
  // UC Model Provider Service — its own UC securable with a Catalog Explorer
  // detail page, so it links differently from an endpoint even though its name
  // is a three-part UC FQN. Flattened from the proto `model_provider_service`
  // oneof variant.
  | (TraceServiceInfoBase & { kind: 'MPS' })
  | (TraceServiceInfoBase & { kind: 'MCP' })
  | (TraceServiceInfoBase & { kind: 'TOOL' })
  | (TraceServiceInfoBase & { kind: 'UNKNOWN' });
...
export type ModelTraceInfoV3 = {
  ...
  // Root span id associated with this trace.
  span_id?: string;
  // Service that produced this trace. Pre-flattened by the API layer from the
  // structured `TraceService` proto oneof into a display name plus a kind
  // that drives the pill label shown next to the name in the table.
  service?: TraceServiceInfo;
  // Caller principal (e.g. service principal or user identity).
  principal?: string;
  trace_metadata?: {
    [key: string]: string;
  };
  assessments?: Assessment[];
  // Root-span error message, surfaced in the status-badge tooltip; typically
