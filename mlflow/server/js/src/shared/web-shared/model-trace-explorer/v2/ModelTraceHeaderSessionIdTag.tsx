...
export const ModelTraceHeaderSessionIdTag = ({
  ...
  const selectedTraceId = new URLSearchParams(location.search ?? '').get(SELECTED_TRACE_ID_QUERY_PARAM);
  // If already on the same session and same selected trace, clicking the Session ID should copy it to clipboard
  // instead of navigating (which would be a no-op). If the trace differs, keep the link so the click jumps there.
  const isOnSessionPage = location.pathname.includes(`/chat-sessions/${sessionId}`);
  const isOnCurrentTraceSessionPage = isOnSessionPage && (!traceId || selectedTraceId === traceId);

  if (isOnCurrentTraceSessionPage) {
    return (
