...
import { restoreServerSearchHierarchy, useServerSideTraceSearch } from './useServerSideTraceSearch';
...
describe('useServerSideTraceSearch', () => {
  ...
  it('uses a caller-provided parser for redesigned explorer nodes', async () => {
    mockGetExperimentTraceV3.mockResolvedValue({
      trace: {
        trace_info: MOCK_TRACE_INFO,
        spans: [{ span_id: 's1', name: 'matched-span' }],
    ...
    const parseTraceToTree = jest.fn(() => [{ key: 's1', title: 'v2-node', tokenUsage: { total_tokens: 42 } }]);
    const { result } = renderHook(() =>
      useServerSideTraceSearch({ modelTraceInfo: MOCK_TRACE_INFO, enabled: true, parseTraceToTree }),
    );

    act(() => result.current.setSearchFilter('matched'));
    await act(async () => {
      jest.advanceTimersByTime(500);
    });

    expect(parseTraceToTree).toHaveBeenCalledTimes(1);
    expect(result.current.filteredTreeNodes).toEqual([
      { key: 's1', title: 'v2-node', tokenUsage: { total_tokens: 42 } },
    ]);
  });
...
describe('restoreServerSearchHierarchy', () => {
  type TestNode = {
    key: string;
    children?: TestNode[];
  ...
  const sourceNodes: TestNode[] = [
    {
      key: 'root',
      children: [{ key: 'matching-child' }, { key: 'exception-child', events: [{ name: 'exception' }] }],
    },
  ];

  it('keeps parent context and exception spans around server matches', () => {
    expect(restoreServerSearchHierarchy(sourceNodes, [{ key: 'matching-child' }], true, true)).toEqual([
      {
        key: 'root',
        children: [
          { key: 'matching-child', children: [] },
          { key: 'exception-child', events: [{ name: 'exception' }], children: [] },
        ],
      },
    ]);
  });

  it('promotes matches when parent display is disabled', () => {
    expect(restoreServerSearchHierarchy(sourceNodes, [{ key: 'matching-child' }], false, false)).toEqual([
      { key: 'matching-child', children: [] },
    ]);
  });
});
