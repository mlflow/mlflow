...
2. "$spanRef" markers — fill a feedback control's "spanId" so it targets the right span in EVERY trace. Write one of: { "$spanRef": "root" }, { "$spanRef": { "type": "<SPAN_TYPE>", "nth"?: <n> } }, or { "$spanRef": { "name": "<span name>" } }. If nothing matches, the host drops the spanId and logs the feedback at trace level. Prefer a type + nth selector over a name copied from the authoring trace. A feedback control's "spanId" uses the WRAPPED "$spanRef" marker; a spanField's "spanRef" uses the BARE selector.
