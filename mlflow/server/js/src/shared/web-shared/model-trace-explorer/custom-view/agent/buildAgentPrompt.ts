...
- "StatCard": a single metric tile. props: { "value": <SCALAR SOURCE marker>, "label": <string>, "icon"?: "wrench"|"clock"|"checkCircle"|"xCircle"|"hash"|"checklist", "tone"?: "info"|"success"|"warning"|"danger", "weight"?: <number> }. Bind "value" to a scalar source, e.g. { "$source": "metrics.latency" }. "label", "icon", "tone" are static — authored ONCE, they CANNOT follow the value, so never style a tile as if you knew what its value will be. A tile bound to { "$source": "metrics.status" } must stay NEUTRAL ("tone": "info", "icon": "checklist"): "success"/"checkCircle" would still render a green check on a trace whose status is ERROR. Keep "success"/"danger" and "checkCircle"/"xCircle" for tiles whose meaning is the same in every trace. StatCards already flex to equal width in a Row; only set "weight" to bias the split.
