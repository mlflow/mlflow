...
import React__default, { forwardRef, useMemo, useRef, useEffect, useState, useContext, createContext, useCallback, useLayoutEffect } from 'react';
import { I as Icon, t as DesignSystemEventSuppressInteractionProviderContext, v as DesignSystemEventSuppressInteractionTrueContextValue, u as useDesignSystemTheme, s as safex, e as useDesignSystemEventComponentCallbacks, f as DesignSystemEventProviderComponentTypes, j as useNotifyOnFirstView, d as DesignSystemEventProviderAnalyticsEventTypes, w as augmentWithDataComponentProps, T as Typography, B as Button, C as CloseIcon, x as addDebugOutlineStylesIfEnabled, b as getAnimationCss, y as getDarkModePortalStyles, z as getShadowScrollStyles, A as useDesignSystemContext, D as DesignSystemAntDConfigProvider, a as addDebugOutlineIfEnabled, E as RestoreAntDDefaultClsPrefix, k as DangerIcon, i as importantify, o as ChevronRightIcon, n as ChevronLeftIcon, W as WarningIcon, L as LoadingIcon, G as DEFAULT_SPACING_UNIT, R as Root$1, l as Trigger$1, m as Content$2 } from './Popover-Ct_RJSr_.js';
import { css, keyframes, Global, createElement } from '@emotion/react';
import noop from 'lodash/noop';
import { ResizableBox } from 'react-resizable';
import pick from 'lodash/pick';
import compact from 'lodash/compact';
import * as RadixTooltipOriginal from '@radix-ui/react-tooltip';
import * as RadixTooltipPatched from '@radix-ui/react-tooltip-patch';
import { Modal as Modal$2 } from 'antd';
...
const modalContentStyles = (theme, maxedOutHeight, useNewShadows, useNewBorderRadii, useNewBorderColors)=>{
...
function closeButtonComponentId$1(componentId) {
    return componentId ? `${componentId}.footer.cancel` : 'modal.close';
}
    ...
    return /*#__PURE__*/ jsxs(Fragment, {
        children: [
            cancelText && /*#__PURE__*/ jsx(Button, {
                // eslint-disable-next-line @databricks/no-dynamic-property-value -- http://go/static-frontend-log-property-strings
                componentId: closeButtonComponentId$1(componentId),
...
function ModalInternal$1({ componentId, analyticsEvents = [
    ...
    const dialogContent = /*#__PURE__*/ jsx(DialogPrimitive.Content, {
        ...
        children: /*#__PURE__*/ jsxs("div", {
            ...
            css: [
                ...
                modalContentStyles(theme, verticalSizing === 'maxed_out', useNewShadows, useNewBorderRadii, useNewBorderColors)
            ],
            style: {
    ...
    const dialogContentWithMask = /*#__PURE__*/ jsx("div", {
        role: "document",
        "aria-hidden": !open,
        className: classnames(`${classNamePrefix}-modal`, wrapClassName),
    ...
    return /*#__PURE__*/ jsx(DialogPrimitive.Root, {
        ...
        children: /*#__PURE__*/ jsx(DialogPrimitive.Portal, {
            ...
            forceMount: destroyOnClose ? undefined : true,
            children: visible ? modalRender ? modalRender(dialogContentWithMask) : dangerouslySetAntdProps?.modalRender ? dangerouslySetAntdProps.modalRender(dialogContentWithMask) : dialogContentWithMask : null
        })
    });
}
...
const useTooltipStyles = ({ maxWidth, usePatchedTooltip })=>{
    const { theme, classNamePrefix: clsPrefix } = useDesignSystemTheme();
    const useNewShadows = safex('databricks.fe.designsystem.useNewShadows', false);
    const dataStateAttr = usePatchedTooltip ? 'tooltip-data-state' : 'data-state';
 ...
 */ const Tooltip = ({ children, content, defaultOpen = false, delayDuration = 350, side = 'top', sideOffset, align = 'center', maxWidth = 250, componentId, analyticsEvents = [
    ...
    const { theme } = useDesignSystemTheme();
    const { getPopupContainer } = useDesignSystemContext();
    const usePatchedTooltip = safex('databricks.fe.designsystem.usePatchedTooltip', false);
    const tooltipStyles = useTooltipStyles({
        maxWidth,
        usePatchedTooltip
    });
    const RadixTooltip = usePatchedTooltip ? RadixTooltipPatched : RadixTooltipOriginal;
    const memoizedAnalyticsEvents = useMemo(()=>analyticsEvents, [
        analyticsEvents
    ]);
    const eventContext = useDesignSystemEventComponentCallbacks({
...
const NavButton = /*#__PURE__*/ React__default.forwardRef(({ active, disabled, icon, onClick, children, dangerouslyAppendEmotionCSS, 'aria-label': ariaLabel, ...restProps }, ref)=>{
    const { theme } = useDesignSystemTheme();
    return /*#__PURE__*/ jsx("div", {
        css: [
            active ? importantify({
...
function VerticalWizardContent({ width, height, steps: wizardSteps, currentStepIndex, localizeStepNumber, onStepsChange, title, padding, verticalCompactButtonContent, enableClickingToSteps, verticalDocumentationSidebarConfig, hideDescriptionForFutureSteps = false, contentMaxWidth, ...footerProps }) {
    const { theme } = useDesignSystemTheme();
    const stepperSteps = useStepperStepsFromWizardSteps(wizardSteps, currentStepIndex, hideDescriptionForFutureSteps);
    const expandContentToFullHeight = wizardSteps[currentStepIndex].expandContentToFullHeight ?? true;
    const disableDefaultScrollBehavior = wizardSteps[currentStepIndex].disableDefaultScrollBehavior ?? false;
...
//# sourceMappingURL=WizardStepContentWrapper-D5_Hpkqa.js.map
