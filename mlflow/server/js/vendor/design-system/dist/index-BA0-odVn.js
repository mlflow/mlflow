...
const getInputStyles = (clsPrefix, theme, param, param1)=>{
    let { validationState, type, hasValue, useNewShadows, useNewFormUISpacing, useNewBorderRadii, locked } = param, { useFocusWithin = false } = param1;
    const inputClass = `.${clsPrefix}-input`;
...
const getInputEmotionStyles = (clsPrefix, theme, param)=>{
    let { validationState, type, hasValue, useNewShadows, useNewBorderRadii, locked } = param;
...
const Input$1 = /*#__PURE__*/ forwardRef(function Input(param, ref) {
    let { validationState, autoComplete = 'off', dangerouslySetAntdProps, dangerouslyAppendEmotionCSS, onChange, onClear, onFocus, onPressEnter, onCompositionStart, onCompositionEnd, componentId, shouldPreventFormSubmission, analyticsEvents, readOnly, locked, ...props } = param;
    ...
    const { useNewShadows, useNewBorderRadii } = useDesignSystemSafexFlags();
    const memoizedAnalyticsEvents = useMemo(()=>analyticsEvents !== null && analyticsEvents !== void 0 ? analyticsEvents : emitOnView ? [
    ...
    const onFormSubmit = useCallback((e)=>{
        var _formContext_formRef;
        if (!((_formContext_formRef = formContext.formRef) === null || _formContext_formRef === void 0 ? void 0 : _formContext_formRef.current)) return;
        e.preventDefault();
...
export { ClockIcon as C, Input as I, LockIcon as L, MegaphoneIcon as M, SearchIcon as S, getInputStyles as g, useCallbackOnEnter as u };
//# sourceMappingURL=index-BA0-odVn.js.map
