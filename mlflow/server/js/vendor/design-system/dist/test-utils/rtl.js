...
import { s as selectClasses, c as createMarkdownTable } from '../common-Ce1R0mCb.js';
 ...
 */ function getTableRowByCellText(tableElement, cellText) {
    let { columnHeaderName } = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {};
    const columnHeaderIndex = columnHeaderName === undefined ? 0 : getColumnHeaderIndex(tableElement, columnHeaderName);
    const matchingRows = within(tableElement).getAllByRole('row')// Skip first row (table header)
    .slice(1).filter((row)=>{
 ...
 */ function toMarkdownTable(tableElement) {
    ...
    const columns = within(headerRow).getAllByRole('columnheader').map((column)=>column.textContent ?? '');
