...
describe('isWizardStepEnabled()', () => {
  function expectIsWizardStepEnabled({
    steps,
    currentStepIdx,
    expectedEnabled,
  }: {
  ...
  test('Only first step is enabled if all steps are in upcoming', () => {
    const steps = [{ status: 'upcoming' }, { status: 'upcoming' }, { status: 'upcoming' }] as WizardStep[];
  ...
  test('The first upcoming step after a series of completed steps is enabled', () => {
    const steps = [{ status: 'completed' }, { status: 'upcoming' }, { status: 'upcoming' }] as WizardStep[];
    const currentStepIdx = 0;
  ...
  test('warning, error, or completed are enabled', () => {
    const steps = [{ status: 'completed' }, { status: 'warning' }, { status: 'error' }] as WizardStep[];
    const currentStepIdx = 0;
...
describe('useStepperStepsFromWizardSteps', () => {
  test.each([
    { currentStepIdx: 1, step1Status: 'upcoming' as const },
    { currentStepIdx: 0, step1Status: 'error' as const },
  ])(
    'description should be undefined for step not reached yet and rendered for steps reached if hideDescriptionForFutureSteps is true',
    ({ currentStepIdx, step1Status }) => {
