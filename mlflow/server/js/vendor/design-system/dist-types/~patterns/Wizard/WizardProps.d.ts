...
export interface WizardProps {
     ...
     * When the wizard container is narrower than the compact breakpoint (see compactStepperBreakpoint),
     * the vertical stepper is hidden and this popover trigger is shown instead.
     * Compact mode is driven by the wizard container width (ResizeObserver), not viewport width.
     *
  ...
  Trigger. This allows the client to display different help based on the contentId trigger clicked.
    ...
    /**
     * Container width breakpoint (in px) below which the vertical stepper switches to compact mode
     * (popover trigger instead of inline stepper). Uses ResizeObserver on the wizard container,
     * so behavior is based on the container width, not the viewport.
     *
     * @default theme.responsive.breakpoints.lg
     */
    compactStepperBreakpoint?: number;
    /**
     * Whether the compact stepper/sidebar breakpoint is driven by viewport or container width.
     * - `'viewport'`: uses viewport width; resizing the browser window toggles compact mode.
     * - `'container'`: uses container width; use when the wizard is inside a panel or sidebar.
     *
     * @default 'viewport'
     */
    compactBreakpointBehavior?: 'container' | 'viewport';
    /**
     * When provided, the compact stepper will be rendered into this DOM element via a React portal
     * instead of inline above the content card. This allows the parent to place the compact stepper
     * in a custom location (e.g., inside a header).
     */
    compactStepperPortalTarget?: HTMLElement | null;
}
