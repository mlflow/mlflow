    ...
    if _should_add_pyfunc_to_model(built_pipeline):
        ...
        if signature is None:
            mlflow_model.signature = _get_default_pipeline_signature(
                built_pipeline, input_example, model_config
            )
...
def _get_default_pipeline_signature(pipeline, example=None, model_config=None) -> ModelSignature:
    ...
    if example:
        try:
            prediction = generate_signature_output(pipeline, example, model_config)
...
@experimental
def generate_signature_output(pipeline, data, model_config=None):
    """
    ...
    :param model_config: Any additional inference configuration, provided as kwargs, to inform
    ...
    return _TransformersWrapper(pipeline=pipeline, model_config=model_config).predict(data)
