"""``mlflow.genai.assert_batch`` -- the imperative assertion API.
...
def _resolve_trace(
    ...
    raise TypeError(
        'assert_batch(trace=...) must be a Trace, the literal "auto", or None. '
...
def assert_batch(
    ...
    *,
    ...
    if not assertions:
        raise ValueError(
            "assert_batch(...) requires at least one assertion. Pass a rubric "
    ...
    session.record(test_name or "<assert_batch>", results)
    ...
    # tells the user *why* it failed, not just that assert_batch() raised.
    if len(failures) == 1:
