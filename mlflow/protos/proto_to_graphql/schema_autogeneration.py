from autogeneration_utils import INDENT, INDENT2, debugLog, get_method_name, get_descriptor_full_pascal_name
...
def generate_schema(state):
    ...
    # types, inputs, queries, mutations, oneofs, input oneofs, enum
...
def get_graphene_type_for_field(field, is_input):
    if field.type == FieldDescriptor.TYPE_ENUM:
        referenced_class_name = get_descriptor_full_pascal_name(field.enum_type)
        ...
        if is_input:
            referenced_class_name = f"{get_descriptor_full_pascal_name(field.message_type)}Input"
            field_type_base = f"graphene.InputField({referenced_class_name})"
        else:
            referenced_class_name = get_descriptor_full_pascal_name(field.message_type)
...
def generate_resolver_function(method):
    ...
    function_builder += f"\n{INDENT2}request_message = mlflow.protos.service_pb2.{pascal_case_method_name}()" # TODO: Sub out service_pb2
