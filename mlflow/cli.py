...
@cli_args.ACTIVATE_SCHEDULER
@click.option("--scheduler-configuration", default=None,
              help="Path to the periodic jobs configuration file.")
def server(backend_store_uri, default_artifact_root, host, port, workers, static_prefix,
           gunicorn_opts, waitress_opts, activate_scheduler=False, scheduler_configuration=None):
    """
    ...
    try:
        _run_server(backend_store_uri, default_artifact_root, host, port, static_prefix, workers,
                    gunicorn_opts, waitress_opts, activate_scheduler, scheduler_configuration)
    except ShellCommandException:
        eprint("Running the mlflow server failed. Please see the logs above for details.")
        sys.exit(1)
