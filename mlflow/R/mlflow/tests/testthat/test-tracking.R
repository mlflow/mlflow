...
test_that("mlflow_log_param()/mlflow_get_param() work properly", {
  ...
  client <- mlflow_client()
  expect_identical(
    mlflow_get_param(client = client, "some_param", run_id = mlflow_get_active_run()$info$run_uuid),
...
test_that("mlflow_log_metric()/mlflow_get_metric() work properly", {
  ...
  expect_identical(
    mlflow_get_metric(
      client = client, run_id = mlflow_get_active_run()$info$run_uuid,
      "some_metric"
    ),
    data.frame(
...
test_that("mlflow_get_metric_history() works properly", {
  ...
  metric_history <- mlflow_get_metric_history(
    client = client, run_id = mlflow_get_active_run()$info$run_uuid, "some_metric"
  )
  expected <- data.frame(
