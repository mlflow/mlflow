...
test_that("mlflow_cli handles explicit and null clients when setting URI env", {
  old_tracking <- mlflow_get_tracking_uri()
  old_registry <- .globals$registry_uri
  on.exit({
    mlflow_set_tracking_uri(old_tracking)
  ...
  mlflow_set_tracking_uri("http://global-tracking")
  mlflow_set_registry_uri("http://global-registry")
  ...
  expect_equal(calls[[1]]$tracking_uri, "databricks")
  expect_equal(calls[[1]]$registry_uri, "databricks-uc")
  expect_equal(calls[[2]]$tracking_uri, "http://global-tracking")
  expect_equal(calls[[2]]$registry_uri, "http://global-registry")
})
