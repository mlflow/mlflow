...
test_that("http(s) clients work as expected", {
  with_mock(.env = "httr", GET = function(url, query, timeout, config, headers) {
    ...
    with_mock(.env = "mlflow", mlflow_register_local_server = function(...) NA, {
      ...
      with_envvar(env, {
        ...
        host_creds <- client1$get_host_creds()
        expect_true(host_creds$host == http_host)
        expect_true(host_creds$username == "DonaldDuck")
        expect_true(host_creds$password == "Quack")
        expect_true(host_creds$token == "$$$")
        expect_true(host_creds$insecure == "True")
        https_host <- "https://remote"
        client2 <- mlflow:::mlflow_client("https://remote")
        host_creds <- client2$get_host_creds()
        expect_true(host_creds$host == https_host)
      ...
      with_mock(.env = "mlflow", mlflow_server = function(...) list(server_url = "local_server"), {
        ...
        host_creds <- client3$get_host_creds()
        expect_true(host_creds$host == "local_server")
      })
    })
  })
})
