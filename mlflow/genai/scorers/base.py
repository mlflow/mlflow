...
def scorer(
        ...
        *,
        ...
        aggregations: list[Union[Literal["min", "max", "mean", "median", "variance", "p90", "p99"], Callable]] = None
    ):
    """
    Syntactic sugar that ensures the decorated function has the correct parameters (with inputs
    as required, outputs, expectations, trace as optional) and returns float | bool | str | Assessment.
    """
    ...
    if "inputs" not in params:
        raise TypeError(f"{func.__name__} must accept 'inputs' as a required keyword-only argument.")
    ...
    def wrapped_call(self, *, inputs, outputs=None, expectations=None, trace=None):
        ...
        if not isinstance(result, (float, bool, str, Assessment)):
            raise ValueError(
                f"{func.__name__} must return one of float, bool, str, or Assessment. Got {type(result).__name__}"
            )
        return result
