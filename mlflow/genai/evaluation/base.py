import mlflow
from mlflow.genai.scorers import Scorer, BuiltInScorer
...
def evaluate(
    ...
    if mlflow.get_tracking_uri() != "databricks":
        raise ValueError(
            "The genai evaluation function is only supported on Databricks. Please set the tracking URI to Databricks."
        )
    ...
    for scorer in builtin_scorers:
        scorer(evaluation_config)

    extra_metrics = []
    for scorer in custom_scorers:
        extra_metrics.append(
            # TODO: is there a better way to turn this into a metric?
            make_metric(
                eval_fn=scorer,
