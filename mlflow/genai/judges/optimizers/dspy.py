...
class DSPyAlignmentOptimizer(AlignmentOptimizer):
    ...
    def _extract_text_from_data(self, data: Any, field_type: str) -> str:
        ...
        if isinstance(data, dict):
            ...
            if field_type == 'request':
                ...
                raise AssertionError(f"Invalid field_type: {field_type}. Must be 'request' or 'response'.")
    ...
    def _extract_request_from_trace(self, trace: Trace) -> str:
        ...
        request_data = (
            trace.data.request if hasattr(trace.data, 'request') 
    ...
    def _extract_response_from_trace(self, trace: Trace) -> str:
        ...
        response_data = (
            trace.data.response if hasattr(trace.data, 'response') 
    ...
    def _trace_to_dspy_example(self, trace: Trace, judge_name: str) -> Optional[Any]:
        """
        ...
        try:
            ...
            if trace.info.assessments:
                for assessment in trace.info.assessments:
                    if (assessment.name == sanitized_judge_name and 
                        assessment.source.source_type == 'HUMAN'):
                        expected_result = assessment
                        break
            
            if not expected_result:
                self._logger.warning(f"No human assessment found for judge '{judge_name}' in trace {trace.info.trace_id}")
                return None
            
            if not expected_result.feedback:
                self._logger.warning(f"No feedback found in assessment for trace {trace.info.trace_id}")
                return None
    ...
    def align(self, judge: Judge, traces: List[Trace]) -> Judge:
        ...
        try:
            ...
            with dspy.context(lm=dspy_model):
                ...
                self._logger.info(f"Created {len(dspy_examples)} valid examples from {len(traces)} traces")
                ...
                val_examples = dspy_examples[split_point:] if len(dspy_examples) > split_point else train_examples[:1]
                ...
                optimized_program = self._dspy_optimize(program, train_examples, val_examples, agreement_metric)
            ...
            raise MlflowException(f"Alignment optimization failed: {str(e)}")
