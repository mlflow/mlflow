...
def is_conversation_safe(
    *,
    ...
    """
    LLM judge determines whether an entire multi-turn conversation is safe.

    This judge evaluates the complete conversation for various safety concerns including:
    ...
    Returns:
        A :py:class:`mlflow.entities.assessment.Feedback~` object with a "yes" or "no"
        value indicating whether the entire conversation is safe.

    Example:

        .. code-block:: python

            from mlflow.genai.judges import is_conversation_safe

            # Safe conversation
            feedback = is_conversation_safe(
                messages=[
                    {"role": "user", "content": "What is the capital of France?"},
                    {"role": "assistant", "content": "The capital of France is Paris."},
                    {"role": "user", "content": "What is the population?"},
                    {"role": "assistant", "content": "About 2.1 million people."},
                ]
            )
            print(feedback.value)  # "yes"

            # Unsafe conversation with jailbreak attempt
            feedback = is_conversation_safe(
                messages=[
            ...
            print(feedback.value)  # "no" (contains jailbreak attempt)
    """
    from mlflow.genai.judges.prompts.conversational_safety import (
        CONVERSATIONAL_SAFETY_ASSESSMENT_NAME,
        format_conversation,
        get_prompt,
    )

    if not messages:
        raise MlflowException("messages cannot be empty")

    model = model or get_default_model()
    assessment_name = name or CONVERSATIONAL_SAFETY_ASSESSMENT_NAME

    # Format the conversation for the prompt
    conversation_str = format_conversation(messages)
    prompt = get_prompt(conversation=conversation_str)
    feedback = invoke_judge_model(
        model,
        prompt,
        assessment_name=assessment_name,
