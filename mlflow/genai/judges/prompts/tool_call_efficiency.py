...
def _format_tools_called(tools_called: list[dict[str, Any]]) -> str:
    ...
    Args:
        tools_called: The sequence of tools that were called by the agent.
            Each element should be a dictionary containing "name", "inputs", and "outputs".
    ...
    Example:
        ...
        >>> #   Output: París
    """
    valid_tools = [tool for tool in tools_called if "name" in tool]

    tools_called_parts = []
    for idx, tool in enumerate(valid_tools, start=1):
        tool_name = tool["name"]
        tool_args = tool.get("inputs", {})
        tool_output = tool.get("outputs", "(no output)")
...
def get_prompt(
    request: str, tools_called: list[dict[str, Any]], available_tools: list["ChatTool"]
) -> str:
