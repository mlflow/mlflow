...
def _serve():
    ...
    if pyfunc.ENV in conf:
        ...
        env_name = "custom_env"
        os.system(
            "conda env create -n {en} -f {ep}".format(en=env_name, ep=env_path_dst))
    ...
    nginx_conf = resource_filename(
        mlflow.sagemaker.__name__, "container/scoring_server/nginx.conf")
    ...
    signal.signal(signal.SIGTERM, lambda a,
                  b: _sigterm_handler(nginx.pid, gunicorn.pid))
