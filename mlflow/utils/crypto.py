"""
Cryptographic utilities for MLflow secrets management (encryption and decryption).

This module provides envelope encryption (dual-layer security) for secrets using:

KEK (Key Encryption Key): Derived from MLFLOW_SECRETS_KEK_PASSPHRASE
DEK (Data Encryption Key): Random AES-256 key per secret
AES-256-GCM: Authenticated encryption with associated data (AEAD)

For more information on the approach here, see https://csrc.nist.gov/pubs/fips/197/final for
a description of AES, and https://nvlpubs.nist.gov/nistpubs/FIPS/NIST.FIPS.197-upd1.pdf for a
description of the Galois/Counter Mode of operation (GCM).

General Order of operations:
    1. Derive KEK from passphrase that is set server-side in an ENV VAR
        MLFLOW_SECRETS_KEK_PASSPHRASE (PBKDF2-HMAC-SHA256)
    2. Generate random DEK for each secret that will be stored with the encyrypted secret
        in the database secrets table
    3. Encrypt secret value with the generated DEK (AES-256-GCM)
    4. Wrap (encrypt) DEK with KEK (double layer protection)
    5. Store encrypted value + wrapped DEK in database along with the nonce (IV) used for AES-GCM
    6. To decrypt, unwrap DEK with KEK, then decrypt secret value with DEK (server-side only)
"""
...
#
# NOTE: We use a fixed salt because our use case is KEY DERIVATION, not password storage.
...
class KEKManager:
    """
    Manages Key Encryption Keys (KEK) for MLflow secrets.
    ...
    and updating DEKs in the database) or via the MLFLOW_SECRETS_KEK_PASSPHRASE environment
    variable (which is used during normal server operation).

    Args:
        passphrase: Optional passphrase. If None, reads from MLFLOW_SECRETS_KEK_PASSPHRASE env var.
        kek_version: Optional version identifier for this KEK. If None, reads from
            MLFLOW_SECRETS_KEK_VERSION env var (default 1). Used to track which KEK version
    ...
    def __init__(self, passphrase: str | None = None, kek_version: int | None = None):
        ...
        if passphrase is None:
            passphrase = os.getenv("MLFLOW_SECRETS_KEK_PASSPHRASE")

        if kek_version is None:
            kek_version = int(os.getenv("MLFLOW_SECRETS_KEK_VERSION", "1"))
...
def rotate_secret_encryption(
    ...
    4. Update MLFLOW_SECRETS_KEK_PASSPHRASE to the NEW passphrase in deployment config
