    ...
    try:
        ...
        if settings.store_hit:
            ...
            if original_name in patch.destination.__dict__:
                raise RuntimeError(
                    f'Patch for {patch.destination.__name__}.{patch.name} already exists.')
...
def revert(patch):
    ...
    #  2. We need bypass the descriptor protocol, `getattr` will call
    #     `__get__` method on the raw attribute object and may return a new object
    #     generated by `raw_attribute.__get__()`
