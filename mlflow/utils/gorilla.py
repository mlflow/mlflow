...
def apply(patch):
    ...
    try:
        ...
        if settings.store_hit:
            ...
            # Note: the patching rule is, we only patch the destination class,
            #   it don't modify the parent class. The following `setattr` will only add attribute
            ...
            # TODO:
            #  If old patch already exists,
            #  Current implementation is overwrite old patch if the patch
            #  in current destination (not include parent class) exists
            #  Overwrite will happen in this case:
            #   user enable autolog twice.
            #  We should improve this, when autolog() called in second time, remove old patches
            #  first, then enable new patches (with new autolog config), this way is safer.
            setattr(patch.destination, patch.name, patch.obj)
...
def revert(patch):
    ...
    # Note: We cannot use `original = getattr(patch.destination, 'original_name')` here, because:
    #  1. `getattr` may get attribute inherit from parent. But according to the patching rule,
    #     we only patch on the destination class and the original object will be only stored as
    #     destination class attributes.
    #  2. If the attribute defined a getter, `getattr` will call
    #     `__get__` method on the raw attribute object and return a new object
    #     generated by `patch.destination.__dict__[original_name].__get__()`
    #     but for such case we must restore the original raw attribute.
    ...
    original = patch.destination.__dict__[original_name]
