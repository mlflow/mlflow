...
class SearchUtils(object):
    ...
    _VALID_IDENTIFIERS = set(_IDENTIFIERS + list(_ALTERNATE_METRIC_IDENTIFIERS) +
                             list(_ALTERNATE_PARAM_IDENTIFIERS) + list(_ALTERNATE_TAG_IDENTIFIERS) +
    ...
    def _get_value_for_sort(cls, run, key_type, key, ascending):
        ...
        is_null_or_nan = sort_value is None or (isinstance(sort_value, float) and
                                                math.isnan(sort_value))
        if ascending:
    ...
    def _parse_filter_for_model_registry(cls, filter_string, valid_search_keys):
        ...
        if len(invalids) > 0:
            ...
            raise MlflowException(
                ...
                error_code=INVALID_PARAMETER_VALUE)
        return [
            cls._get_comparison_for_model_registry(si, valid_search_keys) for si in statement.tokens
            if isinstance(si, Comparison)
        ]

    @classmethod
    def parse_filter_for_model_versions(cls, filter_string):
        return cls._parse_filter_for_model_registry(filter_string,
