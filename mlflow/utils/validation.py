...
# contain dots or whitespace. The goal is to make our names be valid, unique path names too in
# order to treat them as such in the UI and file store.
_METRIC_AND_PARAM_NAME_REGEX = re.compile(
    r"^([\w.\- ]*[\w\-][\w.\- ]*)(/[\w.\- ]*[\w\-][\w.\- ]*)*$")
...
def _validate_metric_name(name):
    ...
    if _METRIC_AND_PARAM_NAME_REGEX.match(name) is None:
        raise Exception("Invalid metric name: '%s'. %s" % (name, CONCISE_EXPLANATION_MESSAGE))


def _validate_param_name(name):
    """Check that `name` is a valid parameter name and raise an exception if it isn't."""
    if _METRIC_AND_PARAM_NAME_REGEX.match(name) is None:
        raise Exception("Invalid parameter name: '%s'. %s" % (name, CONCISE_EXPLANATION_MESSAGE))
