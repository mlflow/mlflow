...
"""
Thin wrappers for psutil functions.
As of 2023 Nov psutil doesn't have binary for Arm64 Linux, requireing building from CPython source. To
avoid headache of building with GCC, we make psutil optional and show user-friendly message when it's
required but not installed.
"""


def _check_and_import_psutil():
    try:
        import psutil
    except ImportError as e:
        raise ImportError(
            str(e) + "\n\nHINT: The `psutil` package is required to log system metrics in "
            "MLflow. Run `pip install psutil --upgrade` to install the package. If you are"
            " on an Arm64-based Linux system, you may need to have GCC installed to build "
