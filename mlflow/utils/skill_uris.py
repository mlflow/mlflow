...
def _split_uri(uri: str, scheme: str) -> tuple[str, str, str | None, str | None]:
    ...
    alias: str | None = None
    if "@" in remainder:
        match remainder.split("@", 1):
            case [name, alias]:
                pass
    elif "/" in remainder:
        match remainder.split("/", 1):
            case [name, version_str]:
                pass
    else:
        name = remainder
...
def parse_skill_uri(uri: str) -> ParsedSkillUri:
    ...
    if version_str is not None:
        try:
            ...
            raise MlflowException.invalid_parameter_value(
                f"Invalid skill version {version_str!r}: must be an integer."
            ) from None
...
def format_skill_uri(parsed: ParsedSkillUri) -> str:
    prefix = _SKILL_SCHEME
    if parsed.organization:
        prefix += f"@{parsed.organization}/"
    result = f"{prefix}{parsed.name}"
    if parsed.version is not None:
        result += f"/{parsed.version}"
    elif parsed.alias is not None:
        result += f"@{parsed.alias}"
    return result
