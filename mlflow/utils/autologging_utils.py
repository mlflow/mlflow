...
def param_logger(fn, args, kwargs, unlogged=[]):
    """
    ...
    kwargs_and_default_names = all_param_names[len(all_param_names) - len(args):]

    # If there are more parameter names than default values left, we know that the parameters not
    # covered by the default values are passed in as kwargs (assuming all non-default parameters
    ...
    for param in default_params:
        if param[0] not in kwargs:
            default_params_to_be_logged.append(param)
