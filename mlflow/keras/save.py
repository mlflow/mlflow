...
def save_model(
    ...
    Args:
        ...
        path: string, local path where the MLflow model is to be saved.
        save_exported_model: bool, defaults to False. If True, save Keras model in exported model
            format, otherwise save in `.keras` format. For more information, please refer to
        ...
        save_model_kwargs: dict, defaults to None. A dict of kwargs to pass to `keras.Model.save`
            method.
...
def log_model(
    ...
    Args:
        model: an instance of `keras.Model`. The Keras model to be saved.
        artifact_path: string, the run-relative path to which to log model artifacts.
        save_exported_model: bool, defaults to False. If True, save Keras model in exported model
            format, otherwise save in `.keras` format. For more information, please refer to
            https://keras.io/guides/serialization_and_saving/.
        conda_env: {{ conda_env }}
        signature: {{ signature }}
        input_example: {{ input_example }}
        registered_model_name: string, defaults to None. If set, create a model version under
            `registered_model_name`, also create a registered model if one with the given name does
            not exist.
        await_registration_for: int, defaults to
