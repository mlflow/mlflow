...
from sqlalchemy import (Column, String, ForeignKey, Integer, BigInteger, PrimaryKeyConstraint,
...
class SqlModelVersion(Base):
    ...
    # entity mappers
    def to_mlflow_entity(self):
        return ModelVersion(self.name, self.version, self.creation_time, self.last_updated_time,
                            self.description, self.user_id, self.current_stage, self.source,
                            self.run_id, self.status, self.status_message,
                            [tag.to_mlflow_entity()
                             for tag in self.model_version_tags], self.run_link)
...
class SqlModelVersionTag(Base):
    ...
    __table_args__ = (PrimaryKeyConstraint('key', 'name', 'version', name='model_version_tag_pk'),
                      ForeignKeyConstraint(('name', 'version'),
                                           ('model_versions.name', 'model_versions.version'),
                                           onupdate='cascade'))

    def __repr__(self):
        return '<SqlModelVersionTag ({}, {}, {}, {})>'.format(self.name, self.version, self.key,
                                                              self.value)
