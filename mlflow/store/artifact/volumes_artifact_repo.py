...
class VolumesRestArtifactRepository(ArtifactRepository):
    """
    Stores artifacts on Volumes using the Files REST API.

    This repository is used with URIs of the form `dbfs:/Volumes/<path>`. The repository can only be used
    ...
    def _is_directory(self, artifact_path):
        volume_path = self._get_volumes_path(artifact_path) if artifact_path else self._get_volumes_path("")
...
def volumes_artifact_repo_factory(artifact_uri):
    ...
    This factory method is used with URIs of the form ``dbfs:/Volumes/<path>``. Volume-backed artifact
