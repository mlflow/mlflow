...
# create_multipart_upload additionally accepts these SSE-KMS args, which aren't valid for
# upload_part/complete_multipart_upload/abort_multipart_upload. Unlike those calls, arbitrary
# MLFLOW_S3_UPLOAD_EXTRA_ARGS keys (e.g. ACL) are intentionally not forwarded.
# https://boto3.amazonaws.com/v1/documentation/api/latest/reference/services/s3/client/create_multipart_upload.html
ALLOWED_MPU_CREATE_ARGS = ALLOWED_MPU_EXTRA_ARGS | frozenset({
    "BucketKeyEnabled",
...
class S3ArtifactRepository(
    ...
    def create_multipart_upload(self, local_file, num_parts=1, artifact_path=None):
        ...
        create_args = {k: v for k, v in extra_args.items() if k in ALLOWED_MPU_CREATE_ARGS}
