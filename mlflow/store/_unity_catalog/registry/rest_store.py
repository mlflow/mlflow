...
from mlflow.protos.databricks_uc_registry_service_pb2 import (
    UnityCatalogModelRegistryService,
)
...
# (UnityCatalogModelRegistryService); prompt endpoints remain on /api/2.0/mlflow/unity-catalog/*.
_METHOD_TO_INFO = {
    **extract_api_info_for_service(UnityCatalogPromptService, _REST_API_PATH_PREFIX),
    **extract_api_info_for_service(UnityCatalogModelRegistryService, _UC_OSS_REST_API_PATH_PREFIX),
}
_METHOD_TO_ALL_INFO = {
...
class UcModelRegistryStore(BaseRestStore):
    ...
    def create_registered_model(self, name, tags=None, description=None, deployment_job_id=None):
        ...
        endpoint, method = self._get_endpoint_from_method(CreateRegisteredModel)
        try:
    ...
    def update_registered_model(self, name, description=None, deployment_job_id=None):
        ...
        endpoint, method = self._get_endpoint_from_method(UpdateRegisteredModel)
    ...
    def search_registered_models(
        ...
        endpoint, method = self._get_endpoint_from_method(ListRegisteredModels)
        resp = self._edit_endpoint_and_call(
            endpoint=endpoint,
            method=method,
            req_body=req_body,
    ...
    def get_registered_model(self, name):
        ...
        """
        full_name = get_full_name_from_sc(name, self.spark)
        native_req = message_to_json(GetRegisteredModel(full_name=full_name))
        # The server json_inlines the wrapper, so the response is a flat
        # RegisteredModelInfo (governance fields + enrichment) at the top level.
        endpoint, method = self._get_endpoint_from_method(GetRegisteredModel)
        native_resp = self._edit_endpoint_and_call(
            endpoint=endpoint,
            method=method,
            req_body=native_req,
    ...
    def _create_model_version_with_optional_signature_validation(
        ...
        with self._local_model_dir(source, local_model_path) as local_model_dir:
            ...
            endpoint, method = self._get_endpoint_from_method(CreateModelVersion)
            ...
            # The native CreateModelVersion response carries an int64 version; coerce so the
            # FinalizeModelVersion request (int64 field) and the URL path segment are well-typed
            ...
            endpoint, method = self._get_endpoint_from_method(FinalizeModelVersion)
    ...
    def _get_artifact_repo(self, name, version, storage_location):
        ...
        if is_databricks_sdk_models_artifact_repository_enabled(self.get_host_creds()):
            return DatabricksSDKModelsArtifactRepository(name, version)
    ...
    def update_model_version(self, name, version, description):
        ...
        endpoint, method = self._get_endpoint_from_method(UpdateModelVersion)
        native_resp = self._edit_endpoint_and_call(
            endpoint=endpoint,
            method=method,
            req_body=native_req,
    ...
    def get_model_version(self, name, version):
        ...
        endpoint, method = self._get_endpoint_from_method(GetModelVersion)
        native_resp = self._edit_endpoint_and_call(
            endpoint=endpoint,
            method=method,
            req_body=native_req,
    ...
    def search_model_versions(
        ...
        endpoint, method = self._get_endpoint_from_method(ListModelVersions)
        resp = self._edit_endpoint_and_call(
            endpoint=endpoint,
            method=method,
            req_body=req_body,
    ...
    def get_model_version_by_alias(self, name, alias):
        ...
        """
        full_name = get_full_name_from_sc(name, self.spark)
        native_req = message_to_json(GetModelVersionByAlias(full_name=full_name, alias=alias))
        endpoint, method = self._get_endpoint_from_method(GetModelVersionByAlias)
        native_resp = self._edit_endpoint_and_call(
            endpoint=endpoint,
            method=method,
            req_body=native_req,
