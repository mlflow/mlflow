...
def _validated_dimension_attributes(value: Any, span_key: tuple[str, str]) -> dict[str, Any]:
    if isinstance(value, str):
        try:
            value = json.loads(value)
        except (TypeError, ValueError) as e:
            raise RuntimeError(
                "Cannot prepopulate spans.dimension_attributes: malformed JSON for "
                f"span {span_key!r}"
            ) from e
    if value is None:
        return {}
    ...
    if not isinstance(value, dict) or unsupported:
        ...
        raise RuntimeError(
            "Cannot prepopulate spans.dimension_attributes: unsupported content for "
            f"span {span_key!r}: {details}"
        )
    return value


def _dimension_attributes_state(value: Any, has_dimension_attributes: bool) -> int | None:
...
def _trace_batch(
    ...
    for row in rows:
        ...
        expected["trace_name"] = trace_names[row.request_id]
...
def _span_batch(
    ...
    for row in rows:
        span_key = (row.trace_id, row.span_id)
        dimensions = _validated_dimension_attributes(row.dimension_attributes, span_key)
        ...
        expected["dimension_attributes_state"] = _dimension_attributes_state(
...
def prepopulate_trace_analytics(
    ...
    *,
    ...
    with engine.connect() as connection:
        ...
        _validate_dimension_attributes(connection, source_tables, batch_size)
