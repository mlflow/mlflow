...
class RestStore(AbstractStore):
    ...
    def start_trace(self, trace_info: TraceInfo) -> TraceInfo:
        ...
        Args:
            trace_info: The TraceInfo object to create.
        ...
        try:
            ...
            if e.error_code == databricks_pb2.ErrorCode.Name(databricks_pb2.ENDPOINT_NOT_FOUND):
                _logger.debug("Server does not support StartTraceV3 API yet. Falling back to V2 API.")
    ...
    def search_traces(
        ...
        if sql_warehouse_id is None:
            ...
            v3_endpoint = get_search_traces_endpoint()

            try:
                response_proto = self._call_endpoint(SearchTracesV3, req_body, v3_endpoint)
            except Exception as e:
                if e.error_code == databricks_pb2.ErrorCode.Name(databricks_pb2.ENDPOINT_NOT_FOUND):
                    _logger.debug("Server does not support SearchTracesV3 API yet. Falling back to V2 API.")
    ...
    def delete_trace_tag(self, trace_id: str, key: str):
        ...
        self._call_endpoint(DeleteTraceTag, req_body)
    ...
    ############################################################################################
    # Deprecated MLflow Tracing APIs. Kept for backward compatibility but do not use for new features.
    ############################################################################################
    def deprecated_start_trace(
        self,
        experiment_id: str,
        timestamp_ms: int,
