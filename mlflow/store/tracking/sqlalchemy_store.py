    ...
    def _log_metrics(self, run_id, metrics):
        ...
        with self.ManagedSessionMaker() as session:
            ...
            try:
                ...
                # Collect all existing metric entities across all key batches before
                # filtering, so that metrics from unqueried batches are not incorrectly
                # included in the insert (see https://github.com/mlflow/mlflow/issues/19144)
                ...
                for metric_key_batch in metric_key_batches:
                    ...
                    existing_metrics.update(m.to_mlflow_entity() for m in metric_history)
                # Filter out metrics that already exist in the database
