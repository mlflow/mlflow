...
def upgrade():
    # Use SQLAlchemy inspector to check existing tables, enabling idempotent upgrades.
    # This is needed because some databases (notably MySQL) auto-commit DDL statements,
    # so a partially-applied migration cannot be rolled back. On retry the tables already
    # exist and the plain CREATE TABLE would raise an error. By checking existence first
    # we allow a failed migration to be safely re-run. See:
    ...
    # Drop and recreate the trigger to ensure idempotency: if a previous migration attempt
    # created the tables but failed before (or during) trigger creation, the trigger may or
    # may not exist.  Dropping first (with IF EXISTS guards) is safe and guarantees the
    # trigger is always in the correct state after a successful upgrade.
