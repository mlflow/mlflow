...
def autolog():
    ...
    Datasource information is logged under the current active MLflow run,
    creating an active run if necessary. Note that autologging of Spark ML (MLlib) models is not
    currently supported via this API.
    ...
    This method assumes a SparkSession already exists & should be called on the Spark driver,
    not on the executors (i.e. do not call this method within a parallelized by
    Spark).
