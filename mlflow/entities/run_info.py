...
def check_run_is_active(run_info):
    if run_info.lifecycle_stage != LifecycleStage.ACTIVE:
        raise MlflowException("The run {} must be in 'active' lifecycle_stage.".format(
            run_info.run_id))


def check_run_is_deleted(run_info):
    if run_info.lifecycle_stage != LifecycleStage.DELETED:
        raise MlflowException("The run {} must be in 'deleted' lifecycle_stage.".format(
            run_info.run_id))
...
class RunInfo(_MLflowObject):
    ...
    def get_orderable_attributes(cls):
        ...
        return sorted([
            p for p in cls.__dict__ if isinstance(getattr(cls, p), searchable_attribute) or
