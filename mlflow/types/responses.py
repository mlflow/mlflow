...
class ResponsesAgentRequest(BaseRequestPayload):
    ...
    Args:
        input: List of simple `role` and `content` messages or output items. See examples at
            https://mlflow.org/docs/latest/genai/serving/responses-agent#logging-and-serving and
            https://mlflow.org/docs/latest/genai/serving/responses-agent#creating-agent-output.
...
class ResponsesAgentStreamEvent(BaseModel):
    ...
    See examples at https://mlflow.org/docs/latest/genai/serving/responses-agent#streaming-agent-output
