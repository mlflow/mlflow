...
class SparkAutologgingSuite extends FunSuite with Matchers with BeforeAndAfterAll
  ...
  test("Delegates to repl-ID-aware listener if Session UUID property is set in SparkSession") {
    ...
    assert(MockPublisherNoSessionUUID.sparkQueryListener.isInstanceOf[SparkDataSourceListener])
    assert(
      !MockPublisherNoSessionUUID.sparkQueryListener.isInstanceOf[ReplAwareSparkDataSourceListener]
    )
    ...
    assert(
      MockPublisherWithSessionUUID.sparkQueryListener.isInstanceOf[ReplAwareSparkDataSourceListener]
    )
  }
