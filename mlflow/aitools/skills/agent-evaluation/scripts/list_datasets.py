...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="List and compare MLflow evaluation datasets"
    )
    parser.add_argument(
        '--dataset-name',
        help='Specific dataset to display'
    )
    parser.add_argument(
        '--show-samples',
        type=int,
        default=5,
        help='Number of sample queries to show (default: 5)'
    )
    parser.add_argument(
        '--non-interactive',
        action='store_true',
        help='List without interactive selection'
    )
...
def main():
    ...
    # Search for datasets
    print("Searching for datasets in experiment...")
    client = MlflowClient()
    ...
    dataset_info.sort(key=lambda x: x.get('count', 0) if isinstance(x.get('count'), int) else 0, reverse=True)
    ...
    for i, info in enumerate(dataset_info, 1):
        ...
        if 'error' in info:
            ...
            if info['count'] > 20 and 'HIGH' in info['diversity']:
                print(f"   → RECOMMENDED (most comprehensive)")
