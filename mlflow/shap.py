...
def log_explanation(predict_function, features, artifact_path=None, sampling_strategy='random'):
    ...
    :param features:
        ...
        types listed above. For non-numeric data, ``sampling_strategy`` must be set to
        "random".

        .. note::
            Background data for `shap.KernelExplainer`_ is generated by subsampling ``features``
            with either `np.random.choice`_ or `shap.kmeans`_. The background data size is
    ...
    :param sampling_strategy: {'kmeans', 'random'}, default='random'
        The sampling strategy to use when collecting background data to compute SHAP values.
        If unspecified, defaults to "random".

    :return: Artifact URI of the logged explanations.

    .. _np.random.choice: https://numpy.org/doc/stable/reference/random/generated/numpy.random.choice.html
