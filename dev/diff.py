...
def fetch_pr_diff(owner: str, repo: str, pr_number: str, token: str | None = None) -> str:
    url = f"https://api.github.com/repos/{owner}/{repo}/pulls/{pr_number}"
    headers = {"Accept": "application/vnd.github.v3.diff", "User-Agent": "MLflow-PR-Diff-Fetcher"}
    if token:
    ...
    try:
        ...
        if e.code == 404:
            ...
            raise ValueError(f"Failed to fetch PR diff: HTTP {e.code}")
...
def main() -> None:
    ...
    parser.add_argument("--no-stats", action="store_true", help="Do not show diff statistics")

    args = parser.parse_args()
    owner, repo, pr_number = parse_pr_url(args.pr)
    token = os.environ.get("GITHUB_TOKEN")
    full_diff = fetch_pr_diff(owner, repo, pr_number, token)
