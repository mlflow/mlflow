...
def _start_fake_server(port: int = FAKE_SERVER_PORT, workers: int = 8):
    ...
    with log_file.open("w") as f:
        proc = subprocess.Popen(
            [*prefix, "gunicorn", "fake_server:app",
...
def _check_docker():
    try:
        ...
        console.print("[red]Docker is not installed. Install it at https://docs.docker.com/get-docker/[/red]")
        sys.exit(1)
...
def _install_psycopg2():
    prefix = _uv_prefix()
    cmd = [*prefix, "pip", "install", "psycopg2-binary", "-q"] if prefix else [
...
def _setup_endpoint(
    ...
    secret_id = api_post("gateway/secrets/create", {
    ...
    model_def_id = api_post("gateway/model-definitions/create", {
    ...
    api_post("gateway/endpoints/create", {
        "name": endpoint_name,
        "model_configs": [
...
def _start_nginx(instance_ports: list[int], lb_port: int = MLFLOW_LB_PORT, container_name: str = "benchmark-nginx"):
    ...
    with Progress(
        ...
        subprocess.run(
            ["docker", "run", "--rm", "-d",
             ...
             "-v", f"{nginx_dir / 'nginx.conf'}:/etc/nginx/nginx.conf:ro",
             "-v", f"{conf_d}:/etc/nginx/conf.d:ro",
             "-p", f"{lb_port}:{lb_port}",
             "nginx:alpine"],
            check=True,
            capture_output=True,
...
def _add_benchmark_args(p: argparse.ArgumentParser, default_requests: int, default_max_concurrent: int):
    p.add_argument("--requests", type=int, default=int(os.environ.get("REQUESTS", str(default_requests))))
    p.add_argument("--max-concurrent", type=int, default=int(os.environ.get("MAX_CONCURRENT", str(default_max_concurrent))))
    p.add_argument("--runs", type=int, default=int(os.environ.get("RUNS", "3")))
    p.add_argument("--fake-delay-ms", type=int, default=int(os.environ.get("FAKE_RESPONSE_DELAY_MS", "50")))
...
def main():
    ...
    p_single.add_argument("--workers", type=int, default=int(os.environ.get("TRACKING_SERVER_WORKERS", "4")))
    p_single.add_argument("--backend", choices=["sqlite", "postgres"], default="sqlite")
    p_single.add_argument("--no-usage-tracking", dest="usage_tracking", action="store_false", default=True)
    p_single.add_argument("--mlflow-port", type=int, default=int(os.environ.get("MLFLOW_PORT", str(MLFLOW_LB_PORT))))
    p_single.add_argument("--fake-server-port", type=int, default=int(os.environ.get("FAKE_SERVER_PORT", str(FAKE_SERVER_PORT))))
    ...
    p_multi.add_argument("--workers", type=int, default=int(os.environ.get("WORKERS_PER_INSTANCE", "4")))
    p_multi.add_argument("--no-usage-tracking", dest="usage_tracking", action="store_false", default=True)
    p_multi.add_argument("--lb-port", type=int, default=int(os.environ.get("LB_PORT", str(MLFLOW_LB_PORT))))
    p_multi.add_argument("--base-port", type=int, default=int(os.environ.get("BASE_PORT", str(INSTANCE_BASE_PORT))))
    p_multi.add_argument("--fake-server-port", type=int, default=int(os.environ.get("FAKE_SERVER_PORT", str(FAKE_SERVER_PORT))))
