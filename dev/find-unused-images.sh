...
# Find unused images under docs/ (basename matching).
...
git ls-files docs/ \
  | grep -Ei '\.(png|jpe?g|gif|webp|ico|avif)$' \
  > "$tmp_images"
...
"$repo_root/bin/rg" -o --no-heading --no-line-number \
  '[^"'\''[:space:]()]+\.(png|jpe?g|gif|webp|ico|avif)\b' \
  . \
  | sed 's#.*/##' \
  | sort -u \
  > "$tmp_used" || true
...
echo "$unused_paths"
echo >&2
echo 'Unused images found. Run `./dev/find-unused-images.sh | xargs rm` to remove them.' >&2
exit 1
