"""
Update the maximum package versions in `mlflow/ml-package-versions.yml`.

# Usage

```
flavors update
flavors update --skip-yml
```
"""
...
def update_ml_package_versions_py(config_path):
    with open(config_path) as f:
        ...
        Path(dst).write_text(
            f"""\
# This file was auto-generated by `uv run flavors update`.
        ...
        for flavor_key, config in flavors:
            ...
            for category in ["autologging", "models"]:
                ...
                if category in config and "minimum" in config[category]:
                    ...
                    elif PkgVersion(min_supported_version) > PkgVersion(old_min_version):
                        new_src = update_version(
