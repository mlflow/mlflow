...
def run_pytest(test_paths: list[str], duration_path: str, verbose: bool = True) -> bool:
    """
...
def merge_duration_files(existing_path: Path, new_path: Path) -> dict[str, float]:
...
def update_durations() -> bool:
    ...
    try:
        ...
        success = run_pytest(test_paths=["tests/"], duration_path=str(temp_file))

        if not success:
            print("Warning: No duration file was created")
            return False
