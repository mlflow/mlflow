...
def validate_requirements_yaml(requirements_yaml):
    """
    Validates requirements yaml file
    :param requirements_yaml: requirements yaml
    """
    assert isinstance(
...
def main(args):
    ...
    with open(args.requirements_yaml_location, "r", encoding="utf-8") as file:
        requirements_yaml = yaml.load(file, Loader=yaml.SafeLoader)
    validate_requirements_yaml(requirements_yaml)
    requirements_txt_content = generate_requirements_txt_content(requirements_yaml)
    with open(args.requirements_txt_output_location, "w", encoding="utf-8") as file:
        # Write requirements file content with a trailing newline
        file.write(AUTOGENERATED_HEADER + "\n")
