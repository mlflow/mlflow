...
def gen_stub_files(
    ...
    # The plugin version is pinned in dev/Dockerfile.protos because it determines
    # the generated bytes that CI's reproducibility check compares against.
...
def apply_stub_service_compat(
    ...
    for proto_file in proto_files:
        ...
        pyi_path = out_dir / str(proto_file).replace(".proto", "_pb2.pyi")
        service_names = sorted(set(service_re.findall(proto_path.read_text())))
        missing = [name for name in service_names if f"class {name}(" not in pyi_path.read_text()]
        if not missing:
            continue
        lines = [
            "",
