...
def run_command(cmd: str, capture: bool = True, check: bool = True) -> Optional[str]:
...
def get_recent_run_id(repo: str, branch: str = "master", status: str = "completed") -> Optional[str]:
...
def determine_run_id(repo: str, specified_run_id: Optional[str] = None) -> Optional[str]:
...
def get_run_info(run_id: str, repo: str) -> Dict:
    ...
    cmd = f'gh api repos/{repo}/actions/runs/{run_id} --jq "{{id, html_url, status, conclusion, created_at, updated_at}}"'
...
def parse_job_name(job_name: str) -> Tuple[str, Optional[str]]:
...
def analyze_jobs(jobs_data: List[Dict]) -> Dict:
    ...
    for job in jobs_data:
        ...
        for step in job.get('steps', []):
            if step['status'] != 'completed' or not step.get('started_at') or not step.get('completed_at'):
                continue
...
def print_human_readable(run_info: Dict, analysis: Dict):
    ...
    for job_name, data in sorted(analysis.items()):
        ...
        print(f"│ {job_name:<15} │ {data['splits']:<7} │ {format_duration(int(stats['min'])):<7} │ {format_duration(int(stats['max'])):<7} │ {format_duration(int(stats['average'])):<7} │ {stats['variance']:<8.1f} │")
    ...
    for job_name, data in sorted(analysis.items()):
        ...
        for step_name, stats in sorted(data['steps'].items()):
            if stats['min'] > 0:  # Only show steps that actually ran
                ...
                print(f"│ {step_display:<24} │ {format_duration(int(stats['min'])):<7} │ {format_duration(int(stats['max'])):<7} │ {format_duration(int(stats['average'])):<7} │ {stats['variance']:<8.1f} │")
        
        print("└──────────────────────────┴─────────┴─────────┴─────────┴──────────┘")


def print_json_output(run_info: Dict, analysis: Dict):
