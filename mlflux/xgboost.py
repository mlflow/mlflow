"""
The ``mlflux.xgboost`` module provides an API for logging and loading XGBoost models.
...
from mlflux.models import Model, ModelInputExample, infer_signature
...
from mlflux.utils.autologging_utils import (  # pylint: disable=unused-import
...
def save_model(
    ...
    pyfunc.add_to_model(
        mlflow_model, loader_module="mlflux.xgboost", data=model_data_subpath, env=conda_env_subpath
    )
...
def log_model(
    ...
    **kwargs
    ...
    """
    Log an XGBoost model as an mlflux artifact for the current run.
...
def _load_pyfunc(path):
    ...
    :param path: Local filesystem path to the mlflux Model with the ``xgboost`` flavor.
    """
...
def autolog(
    ...
    """
    Enables (or disables) and configures autologging from XGBoost to mlflux. Logs the following:
    ...
    :param disable_for_unsupported_versions: If ``True``, disable autologging for versions of
                      xgboost that have not been tested against this version of the mlflux client
                      or are incompatible.
    :param silent: If ``True``, suppress all event logs and warnings from mlflux during XGBoost
    ...
    def train(original, *args, **kwargs):
        ...
        autologging_client.log_params(run_id=mlflux.active_run().info.run_id, params=booster_params)
        ...
        autologging_client.log_params(
            run_id=mlflux.active_run().info.run_id, params=params_to_log_for_fn
        )
