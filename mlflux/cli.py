...
import mlflux.deployments.cli
import mlflux.projects as projects
import mlflux.runs
import mlflux.store.artifact.cli
...
from mlflux.utils.process import ShellCommandException
from mlflux.utils.uri import is_local_uri
...
@click.option(
    ...
    "mlflux downloads artifacts from distributed URIs passed to parameters of "
...
@click.option(
    ...
    "Note: this argument is used internally by the mlflux project APIs "
...
def run(
    ...
    """
    Run an mlflux project from the given URI.
...
def ui(backend_store_uri, default_artifact_root, port, host):
    """
    Launch the mlflux tracking UI for local viewing of run results. To launch a production
    server, use the "mlflux server" command instead.
    ...
    address).
    """
    from mlflux.server import _run_server
    from mlflux.server.handlers import initialize_backend_stores
    ...
    try:
        ...
        eprint("Running the mlflux server failed. Please see the logs above for details.")
        sys.exit(1)
...
def server(
    ...
    """
    Run the mlflux tracking server.
...
cli.add_command(mlflux.deployments.cli.commands)
cli.add_command(mlflux.experiments.commands)
cli.add_command(mlflux.store.artifact.cli.commands)
cli.add_command(mlflux.runs.commands)
cli.add_command(mlflux.db.commands)

try:
    # pylint: disable=unused-import
    import mlflux.models.cli
    import mlflux.azureml.cli
    import mlflux.sagemaker.cli

    cli.add_command(mlflux.azureml.cli.commands)
    cli.add_command(mlflux.sagemaker.cli.commands)
    cli.add_command(mlflux.models.cli.commands)
