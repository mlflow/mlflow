"""
The ``mlflux.spacy`` module provides an API for logging and loading spaCy models.
...
from mlflux.models import Model, ModelSignature
...
def save_model(
    ...
    if os.path.exists(path):
        raise MlflowException(
            "Unable to save mlflux model to {path} - path '{path}' "
    ...
    if any(
        ...
        _logger.warning(
            ...
            "can be loaded back via `mlflux.spacy.load_model`, but cannot be loaded back using "
            "pyfunc APIs like `mlflux.pyfunc.load_model` or via the `mlflux models` CLI commands. "
            "mlflux will only generate the pyfunc flavor for spacy models containing a pipeline "
...
def log_model(
    ...
    **kwargs
    ...
    """
    Log a spaCy model as an mlflux artifact for the current run.
...
def _load_pyfunc(path):
    ...
    :param path: Local filesystem path to the mlflux Model with the ``spacy`` flavor.
    """
...
def load_model(model_uri):
    ...
    # Flavor configurations for models saved in mlflux version <= 0.8.0 may not contain a
