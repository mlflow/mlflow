...
# Environment variable indicating a path to a conda installation. mlflux will default to running
# "conda" if unset
...
def get_conda_bin_executable(executable_name):
    ...
    ``mlflux.projects.MLFLOW_CONDA_HOME`` environment variable. If
    ``mlflux.projects.MLFLOW_CONDA_HOME`` is unspecified, this method simply returns the passed-in
    executable name.
    """
...
def _get_conda_env_name(conda_env_path, env_id=None):
    ...
    return "mlflux-%s" % hashlib.sha1(conda_env_contents.encode("utf-8")).hexdigest()
...
def get_or_create_conda_env(conda_env_path, env_id=None):
    ...
    try:
        ...
        raise ExecutionException(
            ...
            "You can also configure mlflux to look for a specific "
