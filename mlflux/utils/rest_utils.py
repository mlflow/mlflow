...
from mlflux import __version__
from mlflux.protos import databricks_pb2
from mlflux.protos.databricks_pb2 import INVALID_PARAMETER_VALUE
from mlflux.utils.proto_json_utils import parse_dict
from mlflux.utils.string_utils import strip_suffix
...
_DEFAULT_HEADERS = {"User-Agent": "mlflux-python-client/%s" % __version__}
...
def http_request(
    ...
    :param host_creds: A :py:class:`mlflux.rest_utils.MlflowHostCreds` object containing
    ...
    from mlflux.tracking.request_header.registry import resolve_request_headers
...
class MlflowHostCreds(object):
    """
    Provides a hostname and optional authentication for talking to an mlflux tracking server.
    :param host: Hostname (e.g., http://localhost:5000) to mlflux server. Required.
