...
asynchronous execution capabilities for a subset of mlflux Tracking logging operations used most
...
from mlflux.entities import Param, RunTag, Metric
from mlflux.exceptions import MlflowException
from mlflux.tracking.client import MlflowClient
from mlflux.utils import chunk_list, _truncate_dict
...
class RunOperations:
    """
    Represents a collection of operations on one or more mlflux Runs, such as run creation
    or metric logging.
    """
    ...
    def await_completion(self):
        """
        Blocks on completion of the mlflux Run operations.
        """
        failed_operations = []
...
class MlflowAutologgingQueueingClient:
    """
    Efficiently implements a subset of mlflux Tracking's  `MlflowClient` and fluent APIs to provide
    ...
    such as `create_run` and `log_metrics`, enqueue data for future persistence to mlflux
    ...
    def _flush_pending_operations(self, pending_operations):
        ...
        NB: Operations are not parallelized on a per-run basis because mlflux's File Store, which
...
class _PendingRunOperations:
    """
    Represents a collection of queued / pending mlflux Run operations.
    """
    ...
    def enqueue(self, params=None, tags=None, metrics=None, create_run=None, set_terminated=None):
        """
        Enqueues a new pending logging operation for the associated mlflux Run.
        """
        if create_run:
