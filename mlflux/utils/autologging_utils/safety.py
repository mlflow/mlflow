...
import mlflux
import mlflux.utils.autologging_utils
...
from mlflux.utils.autologging_utils.events import AutologgingEventLogger
from mlflux.utils.autologging_utils.logging_and_warnings import (
...
def try_mlflow_log(fn, *args, **kwargs):
    ...
    try:
        ...
        if is_testing():
            ...
            warnings.warn("Logging to mlflux failed: " + str(e), stacklevel=2)
...
def with_managed_run(autologging_integration, patch_function, tags=None):
    ...
    `patch_function` with an active mlflux run. The following properties apply:

        - An mlflux run is only created if there is no active run present when the
    ...
    def create_managed_run():
        managed_run = mlflux.start_run(tags=tags)
        _logger.info(
            "Created mlflux autologging run with ID '%s', which will track hyperparameters,"
    ...
    if inspect.isclass(patch_function):
        ...
        class PatchWithManagedRun(patch_function):
            ...
            def _patch_implementation(self, original, *args, **kwargs):
                ...
                if self.managed_run:
                    try_mlflow_log(mlflux.end_run, RunStatus.to_string(RunStatus.FINISHED))

                return result

            def _on_exception(self, e):
                if self.managed_run:
                    try_mlflow_log(mlflux.end_run, RunStatus.to_string(RunStatus.FAILED))
...
def safe_patch(
    ...
    :param manage_run: If `True`, applies the `with_managed_run` wrapper to the specified
                       `patch_function`, which automatically creates & terminates an mlflux
                       active run during patch code execution if necessary. If `False`,
                       does not apply the `with_managed_run` wrapper to the specified
                       `patch_function`.
    """
    from mlflux.utils.autologging_utils import get_autologging_config, autologging_is_disabled
    ...
    def safe_patch_function(*args, **kwargs):
        ...
        warnings.
        """
        # Reroute warnings encountered during the patch function implementation to an mlflux event
        # logger, and enforce silent mode if applicable (i.e. if the corresponding autologging
        # integration was called with `silent=True`), hiding mlflux event logging statements and
        # hiding all warnings in the autologging preamble and postamble (i.e. the code surrounding
        # the user's original / underlying ML function). Non-mlflux warnings are enabled during the
        ...
        with set_mlflow_events_and_warnings_behavior_globally(
            # mlflux warnings emitted during autologging training sessions are likely not
            # actionable and result from the autologging implementation invoking another mlflux
            # API. Accordingly, we reroute these warnings to the mlflux event logger with level
            ...
            # non-mlflux Warnings emitted during the autologging preamble (before the original /
            ...
            # these warnings to the mlflux event logger with level WARNING. For reference, see
            ...
            if is_testing():
                preexisting_run_for_testing = mlflux.active_run()
            ...
            # (i.e. runs created manually via `mlflux.start_run()`)
            exclusive = get_autologging_config(autologging_integration, "exclusive", False)
            user_created_fluent_run_is_active = (
                mlflux.active_run() and not _AutologgingSessionManager.active_session()
            )
            ...
            if (
                ...
                or mlflux.utils.autologging_utils._AUTOLOGGING_GLOBALLY_DISABLED
            ):
            ...
            # The active mlflux run (if any) associated with patch code execution
            ...
            with _AutologgingSessionManager.start_session(autologging_integration) as session:
                    ...
                    def call_original(*og_args, **og_kwargs):
                        try:
                            ...
                            if is_testing():
                                ...
                                # created an mlflux run or 2. the patch code will not create an
                                # mlflux run during the current execution. Here, we capture a
                                ...
                                patch_function_run_for_testing = mlflux.active_run()

                            nonlocal original_has_been_called
                            original_has_been_called = True

                            nonlocal original_result
                            # Show all non-mlflux warnings as normal (i.e. not as event logs)
                ...
                if is_testing() and not preexisting_run_for_testing:
                    # If an mlflux run was created during the execution of patch code, verify that
                    # it is no longer active and that it contains expected autologging tags
                    assert not mlflux.active_run(), (
...
def _validate_autologging_run(autologging_integration, run_id):
    """
    For testing purposes, verifies that an mlflux run produced by an `autologging_integration`
