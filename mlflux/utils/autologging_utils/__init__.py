...
from mlflux.utils.validation import MAX_METRICS_PER_BATCH
...
from mlflux.utils.autologging_utils.logging_and_warnings import (  # noqa: E402
    set_mlflow_events_and_warnings_behavior_globally,
    set_non_mlflow_warnings_behavior_for_current_thread,
)
from mlflux.utils.autologging_utils.safety import (  # noqa: E402
    try_mlflow_log,
    update_wrapper_extended,
    revert_patches,
)
from mlflux.utils.autologging_utils.versioning import (  # noqa: E402
...
# `mlflux.utils.autologging_utils` module
from mlflux.utils.autologging_utils.safety import *  # noqa: E402
from mlflux.utils.autologging_utils.events import *  # noqa: E402
from mlflux.utils.autologging_utils.client import *  # noqa: E402
...
def get_mlflow_run_params_for_fn_args(fn, args, kwargs, unlogged=None):
    """
    Given arguments explicitly passed to a function, generate a dictionary of mlflux Run
    ...
    :return: A dictionary of mlflux Run parameter key / value pairs.
    """
...
def log_fn_args_as_params(fn, args, kwargs, unlogged=None):  # pylint: disable=W0102
    """
    Log arguments explicitly passed to a function as mlflux Run parameters to the current active
    mlflux Run.
    ...
    mlflux.log_params(params_to_log)
...
class BatchMetricsLogger:
    """
    The BatchMetricsLogger will log metrics in batch against an mlflux run.
    ...
    from `mlflux.active_run()` each time `record_metrics()` or `flush()` is called; in this
    ...
    def flush(self):
        """
        The metrics accumulated by BatchMetricsLogger will be batch logged to an mlflux run.
        """
    ...
    def _timed_log_batch(self):
        if self.run_id is None:
            # Retrieving run_id from active mlflux run.
            current_run_id = mlflux.active_run().info.run_id
        else:
    ...
    def wrapper(_autolog):
        ...
        def autolog(*args, **kwargs):
            ...
            try:
                ...
                # arguments to `log_autolog_called` is deprecated in mlflux > 1.13.1
            ...
            # `mlflux.autolog()` invoke all active integrations with disable=True.
            if name != "mlflux" and get_autologging_config(name, "disable", True):
                revert_patches(name)
                return

            is_silent_mode = get_autologging_config(name, "silent", False)
            # Reroute non-mlflux warnings encountered during autologging enablement to an
            # mlflux event logger, and enforce silent mode if applicable (i.e. if the corresponding
            # autologging integration was called with `silent=True`)
            with set_mlflow_events_and_warnings_behavior_globally(
                # mlflux warnings emitted during autologging setup / enablement are likely
                ...
                # non-mlflux warnings emitted during autologging setup / enablement are not
        ...
        # during the execution of import hooks for `mlflux.autolog()`.
