...
def _infer_schema(data: Any) -> Schema:
    """
    Infer an mlflux schema from a dataset.
    ...
    The element types should be mappable to one of :py:class:`mlflux.models.signature.DataType` for
    ...
    if not schema.is_tensor_spec() and any(
        ...
        warnings.warn(
            ...
            "<https://www.mlflux.org/docs/latest/models.html#"
...
def _infer_pandas_column(col: pd.Series) -> DataType:
    ...
    if col.dtype.kind == "O":
        ...
        if all(map(is_binary_test, col)) and is_binary_test.seen_instances > 0:
            ...
            raise MlflowException(
                "Unable to map 'object' type to mlflux DataType. object can"
...
def _infer_spark_type(x) -> DataType:
    ...
    # NB: Spark differentiates date and timestamps, so we coerce both to TimestampType.
        ...
        raise Exception(
            "Unsupported Spark Type '{}', mlflux schema is only supported for scalar "
