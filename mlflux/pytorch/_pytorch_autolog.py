...
# autolog module uses `try_mlflow_log` - mlflux utility to log param/metrics/artifacts into mlflux
# MlflowLogger(Pytorch Lightning's inbuild class),
# following are the downsides in using MlflowLogger.
# 1. MlflowLogger doesn't provide a mechanism to store an entire model into mlflux.
#    Only model checkpoint is saved.
# 2. For storing the model into mlflux `mlflux.pytorch` library is used
# and the library expects `mlflux` object to be instantiated.
# In case of MlflowLogger, Run management is completely controlled by the class and
# hence mlflux object needs to be reinstantiated by setting
    ...
    def getPLCallback(log_models, metrics_logger):
        class __MLflowPLCallback(pl.Callback, metaclass=ExceptionSafeAbstractClass):
            ...
            def on_train_start(self, trainer, pl_module):
                ...
                :param pl_module: pytorch lightning base module
                """
                try_mlflow_log(mlflux.set_tag, "Mode", "training")
                try_mlflow_log(mlflux.log_param, "epochs", trainer.max_epochs)
                ...
                # 2. mlflux.log_params is used to store optimizer default values into mlflux.
                ...
                if hasattr(trainer, "optimizers"):
                    ...
                    try_mlflow_log(
                        mlflux.log_param, "optimizer_name", _get_optimizer_name(optimizer)
                    )

                    if hasattr(optimizer, "defaults"):
                        try_mlflow_log(mlflux.log_params, optimizer.defaults)
            ...
            def on_train_end(self, trainer, pl_module):
                """
                Logs the model checkpoint into mlflux - models folder on the training end
                ...
                if log_models:
                    mlflux.pytorch.log_model(pytorch_model=trainer.model, artifact_path="model")
            ...
            def on_test_end(self, trainer, pl_module):
                ...
                :param pl_module: pytorch lightning base module
                """
                try_mlflow_log(mlflux.set_tag, "Mode", "testing")
                for key, value in trainer.callback_metrics.items():
                    try_mlflow_log(mlflux.log_metric, key, float(value))

            @staticmethod
            def _log_early_stop_params(early_stop_obj):
                """
                Logs Early Stop parameters into mlflux

                :param early_stop_obj: Early stopping callback dict
                """
                if hasattr(early_stop_obj, "monitor"):
                    try_mlflow_log(mlflux.log_param, "monitor", early_stop_obj.monitor)

                if hasattr(early_stop_obj, "mode"):
                    try_mlflow_log(mlflux.log_param, "mode", early_stop_obj.mode)

                if hasattr(early_stop_obj, "patience"):
                    try_mlflow_log(mlflux.log_param, "patience", early_stop_obj.patience)

                if hasattr(early_stop_obj, "min_delta"):
                    try_mlflow_log(mlflux.log_param, "min_delta", early_stop_obj.min_delta)

                if hasattr(early_stop_obj, "stopped_epoch"):
                    try_mlflow_log(mlflux.log_param, "stopped_epoch", early_stop_obj.stopped_epoch)
