"""
The ``mlflux.pytorch`` module provides an API for logging and loading PyTorch models. This module
...
import mlflux
import shutil
import mlflux.pyfunc.utils as pyfunc_utils
...
def get_default_conda_env():
    ...
    .. code-block:: python
        ...
        with mlflux.start_run() as run:
            mlflux.pytorch.log_model(model, "model")

        # Fetch the associated conda environment
        env = mlflux.pytorch.get_default_conda_env()
        print("conda env: {}".format(env))

    .. code-block:: text
        :caption: Output

        conda env {'name': 'mlflux-env',
    ...
    return _mlflow_conda_env(
        additional_pip_deps=[
            ...
            # and `log_model()`: `mlflux.pytorch.pickle_module`.
...
def log_model(
    ...
    **kwargs
    ...
    """
    Log a PyTorch model as an mlflux artifact for the current run.
    ...
    .. code-block:: python
        ...
        with mlflux.start_run() as run:
            ...
            mlflux.pytorch.log_model(scripted_pytorch_model, "scripted_model")
...
def save_model(
    ...
    **kwargs
    ...
    # supplying the MLmodel configuration to `mlflux.pytorch._load_pyfunc`
...
def _load_pyfunc(path, **kwargs):
    ...
    :param path: Local filesystem path to the mlflux Model with the ``pytorch`` flavor.
    """
...
def log_state_dict(state_dict, artifact_path, **kwargs):
    """
    Log a state_dict as an mlflux artifact for the current run.

    .. warning::
        This function just logs a state_dict as an artifact and doesn't generate
        an :ref:`mlflux Model <models>`.
    ...
    .. code-block:: python
        ...
        with mlflux.start_run():
            ...
            mlflux.pytorch.log_state_dict(state_dict, artifact_path="model")
        ...
        with mlflux.start_run():
            ...
            mlflux.pytorch.log_state_dict(state_dict, artifact_path="checkpoint")
    """

    with TempDir() as tmp:
        local_path = tmp.path()
        save_state_dict(state_dict=state_dict, path=local_path, **kwargs)
        mlflux.log_artifacts(local_path, artifact_path)
...
def load_state_dict(state_dict_uri, **kwargs):
    ...
    .. code-block:: python
        ...
        with mlflux.start_run():
            artifact_path = "model"
            mlflux.pytorch.log_state_dict(model.state_dict(), artifact_path)
            state_dict_uri = mlflux.get_artifact_uri(artifact_path)

        state_dict = mlflux.pytorch.load_state_dict(state_dict_uri)
    """
    import torch
...
def autolog(
    ...
    <https://pytorch-lightning.readthedocs.io/en/latest>`_ to mlflux.
    ...
    <https://github.com/mlflux/mlflux/tree/master/examples/pytorch/MNIST>`_ for
    ...
    **Note**: Autologging is only supported for PyTorch Lightning models,
    ...
    :param silent: If ``True``, suppress all event logs and warnings from mlflux during PyTorch
    ...
    .. code-block:: python
        ...
        # loop step, (no validation, no testing). It illustrates how you can use mlflux
    ...
    .. figure:: ../_static/images/pytorch_lightening_autolog.png
        ...
        PyTorch autologged mlflux entities
    """
    import pytorch_lightning as pl
    from mlflux.pytorch._pytorch_autolog import _create_patch_fit
