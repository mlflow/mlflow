"""
This module contains the base interface implemented by mlflux model deployment plugins.
...
def run_local(target, name, model_uri, flavor=None, config=None):  # pylint: disable=W0613
    ...
    :param name:  Unique name to use for deployment. If another deployment exists with the same
                     ...
                     :py:class:`mlflux.exceptions.MlflowException`
...
def target_help():
    ...
    displayed when users invoke the ``mlflux deployments help -t <target-name>`` CLI. This
...
class BaseDeploymentClient(abc.ABC):
    ...
    .. Note::
        Subclasses should raise :py:class:`mlflux.exceptions.MlflowException` in error cases (e.g.
    ...
    def create_deployment(self, name, model_uri, flavor=None, config=None):
        ...
        :py:class:`mlflux.exceptions.MlflowException`. See target-specific plugin documentation
    ...
    def get_deployment(self, name):
        ...
        py:class:`mlflux.exception.MlflowException` if no deployment exists with the provided
        ID.
    ...
    def predict(self, deployment_name, df):
        ...
        Note that the input/output types of this method matches that of `mlflux pyfunc predict`
