...
from mlflux.deployments import interface
from mlflux.utils.proto_json_utils import NumpyEncoder, _get_jsonable_obj


def _user_args_to_dict(user_list):
    # Similar function in mlflux.cli is throwing exception on import
    user_dict = {}
    for s in user_list:
        try:
            name, value = s.split("=")
...
target_details = click.option(
    ...
    help="""
                                   ...
                                   `mlflux deployments help --target-name <target-name>` for
                                   ...
                                   instructions at
                                   https://mlflux.org/docs/latest/plugins.html#community-plugins
                                   """.format(
...
@click.group(
    "deployments",
    help="""
    Deploy mlflux models to custom targets.
    Run `mlflux deployments help --target-name <target-name>` for
    ...
    https://mlflux.org/docs/latest/plugins.html#writing-your-own-mlflux-plugins.
""".format(
        supported_targets_msg=supported_targets_msg
    ),
)
def commands():
    """
    Deploy mlflux models to custom targets. Support is currently installed for
    the following targets: {targets}. Run `mlflux deployments help --target-name <target-name>` for
    more details on the supported URI format and config options for a given target.
    ...
    at https://mlflux.org/docs/latest/plugins.html#community-plugins.
...
@click.option(
    ...
    " https://mlflux.org/docs/latest/tracking.html"
    "#artifact-stores",
)  # optional model_uri
@click.option(
    "--flavor",
...
def explain(target, name, input_path, output_path):
    ...
    understanding/debugging predictions. Run `mlflux deployments help` or
    ...
    https://www.mlflux.org/docs/latest/models.html#built-in-deployment-tools
    """
    import pandas as pd
