...
from mlflux.deployments.utils import parse_target_uri
...
def get_deploy_client(target_uri):
    """
    Returns a subclass of :py:class:`mlflux.deployments.BaseDeploymentClient` exposing standard
    ...
    :py:class:`mlflux.deployments.BaseDeploymentClient`. You can also run
    ``mlflux deployments help -t <target-uri>`` via the CLI for more details on target-specific
    ...
    .. code-block:: python
        ...
        from mlflux.deployments import get_deploy_client
        import pandas as pd
...
def _target_help(target):
    ...
    to be displayed when users invoke the ``mlflux deployments help -t <target-name>`` CLI.
