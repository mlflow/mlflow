"""
The ``mlflux.onnx`` module provides APIs for logging and loading ONNX models in the mlflux Model
format. This module exports mlflux Models with the following flavors:
...
def save_model(
    ...
    pyfunc.add_to_model(
        mlflow_model, loader_module="mlflux.onnx", data=model_data_subpath, env=conda_env_subpath
    )
...
class _OnnxModelWrapper:
    ...
    def predict(self, data):
        ...
        if isinstance(data, dict):
            ...
            if len(self.inputs) != 1:
                ...
                raise MlflowException(
                    ...
                    "input. "
                    "Numpy arrays can only be used as input for mlflux ONNX "
        ...
        # https://github.com/mlflux/mlflux/issues/1286. Meanwhile, we explicitly cast the input to
...
def load_model(model_uri):
    ...
    :param model_uri: The location, in URI format, of the mlflux model, for example:
                      ...
                      `Artifacts Documentation <https://www.mlflux.org/docs/latest/
...
def log_model(
    ...
    """
    Log an ONNX model as an mlflux artifact for the current run.
