...
from mlflux.projects.submitted_run import LocalSubmittedRun
from mlflux.projects.backend.abstract_backend import AbstractBackend
...
from mlflux.utils.conda import get_conda_command, get_or_create_conda_env
...
from mlflux.utils.mlflow_tags import MLFLOW_PROJECT_ENV
...
class LocalBackend(AbstractBackend):
    def run(
        ...
        # Otherwise, invoke `mlflux run` in a subprocess
...
def _invoke_mlflow_run_subprocess(
    ...
    """
    Run an mlflux project asynchronously by invoking ``mlflux run`` in a subprocess, returning
    a SubmittedRun that can be used to query run status.
    """
    _logger.info("=== Asynchronously launching mlflux run with ID %s ===", run_id)
    ...
    env_vars.update(get_databricks_env_vars(mlflux.get_tracking_uri()))
...
def _build_mlflow_run_cmd(
    ...
    """
    Build and return an array containing an ``mlflux run`` command that can be invoked to locally
    run the project at the specified URI.
    """
    mlflow_run_arr = ["mlflux", "run", uri, "-e", entry_point, "--run-id", run_id]
...
def _run_mlflow_run_cmd(mlflow_run_arr, env_map):
    """
    Invoke ``mlflux run`` in a subprocess, which in turn runs the entry point in a child process.
    Returns a handle to the subprocess. Popen launched to invoke ``mlflux run``.
    """
    final_env = os.environ.copy()
    final_env.update(env_map)
    # Launch `mlflux run` command as the leader of its own process group so that we can do a
...
def _run_entry_point(command, work_dir, experiment_id, run_id):
    ...
    :param run_id: mlflux run ID associated with the entry point execution.
    """
    env = os.environ.copy()
    env.update(get_run_env_vars(run_id, experiment_id))
    env.update(get_databricks_env_vars(tracking_uri=mlflux.get_tracking_uri()))
...
def _get_docker_command(image, active_run, docker_args=None, volumes=None, user_env_vars=None):
    from mlflux.projects.docker import get_docker_tracking_cmd_and_envs
