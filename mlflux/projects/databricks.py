...
from mlflux.projects.utils import MLFLOW_LOCAL_BACKEND_RUN_ID_CONFIG
from mlflux.protos.databricks_pb2 import INVALID_PARAMETER_VALUE
from mlflux.utils import rest_utils, file_utils, databricks_utils
...
from mlflux.utils.uri import is_databricks_uri, is_http_uri
from mlflux.version import is_release_version, VERSION

# Base directory within driver container for storing files related to mlflux
DB_CONTAINER_BASE = "/databricks/mlflux"
...
DB_TARFILE_ARCHIVE_NAME = "mlflux-project"
# Base directory within DBFS for storing code for project runs for experiments
DBFS_EXPERIMENT_DIR_BASE = "mlflux-experiments"
...
def before_run_validations(tracking_uri, backend_config):
    ...
    if backend_config is None:
        raise ExecutionException(
            "Backend spec must be provided when launching mlflux project " "runs on Databricks."
        )
    elif "existing_cluster_id" in backend_config:
        raise MlflowException(
            message=(
                "mlflux Project runs on Databricks must provide a *new cluster* specification."
                " Project execution against existing clusters is not currently supported. For more"
                " information, see https://mlflux.org/docs/latest/projects.html"
                "#run-an-mlflux-project-on-databricks"
            ),
            error_code=INVALID_PARAMETER_VALUE,
        )
    if not is_databricks_uri(tracking_uri) and not is_http_uri(tracking_uri):
        raise ExecutionException(
            "When running on Databricks, the mlflux tracking URI must be of the form "
            ...
            "Please specify a valid tracking URI via mlflux.set_tracking_uri or by setting the "
...
class DatabricksJobRunner(object):
    """
    Helper class for running an mlflux project as a Databricks Job.
    ...
    def _upload_project_to_dbfs(self, project_dir, experiment_id):
        ...
        :param project_dir: Path to a directory containing an mlflux project to upload to DBFS (e.g.
    ...
    def _run_shell_command_job(self, project_uri, command, env_vars, cluster_spec):
        ...
        :param cluster_spec: Dictionary containing a `Databricks cluster specification
                             ...
                             will add mlflux to the library list. This function does not support
        ...
        req_body_json = {
            "run_name": "mlflux Run for %s" % project_uri,
...
def _get_cluster_mlflow_run_cmd(project_dir, run_id, entry_point, parameters):
    mlflow_run_arr = list(
        map(shlex_quote, ["mlflux", "run", project_dir, "--entry-point", entry_point])
    )
    if run_id:
...
def _get_databricks_run_cmd(dbfs_fuse_tar_uri, run_id, entry_point, parameters):
    """
    Generate mlflux CLI command to run on Databricks cluster in order to launch a run on Databricks.
    """
...
class DatabricksSubmittedRun(SubmittedRun):
    """
    Instance of SubmittedRun corresponding to a Databricks Job run launched to run an mlflux
    ...
    def _print_description_and_log_tags(self):
        _logger.info(
            "=== Launched mlflux run as Databricks job run with ID %s."
