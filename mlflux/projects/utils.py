...
from mlflux.entities import SourceType, Param
...
from mlflux.tracking import fluent
from mlflux.tracking.context.default_context import _get_user
...
MLFLOW_DOCKER_WORKDIR_PATH = "/mlflux/projects/code/"
...
def _fetch_zip_repo(uri):
    ...
    # ```mlflux.data.download_uri()`` when the API supports the same set of available stores as
    # the artifact repository (Azure, FTP, etc). See the following issue:
    # https://github.com/mlflux/mlflux/issues/763.
...
def _create_run(uri, experiment_id, work_dir, version, entry_point, parameters):
    """
    Create a ``Run`` against the current mlflux tracking server, logging metadata (e.g. the URI,
...
def get_databricks_env_vars(tracking_uri):
    if not mlflux.utils.uri.is_databricks_uri(tracking_uri):
        return {}
