"""
The ``mlflux.projects`` module provides an API for running mlflux projects locally or remotely.
"""
import json
import yaml
import os
...
import mlflux.projects.databricks
import mlflux.tracking as tracking
from mlflux.entities import RunStatus
from mlflux.exceptions import ExecutionException, MlflowException
from mlflux.projects.submitted_run import SubmittedRun
...
from mlflux.projects.backend import loader
from mlflux.tracking.fluent import _get_experiment_id
from mlflux.utils.mlflow_tags import MLFLOW_PROJECT_ENV, MLFLOW_PROJECT_BACKEND
import mlflux.utils.uri
...
def _run(
    ...
    if backend_name == "databricks":
        ...
        from mlflux.projects.databricks import run_databricks
        ...
        from mlflux.projects.docker import (
            build_docker_image,
            validate_docker_env,
            validate_docker_installation,
        )
        from mlflux.projects import kubernetes as kb
...
def run(
    ...
    """
    Run an mlflux project. The project can be local or stored at a Git URI.

    mlflux provides built-in support for running projects locally or remotely on a Databricks or
    ...
    :raises: :py:class:`mlflux.exceptions.ExecutionException` If a run launched in blocking mode
             is unsuccessful.

    :param uri: URI of project to run. A local filesystem path
                or a Git repository URI (e.g. https://github.com/mlflux/mlflux-example)
    ...
    :param experiment_name: Name of experiment under which to launch the run.
    :param experiment_id: ID of experiment under which to launch the run.
    :param backend: Execution backend for the run: mlflux provides built-in support for "local",
    ...
    :param backend_config: A dictionary, or a path to a JSON file (must end in '.json'), which will
                           ...
                           at https://www.mlflux.org/docs/latest/projects.html.
    :param use_conda: If True (the default), create a new Conda environment for the run and
                      ...
                      dependencies.
    :param storage_dir: Used only if ``backend`` is "local". mlflux downloads artifacts from
    ...
    :param synchronous: Whether to block while waiting for a run to complete. Defaults to True.
                        ...
                        error out as well.
    :param run_id: Note: this argument is used internally by the mlflux project APIs and should
                   not be specified. If specified, the run ID will be used instead of
                   creating a new run.
    :return: :py:class:`mlflux.projects.SubmittedRun` exposing information (e.g. run ID)
    ...
    .. code-block:: python
        ...
        project_uri = "https://github.com/mlflux/mlflux-example"
        params = {"alpha": 0.5, "l1_ratio": 0.01}

        # Run mlflux project and create a reproducible conda environment
        # on a local host
        mlflux.run(project_uri, parameters=params)
    ...
    .. code-block:: text
        ...
        ... mlflux.projects: === Run (ID '6a5109febe5e4a549461e149590d0a7c') succeeded ===
    """
    ...
    if backend == "databricks":
        mlflux.projects.databricks.before_run_validations(mlflux.get_tracking_uri(), backend_config)
