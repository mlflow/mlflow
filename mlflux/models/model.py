...
_LOG_MODEL_METADATA_WARNING_TEMPLATE = (
    ...
    "In addition to exporting model artifacts, mlflux clients 1.7.0 and above "
    "attempt to record model metadata to the  tracking store. If logging to a "
    "mlflux server via REST, consider  upgrading the server version to mlflux "
    "1.7.0 or above."
)
...
class Model(object):
    """
    An mlflux Model that can support multiple model flavors. Provides APIs for implementing
    new Model flavors.
    """
    ...
    def log(
        ...
        **kwargs
        ...
        :param signature: (Experimental) :py:class:`ModelSignature` describes model input
                          and output :py:class:`Schema <mlflux.types.Schema>`. The model signature
        ...
        with TempDir() as tmp:
            ...
            mlflux.tracking.fluent.log_artifacts(local_path, artifact_path)
            try:
                mlflux.tracking.fluent._record_logged_model(mlflow_model)
            except MlflowException:
                # We need to swallow all mlflux exceptions to maintain backwards compatibility with
                # older tracking servers. Only print out a warning for now.
                _logger.warning(_LOG_MODEL_METADATA_WARNING_TEMPLATE, mlflux.get_artifact_uri())
            if registered_model_name is not None:
                run_id = mlflux.tracking.fluent.active_run().info.run_id
