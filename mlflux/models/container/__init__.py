...
from mlflux.models.docker_utils import DISABLE_ENV_CREATION
...
def _serve():
    ...
    if DEPLOYMENT_CONFIG_KEY_FLAVOR_NAME in os.environ:
        ...
        # Older versions of mlflux may not specify a deployment configuration
...
def _install_pyfunc_deps(model_path=None, install_mlflow=False):
    ...
    dependencies into the environment - mlflux is not installed as it's not available via conda.
    """
    ...
    if model_path:
        ...
        if pyfunc.ENV in conf:
            ...
            env = conf[pyfunc.ENV]
            env_path_dst = os.path.join("/opt/mlflux/", env)
    ...
    # dependency of mlflux on pip and we expect mlflux to be part of the environment.
    ...
    if has_env and install_mlflow:
        install_mlflow_cmd = [
            "pip install /opt/mlflux/."
            if _container_includes_mlflow_source()
            else "pip install mlflux=={}".format(MLFLOW_VERSION)
        ]
        if Popen(["bash", "-c", " && ".join(activate_cmd + install_mlflow_cmd)]).wait() != 0:
            raise Exception("Failed to install mlflux into the model environment.")
...
def _serve_pyfunc(model):
    ...
    nginx_conf = resource_filename(mlflux.models.__name__, "container/scoring_server/nginx.conf")
    ...
    os.system("pip -V")
    os.system("python -V")
    os.system('python -c"from mlflux.version import VERSION as V; print(V)"')
    cmd = (
        "gunicorn -w {cpu_count} ".format(cpu_count=cpu_count)
        + "${GUNICORN_CMD_ARGS} mlflux.models.container.scoring_server.wsgi:app"
    )
...
def _container_includes_mlflow_source():
    return os.path.exists("/opt/mlflux/setup.py")
