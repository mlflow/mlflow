...
from mlflux.utils.proto_json_utils import NumpyEncoder, _dataframe_from_json, parse_tf_serving_input
...
class _Example(object):
    """
    Represents an input example for mlflux model.

    Contains jsonable data that can be saved with the model and meta data about the exported format
    that can be saved with :py:class:`Model <mlflux.models.Model>`.
    ...
    The _Example metadata contains the following information:
        ...
        - One of the following metadata based on the `type`:
            ...
            - format: For tensors, this attribute specifies the standard being used to store an
                      input example. mlflux uses a JSON-formatted string representation of T
                      F serving input.

    Storage Format:

    The examples are stored as json for portability and readability. Therefore, the contents of the
    example(s) must be jsonable. mlflux will make the following conversions automatically on behalf
    of the user:
    ...
    The metadata is a dictionary with the following fields:
      ...
      -  One of the following metadata based on the `type`:
            ...
            - 'format: Used to store tensors. Determines the standard used to store a tensor input
                       example. mlflux uses a JSON-formatted string representation of TF serving
                       input.
...
def _read_example(mlflow_model: Model, path: str):
    ...
    if example_type not in ["dataframe", "ndarray"]:
        raise MlflowException(
            "This version of mlflux can not load example of type {}".format(example_type)
        )
