...
def serve(model_uri, port, host, workers, no_conda=False, install_mlflow=False):
    """
    Serve a model saved with mlflux by launching a webserver on the specified host and port.
    ...
    documentation: https://www.mlflux.org/docs/latest/models.html#built-in-deployment-tools.
    ...
    .. code-block:: bash
        ...
        $ mlflux models serve -m runs:/my-run-id/model-path &
...
def predict(
    ...
    """
    Generate predictions in json format using a saved mlflux model. For information about the input
...
@cli_args.MODEL_URI
@click.option("--name", "-n", default="mlflux-pyfunc-servable", help="Name to use for built image")
@cli_args.INSTALL_MLFLOW
def build_docker(model_uri, name, install_mlflow):
    """
    **EXPERIMENTAL**: Builds a Docker image whose default entrypoint serves the specified mlflux
    ...
    .. code:: bash
        ...
        mlflux models build-docker -m "runs:/some-run-uuid/my-model" -n "my-image-name"
    ...
    See https://www.mlflux.org/docs/latest/python_api/mlflux.pyfunc.html for more information on the
...
def _get_flavor_backend(model_uri, **kwargs):
    from mlflux.models.flavor_backend_registry import get_flavor_backend
