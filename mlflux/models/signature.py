"""
The :py:mod:`mlflux.models.signature` module provides an API for specification of model signature.

Model signature defines schema of model input and output. See :py:class:`mlflux.types.schema.Schema`
...
from mlflux.types.schema import Schema
from mlflux.types.utils import _infer_schema
...
class ModelSignature(object):
    ...
    ModelSignature can be :py:func:`inferred <mlflux.models.infer_signature>` from training dataset
    and model predictions using or constructed by hand by passing an input and output
    :py:class:`Schema <mlflux.types.Schema>`.
    """

    def __init__(self, inputs: Schema, outputs: Schema = None):
        if not isinstance(inputs, Schema):
            raise TypeError(
                "inputs must be mlflux.models.signature.Schema, got '{}'".format(type(inputs))
            )
        if outputs is not None and not isinstance(outputs, Schema):
            raise TypeError(
                "outputs must be either None or mlflux.models.signature.Schema, "
...
def infer_signature(
    ...
    """
    Infer an mlflux model signature from the training data (input) and model predictions (output).

    The signature represents model input and output as data frames with (optionally) named columns
    and data type specified as one of types defined in :py:class:`mlflux.types.DataType`.
    ...
    The element types should be mappable to one of :py:class:`mlflux.types.DataType`.

    For pyspark.sql.DataFrame inputs, columns of type DateType and TimestampType are both inferred
    as type :py:data:`datetime <mlflux.types.DataType.datetime>`, which is coerced to
