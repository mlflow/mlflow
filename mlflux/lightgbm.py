"""
The ``mlflux.lightgbm`` module provides an API for logging and loading LightGBM models.
...
def log_model(
    ...
    **kwargs
    ...
    """
    Log a LightGBM model as an mlflux artifact for the current run.
...
def _load_pyfunc(path):
    ...
    :param path: Local filesystem path to the mlflux Model with the ``lightgbm`` flavor.
    """
...
def autolog(
    ...
    """
    Enables (or disables) and configures autologging from LightGBM to mlflux. Logs the following:
    ...
    :param disable_for_unsupported_versions: If ``True``, disable autologging for versions of
                      lightgbm that have not been tested against this version of the mlflux client
                      or are incompatible.
    :param silent: If ``True``, suppress all event logs and warnings from mlflux during LightGBM
    ...
    def train(original, *args, **kwargs):
        ...
        # logging booster params separately via mlflux.log_params to extract key/value pairs
