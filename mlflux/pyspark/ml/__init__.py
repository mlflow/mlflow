...
import mlflux
from mlflux.entities import Metric, Param
from mlflux.tracking.client import MlflowClient
from mlflux.utils import _chunk_dict, _truncate_dict, _get_fully_qualified_class_name
...
from mlflux.utils.mlflow_tags import MLFLOW_AUTOLOGGING, MLFLOW_PARENT_RUN_ID
...
def _read_log_model_allowlist():
    ...
    if not spark_session:
        _logger.info(
            ...
            "prior to calling mlflux.pyspark.ml.autolog() and specify the path to your allowlist "
            "file via the spark.mlflux.pysparkml.autolog.logModelAllowlistFile conf."
        )
        return _read_log_model_allowlist_from_file(builtin_allowlist_file)

    allowlist_file = spark_session.sparkContext._conf.get(
        "spark.mlflux.pysparkml.autolog.logModelAllowlistFile", None
    )
    if allowlist_file:
        try:
...
def _get_warning_msg_for_skip_log_model(model):
    return (
        ...
        "one or more of its nested models are not allowlisted. Call mlflux.spark.log_model() "
        "to explicitly log the model, or specify a custom allowlist via the "
        "spark.mlflux.pysparkml.autolog.logModelAllowlistFile Spark conf "
        "(see mlflux.pyspark.ml.autolog docs for more info)."
    )
...
def _create_child_runs_for_parameter_search(parent_estimator, parent_model, parent_run, child_tags):
    ...
    for i in range(len(estimator_param_maps)):
        ...
        for params_batch, metrics_batch in zip_longest(
            ...
            # imposed by corresponding mlflux Tracking APIs (e.g., LogParam, LogMetric)
...
def _log_estimator_params(param_map):
    ...
    for chunk in _chunk_dict(param_map, chunk_size=MAX_PARAMS_TAGS_PER_BATCH,):
        ...
        try_mlflow_log(mlflux.log_params, truncated)
...
def autolog(
    ...
    **Logged information**
      ...
      **Artifacts**
        - An mlflux Model with the :py:mod:`mlflux.spark` flavor containing a fitted estimator
          (logged by :py:func:`mlflux.spark.log_model()`). Note that large models may not be
    ...
    **How does autologging work for meta estimators?**
    ...
    :param log_models: If ``True``, if trained models are in allowlist, they are logged as mlflux
                       ...
                       the "spark.mlflux.pysparkml.autolog.logModelAllowlistFile" Spark config
    ...
    :param disable_for_unsupported_versions: If ``True``, disable autologging for versions of
                      pyspark that have not been tested against this version of the mlflux
                      client or are incompatible.
    :param silent: If ``True``, suppress all event logs and warnings from mlflux during pyspark ML
                   autologging. If ``False``, show all events and warnings during pyspark ML
                   autologging.

    **The default log model allowlist in mlflux**
        .. literalinclude:: ../../../mlflux/pyspark/ml/log_model_allowlist.txt
           :language: text
    """
    ...
    def _log_pretraining_metadata(estimator, params):
        ...
        if artifact_dict:
            try_mlflow_log(mlflux.log_dict, artifact_dict, artifact_file="estimator_info.json")

        _log_estimator_params(param_map)

        try_mlflow_log(mlflux.set_tags, _get_estimator_info_tags(estimator))
    ...
    def _log_posttraining_metadata(estimator, spark_model, params):
        ...
        if _is_parameter_search_estimator(estimator):
            ...
            _log_parameter_search_results_as_artifact(
                estimator_param_maps, metrics, metric_key, mlflux.active_run().info.run_id
            )
            ...
            try_mlflow_log(mlflux.log_dict, best_param_map, artifact_file="best_parameters.json")
        ...
        if log_models:
            if _should_log_model(spark_model):
                ...
                try_mlflow_log(
                    mlflux.spark.log_model, spark_model, artifact_path="model",
                )
                if _is_parameter_search_model(spark_model):
                    try_mlflow_log(
                        mlflux.spark.log_model, spark_model.bestModel, artifact_path="best_model",
                    )
            else:
