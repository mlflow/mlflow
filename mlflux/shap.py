    ...
    if hasattr(model, "inner_model"):
        ...
        if isinstance(unwrapped_model, types.MethodType):
            ...
            try:
                ...
                if issubclass(type(model_object), sklearn.base.BaseEstimator):
                    return mlflux.sklearn.FLAVOR_NAME
            except ImportError:
                pass
        ...
        try:
            ...
            if issubclass(type(unwrapped_model), torch.nn.Module):
                return mlflux.pytorch.FLAVOR_NAME
        except ImportError:
            pass
...
def get_default_conda_env():
    ...
    :return: The default Conda environment for
             mlflux Models produced by calls to
...
def _log_artifact_contextmanager(out_file, artifact_path=None):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        yield tmp_path
        mlflux.log_artifact(tmp_path, artifact_path)
...
def log_explanation(predict_function, features, artifact_path=None):
    ...
    return append_to_uri_path(mlflux.active_run().info.artifact_uri, artifact_path)
...
def log_explainer(
    ...
    """
    Log an SHAP explainer as an mlflux artifact for the current run.

    :param explainer: SHAP explainer to be saved.
    :param artifact_path: Run-relative artifact path.
    :param serialize_model_using_mlflow: When set to True, mlflux will extract the underlying
                                        model and serialize it as an MLmodel, otherwise it
                                        uses SHAP's internal serialization. Defaults to True.
                                        Currently mlflux serialization is only supported for
...
def save_explainer(
    ...
    """
    Save a SHAP explainer to a path on the local file system. Produces an mlflux Model
    ...
    if serialize_model_using_mlflow:
        ...
        if underlying_model_flavor != _UNKNOWN_MODEL_FLAVOR:
            ...
            warnings.warn(
                "Unable to serialize underlying model using mlflux, will use SHAP serialization"
            )

        if underlying_model_flavor == mlflux.sklearn.FLAVOR_NAME:
            mlflux.sklearn.save_model(explainer.model.inner_model.__self__, underlying_model_path)
        elif underlying_model_flavor == mlflux.pytorch.FLAVOR_NAME:
            mlflux.pytorch.save_model(explainer.model.inner_model, underlying_model_path)
...
def load_explainer(model_uri):
    ...
    if underlying_model_flavor != _UNKNOWN_MODEL_FLAVOR:
        ...
        if underlying_model_flavor == mlflux.sklearn.FLAVOR_NAME:
            model = mlflux.sklearn._load_pyfunc(underlying_model_path).predict
        elif underlying_model_flavor == mlflux.pytorch.FLAVOR_NAME:
            model = mlflux.pytorch._load_model(os.path.join(underlying_model_path, "data"))
...
def _load_explainer(explainer_file, model=None):
    """
    Load a SHAP explainer saved as an mlflux artifact on the local file system.

    :param explainer_file: Local filesystem path to the mlflux Model saved with the ``shap`` flavor
