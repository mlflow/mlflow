"""
The ``mlflux.azureml`` module provides an API for deploying mlflux models to Azure
Machine Learning.
"""
import sys
import os
...
from mlflux import get_tracking_uri, get_registry_uri
from mlflux import pyfunc
from mlflux import register_model as mlflow_register_model
...
from mlflux.utils import get_unique_resource_id
from mlflux.utils.annotations import experimental
from mlflux.utils.file_utils import TempDir, _copy_file_or_tree, _copy_project
from mlflux.version import VERSION as mlflow_version
...
def build_image(
    ...
    :ref:`mlflux deployment tools documentation <azureml_deployment>`.

    :param model_uri: The location, in URI format, of the mlflux model used to build the Azure
    ...
    :param mlflow_home: Path to a local copy of the mlflux GitHub repository. If specified, the
                        image will install mlflux from this directory. Otherwise, it will install
                        mlflux from pip.
    ...
    .. code-block:: python
        ...
        # Build an Azure ML Container Image for an mlflux model
        azure_image, azure_model = mlflux.azureml.build_image(model_uri="<model_uri>",
    ...
    """
    # The Azure ML SDK is only compatible with Python 3. However, the `mlflux.azureml` module should
    ...
    if model_python_version is not None and Version(model_python_version) < Version("3.0.0"):
        raise MlflowException(
            message=(
                ...
                " the following mlflux GitHub issue for a thorough explanation of this"
                " limitation and a workaround to enable support for deploying models"
                " trained in Python 2: https://github.com/mlflux/mlflux/issues/668"
            ),
...
def deploy(
    ...
    """
    Register an mlflux model with Azure ML and deploy a websevice to Azure Container Instances (ACI)
    ...
    .. code-block:: python
        ...
        # Create an Azure Container Instance webservice for an mlflux model
        azure_service, azure_model = mlflux.azureml.deploy(model_uri="<model_uri>",
    ...
    with TempDir(chdr=True) as tmp:
        ...
        # produced it. This returns an mlflux Model object however, so we'll still need the
        ...
        if mlflow_home is not None:
            path = tmp.path("dist")
            _logger.info("Bulding temporary mlflux wheel in %s", path)
...
def _build_tags(model_uri, model_python_version=None, user_tags=None, run_id=None):
    """
    :param model_uri: URI to the mlflux model.
...
def _create_dockerfile(output_path, mlflow_path=None):
    ...
    :param mlflow_path: Path to a local copy of the mlflux GitHub repository. If specified, the
                        Dockerfile command for mlflux installation will install mlflux from this
                        directory. Otherwise, it will install mlflux from pip.
    """
    ...
    if mlflow_path is not None:
        ...
        mlflow_install_cmd = "RUN pip install mlflux=={mlflow_version}".format(
            mlflow_version=mlflow_version
        )
    else:
        raise MlflowException(
            "You are running a 'dev' version of mlflux: `{mlflow_version}` that cannot be"
            " installed from pip. In order to build a container image, either specify the"
            " path to a local copy of the mlflux GitHub repository using the `mlflow_home`"
            " parameter or install a release version of mlflux from pip".format(
                mlflow_version=mlflow_version
            )
        )
...
def _create_mlflow_wheel(mlflow_dir, out_dir):
    """
    Create the wheel of mlflux by using setup.py bdist_wheel in the outdir.

    :param mlflow_dir: The absolute path to base of the mlflux Repo to create a wheel from..
    ...
    if len(files) < 1:
        raise MlflowException(
            "Error creating mlflux Wheel - couldn't"
...
from mlflux.pyfunc.scoring_server import parse_json_input, _get_jsonable_obj
