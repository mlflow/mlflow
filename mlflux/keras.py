"""
The ``mlflux.keras`` module provides an API for logging and loading Keras models. This module
...
def save_model(
    ...
    **kwargs
    ...
    :param mlflow_model: mlflux model config this flavor is being added to.
    :param custom_objects: A Keras ``custom_objects`` dictionary mapping names (strings) to
                           custom classes or functions associated with the Keras model. mlflux saves
                           these custom layers using CloudPickle and restores them automatically
                           when the model is loaded with :py:func:`mlflux.keras.load_model` and
                           :py:func:`mlflux.pyfunc.load_model`.
    :param keras_module: Keras module to be used to save / load the model
                         (``keras`` or ``tf.keras``). If not provided, mlflux will
    ...
    .. code-block:: python
        ...
        # Save the model as an mlflux Model
        mlflux.keras.save_model(keras_model, keras_model_path)
    """
    if keras_module is None:
    ...
    pyfunc.add_to_model(
        mlflow_model, loader_module="mlflux.keras", data=data_subpath, env=_CONDA_ENV_SUBPATH
    )
...
def log_model(
    ...
    **kwargs
    ...
    """
    Log a Keras model as an mlflux artifact for the current run.
    ...
    :param conda_env: Either a dictionary representation of a Conda environment or
                      ...
                      :func:`mlflux.keras.get_default_conda_env()` environment is added to
    ...
    .. code-block:: python
        ...
        with mlflux.start_run() as run:
            mlflux.keras.log_model(keras_model, "models")
    """
    Model.log(
...
def _save_custom_objects(path, custom_objects):
    ...
    :param custom_objects: Keras ``custom_objects`` is a dictionary mapping
                           ...
                           during deserialization. mlflux saves these custom layers using
                           CloudPickle and restores them automatically when the model is
                           loaded with :py:func:`mlflux.keras.load_model` and
                           :py:func:`mlflux.pyfunc.load_model`.
    """
    import cloudpickle
...
def _load_model(model_path, keras_module, save_format, **kwargs):
    ...
    # extension to align with prior behavior of mlflux logging
    if save_format == "h5":
...
def _load_pyfunc(path):
    ...
    :param path: Local filesystem path to the mlflux Model with the ``keras`` flavor.
    """
    import tensorflow as tf
...
def load_model(model_uri, **kwargs):
    ...
    .. code-block:: python
        ...
        keras_model = mlflux.keras.load_model("runs:/96771d893a5e46159d9f3b49bf9013e2" + "/models")
...
def autolog(
    ...
    """
    Enables (or disables) and configures autologging from Keras to mlflux. Autologging captures
    ...
    **Artifacts**
     ...
     - `mlflux Model <https://mlflux.org/docs/latest/models.html>`_ (Keras model) on training end
    ...
    mlflux will detect if an ``EarlyStopping`` callback is used in a ``fit()`` or
    ``fit_generator()`` call, and if the ``restore_best_weights`` parameter is set to be ``True``,
    then mlflux will log the metrics associated with the restored model as a final, extra step.
    ...
    If ``restore_best_weights`` is set to be ``False``, then mlflux will not log an additional step.

    Regardless of ``restore_best_weights``, mlflux will also log ``stopped_epoch``,
    ...
    mlflux will also log the parameters of the ``EarlyStopping`` callback,
    ...
    :param disable_for_unsupported_versions: If ``True``, disable autologging for versions of
                      keras that have not been tested against this version of the mlflux client
                      or are incompatible.
    :param silent: If ``True``, suppress all event logs and warnings from mlflux during Keras
    ...
    def getKerasCallback(metrics_logger):
        class __MLflowKerasCallback(keras.callbacks.Callback, metaclass=ExceptionSafeClass):
            ...
            def on_train_begin(self, logs=None):  # pylint: disable=unused-argument
                try_mlflow_log(mlflux.log_param, "num_layers", len(self.model.layers))
                try_mlflow_log(
                    mlflux.log_param, "optimizer_name", type(self.model.optimizer).__name__
                )
                if hasattr(self.model.optimizer, "lr"):
                    ...
                    try_mlflow_log(mlflux.log_param, "learning_rate", lr)
                if hasattr(self.model.optimizer, "epsilon"):
                    ...
                    try_mlflow_log(mlflux.log_param, "epsilon", epsilon)
    ...
    def _log_early_stop_callback_params(callback):
        if callback:
            ...
            try_mlflow_log(mlflux.log_params, earlystopping_params)
    ...
    def fit_generator(original, self, *args, **kwargs):
        ...
        To avoid unintentional creation of nested mlflux runs caused by a patched
