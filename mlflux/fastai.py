"""
The ``mlflux.fastai`` module provides an API for logging and loading fast.ai models. This module
...
from mlflux.models import Model, ModelSignature, ModelInputExample
...
def get_default_conda_env(include_cloudpickle=False):
    """
    :return: The default Conda environment as a dictionary for mlflux Models produced by calls to
    ...
    .. code-block:: python
        ...
        # Start mlflux session and log the fastai learner model
        with mlflux.start_run():
           model.fit(epochs, learning_rate)
           mlflux.fastai.log_model(model, "model")

        # Fetch the default conda environment
        env = mlflux.fastai.get_default_conda_env()
        print("conda environment: {}".format(env))

    .. code-block:: text
        :caption: Output

        conda environment: {'name': 'mlflux-env',
...
def save_model(
    ...
    **kwargs
    ...
    .. code-block:: python
        ...
        # Start mlflux session and save model to current working directory
        with mlflux.start_run():
            model.fit(epochs, learning_rate)
            mlflux.fastai.save_model(model, 'model')

        # Load saved model for inference
        model_uri = "{}/{}".format(os.getcwd(), 'model')
        loaded_model = mlflux.fastai.load_model(model_uri)
    ...
    pyfunc.add_to_model(
        mlflow_model, loader_module="mlflux.fastai", data=model_data_subpath, env=conda_env_subpath
    )
...
def log_model(
    ...
    **kwargs
    ...
    """
    Log a fastai model as an mlflux artifact for the current run.
    ...
    .. code-block:: python
        ...
        def main(epochs=5, learning_rate=0.01):
           ...
           # Start mlflux session and log model
...
def _load_pyfunc(path):
    ...
    :param path: Local filesystem path to the mlflux Model with the ``fastai`` flavor.
    """
...
def load_model(model_uri):
    ...
    :param model_uri: The location, in URI format, of the mlflux model. For example:
                      ...
                      `Referencing Artifacts <https://www.mlflux.org/docs/latest/tracking.html#
...
def autolog(
    ...
    """
    Enable automatic logging from Fastai to mlflux.
    Logs loss and any other metrics specified in the fit
    function, and optimizer data as parameters. Model checkpoints
    are logged as artifacts to a 'models' directory.

    mlflux will also log the parameters of the
    ...
    :param disable_for_unsupported_versions: If ``True``, disable autologging for versions of
                      fastai that have not been tested against this version of the mlflux client
                      or are incompatible.
    :param silent: If ``True``, suppress all event logs and warnings from mlflux during Fastai
    ...
    .. code-block:: python
        ...
        # https://github.com/mlflux/mlflux/tree/master/examples/fastai
        ...
        def print_auto_logged_info(r):
            tags = {k: v for k, v in r.data.tags.items() if not k.startswith("mlflux.")}
        ...
        def main(epochs=5, learning_rate=0.01):
            ...
            print_auto_logged_info(mlflux.get_run(run_id=run.info.run_id))
    ...
    .. figure:: ../_static/images/fastai_autolog.png
        ...
        Fastai autologged mlflux entities
    """
    ...
    def getFastaiCallback(metrics_logger, learner):
        class __MLflowFastaiCallback(LearnerCallback, metaclass=ExceptionSafeClass):
            ...
            def on_train_begin(self, **kwargs):
                ...
                try_mlflow_log(mlflux.log_param, "num_layers", len(info))
                try_mlflow_log(mlflux.log_param, "opt_func", self.opt_func.func.__name__)

                if hasattr(self.opt, "true_wd"):
                    try_mlflow_log(mlflux.log_param, "true_wd", self.opt.true_wd)

                if hasattr(self.opt, "bn_wd"):
                    try_mlflow_log(mlflux.log_param, "bn_wd", self.opt.bn_wd)

                if hasattr(self.opt, "train_bn"):
                    try_mlflow_log(mlflux.log_param, "train_bn", self.train_bn)
    ...
    def _log_one_cycle_callback_params(callback):
        if callback:
            try:
                ...
                try_mlflow_log(mlflux.log_params, params)
            except Exception:  # pylint: disable=W0703
                return
