"""
The ``mlflux.sagemaker`` module provides an API for deploying mlflux models to Amazon SageMaker.
"""
import os
...
from mlflux.models.container import SUPPORTED_FLAVORS as SUPPORTED_DEPLOYMENT_FLAVORS
from mlflux.models.container import DEPLOYMENT_CONFIG_KEY_FLAVOR_NAME

DEFAULT_IMAGE_NAME = "mlflux-pyfunc"
...
# Deprecated as of mlflux 1.0.
DEPRECATED_IMAGE_NAME_ENV_VAR = "SAGEMAKER_DEPLOY_IMG_URL"

DEFAULT_BUCKET_NAME_PREFIX = "mlflux-sagemaker"
...
def _get_preferred_deployment_flavor(model_config):
    """
    Obtains the flavor that mlflux would prefer to use when deploying the model.
    If the model does not contain any supported flavors for deployment, an exception
    will be thrown.

    :param model_config: An mlflux model object
...
def push_image_to_ecr(image=DEFAULT_IMAGE_NAME):
    ...
    fullname = _full_template.format(
        account=account, region=region, image=image, version=mlflux.version.VERSION
    )
...
def deploy(
    ...
    """
    Deploy an mlflux model on AWS SageMaker.
    ...
    :ref:`mlflux deployment tools documentation <sagemaker_deployment>`.

    :param app_name: Name of the deployed application.
    :param model_uri: The location, in URI format, of the mlflux model to deploy to SageMaker.
                      For example:
    ...
    :param execution_role_arn: The name of an IAM role granting the SageMaker service permissions to
                               access the specified Docker image and S3 bucket containing mlflux
                               ...
                               creating a SageMaker model from the specified mlflux model. It is
    ...
    :param image_url: URL of the ECR-hosted Docker image the model should be deployed into, produced
                      by ``mlflux sagemaker build-and-push-container``. This parameter can also
    ...
    :param mode: The mode in which to deploy the application. Must be one of the following:
                 ...
                 ``mlflux.sagemaker.DEPLOYMENT_MODE_ADD``
    ...
    :param archive: If ``True``, any pre-existing SageMaker application resources that become
                    ...
                    ``mlflux.sagemaker.DEPLOYMENT_MODE_REPLACE`` mode) are preserved.
    ...
    :param flavor: The name of the flavor of the model to use for deployment. Must be either
                   ``None`` or one of mlflux.sagemaker.SUPPORTED_DEPLOYMENT_FLAVORS. If ``None``,
...
def run_local(model_uri, port=5000, image=DEFAULT_IMAGE_NAME, flavor=None):
    ...
    :param model_uri: The location, in URI format, of the mlflux model to serve locally,
                      for example:
...
def _get_default_image_url(region_name):
    ...
    return (repository_conf["repositoryUri"] + ":{version}").format(version=mlflux.version.VERSION)
...
def _create_sagemaker_endpoint(
    ...
    :param model_uri: URI of the mlflux model to associate with the specified SageMaker endpoint.
...
def _update_sagemaker_endpoint(
    ...
    :param vpc_config: A dictionary specifying the VPC configuration to use when creating the
                       new SageMaker model associated with this SageMaker endpoint.
    :param mode: either mlflux.sagemaker.DEPLOYMENT_MODE_ADD or
                 mlflux.sagemaker.DEPLOYMENT_MODE_REPLACE.
    :param role: SageMaker execution ARN role.
    :param sage_client: A boto3 client for SageMaker.
    :param s3_client: A boto3 client for S3.
...
def _create_sagemaker_model(
    ...
    :param model_uri: URI of the mlflux model associated with the new SageMaker model.
