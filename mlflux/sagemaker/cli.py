...
import mlflux
import mlflux.sagemaker
from mlflux.sagemaker import DEFAULT_IMAGE_NAME as IMAGE
from mlflux.utils import cli_args
import mlflux.models.docker_utils
...
@click.option(
    "--mode",
    default=mlflux.sagemaker.DEPLOYMENT_MODE_CREATE,
    help="The mode in which to deploy the application."
    " Must be one of the following: {mds}".format(mds=", ".join(mlflux.sagemaker.DEPLOYMENT_MODES)),
)
@click.option(
    "--archive",
    "-ar",
    ...
    help=(
        ...
        " `--async`.".format(mode_replace=mlflux.sagemaker.DEPLOYMENT_MODE_REPLACE)
    ),
)
@click.option(
    "--instance-type",
    "-t",
    default=mlflux.sagemaker.DEFAULT_SAGEMAKER_INSTANCE_TYPE,
...
@click.option(
    "--instance-count",
    "-c",
    default=mlflux.sagemaker.DEFAULT_SAGEMAKER_INSTANCE_COUNT,
...
@click.option(
    ...
    help=(
        ...
        " from the model's available flavors.".format(
            supported_flavors=mlflux.sagemaker.SUPPORTED_DEPLOYMENT_FLAVORS
        )
    ),
)
@click.option(
...
def deploy(
    ...
    https://www.mlflux.org/docs/latest/models.html#sagemaker-deployment.
    """
    if vpc_config is not None:
        with open(vpc_config, "r") as f:
            vpc_config = json.load(f)

    mlflux.sagemaker.deploy(
        app_name=app_name,
        model_uri=model_uri,
...
def run_local(model_uri, port, image, flavor):
    ...
    """
    mlflux.sagemaker.run_local(model_uri=model_uri, port=port, image=image, flavor=flavor)
...
def build_and_push_container(build, push, container, mlflow_home):
    """
    Build new mlflux Sagemaker image, assign it a name, and push to ECR.

    This function builds an mlflux Docker image.
    ...
    if build:
        ...
        ENTRYPOINT ["python", "-c", "import sys; from mlflux.models import container as C; \
        C._init(sys.argv[1])"]
        """
        ...
        def setup_container(_):
            return "\n".join(
                [
                    ...
                    'RUN python -c "from mlflux.models.container import _install_pyfunc_deps;'
                    '_install_pyfunc_deps(None, False)"',
                ]
            )

        mlflux.models.docker_utils._build_image(
            container,
    ...
    if push:
        mlflux.sagemaker.push_image_to_ecr(container)
