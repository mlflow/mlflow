"""
The ``mlflux.catboost`` module provides an API for logging and loading CatBoost models.
...
from mlflux.models import Model, ModelInputExample
...
def save_model(
    ...
    **kwargs
    ...
    pyfunc.add_to_model(
        mlflow_model, loader_module="mlflux.catboost", env=conda_env_subpath, **model_bin_kwargs,
    )
...
def log_model(
    ...
    **kwargs
    ...
    """
    Log a CatBoost model as an mlflux artifact for the current run.
...
def _load_pyfunc(path):
    ...
    :param path: Local filesystem path to the mlflux Model with the ``catboost`` flavor.
    """
