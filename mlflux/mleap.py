"""
The ``mlflux.mleap`` module provides an API for saving Spark MLLib models using the
...
from mlflux.utils.annotations import keyword_only
...
def log_model(
    ...
    """
    Log a Spark MLLib model in MLeap format as an mlflux artifact
    ...
    .. code-block:: python
        ...
        # log parameters
        mlflux.log_param("max_iter", 10)
        mlflux.log_param("reg_param", 0.001)
        # log the Spark MLlib model in MLeap format
        mlflux.mleap.log_model(spark_model=model, sample_input=test_df, artifact_path="mleap-model")
    """
    return Model.log(
...
def save_model(
    ...
    :param mlflow_model: :py:mod:`mlflux.models.Model` to which this flavor is being added.
...
def add_to_model(mlflow_model, path, spark_model, sample_input):
    """
    Add the MLeap flavor to an existing mlflux model.
    ...
    try:
        ...
        _logger.warning(
            ...
            "mlflux release. Please upgrade to a newer mleap version.",
            mleap_version,
        )
