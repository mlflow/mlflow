...
# NB: We need to be careful what we import form mlflux here. Scoring server is used from within
# model's conda environment. The version of mlflux doing the serving (outside) and the version of
# mlflux in the model's conda environment (inside) can differ. We should therefore keep mlflux
...
from mlflux.types import Schema
from mlflux.utils import reraise
...
try:
    from mlflux.pyfunc import load_model, PyFuncModel
except ImportError:
    from mlflux.pyfunc import load_pyfunc as load_model
from mlflux.protos.databricks_pb2 import MALFORMED_REQUEST, BAD_REQUEST
from mlflux.server.handlers import catch_mlflow_exception
...
def _handle_serving_error(error_message, error_code, include_traceback=True):
    ...
    :param error_code: An appropriate error code for the reraised exception. This should be one of
                       the codes listed in the `mlflux.protos.databricks_pb2` proto.
