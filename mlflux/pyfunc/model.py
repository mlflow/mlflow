"""
The ``mlflux.pyfunc.model`` module defines logic for saving and loading custom "python_function"
...
def get_default_conda_env():
    ...
    :return: The default Conda environment for mlflux Models produced by calls to
             :func:`save_model() <mlflux.pyfunc.save_model>`
             and :func:`log_model() <mlflux.pyfunc.log_model>` when a user-defined subclass of
...
class PythonModel(object):
    ...
    By subclassing :class:`~PythonModel`, users can create customized mlflux models with the
    ...
    def load_context(self, context):
        ...
        :func:`~PythonModel.predict` when evaluating inputs. When loading an mlflux model with
...
class PythonModelContext(object):
    ...
    :func:`save_model() <mlflux.pyfunc.save_model>` and
    :func:`log_model() <mlflux.pyfunc.log_model>` persistence methods, using the contents specified
...
def _save_model_with_class_artifacts_params(
    ...
    :param mlflow_model: The model configuration to which to add the ``mlflux.pyfunc`` flavor.
    """
