...
import subprocess
import posixpath
from mlflux.models import FlavorBackend
from mlflux.models.docker_utils import _build_image, DISABLE_ENV_CREATION
from mlflux.pyfunc import ENV, scoring_server

from mlflux.utils.conda import get_or_create_conda_env, get_conda_bin_executable, get_conda_command
...
from mlflux.version import VERSION
...
class PyFuncBackend(FlavorBackend):
    ...
    def predict(
        ...
        """
        Generate predictions using generic python model saved with mlflux.
        Return the prediction results as a JSON.
        """
        local_path = _download_artifact_from_uri(model_uri)
        # NB: Absolute windows paths do not work with mlflux apis, use file uri to ensure
        # platform compatibility.
        local_uri = path_to_local_file_uri(local_path)
        if not self._no_conda and ENV in self._config:
            conda_env_path = os.path.join(local_path, self._config[ENV])
            command = (
                'python -c "from mlflux.pyfunc.scoring_server import _predict; _predict('
                "model_uri={model_uri}, "
    ...
    def serve(self, model_uri, port, host):
        ...
        if os.name != "nt":
            command = (
                ...
                " -- mlflux.pyfunc.scoring_server.wsgi:app"
            ).format(host=host, port=port, nworkers=self._nworkers)
        else:
            command = (
                "waitress-serve --host={host} --port={port} "
                "--ident=mlflux mlflux.pyfunc.scoring_server.wsgi:app"
    ...
    def build_image(self, model_uri, image_name, install_mlflow=False, mlflow_home=None):
        def copy_model_into_container(dockerfile_context_dir):
            ...
            return """
                ...
                RUN python -c \
                'from mlflux.models.container import _install_pyfunc_deps;\
        ...
        pyfunc_entrypoint = (
            'ENTRYPOINT ["python", "-c", "from mlflux.models import container as C; C._serve()"]'
        )
        _build_image(
...
def _execute_in_conda_env(conda_env_path, command, install_mlflow, command_env=None):
    ...
    if install_mlflow:
        if "MLFLOW_HOME" in os.environ:  # dev version
            ...
            install_mlflow = "pip install mlflux=={} 1>&2".format(VERSION)
