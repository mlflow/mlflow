"""
The ``python_function`` model flavor serves as a default model interface for mlflux Python models.
Any mlflux Python model is expected to be loadable as a ``python_function`` model.

In addition, the ``mlflux.pyfunc`` module defines a generic :ref:`filesystem format
...
The ``mlflux.pyfunc`` module also defines utilities for creating custom ``pyfunc`` models
using frameworks and inference logic that may not be natively included in mlflux. See
...
<mlflux.pyfunc.PyFuncModel>`, which is an mlflux wrapper around the model implementation and model
metadata (MLmodel file). You can score the model by calling the :py:func:`predict()
<mlflux.pyfunc.PyFuncModel.predict>` method, which has the following signature::
...
- loader_module [required]:
         ...
         e.g. ``mlflux.sklearn``, it will be imported using ``importlib.import_module``.
...
mlflux's persistence modules provide convenience functions for creating models with the
``pyfunc`` flavor in a variety of machine learning frameworks (scikit-learn, Keras, Pytorch, and
more); however, they do not cover every use case. For example, you may want to create an mlflux
model with the ``pyfunc`` flavor using a framework that mlflux does not natively support.
Alternatively, you may want to build an mlflux model that executes custom logic when evaluating
queries, such as preprocessing and postprocessing routines. Therefore, ``mlflux.pyfunc``
...
serialized TensorFlow graph is an artifact. An mlflux model directory is also an artifact.
...
1. Programmatically defining a new mlflux model, including its attributes and artifacts.

   Given a set of artifact URIs, :meth:`save_model()` and :meth:`log_model()` can
   automatically download artifacts from their URIs and create an mlflux model directory.
...
2. Interpreting pre-existing data as an mlflux model.

   If you already have a directory containing model data, :meth:`save_model()` and
   :meth:`log_model()` can import the data as an mlflux model. The ``data_path`` parameter
   ...
   implementation in mlflux.keras <https://github.com/mlflux/mlflux/blob/
   74d75109aaf2975f5026104d6125bb30f4e3f744/mlflux/keras.py#L157-L187>`_.
...
- If you have already collected all of your model data in a single location, the second
  workflow allows it to be saved in mlflux format directly, without enumerating constituent
  artifacts.
"""
...
from mlflux.pyfunc.model import PythonModel, PythonModelContext  # pylint: disable=unused-import
from mlflux.pyfunc.model import get_default_conda_env
from mlflux.tracking.artifact_utils import _download_artifact_from_uri
from mlflux.types import DataType, Schema, TensorSpec
from mlflux.types.utils import clean_tensor_type
from mlflux.utils import PYTHON_VERSION, get_major_minor_py_version
from mlflux.utils.annotations import deprecated
from mlflux.utils.file_utils import TempDir, _copy_file_or_tree
from mlflux.utils.model_utils import _get_flavor_configuration
from mlflux.utils.environment import _log_pip_requirements
...
def _enforce_mlflow_datatype(name, values: pandas.Series, t: DataType):
    ...
    if t == DataType.binary and values.dtype.kind == t.binary.to_numpy().kind:
        ...
        # element in the array (column). Since mlflux binary type is length agnostic, we ignore
    ...
    if t == DataType.datetime and values.dtype.kind == t.to_numpy().kind:
        ...
        # denotes nanosecond precision. Since mlflux datetime type is precision agnostic, we
    ...
    if is_upcast:
        ...
        if (
            ...
            hint = (
                ...
                "Missing Values <https://www.mlflux.org/docs/latest/models.html#"
...
class PyFuncModel(object):
    """
    mlflux 'python function' model.

    Wrapper around model implementation and metadata. This class is not meant to be constructed
    directly. Instead, instances of this class are constructed and returned from
    :py:func:`load_model() <mlflux.pyfunc.load_model>`.

    ``model_impl`` can be any Python object that implements the `Pyfunc interface
    <https://mlflux.org/docs/latest/python_api/mlflux.pyfunc.html#pyfunc-inference-api>`_, and is
    ...
    def predict(self, data: PyFuncInput) -> PyFuncOutput:
        ...
        <https://www.mlflux.org/docs/latest/models.html#signature-enforcement>`_ for more details."
    ...
    def __repr__(self):
        ...
        return yaml.safe_dump({"mlflux.pyfunc.loaded_model": info}, default_flow_style=False)
...
def load_model(model_uri: str, suppress_warnings: bool = True) -> PyFuncModel:
    ...
    if CODE in conf and conf[CODE]:
        ...
        mlflux.pyfunc.utils._add_code_to_system_path(code_path=code_path)
    data_path = os.path.join(local_path, conf[DATA]) if (DATA in conf) else local_path
    model_impl = importlib.import_module(conf[MAIN])._load_pyfunc(data_path)
    return PyFuncModel(model_meta=model_meta, model_impl=model_impl)


@deprecated("mlflux.pyfunc.load_model", 1.0)
...
def spark_udf(spark, model_uri, result_type="double"):
    ...
    .. code-block:: python
        ...
        predict = mlflux.pyfunc.spark_udf(spark, "/my/local/model")
        df.withColumn("prediction", predict(struct("name", "age"))).show()

    :param spark: A SparkSession object.
    :param model_uri: The location, in URI format, of the mlflux model with the
                      :py:mod:`mlflux.pyfunc` flavor. For example:
    ...
    def predict(*args):
        ...
        if type(elem_type) == IntegerType:
            ...
            result = result.select_dtypes([np.byte, np.ubyte, np.short, np.ushort, np.int, int])
...
def save_model(
    ...
    **kwargs
    ...
    :param mlflow_model: :py:mod:`mlflux.models.Model` configuration to which to add the
    ...
    :param artifacts: A dictionary containing ``<name, artifact_uri>`` entries. Remote artifact URIs
                      ...
                      in :func:`PythonModel.load_context() <mlflux.pyfunc.PythonModel.load_context>`
                      and :func:`PythonModel.predict() <mlflux.pyfunc.PythonModel.predict>`.
    ...
    if first_argument_set_specified:
        ...
        return mlflux.pyfunc.model._save_model_with_class_artifacts_params(
            path=path,
            python_model=python_model,
            artifacts=artifacts,
            conda_env=conda_env,
...
def log_model(
    ...
    """
    Log a Pyfunc model with custom inference logic and optional data dependencies as an mlflux
