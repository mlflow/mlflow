"""
The ``mlflux.tensorflow`` module provides an API for logging and loading TensorFlow models.
...
from mlflux.models.model import MLMODEL_FILE_NAME, _LOG_MODEL_METADATA_WARNING_TEMPLATE
from mlflux.models.signature import ModelSignature
from mlflux.models.utils import ModelInputExample, _save_example
from mlflux.protos.databricks_pb2 import DIRECTORY_NOT_EMPTY
from mlflux.tracking import MlflowClient
from mlflux.tracking.artifact_utils import _download_artifact_from_uri, get_artifact_uri
from mlflux.utils.annotations import keyword_only, experimental
from mlflux.utils.environment import _mlflow_conda_env, _log_pip_requirements
from mlflux.utils.file_utils import _copy_file_or_tree, TempDir
...
def log_model(
    ...
    """
    Log a *serialized* collection of TensorFlow graphs and variables as an mlflux model
...
def save_model(
    ...
    """
    Save a *serialized* collection of TensorFlow graphs and variables as an mlflux model
    ...
    :param tf_signature_def_key: A string identifying the input/output signature associated with the
                                 ...
                                 ``signature_def_map`` parameter of the
                                 ``tf.saved_model.builder.savedmodelbuilder`` method.
    :param path: Local path where the mlflux model is to be saved.
    :param mlflow_model: mlflux model configuration to which to add the ``tensorflow`` flavor.
    :param conda_env: Either a dictionary representation of a Conda environment or the path to a
                      Conda environment yaml file. If provided, this decsribes the environment
                      this model should be run in. At minimum, it should specify the dependencies
                      contained in :func:`get_default_conda_env()`. If ``None``, the default
    ...
    pyfunc.add_to_model(mlflow_model, loader_module="mlflux.tensorflow", env=conda_env_subpath)
...
def load_model(model_uri, tf_sess=None):
    """
    Load an mlflux model that contains the TensorFlow flavor from the specified path.
    ...
    *With TensorFlow version <2.0.0, this method must be called within a TensorFlow graph context.*
    ...
    :param tf_sess: The TensorFlow session in which to load the model. If using TensorFlow
                    ...
                    session is passed to this function, mlflux will attempt to load the model using
    ...
    .. code-block:: python
        ...
        with tf_graph.as_default():
            signature_definition = mlflux.tensorflow.load_model(model_uri="model_uri",
                                    tf_sess=tf_sess)
...
def _get_and_parse_flavor_configuration(model_path):
    """
    :param path: Local filesystem path to the mlflux Model with the ``tensorflow`` flavor.
...
def _load_pyfunc(path):
    """
    Load PyFunc implementation. Called by ``pyfunc.load_pyfunc``. This function loads an mlflux
...
def _log_artifacts_with_warning(**kwargs):
    try_mlflow_log(mlflux.log_artifacts, **kwargs)
...
def _flush_queue():
    """
    Flush the metric queue and log contents in batches to mlflux.
...
# :is_temp - boolean: `True` if the logging directory was created for temporary use by mlflux,
#                     `False` otherwise
    ...
    class __MLflowTfKerasCallback(Callback, metaclass=ExceptionSafeClass):
        ...
        def on_train_begin(self, logs=None):  # pylint: disable=unused-argument
            ...
            if hasattr(opt, "_name"):
                try_mlflow_log(mlflux.log_param, "optimizer_name", opt._name)
            # Elif checks are if the optimizer is a TensorFlow optimizer rather than a Keras one.
                ...
                opt = opt.optimizer
                try_mlflow_log(mlflux.log_param, "optimizer_name", type(opt).__name__)
            if hasattr(opt, "lr"):
        ...
        def on_train_end(self, logs=None):  # pylint: disable=unused-argument
            if log_models:
                try_mlflow_log(mlflux.keras.log_model, self.model, artifact_path="model")
    ...
    class __MLflowTfKeras2Callback(Callback, metaclass=ExceptionSafeClass):
        ...
        def on_train_begin(self, logs=None):  # pylint: disable=unused-argument
            ...
            for attribute in config:
                try_mlflow_log(mlflux.log_param, "opt_" + attribute, config[attribute])
...
def autolog(
    ...
    """
    Enables automatic logging from TensorFlow to mlflux.
    Note that autologging for ``tf.keras`` is handled by :py:func:`mlflux.tensorflow.autolog`,
    not :py:func:`mlflux.keras.autolog`.
    As an example, try running the
    `TensorFlow examples <https://github.com/mlflux/mlflux/tree/master/examples/tensorflow>`_.
    ...
    **tf.keras**
     ...
     - **Artifacts**
      ...
      - `mlflux Model <https://mlflux.org/docs/latest/models.html>`_ (Keras model)
    ...
    **tf.estimator**
     ...
     - **Artifacts**
      ...
      - `mlflux Model <https://mlflux.org/docs/latest/models.html>`_ (TF saved model) on call
    ...
    **TensorFlow Core**
    ...
    <https://www.mlflux.org/docs/latest/tracking.html#tensorflow-and-keras-experimental>`_.
    ...
    :param disable_for_unsupported_versions: If ``True``, disable autologging for versions of
                      tensorflow that have not been tested against this version of the mlflux
                      client or are incompatible.
    :param silent: If ``True``, suppress all event logs and warnings from mlflux during TensorFlow
    ...
    if Version(tensorflow.__version__) < Version("1.12"):
        warnings.warn("Could not log to mlflux. TensorFlow versions below 1.12 are not supported.")
        return
    ...
    def train(original, self, *args, **kwargs):
        ...
        if len(args) >= 3:
            try_mlflow_log(mlflux.log_param, "steps", args[2])
            if len(args) >= 4:
                try_mlflow_log(mlflux.log_param, "max_steps", args[3])
        if "steps" in kwargs:
            try_mlflow_log(mlflux.log_param, "steps", kwargs["steps"])
        if "max_steps" in kwargs:
            try_mlflow_log(mlflux.log_param, "max_steps", kwargs["max_steps"])
    ...
    def export_saved_model(original, self, *args, **kwargs):
        ...
        if _AUTOLOG_RUN_ID:
            _logger.info(
                "Logging TensorFlow Estimator as mlflux Model to run with ID '%s'", _AUTOLOG_RUN_ID
            )
            ...
            def log_model_without_starting_new_run():
                ...
                with TempDir() as tmp:
                    ...
                    try:
                        ...
                        # We need to swallow all mlflux exceptions to maintain backwards
