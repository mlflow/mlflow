"""
The ``mlflux.statsmodels`` module provides an API for logging and loading statsmodels models.
...
from mlflux.utils.validation import _is_numeric
...
def save_model(
    ...
    pyfunc.add_to_model(
        mlflow_model,
        loader_module="mlflux.statsmodels",
...
def log_model(
    ...
    **kwargs
    ...
    """
    Log a statsmodels model as an mlflux artifact for the current run.
...
def _load_pyfunc(path):
    ...
    :param path: Local filesystem path to the mlflux Model with the ``statsmodels`` flavor.
    """
...
class _StatsmodelsModelWrapper:
    ...
    def predict(self, dataframe):
        ...
        if isinstance(model, TimeSeriesModel):
            ...
            # TODO: move this to a specific mlflux.statsmodels.tsa flavor? Time series models
...
def autolog(
    ...
    """
    Enables (or disables) and configures automatic logging from statsmodels to mlflux.
    Logs the following:
    ...
    :param disable_for_unsupported_versions: If ``True``, disable autologging for versions of
                      statsmodels that have not been tested against this version of the mlflux
                      client or are incompatible.
    :param silent: If ``True``, suppress all event logs and warnings from mlflux during statsmodels
    ...
    def prepend_to_keys(dictionary: dict, preffix="_"):
        ...
        and make them compliant with mlflux params & metrics naming rules.
        :param dictionary:
        ...
        try:
            ...
            if should_autolog:
                ...
                if isinstance(model, statsmodels.base.wrapper.ResultsWrapper):
                    ...
                    try_mlflow_log(mlflux.log_metrics, metrics_dict)
