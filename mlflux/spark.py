"""
The ``mlflux.spark`` module provides an API for logging and loading Spark MLlib models. This module
...
:py:mod:`mlflux.mleap`
    ...
    ``mlflux/java`` package. This flavor is produced only if you specify
...
import mlflux
from mlflux import pyfunc, mleap
...
from mlflux.utils.model_utils import _get_flavor_configuration_from_uri
...
def log_model(
    ...
    """
    Log a Spark MLlib model as an mlflux artifact for the current run. This uses the
    MLlib persistence format and produces an mlflux Model with the Spark flavor.

    Note: If no run is active, it will instantiate a run to obtain a run_id.

    :param spark_model: Spark model to be saved - mlflux can only save descendants of
    ...
    :param dfs_tmpdir: Temporary directory path on Distributed (Hadoop) File System (DFS) or local
                       ...
                       created on the DFS are removed. Defaults to ``/tmp/mlflux``.
    ...
    .. code-block:: python
        ...
        mlflux.spark.log_model(model, "spark-model")
    """
    ...
    run_id = mlflux.tracking.fluent._get_or_start_run().info.run_id
    run_root_artifact_uri = mlflux.get_artifact_uri()
    ...
    with TempDir() as tmp:
        ...
        mlflux.tracking.fluent.log_artifacts(tmp_model_metadata_dir, artifact_path)
        if registered_model_name is not None:
            mlflux.register_model(
...
class _HadoopFileSystem:
    ...
    Since mlflux works on local directories, we need this interface to copy the files between
...
def _validate_model(spark_model):
    ...
    if (
        ...
        raise MlflowException(
            "Cannot serialize this model. mlflux can only save descendants of pyspark.Model"
...
def save_model(
    ...
    :param dfs_tmpdir: Temporary directory path on Distributed (Hadoop) File System (DFS) or local
                       ...
                       completes successfully. Defaults to ``/tmp/mlflux``.
...
def load_model(model_uri, dfs_tmpdir=None):
    ...
    :param dfs_tmpdir: Temporary directory path on Distributed (Hadoop) File System (DFS) or local
                       ...
                       destination. Defaults to ``/tmp/mlflux``.
    ...
    .. code-block:: python
        ...
        from mlflux import spark
        model = mlflux.spark.load_model("spark-model")
...
def _load_pyfunc(path):
    ...
    :param path: Local filesystem path to the mlflux Model with the ``spark`` flavor.
    """
...
def autolog(disable=False, silent=False):  # pylint: disable=unused-argument
    ...
    `mlflux-spark JAR
    <http://mlflux.org/docs/latest/tracking.html#automatic-logging-from-spark-experimental>`_
    ...
    Datasource information is cached in memory and logged to all subsequent mlflux runs,
    including the active mlflux run (if one exists when the data is read). Note that autologging of
    Spark ML (MLlib) models is not currently supported via this API. Datasource autologging is
    best-effort, meaning that if Spark is under heavy load or mlflux logging fails for any reason
    (e.g., if the mlflux server is unavailable), logging may be dropped.

    For any unexpected issues with autologging, check Spark driver and executor logs in addition
    to stderr & stdout generated from your mlflux code - datasource information is pulled from
    ...
    .. code-block:: python
        ...
        # ensure the org.mlflux:mlflux-spark:1.11.0 is attached as a library to
        # your cluster
        spark = (SparkSession.builder
                    .config("spark.jars.packages", "org.mlflux:mlflux-spark:1.11.0")
                    .master("local[*]")
                    .getOrCreate())
        ...
        # next-created mlflux run if no run is currently active
    ...
    :param silent: If ``True``, suppress all event logs and warnings from mlflux during Spark
                   datasource autologging. If ``False``, show all events and warnings during Spark
                   datasource autologging.
    """
    from mlflux.utils._spark_utils import _get_active_spark_session
    from mlflux._spark_autologging import _listen_for_spark_activity
    from pyspark.sql import SparkSession
    from pyspark import SparkContext
