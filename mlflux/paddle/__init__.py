"""
The ``mlflux.paddle`` module provides an API for logging and loading paddle models.
...
:py:mod:`mlflux.pyfunc`
    ...
    NOTE: The `mlflux.pyfunc` flavor is only added for paddle models that define `predict()`,
...
def save_model(
    ...
    """
    Save a paddle model to a path on the local file system. Produces an mlflux Model
    containing the following flavors:

        - :py:mod:`mlflux.paddle`
        - :py:mod:`mlflux.pyfunc`. NOTE: This flavor is only included for paddle models
    ...
    .. code-block:: python
        ...
        mlflux.paddle.save_model(model, sk_path_dir)
...
def load_model(model_uri, model=None, **kwargs):
    ...
    .. code-block:: python
        ...
        import mlflux.paddle
        pd_model = mlflux.paddle.load_model("runs:/96771d893a5e46159d9f3b49bf9013e2/pd_models")
...
def log_model(
    ...
    """
    Log a paddle model as an mlflux artifact for the current run. Produces an mlflux Model
    containing the following flavors:
...
def _load_pyfunc(path):
    ...
    :param path: Local filesystem path to the mlflux Model with the ``paddle`` flavor.
    """
