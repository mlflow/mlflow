...
mlflux run. This module is exposed to users at the top-level :py:mod:`mlflux` module.
"""
import os
...
from mlflux.entities import Experiment, Run, RunInfo, RunStatus, Param, RunTag, Metric, ViewType
...
from mlflux.tracking import artifact_utils, _get_store
...
from mlflux.utils.databricks_utils import is_in_databricks_notebook, get_notebook_id
from mlflux.utils.import_hooks import register_post_import_hook
from mlflux.utils.mlflow_tags import MLFLOW_PARENT_RUN_ID, MLFLOW_RUN_NAME
from mlflux.utils.validation import _validate_run_id
...
def set_experiment(experiment_name: str) -> None:
    ...
    .. code-block:: python
        ...
        mlflux.set_experiment("Social NLP Experiments")

        # Get Experiment Details
        experiment = mlflux.get_experiment_by_name("Social NLP Experiments")
...
class ActiveRun(Run):  # pylint: disable=W0223
    """Wrapper around :py:class:`mlflux.entities.Run` to enable using Python ``with`` syntax."""
...
def start_run(
    ...
    """
    Start a new mlflux run, setting it as the active run under which metrics and parameters
    ...
    mlflux sets a variety of default tags on the run, as defined in
    :ref:`mlflux system tags <system_tags>`.
    ...
    :param run_name: Name of new run (stored as a ``mlflux.runName`` tag).
    ...
    :return: :py:class:`mlflux.ActiveRun` object that acts as a context manager wrapping
             the run's state.
    ...
    .. code-block:: python
        ...
        # Create nested runs
        with mlflux.start_run(run_name='PARENT_RUN') as parent_run:
            mlflux.log_param("parent", "yes")
            with mlflux.start_run(run_name='CHILD_RUN', nested=True) as child_run:
                mlflux.log_param("child", "yes")
        ...
        query = "tags.mlflux.parentRunId = '{}'".format(parent_run.info.run_id)
        results = mlflux.search_runs(filter_string=query)
        print(results[["run_id", "params.child", "tags.mlflux.runName"]])
    ...
    .. code-block:: text
        ...
        --
                                     run_id params.child tags.mlflux.runName
    ...
    if len(_active_run_stack) > 0 and not nested:
        raise Exception(
            (
                ...
                + "current run with mlflux.end_run(). To start a nested "
...
def end_run(status: str = RunStatus.to_string(RunStatus.FINISHED)) -> None:
    """End an active mlflux run (if there is one).
    ...
    .. code-block:: python
        ...
        print("Active run: {}".format(mlflux.active_run()))
...
def active_run() -> Optional[ActiveRun]:
    ...
    **Note**: You cannot access currently-active run attributes
    (parameters, metrics, etc.) through the run returned by ``mlflux.active_run``. In order
    to access such attributes, use the :py:class:`mlflux.tracking.MlflowClient` as follows:
...
def get_run(run_id: str) -> Run:
    ...
    .. code-block:: python
        ...
        with mlflux.start_run() as run:
            mlflux.log_param("p", 0)

        run_id = run.info.run_id
        print("run_id: {}; lifecycle_stage: {}".format(run_id,
            mlflux.get_run(run_id).info.lifecycle_stage))
...
def set_tag(key: str, value: Any) -> None:
    ...
    .. code-block:: python
        ...
        with mlflux.start_run():
           mlflux.set_tag("release.version", "2.2.0")
    """
    run_id = _get_or_start_run().info.run_id
...
def delete_tag(key: str) -> None:
    ...
    .. code-block:: python
        ...
        with mlflux.start_run() as run:
            mlflux.set_tags(tags)

        with mlflux.start_run(run_id=run.info.run_id):
            mlflux.delete_tag("engineering_remote")
    """
    run_id = _get_or_start_run().info.run_id
...
def log_artifact(local_path: str, artifact_path: Optional[str] = None) -> None:
    ...
    .. code-block:: python
        ...
        with mlflux.start_run():
            mlflux.log_artifact("features.txt")
    """
    run_id = _get_or_start_run().info.run_id
...
def log_artifacts(local_dir: str, artifact_path: Optional[str] = None) -> None:
    ...
    .. code-block:: python
        ...
        with mlflux.start_run():
            mlflux.log_artifacts("data", artifact_path="states")
    """
    run_id = _get_or_start_run().info.run_id
...
def log_text(text: str, artifact_file: str) -> None:
    ...
    .. code-block:: python
        ...
        with mlflux.start_run():
            ...
            mlflux.log_text("text1", "file1.txt")

            # Log text in a subdirectory of the run's root artifact directory
            mlflux.log_text("text2", "dir/file2.txt")

            # Log HTML text
            mlflux.log_text("<h1>header</h1>", "index.html")
    """
    run_id = _get_or_start_run().info.run_id
...
def log_dict(dictionary: Any, artifact_file: str) -> None:
    ...
    .. code-block:: python
        ...
        with mlflux.start_run():
            ...
            mlflux.log_dict(dictionary, "data.json")

            # Log a dictionary as a YAML file in a subdirectory of the run's root artifact directory
            mlflux.log_dict(dictionary, "dir/data.yml")

            # If the file extension doesn't exist or match any of [".json", ".yaml", ".yml"],
            # JSON format is used.
            mlflux.log_dict(dictionary, "data")
            mlflux.log_dict(dictionary, "data.txt")
    """
    run_id = _get_or_start_run().info.run_id
...
def log_figure(
    ...
    .. code-block:: python
        ...
        with mlflux.start_run():
            mlflux.log_figure(fig, "figure.png")
    ...
    .. code-block:: python
        ...
        with mlflux.start_run():
            mlflux.log_figure(fig, "figure.html")
    """
    run_id = _get_or_start_run().info.run_id
...
def log_image(image: Union["numpy.ndarray", "PIL.Image.Image"], artifact_file: str) -> None:
    ...
    .. code-block:: python
        ...
        with mlflux.start_run():
            mlflux.log_image(image, "image.png")
...
def get_experiment(experiment_id: str) -> Experiment:
    ...
    .. code-block:: python
        ...
        experiment = mlflux.get_experiment("0")
...
def get_experiment_by_name(name: str) -> Optional[Experiment]:
    ...
    :return: An instance of :py:class:`mlflux.entities.Experiment`
    ...
    .. code-block:: python
        ...
        # Case sensitive name
        experiment = mlflux.get_experiment_by_name("Default")
        print("Experiment_id: {}".format(experiment.experiment_id))
        print("Artifact Location: {}".format(experiment.artifact_location))
        print("Tags: {}".format(experiment.tags))
        print("Lifecycle_stage: {}".format(experiment.lifecycle_stage))
...
def list_experiments(
    ...
    """
        ...
        :return: A list of :py:class:`Experiment <mlflux.entities.Experiment>` objects.
        """
...
def create_experiment(name: str, artifact_location: Optional[str] = None) -> str:
    ...
    .. code-block:: python
        ...
        experiment_id = mlflux.create_experiment("Social NLP Experiments")
        experiment = mlflux.get_experiment(experiment_id)
        print("Name: {}".format(experiment.name))
        print("Experiment_id: {}".format(experiment.experiment_id))
        print("Artifact Location: {}".format(experiment.artifact_location))
        print("Tags: {}".format(experiment.tags))
...
def delete_experiment(experiment_id: str) -> None:
    ...
    .. code-block:: python
        ...
        experiment_id = mlflux.create_experiment("New Experiment")
        mlflux.delete_experiment(experiment_id)
...
def get_artifact_uri(artifact_path: Optional[str] = None) -> str:
    ...
    .. code-block:: python
        ...
        with mlflux.start_run():
            mlflux.log_artifact("features.txt", artifact_path="features")
            ...
            artifact_uri = mlflux.get_artifact_uri(artifact_path="features/features.txt")
            print("Artifact uri: {}".format(artifact_uri))
...
def search_runs(
    ...
    :param output_format: The output format to be returned. If ``pandas``, a ``pandas.DataFrame``
                          is returned and, if ``list``, a list of :py:class:`mlflux.entities.Run`
                          is returned.

    :return: If output_format is ``list``: a list of :py:class:`mlflux.entities.Run`. If
    ...
    .. code-block:: python
        ...
        with mlflux.start_run(experiment_id=experiment_id):
            mlflux.log_metric("m", 1.55)
            mlflux.set_tag("s.release", "1.1.0-RC")
        with mlflux.start_run(experiment_id=experiment_id):
            mlflux.log_metric("m", 2.50)
            mlflux.set_tag("s.release", "1.2.0-GA")

        # Search all runs in experiment_id
        df = mlflux.search_runs([experiment_id], order_by=["metrics.m DESC"])
        ...
        df = mlflux.search_runs([experiment_id], filter_string=filter_string)
        print(df[["metrics.m", "tags.s.release", "run_id"]])
    ...
    if output_format == "list":
        return runs  # List[mlflux.entities.run.Run]
...
def list_run_infos(
    ...
    :return: A list of :py:class:`RunInfo <mlflux.entities.RunInfo>` objects that satisfy the
        search expressions.
    ...
    .. code-block:: python
        ...
        # Create two runs
        with mlflux.start_run() as run1:
            mlflux.log_param("p", 0)

        with mlflux.start_run() as run2:
            mlflux.log_param("p", 1)

        # Delete the last run
        mlflux.delete_run(run2.info.run_id)
        ...
        print("Active runs:")
        print_run_infos(mlflux.list_run_infos("0", run_view_type=ViewType.ACTIVE_ONLY))

        print("Deleted runs:")
        print_run_infos(mlflux.list_run_infos("0", run_view_type=ViewType.DELETED_ONLY))

        print("All runs:")
        print_run_infos(mlflux.list_run_infos("0", run_view_type=ViewType.ALL))
...
@autologging_integration("mlflux")
def autolog(
    ...
    .. code-block:: python
        ...
        mlflux.autolog(log_models=False, exclusive=True)
        import sklearn
    ...
    .. code-block:: python
        ...
        mlflux.autolog(log_models=False, exclusive=True)
        import sklearn
        mlflux.sklearn.autolog(log_models=True)

    would enable autologging for `sklearn` with `log_models=True` and `exclusive=False`,
    the latter resulting from the default value for `exclusive` in `mlflux.sklearn.autolog`;
    other framework autolog functions (e.g. `mlflux.tensorflow.autolog`) would use the
    configurations set by `mlflux.autolog` (in this instance, `log_models=False`, `exclusive=True`),
    ...
    :param log_model_signatures: If ``True``,
                                 ...
                                 not logged. Note: Model signatures are mlflux model attributes
                                 and are only collected if ``log_models`` is also ``True``.
    ...
    :param disable_for_unsupported_versions: If ``True``, disable autologging for versions of
                      ...
                      of the mlflux client or are incompatible.
    :param silent: If ``True``, suppress all event logs and warnings from mlflux during autologging
    ...
    # eg: mxnet.gluon is the actual library, mlflux.gluon.autolog is our autolog function for it
    LIBRARY_TO_AUTOLOG_FN = {
        ...
        # Pytorch frameworks under mlflux.pytorch.autolog
    ...
    def setup_autologging(module):
        try:
            ...
            # Only call integration's autolog function with `mlflux.autolog` configs
            # if the integration's autolog function has not already been called by the user.
            # Logic is as follows:
            # - if a previous_config exists, that means either `mlflux.autolog` or
            #   `mlflux.integration.autolog` was called.
            # - if the config contains `CONF_KEY_IS_GLOBALLY_CONFIGURED`, the configuration
            #   was set by `mlflux.autolog`, and so we can safely call `autolog_fn` with
            #   `autologging_params`.
            # - if the config doesn't contain this key, the configuration was set by an
            #   `mlflux.integration.autolog` call, so we should not call `autolog_fn` with
            #   new configs.
