"""
Internal package providing a Python CRUD interface to mlflux experiments and runs.
This is a lower level API than the :py:mod:`mlflux.tracking.fluent` module, and is
exposed in the :py:mod:`mlflux.tracking` module.
"""
...
from mlflux.tracking._tracking_service import utils
...
from mlflux.entities import Param, Metric, RunStatus, RunTag, ViewType, ExperimentTag
from mlflux.store.artifact.artifact_repository_registry import get_artifact_repository
from mlflux.utils.mlflow_tags import MLFLOW_USER
from mlflux.utils.string_utils import is_string_type
from mlflux.utils.uri import add_databricks_profile_info_to_artifact_uri
from collections import OrderedDict


class TrackingServiceClient(object):
    """
    Client of an mlflux Tracking Server that creates and manages experiments and runs.
    """
    ...
    def get_run(self, run_id):
        """
        Fetch the run from backend store. The resulting :py:class:`Run <mlflux.entities.Run>`
        contains a collection of run metadata -- :py:class:`RunInfo <mlflux.entities.RunInfo>`,
        as well as a collection of run parameters, tags, and metrics --
        :py:class:`RunData <mlflux.entities.RunData>`. In the case where multiple metrics with the
        same key are logged for the run, the :py:class:`RunData <mlflux.entities.RunData>` contains
        the most recently logged value at the largest step for each metric.

        :param run_id: Unique identifier for the run.

        :return: A single :py:class:`mlflux.entities.Run` object, if the run exists. Otherwise,
                 raises an exception.
        """
    ...
    def get_metric_history(self, run_id, key):
        ...
        :return: A list of :py:class:`mlflux.entities.Metric` entities if logged, else empty list
        """
    ...
    def create_run(self, experiment_id, start_time=None, tags=None):
        """
        Create a :py:class:`mlflux.entities.Run` object that can be associated with
        metrics, parameters, artifacts, etc.
        Unlike :py:func:`mlflux.projects.run`, creates objects but does not run code.
        Unlike :py:func:`mlflux.start_run`, does not change the "active run" used by
        :py:func:`mlflux.log_param`.

        :param experiment_id: The ID of then experiment to create a run in.
        :param start_time: If not provided, use the current timestamp.
        :param tags: A dictionary of key-value pairs that are converted into
                     :py:class:`mlflux.entities.RunTag` objects.
        :return: :py:class:`mlflux.entities.Run` that was created.
        """
    ...
    def list_run_infos(
        ...
        :return: A :py:class:`PagedList <mlflux.store.entities.PagedList>` of
            :py:class:`RunInfo <mlflux.entities.RunInfo>` objects that satisfy the search
    ...
    def list_experiments(self, view_type=ViewType.ACTIVE_ONLY, max_results=None, page_token=None):
        ...
        :return: A :py:class:`PagedList <mlflux.store.entities.PagedList>` of
                 :py:class:`Experiment <mlflux.entities.Experiment>` objects. The pagination token
    ...
    def get_experiment(self, experiment_id):
        ...
        :return: :py:class:`mlflux.entities.Experiment`
        """
    ...
    def _record_logged_model(self, run_id, mlflow_model):
        ...
        if not isinstance(mlflow_model, Model):
            raise TypeError(
                "Argument 'mlflow_model' should be of type mlflux.models.Model but was "
    ...
    def list_artifacts(self, run_id, path=None):
        ...
        :return: List of :py:class:`mlflux.entities.FileInfo`
        """
    ...
    def set_terminated(self, run_id, status=None, end_time=None):
        ...
        :param status: A string value of :py:class:`mlflux.entities.RunStatus`.
    ...
    def search_runs(
        ...
        :param run_view_type: one of enum values ACTIVE_ONLY, DELETED_ONLY, or ALL runs
                              defined in :py:class:`mlflux.entities.ViewType`.
        ...
        :return: A :py:class:`PagedList <mlflux.store.entities.PagedList>` of
            :py:class:`Run <mlflux.entities.Run>` objects that satisfy the search expressions.
