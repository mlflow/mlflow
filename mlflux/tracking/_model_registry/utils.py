...
from mlflux.utils import rest_utils
...
#    ``ModelRegistryClient``.
#  - via a utility method ``mlflux.set_registry_uri``
...
#    different ``Store`` classes (e.g. ``mlflux.store.tracking.SQLAlchemyStore`` &
#    ``mlflux.store.model_registry.SQLAlchemyStore``).
...
def set_registry_uri(uri: str) -> None:
    ...
    .. code-block:: python
        ...
        mlflux.set_registry_uri("sqlite:////tmp/registry.db")
        mr_uri = mlflux.get_registry_uri()
        print("Current registry uri: {}".format(mr_uri))
        tracking_uri = mlflux.get_tracking_uri()
