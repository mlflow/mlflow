from mlflux.tracking.registry import StoreRegistry
...
class ModelRegistryStoreRegistry(StoreRegistry):
    ...
    `mlflux.registry_store` group can be automatically registered through the
    ...
    def __init__(self):
        super().__init__("mlflux.model_registry_store")
    ...
    def get_store(self, store_uri=None):
        ...
        :return: An instance of `mlflux.store.model_registry.AbstractStore` that fulfills the
                 store URI requirements.
        """
        from mlflux.tracking._model_registry import utils
