"""
Internal package providing a Python CRUD interface to mlflux experiments, runs, registered models,
and model versions. This is a lower level API than the :py:mod:`mlflux.tracking.fluent` module,
and is exposed in the :py:mod:`mlflux.tracking` module.
"""
import contextlib
import logging
import json
...
from mlflux.entities import Experiment, Run, RunInfo, Param, Metric, RunTag, FileInfo, ViewType
from mlflux.store.entities.paged_list import PagedList
from mlflux.entities.model_registry import RegisteredModel, ModelVersion
from mlflux.entities.model_registry.model_version_stages import ALL_STAGES
from mlflux.exceptions import MlflowException
from mlflux.protos.databricks_pb2 import FEATURE_DISABLED
...
from mlflux.tracking._model_registry import utils as registry_utils
...
from mlflux.tracking.artifact_utils import _upload_artifacts_to_databricks
...
from mlflux.utils.uri import is_databricks_uri, construct_run_url
...
class MlflowClient(object):
    """
    Client of an mlflux Tracking Server that creates and manages experiments and runs, and of an
    mlflux Registry Server that creates and manages registered models and model versions. It's a
    ...
    def __init__(self, tracking_uri: Optional[str] = None, registry_uri: Optional[str] = None):
        ...
        :param tracking_uri: Address of local or remote tracking server. If not provided, defaults
                             to the service set by ``mlflux.tracking.set_tracking_uri``. See
                             `Where Runs Get Recorded <../tracking.html#where-runs-get-recorded>`_
                             for more info.
        :param registry_uri: Address of local or remote model registry server. If not provided,
                             defaults to the service set by ``mlflux.tracking.set_registry_uri``. If
    ...
    def _get_registry_client(self):
        ...
        :raises: py:class:`mlflux.exceptions.MlflowException` if the py:class:`ModelRegistryClient`
    ...
    def list_run_infos(
        ...
        .. code-block:: python
            ...
            with mlflux.start_run() as run1:
                mlflux.log_metric("click_rate", 1.55)

            with mlflux.start_run() as run2:
                mlflux.log_metric("click_rate", 2.50)
    ...
    def log_metric(
        ...
        .. code-block:: python
            ...
            # Log the metric. Unlike mlflux.log_metric this method
    ...
    def log_param(self, run_id: str, key: str, value: Any) -> None:
        ...
        .. code-block:: python
            ...
            # Log the parameter. Unlike mlflux.log_param this method
            # does not start a run if one does not exist. It will log
    ...
    def log_batch(
        ...
        .. code-block:: python
            ...
            # Create mlflux entities and a run under the default experiment (whose id is '0').
    ...
    def log_dict(self, run_id: str, dictionary: Any, artifact_file: str) -> None:
        ...
        .. code-block:: python
            ...
            # JSON format is used.
            mlflux.log_dict(run_id, dictionary, "data")
            mlflux.log_dict(run_id, dictionary, "data.txt")
        """
    ...
    def get_latest_versions(self, name: str, stages: List[str] = None) -> ModelVersion:
        ...
        .. code-block:: python
            ...
            # Create two runs Log mlflux entities
            with mlflux.start_run() as run1:
    ...
    def delete_model_version(self, name: str, version: str) -> None:
        ...
        .. code-block:: python
            ...
            # Create two runs and log mlflux entities
            with mlflux.start_run() as run1:
                params = {"n_estimators": 3, "random_state": 42}
                rfr = RandomForestRegressor(**params).fit([[0, 1]], [1])
                mlflux.log_params(params)
    ...
    def get_model_version_download_uri(self, name: str, version: str) -> str:
        ...
        .. code-block:: python
            ...
            with mlflux.start_run() as run:
                mlflux.log_params(params)
                mlflux.sklearn.log_model(rfr, artifact_path="models/sklearn-model")
