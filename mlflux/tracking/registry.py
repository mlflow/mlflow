...
class UnsupportedModelRegistryStoreURIException(MlflowException):
    ...
    def __init__(self, unsupported_uri, supported_uri_schemes):
        message = (
            ...
            " See https://www.mlflux.org/docs/latest/tracking.html#storage for"
            " how to run an mlflux server against one of the supported backend storage"
            " locations."
...
class StoreRegistry:
    ...
    def get_store_builder(self, store_uri):
        ...
        :return: A function that returns an instance of
                 ``mlflux.store.{tracking|model_registry}.AbstractStore`` that fulfills the store
                  URI requirements.
        """
