...
def load_model(model_uri, ctx):
    ...
    .. code-block:: python
        ...
        model = mlflux.gluon.load_model("runs:/" + gluon_random_data_run.info.run_id + "/model")
...
def _load_pyfunc(path):
    ...
    :param path: Local filesystem path to the mlflux Model with the ``gluon`` flavor.
    """
    import mxnet as mx
...
def save_model(
    ...
    :param conda_env: Either a dictionary representation of a Conda environment or
                      ...
                      :func:`mlflux.gluon.get_default_conda_env()` environment is added to
    ...
    .. code-block:: python
        ...
        mlflux.gluon.save_model(net, gluon_model_path)
    """
    ...
    pyfunc.add_to_model(mlflow_model, loader_module="mlflux.gluon", env=conda_env_subpath)
...
def log_model(
    ...
    """
    Log a Gluon model as an mlflux artifact for the current run.
    ...
    .. code-block:: python
        ...
        with mlflux.start_run():
            est.fit(train_data=train_data, epochs=100, val_data=validation_data)
            mlflux.gluon.log_model(net, "model")
    """
    Model.log(
...
def autolog(
    ...
    """
    Enables (or disables) and configures autologging from Gluon to mlflux.
    ...
    :param disable_for_unsupported_versions: If ``True``, disable autologging for versions of
                      gluon that have not been tested against this version of the mlflux client
                      or are incompatible.
    :param silent: If ``True``, suppress all event logs and warnings from mlflux during MXNet Gluon
    ...
    def getGluonCallback(metrics_logger):
        class __MLflowGluonCallback(EpochEnd, TrainEnd, TrainBegin, metaclass=ExceptionSafeClass):
            ...
            def train_begin(self, estimator, *args, **kwargs):
                try_mlflow_log(mlflux.log_param, "num_layers", len(estimator.net))
                if estimator.max_epoch is not None:
                    try_mlflow_log(mlflux.log_param, "epochs", estimator.max_epoch)
                if estimator.max_batch is not None:
                    try_mlflow_log(mlflux.log_param, "batches", estimator.max_batch)
                try_mlflow_log(
                    mlflux.log_param, "optimizer_name", type(estimator.trainer.optimizer).__name__
                )
                if hasattr(estimator.trainer.optimizer, "lr"):
                    try_mlflow_log(
                        mlflux.log_param, "learning_rate", estimator.trainer.optimizer.lr
                    )
                if hasattr(estimator.trainer.optimizer, "epsilon"):
                    try_mlflow_log(mlflux.log_param, "epsilon", estimator.trainer.optimizer.epsilon)
