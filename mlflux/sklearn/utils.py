...
from mlflux.utils.mlflow_tags import MLFLOW_PARENT_RUN_ID
...
def _get_estimator_info_tags(estimator):
    """
    :return: A dictionary of mlflux run tag keys and values
...
def _log_parameter_search_results_as_artifact(cv_results_df, run_id):
    """
    Records a collection of parameter search results as an mlflux artifact
    ...
    :param cv_results_df: A Pandas DataFrame containing the results of a parameter search
                          ...
                          `GridSearchCV`.
    :param run_id: The ID of the mlflux Run to which the artifact should be recorded.
    """
    with TempDir() as t:
...
def _create_child_runs_for_parameter_search(
    ...
    :param cv_estimator: The trained parameter search estimator for which to create
                         child runs.
    :param parent_run: A py:class:`mlflux.entities.Run` object referring to the parent
                       parameter search run for which child runs should be created.
    :param child_tags: An optional dictionary of mlflux tag keys and values to log
                       for each child run.
    """
    import pandas as pd
