...
from mlflux.utils.process import exec_cmd
...
if os.getenv(PROMETHEUS_EXPORTER_ENV_VAR):
    from mlflux.server.prometheus_exporter import activate_prometheus_exporter
...
def serve():
    ...
    text = textwrap.dedent(
        """
    Unable to display mlflux UI - landing page (index.html) not found.

    You are very likely running the mlflux server using a source installation of the Python mlflux
    package.

    If you are a developer making mlflux source code changes and intentionally running a source
    installation of mlflux, you can view the UI by running the Javascript dev server:
    https://github.com/mlflux/mlflux/blob/master/CONTRIBUTING.rst#running-the-javascript-dev-server

    Otherwise, uninstall mlflux via 'pip uninstall mlflux', reinstall an official mlflux release
    from PyPI via 'pip install mlflux', and rerun the mlflux server.
    """
    )
...
def _build_waitress_command(waitress_opts, host, port):
    ...
    return (
        ["waitress-serve"]
        + opts
        + ["--host=%s" % host, "--port=%s" % port, "--ident=mlflux", "mlflux.server:app"]
    )
...
def _build_gunicorn_command(gunicorn_opts, host, port, workers):
    ...
    return ["gunicorn"] + opts + ["-b", bind_address, "-w", "%s" % workers, "mlflux.server:app"]
...
def _run_server(
    ...
    """
    Run the mlflux server, wrapping it in gunicorn or waitress on windows
