...
from mlflux.spark import FLAVOR_NAME

_JAVA_PACKAGE = "org.mlflux.spark.autologging"
...
def _listen_for_spark_activity(spark_context):
    ...
    try:
        ...
        raise MlflowException(
            ...
            "and ensure you have the mlflux-spark JAR attached to your Spark "
            "session as described in "
            "http://mlflux.org/docs/latest/tracking.html#"
            "automatic-logging-from-spark-experimental. "
            "Exception:\n%s" % e
        )

    # Register context provider for Spark autologging
    from mlflux.tracking.context.registry import _run_context_provider_registry
...
class PythonSubscriber(object, metaclass=ExceptionSafeClass):
    ...
    information propagated from Java to the current mlflux run, starting a run if necessary.
    class implements a Java interface (org.mlflux.spark.autologging.MlflowAutologEventSubscriber,
    defined in the mlflux-spark package) that's called-into by autologging logic in the JVM in order
