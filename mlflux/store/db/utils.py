...
from mlflux.protos.databricks_pb2 import INTERNAL_ERROR
from mlflux.store.db.db_types import SQLITE
...
def _get_package_dir():
    """Returns directory containing mlflux python package."""
    current_dir = os.path.dirname(os.path.abspath(__file__))
    return os.path.normpath(os.path.join(current_dir, os.pardir, os.pardir))


def _initialize_tables(engine):
    _logger.info("Creating initial mlflux database tables...")
...
def _verify_schema(engine):
    ...
    if current_rev != head_revision:
        raise MlflowException(
            ...
            "Take a backup of your database, then run 'mlflux db upgrade <database_uri>' "
...
def _get_alembic_config(db_url, alembic_dir=None):
    ...
    directory under mlflux/alembic if unspecified. TODO: remove this argument in mlflux 1.1, as
