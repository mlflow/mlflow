...
from mlflux.store.artifact.dbfs_artifact_repo import dbfs_artifact_repo_factory
from mlflux.store.artifact.ftp_artifact_repo import FTPArtifactRepository
from mlflux.store.artifact.gcs_artifact_repo import GCSArtifactRepository
from mlflux.store.artifact.hdfs_artifact_repo import HdfsArtifactRepository
from mlflux.store.artifact.local_artifact_repo import LocalArtifactRepository
from mlflux.store.artifact.models_artifact_repo import ModelsArtifactRepository
...
from mlflux.utils.uri import get_uri_scheme
...
class ArtifactRepositoryRegistry:
    ...
    the entrypoints `mlflux.artifact_repository` group can be automatically registered through the
    ...
    def register_entrypoints(self):
        # Register artifact repositories provided by other packages
        for entrypoint in entrypoints.get_group_all("mlflux.artifact_repository"):
            try:
    ...
    def get_artifact_repository(self, artifact_uri):
        ...
        :return: An instance of `mlflux.store.ArtifactRepository` that fulfills the artifact URI
                 requirements.
        """
