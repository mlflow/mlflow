    ...
    for line in lines:
        if line.startswith("diff --git"):
            if match := re.match(r"diff --git a/(.*?) b/(.*?)$", line):
                ...
                is_deleted = b_path == "dev/null"
                file_path = a_path if is_deleted else b_path
                ...
                if file_patterns and not any(
                    ...
                    is_masked = is_deleted or is_autogenerated_file(file_path)
            else:
                in_included_file = False
                is_masked = False
                is_deleted = False
    ...
    for line in filtered_diff:
        if line.startswith("@@"):
            ...
            if result_lines:
                result_lines.append("")
            result_lines.append(line)
        elif line.startswith(("index ", "---", "+++")) or line in (
            _MASKED_DIFF_MESSAGE,
            _DELETED_DIFF_MESSAGE,
        ):
            result_lines.append(line)
        elif line.startswith("-"):
