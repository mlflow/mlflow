...
def filter_diff(full_diff: str, file_patterns: list[str] | None = None) -> str:
    """Filter diff output and add line numbers.

    Auto-generated files (protobuf, lock files, generated Java) are included
    with their diff headers but their hunk content is replaced with a mask
    message so reviewers know the PR touches them without being overwhelmed
    by noise.
    """
        ...
        if line.startswith("diff --git"):
            ...
            if is_masked:
                if line.startswith("@@"):
                    ...
                    if (
                        not filtered_diff
                        or filtered_diff[-1] != "[Auto-generated file - diff masked]"
                    ):
                        filtered_diff.append("[Auto-generated file - diff masked]")
    ...
    for line in filtered_diff:
        if line.startswith("@@"):
            ...
            or line.startswith("---")
            or line.startswith("+++")
            or line == "[Auto-generated file - diff masked]"
        ):
            result_lines.append(line)
        elif line.startswith("-"):
