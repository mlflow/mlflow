# ruff: noqa: T201
"""Validate a ui-review JSON payload against the schema in ``.claude/skills/ui-review/``."""
...
DEFAULT_SCHEMA = Path(__file__).parents[3] / "ui-review" / "ui-review-payload.schema.json"
...
def register(subparsers: argparse._SubParsersAction[argparse.ArgumentParser]) -> None:
    parser = subparsers.add_parser(
        "validate-ui-review",
        help="Validate a ui-review payload against the JSON schema",
    )
    parser.add_argument("payload", type=Path, help="Path to the UI-review payload JSON file")
    parser.add_argument(
        "--schema",
        type=Path,
