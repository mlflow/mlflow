...
MLflow projects can be [explicitly created](https://www.mlflow.org/docs/latest/projects.html#specifying-projects) or implicitly used by running `R` files using `mlflow_run()`, from the terminal we can run:
...
# read parameters
column <- mlflow_log_param("column", 1, "column index to use as feature")
...
In order to contribute, follow the [contribution guidelines](../../CONTRIBUTING.rst). After performing python changes, or to pick the latest changes, you can install with the following code. Note that [mlflow/pull/193](https://github.com/databricks/mlflow/pull/193) is currently required to run python projects locally.
