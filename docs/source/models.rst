...
MLflow can deploy models to Azure Machine Learning using the MLflow plugin `azureml-mlflow <https://pypi.org/project/azureml-mlflow/>`_. Models can be deployed to Azure Kubernetes Service (AKS) and the Azure Container Instances (ACI)
platform for real-time serving. 
  
The resulting Azure ML ContainerImage contains a web server that accepts the following data formats as input:
...
Deployments can be generated using both the Python API or MLflow CLI
...
.. code-block:: bash
    ...
    mlflow deployments -n <deployment-name> -m models:/<model-name>/<model-version> -t <azureml-mlflow-tracking-url>
...
.. code-block:: bash
    ...
    mlflow deployments run-local -n <deployment-name> -m models:/<model-name>/<model-version> -t <azureml-mlflow-tracking-url>

For more info, see:

.. code-block:: bash

    mlflow deployments -t azureml --help
