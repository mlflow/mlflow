...
.. code:: bash
    ...
    az ml env set -n <local-env> -g <local-env-resource-group> - set environment to local deployment
    mlflow azureml deploy <parameters> - deploy locally to test the model
    az ml env set -n <cluster-env> -g <cluster-env-resource-group> - set environment to cluster
