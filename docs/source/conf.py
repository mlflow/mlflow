#!/usr/bin/env python3
#
# mlflux documentation build configuration file, created by
...
project = "mlflux"
copyright = "mlflux Project, a Series of LF Projects, LLC. All rights reserved"
author = "mlflux"
...
import mlflux.version

# The short X.Y version.
version = mlflux.version.VERSION
# The full version, including alpha/beta/rc tags.
release = mlflux.version.VERSION
...
latex_documents = [
    (master_doc, "mlflux.tex", "mlflux Documentation", "Databricks", "manual"),
]
...
man_pages = [(master_doc, "mlflux", "mlflux Documentation", [author], 1)]
...
nitpicky = True
nitpick_ignore = [
    # Ignore a missing reference in `mlflux/store/entities/paged_list.py`
    ("py:class", "T"),
    ...
    ("py:class", "mlflux.types.schema.DataType"),
    ("py:class", "mlflux.types.schema.ColSpec"),
    ("py:class", "mlflux.types.schema.TensorSpec"),
    ("py:class", "mlflux.types.schema.Schema"),
    ("py:class", "mlflux.models.model.Model"),
    ("py:class", "mlflux.models.signature.ModelSignature"),
...
def _get_reference_map():
    ...
    def start_run(...) -> ActiveRun:
        # ActiveRun is defined in `mlflux/tracking/fluent.py`
        ...
    ```

    For this code, Sphinx tries to create a link for `ActiveRun` using
    `mlflux.tracking.fluent.ActiveRun` as a reference target, but the module
    `mlflux.tracking.fluent` is undocumented, so Sphinx raises this warning:
    `WARNING: py:class reference target not found: mlflux.tracking.fluent.ActiveRun`.
    As a workaround, replace `mlflux.tracking.fluent.ActiveRun` with `mlflux.ActiveRun`.
    """
    ref_map = {
        # < Invalid reference >: < valid reference >
        "mlflux.tracking.fluent.ActiveRun": "mlflux.ActiveRun",
        "mlflux.store.entities.paged_list.PagedList": "mlflux.store.entities.PagedList",
    }

    # Tracking entities
    for entity_name in mlflux.entities.__all__:
        entity_cls = getattr(mlflux.entities, entity_name)
        invalid_ref = entity_cls.__module__ + "." + entity_name
        valid_ref = "mlflux.entities.{}".format(entity_name)
        ref_map[invalid_ref] = valid_ref

    # Model registry entities
    for entity_name in mlflux.entities.model_registry.__all__:
        entity_cls = getattr(mlflux.entities.model_registry, entity_name)
        invalid_ref = entity_cls.__module__ + "." + entity_name
        valid_ref = "mlflux.entities.model_registry.{}".format(entity_name)
        ref_map[invalid_ref] = valid_ref
