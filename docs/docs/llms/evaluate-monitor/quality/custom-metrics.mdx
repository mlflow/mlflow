...
- [Pass/fail metrics](#pass-fail): `"yes" or "no"` string values render as “Pass” or “Fail” in the UI.
- [Numeric metrics](#numeric): Ordinal values: integers or floats.
- [Boolean metrics](#boolean): `True` or `False`.
...
Boolean metrics evaluate to `True` or `False`. These are useful for binary decisions, such as checking whether a response meets a simple heuristic. If you want the metric to have a special pass/fail treatment in the UI, see [pass/fail metrics](#pass-fail).
...
This example is not copy-pastable as it does not define the LangGraph agent. See the [attached notebook](#notebook) for the fully-runnable example.
