...
You can pass domain-specific examples to the built-in judges by providing a few `"yes"` or `"no"` examples for each type of assessment. These examples are referred to as **few-shot** examples and can help the built-in judges align better with domain-specific rating criteria. See TODO LINK.
