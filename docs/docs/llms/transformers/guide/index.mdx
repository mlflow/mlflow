...
The `transformers` [python_function (pyfunc) model flavor](/model#pyfunc-model-flavor) simplifies
...
The table below shows the mapping of `transformers` pipeline types to the [python_function (pyfunc) model flavor](/model#pyfunc-model-flavor)
...
For a more in-depth guide, check out the [Prompt Templating notebook](/llms/transformers/tutorials/prompt-templating/prompt-templating/)!
...
:::note
Overriding the data type for a pipeline when loading as a [python_function (pyfunc) model flavor](/model#pyfunc-model-flavor) is not supported.
...
The Audio Pipeline types, when loaded as a [python_function (pyfunc) model flavor](/model#pyfunc-model-flavor) have three input types available:
...
[train a Mistral/Llama2 7B model on a single Nvidia A10G GPU in an hour](/llms/transformers/tutorials/fine-tuning/transformers-peft/).
...
Check out the tutorial [Fine-Tuning Open-Source LLM using QLoRA with MLflow and PEFT](/llms/transformers/tutorials/fine-tuning/transformers-peft/)
