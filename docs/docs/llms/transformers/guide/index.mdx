---
sidebar_label: Transformers within MLflow
---
...
please refer to the [MLflow Transformers Flavor](/llms/transformers) page.
...
The `transformers` [python_function (pyfunc) model flavor](/models#pyfunc-model-flavor) simplifies
...
The table below shows the mapping of `transformers` pipeline types to the [python_function (pyfunc) model flavor](/models#pyfunc-model-flavor)
...
For a more in-depth guide, check out the [Prompt Templating notebook](/llms/transformers/tutorials/prompt-templating/prompt-templating.html)!
...
:::note
Overriding the data type for a pipeline when loading as a [python_function (pyfunc) model flavor](/models#pyfunc-model-flavor) is not supported.
...
The Audio Pipeline types, when loaded as a [python_function (pyfunc) model flavor](/models#pyfunc-model-flavor) have three input types available:
...
:::warning
The PEFT model is supported in MLflow 2.11.0 and above and is still in the experimental stage. The API and behavior may change in future releases.
Moreover, the [PEFT](https://huggingface.co/docs/peft/en/index) library is under active development, so not all features
...
[train a Mistral/Llama2 7B model on a single Nvidia A10G GPU in an hour](/llms/transformers/tutorials/fine-tuning/transformers-peft.html).
...
Check out the tutorial [Fine-Tuning Open-Source LLM using QLoRA with MLflow and PEFT](/llms/transformers/tutorials/fine-tuning/transformers-peft.html)
...
Since the model saving/loading behavior for PEFT models is similar to that of `save_pretrained=False`, [the same caveats](/llms/transformers/large-models#caveats-of-save-pretrained) apply to PEFT models.
...
Please refer to [this section](/llms/transformers/large-models#persist-pretrained-guide) for the detailed usage of this API.
