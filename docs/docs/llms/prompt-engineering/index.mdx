...
[MLflow AI Gateway](/llms/deployments), parameter configurations, and prompts to build a variety of models for
...
Every model created with the prompt engineering UI is stored in the [MLflow Model](/model)
format and can be deployed for batch or real time inference. All configurations (prompt templates,
choice of LLM, parameters, etc.) are tracked as [MLflow Runs](/tracking).
...
To use the prompt engineering UI, you need to create one or more [MLflow AI Gateway](/llms/deployments)
completions or chat [Endpoints](/llms/deployments#deployments-endpoints). Follow the
[MLflow AI Gateway Quickstart guide](/llms/deployments#deployments-quickstart) to easily create an endpoint in less than five
...
questions about [MLflow Projects](/projects) even if the answer does not appear in the
specified article.
...
If you'd like to assess your model's performance on specific metrics, MLflow provides the <APILink fn="mlflow.genai.evaluate" />
API. Let's evaluate our model on some [pre-defined metrics](/llms/llm-evaluate#llm-eval-default-metrics)
...
with mlflow.start_run():
    results = mlflow.genai.evaluate(
        predict_fn=mlflow.genai.to_predict_fn(logged_model),
        data=data,
    )
...
![](/images/evaluate_metrics.png)

The <APILink fn="mlflow.genai.evaluate" /> API also supports [custom metrics](/llms/llm-evaluate#llm-eval-custom-scorers),
[static dataset evaluation](/llms/llm-evaluate#llm-eval-static-dataset), and much more. For a
more in-depth guide, see [MLflow LLM Evaluation](/llms/llm-evaluate).
