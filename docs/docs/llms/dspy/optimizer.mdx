...
Historically optimizers have following challenges due to its black box nature:
...
Note that the optimizer tracking feature is available since MLflow `2.21.1`.
...
<table>
    ...
    <tbody>
        <tr>
            ...
            <td>`true`</td>
            <td>`log_traces`</td>
            <td>Whether to generate and log traces for the program. See [MLflow Tracing](/tracing/) for more details about tracing feature.</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>`false`</td>
            <td>`log_evals`</td>
            <td>If set to `True`, a MLflow run is created for each `Evaluation.__call__` call, and metrics and artifacts for the evaluation are logged.</td>
        </tr>
    </tbody>
</table>

# Example Code of DSPy Optimizer Tracking
...
trainset, devset = gms8k.train, gms8k.dev[:50]
...
# What are logged?
MLflow optimizer tracking logs the following information:
...
When both `log_compile` and `log_evals` are set to `True`, MLflow creates a run for `Teleprompter.compile` with child runs for `Evaluation.__call__` calls. 
In the MLflow UI, they are displayed hierarchically as shown below:
<img src="https://i.imgur.com/582t3nV.png" alt="Experiment Page" width="700px"/>
