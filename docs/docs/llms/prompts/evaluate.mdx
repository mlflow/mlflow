...
- **Effective Evaluation**: `mlflow.evaluate` API provides a simple and consistent way to evaluate prompts across different models and datasets without writing boilerplate code.
...
:::tip
    ...
    There are numbers of built-in metrics available in MLflow for evaluating LLMs. You can also define custom metrics including LLM-as-a-Judge. Refer to the [LLM Evaluation Metrics](/llms/llm-evaluate#llm-evaluation-metrics) for more details.
