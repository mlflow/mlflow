...
When you deploy your agent with `agents.deploy()`, agent framework also creates and deploys a “feedback” model version within the same endpoint, which you can query to provide feedback on your agent application. Feedback entries appear as request rows within the [inference table](#agent-inference-tables) associated with your agent serving endpoint.
