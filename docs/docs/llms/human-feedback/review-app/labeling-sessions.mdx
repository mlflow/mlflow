...
- For chat with the bot:
  ...
  - Each human reviewer must have access to the review app workspace or be synced to your :re[Databricks] account with SCIM. 
...
For more info, see TODO LINK below.
...
- TODO LINK for labeling an evaluation dataset.
- TODO LINK for labeling traces. Note that a live agent is not required for this.
