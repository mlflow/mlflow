...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>`true`</td>
      <td>`log_traces`</td>
      <td>Whether to generate and log traces for the model. See [MLflow Tracing](/llms/tracing) for more details about the tracing feature.</td>
    </tr>
    <tr>
...
You can discover how to use the [fluent API here](/llms/tracing#tracing-fluent-apis).
...
![OpenAI Autologging stream traces](/images/tutorials/llms/openai-stream-trace.png)
...
However, you can still record streaming traces. When loading a the logged openai model as pyfunc via <APILink fn="mlflow.pyfunc.load_model" />, the only
...
Saving your async implementation is best done by using the [models from code feature](/models#models-from-code).
