...
When implementing your agent, you'll work with two main output types: `ResponsesAgentResponse` and `ResponsesAgentStreamEvent`. These are the only pydantic objects you should create directly. The remaining classes in `mlflow.types.responses_helpers` serve as validation utilities.
...
tools = [
    ToolInfo(
        ...
        exec_fn=lambda latitude, longitude: latitude
        + longitude,  # dummy tool implementation
    )
]
