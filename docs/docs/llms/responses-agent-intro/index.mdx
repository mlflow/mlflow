...
- yield a `response.output_item.done` event to aggregate all chunks and complete the message
...
class SimpleResponsesAgent(ResponsesAgent):
    ...
    def predict_stream(
        ...
        # the text output item id should be the same item_id as the streamed text deltas
        yield ResponsesAgentStreamEvent(
