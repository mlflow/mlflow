...
`ResponsesAgent` is a subclass of `PythonModel` that provides a framework-agnostic way to create an agent model. Authoring a `ResponsesAgent` provides the following benefits:
- Ensure your model is compatible with OpenAI Responses API (Read more about it in the [Responses API Introduction](./responses-api-intro))
- Support for multiple messages and intermediate steps
...
To author a ResponsesAgent, create a subclass of the ResponsesAgent and implement a predict method using any agent authoring framework. You must be using `pydantic>=2` to use ResponsesAgent.
...
### Streaming a ResponsesAgent


See the [example below](#example-responsesagent) for a full implementation!



## Log a ResponsesAgent

Log your ResponsesAgent using the [Models-from-code](/model/models-from-code) approach. This approach is framework-agnostic and supports all authoring frameworks:
```python
...
Here's an example of a ResponsesAgent that calls OpenAI's gpt-4o model with a simple tool.
...
class ToolCallingAgent(ResponsesAgent):
    ...
    def call_llm(self, input_messages) -> ResponsesStreamEvent:
        return (
    ...
    def handle_tool_call(self, tool_call: dict[str, Any]) -> ResponsesStreamEvent:
        """
        Execute tool calls and return a ResponsesStreamEvent w/ tool output
        """
        ...
        return ResponsesStreamEvent(type="response.output_item.done", item=tool_call_output)
    ...
    def predict(self, request: ResponsesRequest) -> ResponsesResponse:
        ...
        return ResponsesResponse(output=outputs, custom_outputs=request.custom_inputs)
...
tools = [
    ToolInfo(
        ...
        exec_fn=lambda x: x,  # dummy tool implementation
    )
]
