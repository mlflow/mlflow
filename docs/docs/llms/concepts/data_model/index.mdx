...
[`Traces`](trace) are annotated with [`Assessments`](trace/assessment) that describe their quality and operational performance (latency, cost, etc).  [`Assessments`](trace/assessment) are produced by either:
