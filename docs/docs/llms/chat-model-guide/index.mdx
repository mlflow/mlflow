...
:::warning attention
Starting in MLflow 3.0.0, we recommend <APILink fn="mlflow.pyfunc.ResponsesAgent">`ResponsesAgent`</APILink> instead of <APILink fn="mlflow.pyfunc.ChatModel">`ChatModel`</APILink>. See more details in the [ResponsesAgent Introduction](/llms/responses-agent-intro).
:::
...
- Integrate [MLflow Tracing](/tracing) into a custom <APILink fn="mlflow.pyfunc.ChatModel">`mlflow.pyfunc.ChatModel`</APILink> instance.
...
- MLflow version 2.14.0 or higher installed for use of [MLflow Tracing](/tracing).
...
<Tabs>
  <TabItem label="Tracing" value="tracing">
    ...
    [MLflow Tracing](/tracing) allows you to monitor and log the execution of your model's methods, providing valuable insights during debugging and performance optimization.
  ...
  <TabItem label="Pitfalls" value="pitfalls">
    ...
    will always be easier than implementing a custom model. It is recommended to check the [supported GenAI flavors](/llms#model-packaging)
...
- **Models from code**:
  - MLflow recommends using [models from code](/model/models-from-code) when authoring GenAI agents or apps, for robust logging and straightforward
...
This approach also integrates seamlessly with MLflow's evaluation tools, such as <APILink fn="mlflow.evaluate" />, which allows you to compare model
...
how the `model_config` parameter is utilized, [see the guide on model_config usage](/model#python-function-model-interfaces).
:::
