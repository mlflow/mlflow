...
:::tip Streaming predictions with `predict_stream`

In addition to `predict`, Mlflow's `ChatModel` can implement [`predict_stream`](https://mlflow.org/docs/latest/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.ChatModel.predict_stream) which allows for the streaming of [ChatCompletionChunks](https://mlflow.org/docs/latest/api_reference/python_api/mlflow.types.html#mlflow.types.llm.ChatCompletionChunk) returned to the client. This is particularly useful in interactive chat-bot type systems where providing information to users incrementally improves their experience. When implemented on Databricks Mosaic AI Model Serving, a ChatCompletionChunk will actually be sent to the client after the following ChatCompletionChunk has been yielded by the `predict_stream` generator. Therefore you can optionally yield an empty ChatCompletionChunk if you want a chunk to be immediately sent back to the client application.  :::
...
This is the "models from code" approach to model logging, which you can read more about [here](/model/models-from-code).
...
[GenAI model signature example](/model/signatures#genai-model-signature-example) section for details.
...
To learn more about these and other advanced features of the ChatModel API, you can read [this guide](/llms/chat-model-guide).
