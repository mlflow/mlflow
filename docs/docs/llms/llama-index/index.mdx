...
- [MLflow Tracking](/tracking) allows you to track your indices within MLflow and manage the many moving parts that comprise your LlamaIndex project, such as prompts, LLMs, workflows, tools, global configurations, and more.
- [MLflow Model](/model) packages your LlamaIndex index/engine/workflows with all its dependency versions, input and output interfaces, and other essential metadata. This allows you to deploy your LlamaIndex models for inference with ease, knowing that the environment is consistent across different stages of the ML lifecycle.
- [MLflow Evaluate](/llms/llm-evaluate) provides native capabilities within MLflow to evaluate GenAI applications. This capability facilitates the efficient assessment of inference results from your LlamaIndex models, ensuring robust performance analytics and facilitating quick iterations.
- [MLflow Tracing](/tracing) is a powerful observability tool for monitoring and debugging what happens inside the LlamaIndex models, helping you identify potential bottlenecks or issues quickly. With its powerful automatic logging capability, you can instrument your LlamaIndex application without needing to add any code apart from running a single command.
...
<CardGroup>
  <PageCard
    ...
    link="/llms/llama-index/notebooks/llama_index_workflow_tutorial/"
  ...
  <PageCard
    ...
    link="/llms/llama-index/notebooks/llama_index_quickstart/"
...
However, when the index uses external vector stores like `DatabricksVectorSearch` and `QdrantVectorStore`, the index data is stored remotely and they do not support local serialization. Thereby, you cannot log the index with these stores directly. For such cases, you can use the [Model-from-Code](/model#models-from-code) logging that provides more control over the index saving process and allow you to use the external vector store.
...
Mlflow supports logging and loading a LlamaIndex Workflow via the [Model-from-Code](/model#models-from-code) feature. For a detailed example of logging and loading a LlamaIndex Workflow, see the [LlamaIndex Workflows with MLflow](/llms/llama-index/notebooks/llama_index_workflow_tutorial/) notebook.
...
This also applies when deploying the logged LlamaIndex workflow to a production endpoint using [MLflow Deployment](/deployment) or Databricks [Model Serving](https://docs.databricks.com/en/machine-learning/model-serving/index.html).
:::
