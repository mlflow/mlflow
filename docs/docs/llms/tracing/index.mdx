...
If you are new to the tracing or observability concepts, we recommend starting with the [Tracing Concepts Overview](/llms/tracing/overview) guide.
...
:::info Hint
Is your favorite library missing from the list? Consider [contributing to MLflow Tracing](/llms/tracing/contribute) or [submitting a feature request](https://github.com/mlflow/mlflow/issues/new?assignees=&labels=enhancement&projects=&template=feature_request_template.yaml&title=%5BFR%5D) to our Github repository.
:::
...
<Tabs>
  <TabItem value="langchain" label="LangChain / LangGraph" default>
    ...
    In the full example below, the model and its associated metadata will be logged as a run, while the traces are logged separately to the active experiment. To learn more, please visit [LangChain Autologging documentation](/llms/langchain/autologging).
    ...
    ![LangChain Tracing via autolog](/images/llms/tracing/langchain-tracing.png)

  </TabItem>
  <TabItem value="openai" label="OpenAI">
    ### OpenAI Automatic Tracing

    The [MLflow OpenAI flavor](/llms/openai/)'s autologging feature has a direct integration with MLflow tracing. When OpenAI autologging is enabled with <APILink fn="mlflow.openai.autolog" />,
    ...
    To learn more about OpenAI autologging, you can [view the documentation here](/llms/openai/autologging).
  ...
  <TabItem value="swarm" label="Swarm">
    ...
    The [MLflow OpenAI flavor](/llms/openai/) supports automatic tracing for [Swarm](https://github.com/openai/swarm), a multi-agent orchestration
  ...
  <TabItem value="ollama" label="Ollama">
    ### Ollama Automatic Tracing

    [Ollama](https://github.com/ollama/ollama) is an open-source platform that enables users to run large
    language models (LLMs) locally on their devices, such as Llama 3.2, Gemma 2, Mistral, Code Llama, and more.

    Since the local LLM endpoint served by Ollama is compatible with the OpenAI API, you can query it via
    OpenAI SDK and enable tracing for Ollama with <APILink fn="mlflow.openai.autolog" />. Any LLM interactions via
    Ollama will be recorded to the active MLflow Experiment.
    ...
    ![Ollama Tracing](/images/llms/tracing/ollama-tracing.png)

  </TabItem>
  <TabItem value="instructor" label="Instructor">
    ### Instructor Automatic Tracing

    [Instructor](https://python.useinstructor.com) is an open-source Python library built on top of Pydantic,
    simplifying structured LLM outputs with validation, retries, and streaming.

    MLflow Tracing works with Instructor by enabling auto-tracing for the underlying LLM libraries.
    For example, if you use Instructor for OpenAI LLMs, you can enable tracing with <APILink fn="mlflow.openai.autolog" />
    and the generated traces will capture the structured outputs from Instructor.

    Similarly, you can also trace Instructor with other LLM providers, such as Anthropic, Gemini, and LiteLLM,
    by enabling the corresponding autologging in MLflow.
    ...
    ![Instructor Tracing](/images/llms/tracing/instructor-tracing.png)

  </TabItem>
  <TabItem value="llamaindex" label="LlamaIndex">
    ### LlamaIndex Automatic Tracing

    The [MLflow LlamaIndex flavor](/llms/llama-index/)'s autologging feature has a direct integration with MLflow tracing. When LlamaIndex autologging is enabled with <APILink fn="mlflow.llama_index.autolog" />, invocation of components
    ...
    To see the full example of tracing LlamaIndex, please visit [LLamaIndex Tracing documentation](/llms/llama-index).
  ...
  <TabItem value="dspy" label="DSPy">
    ...
    The [MLflow DSPy flavor](/llms/dspy/)'s autologging feature has a direct integration with MLflow tracing. When DSPy autologging is enabled with <APILink fn="mlflow.dspy.autolog" />, invocation of components
  ...
  <TabItem value="anthropic" label="Anthropic">
    ...
    To see the full example of tracing Anthropic, please refer to the [Anthropic Tracing example](https://github.com/mlflow/mlflow/tree/master/examples/anthropic/tracing.py).

    ![Anthropic Tracing](/images/llms/anthropic/anthropic-tracing.png)

  </TabItem>
  <TabItem value="crewai" label="CrewAI">
    ### CrewAI Automatic Tracing

    MLflow Tracing ensures observability for the interactions of CrewAI agents.
    When CrewAI autologging is enabled with <APILink fn="mlflow.crewai.autolog" />,
    ...
    To see the full example of tracing CrewAI, please refer to the [CrewAI Tracing example](https://github.com/mlflow/mlflow/tree/master/examples/crewai/tracing.py).

    ![CrewAI Tracing](/images/llms/crewai/crewai-trace.png)
...
![Trace Decorator](/images/llms/tracing/trace-decorator.png)
...
![External Function tracing](/images/llms/tracing/external-trace.png)

## Tracing Client APIs

:::note
Client APIs are in **Experimental Status** and is subject to change without deprecation warning or notification.
We recommend using the client APIs only when you have specific requirements that are not met by the other APIs.
:::
...
To explore the structure and schema of MLflow Tracing, please see the [Tracing Schema](/llms/tracing/tracing-schema/) guide.
