...
:::warning
We recommend using the client APIs ONLY when the high-level APIs like `@mlflow.trace` and `mlflow.start_span` do not satisfy your requirements. See [this page](fluent-apis) for the usage of those high-level APIs.
:::
