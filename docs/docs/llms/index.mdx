...
- [Manual trace instrumentation with high-level fluent APIs](/tracing/api/manual-instrumentation/): Easy-to-use decorators and context managers for adding tracing with minimal code changes.
- [Low-level client APIs for tracing](/tracing/api/client): Thread-safe methods for detailed and explicit control over trace data management.
