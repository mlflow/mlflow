...
- [Automated tracing with GenAI libraries](MLflow%20Tracing/introduction#automatic-tracing): Seamless integration with libraries such as LangChain, OpenAI, LlamaIndex, and AutoGen, for automatic trace data collection.
- [Manual trace instrumentation with high-level fluent APIs](MLflow%20Tracing/introduction#tracing-fluent-apis): Easy-to-use decorators and context managers for adding tracing with minimal code changes.
- [Low-level client APIs for tracing](MLflow%20Tracing/introduction#tracing-client-apis): Thread-safe methods for detailed and explicit control over trace data management.

To learn more about what tracing is, see our [Tracing Concepts Overview](MLflow%20Tracing/overview) guide. For an in-depth exploration into the structure of
MLflow traces and their schema, see the [Tracing Schema](MLflow%20Tracing/tracing-schema) guide.

If you're interested in contributing to the development of MLflow Tracing, please refer to the [Contribute to MLflow Tracing](MLflow%20Tracing/contribute) guide.

## MLflow AI Gateway for LLMs

Serving as a unified interface, the [MLflow AI Gateway](_)
...
Please refer to [Supported Provider Models](_) for the full list of supported providers and models.
...
Enter [MLflow LLM Evaluation](_). This feature is designed to simplify the evaluation process,
...
Serving as a powerful tool for prompt engineering, the [MLflow Prompt Engineering UI](_) revolutionizes the
...
- **PyFunc Compatibility**:
  ...
  - Utilize the [Models From Code feature](_) for simplified GenAI application development.
...
Empowering developers with advanced tracking capabilities, the [MLflow LLM Tracking System](_) stands out as the
