...
<APILink fn="mlflow.search_experiments" /> and <APILink fn="mlflow.client.MlflowClient.search_experiments">
  `MlflowClient.search_experiments()`
</APILink>
support the same filter string syntax as <APILink fn="mlflow.search_runs" /> and
<APILink fn="mlflow.client.MlflowClient.search_runs">`MlflowClient.search_runs`</APILink>, but the
supported identifiers and comparators are different.

<TOCInline toc={toc} maxHeadingLevel={3} minHeadingLevel={2} />

## Syntax

See [Search Runs Syntax](/search-runs#search-runs-syntax) for more information.
