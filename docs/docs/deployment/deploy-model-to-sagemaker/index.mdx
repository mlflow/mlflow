...
If you are new to MLflow model deployment, please read [MLflow Deployment](/deployment)
...
prediction results. For more details on the endpoints, refer to [Local Inference Server](/deployment/deploy-model-locally#local-inference-server-spec).
...
[official MLflow Docker image](/docker) and manually push it to ECR.
