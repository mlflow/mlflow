...
[Building a Docker Image for MLflow Model](deploy-model-to-kubernetes#build-docker-for-deployment) for more details on how
...
Detailed information about the server and endpoints is available in [Inference Server Specification](deploy-model-locally#local-inference-server-spec).
...
<CardGroup>
  <LogoCard
    description={
      <>
        Deploying a model locally as an inference server is straightforward with MLflow, requiring
        just a single command <code>mlflow models serve</code>.
      </>
    }
    link="deploy-model-locally"
  >
    <span>Deploying a Model Locally</span>
  </LogoCard>
  <LogoCard
    description="Amazon SageMaker is a fully managed service for scaling ML inference containers. MLflow simplifies the deployment process with easy-to-use commands, eliminating the need to write container definitions."
    link="deploy-model-to-sagemaker"
  >
    <span>![Amazon SageMaker Logo](/images/logos/amazon-sagemaker-logo.png)</span>
  </LogoCard>
  <LogoCard
    description="MLflow integrates seamlessly with Azure ML. You can deploy MLflow Model to the Azure ML managed online/batch endpoints, or to Azure Container Instances (ACI) / Azure Kubernetes Service (AKS)."
    link="https://learn.microsoft.com/en-us/azure/machine-learning/how-to-deploy-mlflow-models"
  >
    <span>![AzureML Logo](/images/logos/azure-ml-logo.png)</span>
  </LogoCard>
  <LogoCard
    description="Databricks Model Serving offers a fully managed service for serving MLflow models at scale, with added benefits of performance optimizations and monitoring capabilities."
    link="https://docs.databricks.com/en/mlflow/models.html"
  >
    <span>![Databricks Logo](/images/logos/databricks-logo.png)</span>
  </LogoCard>
  <LogoCard
    description="MLflow Deployment integrates with Kubernetes-native ML serving frameworks such as Seldon Core and KServe (formerly KFServing)."
    link="deploy-model-to-kubernetes"
  >
    <span>![Kubernetes Logo](/images/logos/kubernetes-logo.png)</span>
  </LogoCard>
  <LogoCard
    description="MLflow also supports more deployment targets such as Ray Serve, Redis AI, Torch Serve, Oracle Cloud Infrastructure (OCI), through community-supported plugins."
    link="/plugins#deployment-plugins"
  >
