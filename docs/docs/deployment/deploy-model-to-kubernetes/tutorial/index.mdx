...
and could be unsuitable for production use cases. Flask is not designed for high performance and scale ([why?](/deployment/deploy-model-locally#serving-frameworks)), and also
...
as a single place to manage the entire ML lifecycle. This includes [experiment tracking](/tracking), [model packaging](/models),
[versioning](/model-registry), [evaluation](/model-evaluation), and [deployment](/deployment), which we will cover in this tutorial.
...
<Tabs>
  <TabItem default label="Kubernetes Cluster" value="kubernetes-cluster">
    If you already have access to a Kubernetes cluster, you can install KServe to your cluster by
    following [the official
    instructions](https://github.com/kserve/kserve#hammer_and_wrench-installation).
  </TabItem>
  <TabItem label="Local Machine Emulation" value="local-machine-emulation">
    You can follow [KServe QuickStart](https://kserve.github.io/website/latest/get_started) to set
    up a local cluster with [Kind](https://kind.sigs.k8s.io/docs/user/quick-start) and install
    KServe on it.
  </TabItem>
</Tabs>
...
or how to log manually instead, see the [MLflow Tracking documentation](/tracking).
:::
...
based on the `scoring` metric, which is negative mean absolute error in this example. You can also create parent and child runs manually, please refer to [Create Child Runs](#child_runs)
for more details.
...
:::note
To learn more about hyperparameter tuning with MLflow, please refer to [Hyperparameter Tuning with MLflow and Optuna](/traditional-ml/hyperparameter-tuning-with-child-runs).
:::

## Step 5: Packaging the Model and Dependencies

Since we are using autologging, MLflow automatically logs the [Model](/models) for each run. This process conveniently packages the model weight
and dependencies in a ready-to-deploy format.

:::note
In practice, it is also recommended to use [MLflow Model Registry](/model-registry) for registering and managing your models.
:::
...
[Deploy MLflow Model Locally](/deployment/deploy-model-locally), you can run a local inference server with just a single command.
...
For more information about the request format and response formats, refer to [Inference Server Specification](/deployment/deploy-model-locally.html#local-inference-server-spec).
...
<Tabs>
  ...
  <TabItem label="Using Model URI" value="using-model-uri">
    ...
    Please refer to [Artifact Store](/tracking#artifact-stores) for setup instructions.
...
First, create a JSON file with test data and save it as `test-input.json`. Ensure the request data is formatted for the [V2 Inference Protocol](https://kserve.github.io/website/latest/modelserving/inference_api/#inference-request-json-object),
...
- [MLflow Tracking](/tracking) - Explore more about MLflow Tracking and various ways to manage experiments and models, such as team collaboration.
- [MLflow Model Registry](/model-registry) - Discover more about MLflow Model Registry for managing model versions and stages in a centralized model store.
- [MLflow Deployment](/deployment) - Learn more about MLflow deployment and different deployment targets.
