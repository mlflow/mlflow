...
you are required to use the legacy serialization methods outlined in the [Custom Python Model](/models#custom-python-models) documentation.
:::

:::note
Models from code is only available for [LangChain](/llms/langchain), [LlamaIndex](/llms/llama-index), and custom `pyfunc` models. If you are
writing a custom Python model or GenAI agent/app, use models from code. For conversational GenAI agents, use authoring interfaces
like [ChatModel](/llms/chat-model-guide) to define your agent, which is supported regardless of which other libraries or frameworks
you're using. For other use cases (e.g. classic ML with xgboost), if you are using ML libraries directly, using the provided saving and
logging functionality within specific model flavors is recommended.
:::
...
implementations that do not depend on serialized model weights (such as [LangChain](/llms/langchain) and
[LlamaIndex](/llms/llama-index)).
...
In the legacy mode for custom models, an instance of your subclassed <APILink fn="mlflow.pyfunc.PythonModel">`mlflow.pyfunc.PythonModel`</APILink> is submitted in the call to `log_model`. When called via an object
...
the main script) in the execution environment and instantiating whichever object you define in the call to <APILink fn="mlflow.models.set_model" />, assigning that
object as the inference target.

At no point in this process are there dependencies on serialization libraries such as [pickle](https://docs.python.org/3/library/pickle.html) or
[cloudpickle](https://pypi.org/project/cloudpickle/1.1.1), removing the broad limitations that these serialization packages have, such as:

- **Portability and Compatibility**: Loading a pickle or cloudpickle file in a Python version that was different than the one used to serialize the object does not guarantee compatibility.
...
[Jupyter](https://jupyter.org) (IPython Notebooks) are a very convenient way to work with AI applications and modeling in general. One slight limitation that they
...
Fortunately, the folks that maintain the core kernel that Jupyter uses ([IPython](https://ipython.readthedocs.io/en/stable/interactive/magics.html)) have created a
...
The [%%writefile](https://ipython.readthedocs.io/en/stable/interactive/magics.html#cellmagic-writefile) magic command, when written as the first line of a notebook
...
<Tabs>
  <TabItem label="Simple Example" value="simple-example">
    ...
    The first code block, representing a discrete notebook cell, will create a file named `basic.py` in the same directory as the notebook. The contents of this
  ...
  <TabItem label="Models with Code Paths dependencies" value="models-with-code-paths-dependencies">
    ...
    This model will be logged with MLflow, allowing us to perform predictions using the stored model.
      ...
      To learn more about the `code_paths` feature in MLflow, see the [guidelines on usage here](/model/dependencies#caveats-of-code_paths-option).
  ...
  <TabItem label="Models From Code with LangChain" value="models-from-code-with-langchain">
    ### MLflow's native LangChain Models from Code support

    In this slightly more advanced example, we will explore how to use the [MLflow LangChain integration](/llms/langchain) to define
    ...
    This script utilizes the LangChain library along with MLflow's `autolog` feature for enabling the [capture of traces](/llms/tracing).
    ...
    [LangChain Expression Language (LCEL)](https://python.langchain.com/v0.1/docs/expression_language), as well as the custom default logic
    ...
    When you load this model using <APILink fn="mlflow.langchain.load_model" />, the entire chain defined in `mfc.py` is executed, and the model
...
- [Model API Documentation](/models#model-api)
- [Managing Dependencies in MLflow Models](/model/dependencies)
