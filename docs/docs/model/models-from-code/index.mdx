...
:::note
Models from code is only available for [LangChain](/llms/langchain), [LlamaIndex](/llms/llama-index), and custom Python agents
or GenAI apps written using `pyfunc`. For other use cases (e.g. classic ML with xgboost), if you are using ML libraries
...
[LlamaIndex](/llms/llama-index)). If you are writing a custom Python model or GenAI agent/app, you should use models from
code.

The key difference between Models from Code and legacy serialization of these models is in how a model is represented during serialization.
