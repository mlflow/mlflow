...
outputs, and parameters. Additionally, [MLflow model deployment tools](/model#built-in-deployment) utilize these signatures to ensure that
...
specific to scenarios where [MLflow model deployment tools](/model#built-in-deployment) are used or when models are loaded as `python_function`.
...
:::note
Model signatures play a crucial role in [MLflow model deployment tools](/model#built-in-deployment), particularly for serving models in the Python
...
[pmdarima model flavor](/model#pmdarima-flavor)).
:::
...
To log a langchain model successfully, **it is highly recommended to use** [models from code](/model#models-from-code) feature.
