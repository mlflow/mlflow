...
## Automatic Model Dependency Locking (Experimental)

By default, MLflow only pins the direct dependencies of your model (for example, `scikit-learn` for a `scikit-learn` model), but does **not** lock transitive dependencies (such as `numpy` installed by `scikit-learn`). This can sometimes lead to inconsistencies when deploying or sharing models, as transitive dependencies may change over time.

To ensure fully reproducible environments, MLflow supports **automatic dependency locking**. When enabled, MLflow will lock both direct and transitive dependencies, generating a complete, deterministic set of package versions for your model. Here's an example of `requirements.txt` generated with dependency locking enabled:
...
### How to Enable Dependency Locking

Set the environment variable `MLFLOW_LOCK_MODEL_DEPENDENCIES` to `true` before logging your model:

```sh
export MLFLOW_LOCK_MODEL_DEPENDENCIES=true
```

With this setting, MLflow will use [uv](https://docs.astral.sh/uv/) to resolve and lock all dependencies when saving or logging a model.

### Installing `uv`

If `uv` is not already installed, you can add it with:
