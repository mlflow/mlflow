...
with mlflow.start_run():
    ...
    processed_data = spark.read.csv("hdfs://cluster/processed/features.csv", header=True)
...
vector_data = spark.createDataFrame([
    ([3.0, 4.0], 0),
    ([5.0, 6.0], 1)
], ["features_array", "label"]).select(
    array_to_vector("features_array").alias("features"), 
    "label"
)
...
with mlflow.start_run(run_name="ONNX Conversion"):
    ...
    try:
        ...
        onnx_model = onnxmltools.convert_sparkml(
            ...
            target_opset=None  # Use default opset
        )
