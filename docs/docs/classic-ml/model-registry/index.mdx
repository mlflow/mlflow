...
the model registry via the UI or API. [See here](/tracking/backend-stores) for more information.
...
An alternative way to interact with Model Registry is using the <Link to={"/api_reference/python_api/index.html"} target="\_blank">MLflow model flavor</Link>
o <Link to={"/api_reference/python_api/mlflow.client.html"} target="\_blank">MLflow Client Tracking API</Link> interface. In particular, you can register a model during an
...
To use Databricks unity catalog model registry, set MLflow registry URI to `"databricks-uc"`, and set the two environmental variables
...
in production. In MLflow, you can use registered models and [MLflow Authentication](/auth) to
...
load the model from the Model Registry and score using the <Link to={"/api_reference/python_api/mlflow.pyfunc.html#mlflow.pyfunc.PyFuncModel.predict"} target="\_blank">`predict`</Link>
function.
...
access controls to enable quick development without compromising stability in production. With [MLflow Authentication](/auth),
...
- Use [MLflow Authentication](/auth) to grant appropriate permissions on these models.
