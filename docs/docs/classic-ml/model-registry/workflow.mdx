...
If the destination UC model doesn't exist, one will be created as part of this API call (assuming the right ACLs).
...
instructions to add one. However, to ease the burden of migration, you can also set the environment variable
`MLFLOW_SKIP_SIGNATURE_CHECK_FOR_MIGRATION_TO_DATABRICKS_UC_REGISTRY` to `true` that will work ONLY with the

<APILink fn="mlflow.client.MlflowClient.copy_model_version">`copy_model_version()`</APILink> API as indicated above, and
allow you to migrate the model to Unity Catalog, bypassing the signature requirement.
...
NOTE: not having a signature on your Unity Catalog registered model version will limit the available feature set.
For a full list of what will be unavailable, refer to the [Databricks documentation](https://docs.databricks.com/aws/en/machine-learning/manage-model-lifecycle/#train-and-register-unity-catalog-compatible-models).
...
# Make a dummy model that can be used to increment the version number
def make_dummy_uri() -> tuple[str, str]:
    class _Dummy(mlflow.pyfunc.PythonModel):
    ...
    with mlflow.start_run() as run:
        ...
        return f"runs:/{run.info.run_id}/m", run.info.run_id


# Check if the source model has a particular version number
def src_has_mv(name: str, v: int) -> bool:
    try:
        workspace_client.get_model_version(name, str(v))
        return True
    except Exception:
        return False
...
def copy_model_versions_to_uc(src: str, dst: str) -> None:
    latests = workspace_client.get_latest_versions(src)
    top = max(int(v.version) for v in latests)

    dummy_uri, run_id = make_dummy_uri()
    ...
    for v in range(1, top + 1):
        if src_has_mv(src, v):
            ...
            mlflow.set_registry_uri("databricks-uc")
            mv = uc_client.create_model_version(dst, dummy_uri, run_id)
            # delete immediately so it just bumps numbering
            uc_client.delete_model_version(dst, mv.version)


copy_model_versions_to_uc("my_wmr_model", "mycatalog.myschema.my_uc_model")
```
