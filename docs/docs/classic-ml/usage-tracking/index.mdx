...
The below section outlines the data currently collected in this version of MLflow. You can view the exact data collected in the source code (add link to code).
...
<Table>
  ...
  <tbody>
    <tr>
      ...
      <td>A randomly generated, non-customer/non-personally identifiable UUID is created for each session—defined as each time MLflow is imported</td>
...
<Table>
  ...
  <tbody>
    ...
    <tr>
      <td>create_run</td>
      <td>Whether the packages in PACKAGES_TO_CHECK_IMPORT (add link to code) is imported or not</td>
    </tr>
    <tr>
...
- If you spawn subprocesses (e.g., via subprocess.run) from a clean environment, those subprocesses may not inherit your shell’s environment, and telemetry could remain enabled.
...
Use `mlflow.telemetry.get_telemetry_client` to retrieve the current telemetry client. If it returns None, telemetry is disabled for the current session.

### How to opt-out for your organization?

MLflow fetches telemetry configuration from `https://config.mlflow-telemetry.io`.
When it fails to fetch the configuration, MLflow will not track any telemetry for the session.
With this, you can block requests to this endpoint to prevent telemetry from being enabled.
