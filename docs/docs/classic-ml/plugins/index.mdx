---
title: ML Plugins
description: Extend MLflow with custom plugins for tracking backends, artifact storage, model deployment, and more.
---
...
class MyEvaluator(ModelEvaluator):
    ...
    def evaluate(self, *, model, model_type, dataset, run_id, evaluator_config, **kwargs):
        ...
        metrics = self._compute_custom_metrics(predictions, dataset.labels_data, evaluator_config)
...
mlflow.create_experiment("xet_experiment", artifact_location="xet://username/repo/branch")
```
...
| **mlflow-crusoe**       | Crusoe          | `pip install mlflow-crusoe`       |
...
client.create_deployment(name="my_model", model_uri="models:/MyModel/1", config={"device": "GPU"})
```
...
Bind each run to pre-declared evaluation criteria and record their manifest hash with the run:
...
# A .prml.yaml in the working directory or any ancestor declares the
# metric, comparator and threshold for the evaluation.
with mlflow_falsify.locked_run():
    mlflow.log_metric("accuracy", 0.95)

# Run tags: prml.manifest_hash, prml.metric, prml.comparator,
# prml.threshold, prml.dataset_id, prml.verdict
```
...
class TestPerformance:
    ...
    def test_concurrent_access(self):
        ...
        threads = [threading.Thread(target=create_experiment, args=[i]) for i in range(10)]
