...
```
  ...
  <TabItem value="run-context-plugin" label="Run Context and Provenance">
...
For HPO sweeps, descriptive tags can be lifted to experiment scope via `MLFLOW_FALSIFY_TAG_SCOPE=experiment`. The provider never raises exceptions into your run; on failure it logs a debug message and no-ops.
