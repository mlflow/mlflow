...
The signature is included in the `X-MLflow-Signature` header with the format: `v1,<base64_encoded_signature>`
...
# Use the verify_mlflow_signature function from the previous example
def verify_mlflow_signature(
    payload: str, signature: str, secret: str, delivery_id: str, timestamp: str
) -> bool:
    """Verify the HMAC signature from MLflow webhook"""
