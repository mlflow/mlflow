...
:::warning
Webhooks are only supported with the SQL backend. The file backend does not support webhooks.
:::
...
# Test with first event (REGISTERED_MODEL_CREATED)
result = client.test_webhook(webhook.webhook_id)
...
MLflow implements automatic retry logic to ensure reliable webhook delivery. When a webhook request fails, MLflow will automatically retry the request based on the following rules:

| Status Code                   | Category     | Retry Behavior | Description                                                 |
| ----------------------------- | ------------ | -------------- | ----------------------------------------------------------- |
| **2xx** (200-299)             | Success      | ❌ No Retry    | Success responses                                           |
| **3xx** (300-399)             | Redirect     | ❌ No Retry    | Redirects                                                   |
| **4xx** (400-499, except 429) | Client Error | ❌ No Retry    | Client errors that won't be resolved by retrying            |
| **429**                       | Rate Limit   | ✅ Retry       | Too Many Requests - Rate limit errors                       |
| **500**                       | Server Error | ✅ Retry       | Internal Server Error - Server errors that may be temporary |
| **502**                       | Server Error | ✅ Retry       | Bad Gateway - Gateway errors                                |
| **503**                       | Server Error | ✅ Retry       | Service Unavailable - Service temporarily unavailable       |
| **504**                       | Server Error | ✅ Retry       | Gateway Timeout - Gateway timeout errors                    |
...
async def handle_webhook(
    ...
    print(f"Timestamp: {x_mlflow_timestamp}")
