...
| Scenario                       | Required Parameter(s) | MLflow Behavior                                  |
|--------------------------------|----------------------|--------------------------------------------------|
| All classes present            | *(none)*             | Infers all options, no need for `label_list`     |
| Single-class in eval data      | `label_list`         | Required. Error if not supplied                  |
| Missing classes (multiclass)   | `label_list`         | Recommended. Otherwise, only present classes used|
| Binary, want custom pos_label  | `pos_label`          | Use for metrics (default: last in label_list)    |
...
<details>
  ...
  #### General Options
  <ul>
    <li><code>pos_label</code>: Positive class label for binary classification metrics. Only applies to binary classification. If not specified, MLflow defaults to the last label in <code>label_list</code> (or the inferred list) as the positive class, and logs a warning. <b>bool</b> and <b>int</b> labels are treated as compatible types. When provided, MLflow validates that <code>pos_label</code> exists in <code>label_list</code> (whether explicitly provided or inferred from data). For multiclass classification, <code>pos_label</code> is ignored entirely.</li>
    <li><code>label_list</code>: For single-class or missing-class evaluation data, you <b>must</b> specify the full set of expected classes using <code>label_list</code>. Otherwise, MLflow will raise an error. For normal binary or multiclass data, you do <b>not</b> need to specify <code>label_list</code>—MLflow will infer it automatically.</li>
...
For classification tasks, you can control evaluation behavior with these key parameters:
...
result = mlflow.models.evaluate(
    ...
    model_type="classifier",
    evaluator_config={
        "pos_label": 1,  # Specify positive class for binary metrics
        "label_list": [0, 1],  # Explicit class list
    },
)

# Multiclass classification with averaging strategy
result = mlflow.models.evaluate(
    model_uri,
    eval_data,
    targets="label",
    model_type="classifier",
    evaluator_config={
        "average": "weighted",  # "macro", "micro", or "weighted"
        "label_list": [0, 1, 2, 3],  # All expected classes
    },
)
```
...
- **`pos_label`**: Positive class for binary classification metrics. Only meaningful for binary classification (2 classes). For multiclass, this parameter is ignored.
- **`label_list`**: Complete set of expected classes. Required when evaluation data is missing expected classes.
  > **Note:** All items in `label_list` must be of the same type or convertible to a common numeric type (e.g., int, float, numeric string). Mixing string labels with non-numeric values (e.g., `['cat', 1]`) is not supported and will raise an error.
- **`average`**: Averaging strategy for multiclass metrics ("macro", "micro", "weighted").

:::tip Binary vs Multiclass Behavior
- **Binary classification**: `pos_label` determines which class is treated as positive for metrics like precision, recall, and ROC-AUC
- **Multiclass classification**: `pos_label` is ignored - all classes are evaluated equally using one-vs-rest approach
:::
...
## Handling Edge Cases: Single-Class and Missing Class Scenarios

In production environments, you may encounter evaluation datasets that don't contain all the classes your model was trained on. This commonly occurs with:

- <b>Imbalanced datasets</b> where rare classes are absent in small evaluation batches
- <b>Highly confident models</b> that predict only one class
- <b>Production data drift</b> where new data lacks certain classes

:::note
MLflow will raise an error if your labels are of incompatible types (e.g., mixing strings and integers). However, <b>bool</b> and <b>int</b> labels are treated as compatible and can be mixed. Additionally, strings that can be converted to numbers (e.g., "0", "1.5") are compatible with numeric types (int/float).
:::

### Single-Class Binary Classification

When your evaluation data contains only one class, explicitly specify both expected classes:
...
# Create evaluation dataset with only one class present
X = pd.DataFrame(np.random.randn(50, 2), columns=["feature1", "feature2"])
y = np.zeros(50)  # Only class 0 present in evaluation data

# Evaluate with explicit label_list to handle single-class scenario
result = mlflow.evaluate(
    model=model_uri,
    data=X,
    targets=y,
    model_type="classifier",
    evaluator_config={"label_list": [0, 1]},  # Explicitly specify both classes
)
```

Without <code>label_list</code>, MLflow will raise an error for single-class data:

```python
# This will raise an error for single-class data without label_list
result = mlflow.evaluate(
    model=model_uri,
    data=X,
    targets=y,
    model_type="classifier",
    # Missing evaluator_config with label_list
)
```

**Error Output:**
```
MlflowException: Evaluation dataset for classification must contain at least two unique labels, but only 1 unique label(s) were found.
```

:::tip When to Use `label_list`
For normal binary or multiclass data, you do **not** need to specify `label_list`—MLflow will infer it automatically. Only use `label_list` when your evaluation data is missing expected classes.
:::

### Multiclass with Missing Classes

The same principle applies to multiclass classification. If your evaluation dataset is missing some classes that your model was trained on, specify the complete set of expected classes:

```python
# Example: Model trained on 4 classes (0, 1, 2, 3) but evaluation data only has classes 0 and 2
X = pd.DataFrame(np.random.randn(100, 2), columns=["feature1", "feature2"])
y_missing_classes = np.random.choice([0, 2], size=100)  # Only classes 0 and 2 present

result = mlflow.evaluate(
    model=model_uri,
    data=X,
    targets=y_missing_classes,
    model_type="classifier",
    evaluator_config={"label_list": [0, 1, 2, 3]},  # Specify all expected classes
)
```

This ensures that confusion matrices and per-class metrics are computed for all expected classes, with zero counts for missing classes.

## Transparency and Auditability

- The positive label used for binary metrics is always logged as a run tag (<code>evaluation.positive_label</code>).
- For binary classification, the <code>pos_label</code> is also logged as a metric in the evaluation results for transparency.
- If the default positive label is used, a warning is logged in the run.
- If the model does not support <code>predict_proba</code>, ROC/PR metrics are skipped and this is robustly handled.
