...
def reproducible_evaluation(model, eval_data, targets, random_seed=42):
    ...
    eval_data_sorted = eval_data.sort_values(by=eval_data.columns.tolist()).reset_index(
        drop=True
    )
...
## Handling Single-Class Binary Classification with `label_list`

By default, MLflow infers the set of possible classes from your data. However, for binary classification, some evaluation metrics (like the confusion matrix) require both classes to be present. If your evaluation dataset contains only a single class (e.g., all zeros), you **must** specify the full set of possible classes using the `label_list` parameter in `evaluator_config`.

**Why is this needed?**
This ensures that metrics and plots (such as the confusion matrix) are always computed in a consistent 2x2 format, even if only one class is present in your data.

### Example: Forcing a 2-Class Confusion Matrix
...
# Assume you have a model logged at model_uri
X = pd.DataFrame(np.random.randn(50, 2), columns=["feature1", "feature2"])
y = np.zeros(50)  # Only class 0 present
...
If you do **not** provide a `label_list` of length 2 for single-class data, MLflow will raise an error:
...
> **Tip:** For normal binary or multiclass data, you do **not** need to specify `label_list`—MLflow will infer it automatically.

### Multiclass with Missing Classes

The same principle applies to multiclass classification. If your evaluation dataset is missing some classes that your model was trained on, you can specify the complete set of expected classes:
