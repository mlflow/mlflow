...
## Handling Single-Class Data in Binary Classification

When evaluating a binary classifier, you may encounter a scenario where your evaluation dataset contains only one of the two classes (e.g., all `True` or all `1`). This is common in highly imbalanced datasets or with specific data batches.

By default, `mlflow.evaluate` will raise an error in this case, as it cannot construct a 2x2 confusion matrix from a single class.

**Solution**: You can instruct MLflow to handle this gracefully by explicitly providing the full set of expected labels via the `evaluator_config`. This ensures that metrics like `true_positives` and `true_negatives` are calculated correctly against the complete label space.

Here's how to configure the `default` evaluator to handle a single-class dataset:
...
# 1) Create a toy dataset where only class '1' appears
df = pd.DataFrame(
    {
        "feature": np.random.randn(10),
        "target": np.ones(10, dtype=int),  # All labels are '1'
    }
)


# 2) Define a model that always predicts '1'
class AlwaysOneModel(PythonModel):
...
with mlflow.start_run() as run:
    model_info = mlflow.pyfunc.log_model(
        name="always_one_model",
        python_model=AlwaysOneModel(),
    )

# 4) Evaluate with an explicit label_list
#    This forces a 2x2 confusion matrix, preventing a crash.
result = mlflow.evaluate(
    ...
    evaluator_config={
        "default": {
            "label_list": [0, 1],  # <-- Force a two-class confusion matrix
            "pos_label": 1,  # (Optional) Define the positive class
        }
    },
)

# The metrics are calculated correctly against the full label space
# Expected: true_positives=10, false_negatives=0, true_negatives=0, false_positives=0
print(result.metrics)
```

By setting `label_list: [0, 1]`, you tell MLflow to generate a confusion matrix with both classes, even though class `0` never appears in the data. This correctly results in `true_positives` being 10 and all other confusion matrix entries being 0, which is the expected behavior.
