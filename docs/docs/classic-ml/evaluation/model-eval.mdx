...
- **Imbalanced datasets** where rare classes are absent in small evaluation batches
- **Highly confident models** that predict only one class
- **Production data drift** where new data lacks certain classes

By default, MLflow infers the set of possible classes from your evaluation data. However, for binary classification, some evaluation metrics (like the confusion matrix) require both classes to be present. When your evaluation dataset contains only a single class, you **must** specify the full set of possible classes using the `label_list` parameter in `evaluator_config`.
...
Without `label_list`, MLflow will raise an error for single-class data:
