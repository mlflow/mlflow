...
:::tip
Looking for guidance on searching over Runs? See the [Search Runs](/ml/search-runs) documentation.
:::
...
environments = ["production", "staging", "production", "development", "staging", "production"]
...
specific_model = mlflow.search_logged_models(
    experiment_ids=["1"],
    filter_string="name = 'SVM_model_5'"
)
...
not_svm = mlflow.search_logged_models(
    experiment_ids=["1"],
    filter_string="name != 'SVM_model_4'"
)
```
...
recent_models = mlflow.search_logged_models(
    experiment_ids=["1"],
    filter_string=f"creation_time > {last_week}"
)
```
...
best_models = mlflow.search_logged_models(
    ...
    order_by=[{
        ...
        "ascending": False  # Highest accuracy first
    }]
)
...
for model_type, model in best_by_type.items():
    ...
    print(f"{model_type}: Model ID = {model.model_id}, Run ID = {model.source_run_id}, Accuracy = {accuracy_display}")
```
