...
<details>
  ...
  1. **Format Conversion**: The model is converted to a generic MLflow `pyfunc` model
  2. **Metadata Creation**: Model metadata is stored, including dependencies and signature
  3. **Artifact Storage**: The model is saved to the MLflow artifact store
  4. **Loading Capability**: The model can be loaded back as either a native TensorFlow model or a generic `pyfunc` model
...
A model signature describes the expected input and output formats of your model. While optional, it's a best practice for better model understanding and validation:
...
<details>
  <summary>Automatic Signature Inference</summary>

  MLflow can automatically infer model signatures if you provide example data:
...
<details>
  ...
  with mlflow.start_run(run_name="ensemble_experiment") as parent_run:
      ...
      with mlflow.start_run(run_name="model_1", nested=True) as child_run_1:
          ...
          # Train model_1
          mlflow.tensorflow.log_model(model_1, "model_1")
...
with mlflow.start_run(run_name="hyperparameter_optimization"):
    ...
    mlflow.tensorflow.log_model(final_model, "best_model")
```

### Deployment Preparation

Prepare your TensorFlow model for deployment with MLflow:

<details>
  <summary>Model Serving with MLflow</summary>
  ...
  # Log model with sample input for signature
  with mlflow.start_run() as run:
      # Create example input
      sample_input = tf.random.uniform([1, 28, 28, 3])

      # Add prediction signature
      signature = mlflow.models.infer_signature(
          sample_input.numpy(), model.predict(sample_input)
      )

      # Log model with sample input and signature
      mlflow.tensorflow.log_model(
          model,
          "model",
          signature=signature,
          input_example=sample_input.numpy(),
          pip_requirements=["tensorflow==2.10.0"],
          code_paths=["./preprocessing.py"],  # Include custom code
      )

      model_uri = f"runs:/{run.info.run_id}/model"

  # Deploy model to MLflow server
  import subprocess
  ...
  inference_request = {"inputs": sample_input.numpy().tolist()}

  response = requests.post(
      "http://localhost:5000/invocations",
      data=json.dumps(inference_request),
