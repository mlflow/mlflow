...
your model. Additionally, only Tensorflow >= 2.3.0 is supported. If you are using an older version
...
autologging, simply run <APILink fn="mlflow.tensorflow.autolog" /> or <APILink fn="mlflow.autolog" />.
...
`mlflow.tensorflow.autolog(log_dataset_info=False)`. Please refer to the API
documentation <APILink fn="mlflow.tensorflow.autolog" /> for full customization options.
...
MLflow offers a predefined callback <APILink fn="mlflow.tensorflow.MlflowCallback">`mlflow.tensorflow.MlflowCallback`</APILink> that you can use or
...
You can change the logging frequency in <APILink fn="mlflow.tensorflow.MlflowCallback">`mlflow.tensorflow.MlflowCallback`</APILink>
by setting `log_every_epoch` and `log_every_n_steps`, by default metrics are logged per epoch. Please refer to
...
a class subclassing from [keras.callbacks.Callback](https://www.tensorflow.org/api_docs/python/tf/keras/callbacks/Callback),
...
read [keras.callbacks.Callback](https://www.tensorflow.org/api_docs/python/tf/keras/callbacks/Callback).
...
call <APILink fn="mlflow.tensorflow.log_model" />. After saving, you can load back the model
using <APILink fn="mlflow.tensorflow.load_model" />. The loaded model can be used for inference by calling
the `predict()` method.
...
model to use Tensorflow flavor, but if you are interested, please refer to [MLflow pyfunc model](/model#how-to-load-and-score-python-function-models).
...
`keras` by setting the `keras_model_kwargs` parameter in <APILink fn="mlflow.tensorflow.log_model" />. For
...
For difference between the formats, please refer to [Tensorflow Save and Load Guide](https://www.tensorflow.org/guide/keras/save_and_serialize).
...
To manually set the signature for your model, you can pass a `signature` parameter to <APILink fn="mlflow.tensorflow.log_model" />.
You will need to set the input schema by specifying the `dtype` and `shape` of the input tensors,
and wrap it with <APILink fn="mlflow.types.TensorSpec" />. For example,
