        ...
        for query in test_queries:
            ...
            for hit in similarities:
                search_results.append({
                    ...
                    "rank": len([r for r in search_results if r["query"] == query]) + 1
                })
...
def create_semantic_similarity_dataset():
    ...
    similarity_data = [
        {"text1": "The cat is sleeping", "text2": "A cat is resting", "similarity": 0.85},
        {"text1": "I love programming", "text2": "Coding is my passion", "similarity": 0.80},
        {"text1": "The weather is nice", "text2": "It's raining heavily", "similarity": 0.15},
        {"text1": "Machine learning is exciting", "text2": "AI technology fascinates me", "similarity": 0.75},
        {"text1": "Python is a language", "text2": "The snake slithered away", "similarity": 0.10},
        {"text1": "Data science projects", "text2": "Analytics and statistics work", "similarity": 0.70},
    ]
...
def evaluate_embedding_model_with_mlflow(model_name):
    ...
    with mlflow.start_run(run_name=f"eval_{model_name.replace('/', '_')}"):
        ...
        signature = mlflow.models.infer_signature(
            input_example,
            similarity_model.predict(None, input_example)
        )
        ...
        accuracy_metric = make_metric(
            eval_fn=lambda df, metrics: accuracy_within_threshold_metric(df, metrics, 0.1),
            greater_is_better=True,
...
def create_batch_embedding_pipeline():
    ...
    with mlflow.start_run(run_name="batch_embedding_pipeline"):
        ...
        mlflow.sentence_transformers.log_model(
            ...
            input_example=large_text_dataset[:5]
        )
        
        print(f"Processed {len(large_text_dataset)} documents in {processing_time:.2f} seconds")
...
def comprehensive_model_comparison():
    ...
    with mlflow.start_run(run_name="multi_model_evaluation"):
        ...
        for model_name in models_to_compare:
            ...
            with mlflow.start_run(run_name=f"eval_{model_name.replace('/', '_')}", nested=True):
...
def analyze_speed_quality_tradeoffs():
    ...
    with mlflow.start_run(run_name="speed_quality_analysis"):
        ...
        for config in model_configs:
            ...
            with mlflow.start_run(run_name=f"analysis_{model_name.replace('/', '_')}", nested=True):
                model = SentenceTransformer(model_name)
...
def create_domain_evaluation_pipeline(domain_name, test_cases):
    ...
    with mlflow.start_run(run_name=f"domain_eval_{domain_name}"):
        ...
        best_model = max(domain_results.keys(), 
                        key=lambda x: domain_results[x]["accuracy"])
...
def optimized_batch_encoding():
    ...
    with mlflow.start_run(run_name="batch_optimization"):
        ...
        large_dataset = [f"Document {i} with sample content for encoding." for i in range(5000)]
