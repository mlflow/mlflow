...
A model signature defines the schema of inputs and outputs, enhancing model understanding and enabling validation:

<details>
  <summary>Automatic Signature Inference</summary>

  The simplest way to add a signature is using <APILink fn="mlflow.models.infer_signature"/>:
...
You can also manually define a signature for complete control:
