...
with mlflow.start_run():
    mlflow.spacy.log_model(nlp, "spacy_model")
```
...
with mlflow.start_run():
    ...
    mlflow.spacy.log_model(nlp, "text_classifier")

# Load and use for inference
loaded_model = mlflow.pyfunc.load_model("runs:/<run_id>/text_classifier")
...
def mlflow_logger():
    ...
    def setup_logger(
        ...
        def finalize():
            ...
            mlflow.spacy.log_model(nlp, "trained_model")
            mlflow.end_run()
...
loaded_nlp = mlflow.spacy.load_model(f"runs:/{run_id}/spacy_model")
...
<details>
  ...
  nlp = mlflow.spacy.load_model("runs:/<run_id>/model")
  ...
  classifier = mlflow.pyfunc.load_model("runs:/<run_id>/model")
...
<details>
  ...
  with mlflow.start_run():
      mlflow.spacy.log_model(nlp, "model", signature=signature)
  ```
...
with mlflow.start_run():
    ...
    mlflow.spacy.log_model(nlp, "custom_sentiment_model")
...
<details>
  ...
  def evaluate_multilingual_models():
      ...
      with mlflow.start_run(run_name="multilingual_comparison"):
          ...
          for lang, model_name in models.items():
              try:
                  with mlflow.start_run(run_name=f"{lang}_model", nested=True):
                      ...
                      # Log the model
                      mlflow.spacy.log_model(nlp, f"{lang}_model")
          ...
          mlflow.log_metric("avg_entities_per_lang", 
    ...
    with mlflow.start_run(run_name="pipeline_optimization"):
        ...
        for i, components in enumerate(configurations):
            with mlflow.start_run(run_name=f"config_{i}", nested=True):
                ...
                performance_score = len(pipeline_components) * 10 - processing_time * 100
                ...
                # Log model
                mlflow.spacy.log_model(nlp, "pipeline_model")
...
with mlflow.start_run() as run:
    ...
    model_uri = f"runs:/{run.info.run_id}/spacy_model"
```
...
mlflow models serve -m runs:/<run_id>/spacy_model -p 5000
...
<details>
  ...
  mlflow models serve -m runs:/<run_id>/spacy_model -p 5000 --env-manager conda

  # Enable MLServer for enhanced performance
  mlflow models serve -m runs:/<run_id>/spacy_model -p 5000 --enable-mlserver

  # Set custom host for network access
  mlflow models serve -m runs:/<run_id>/spacy_model -p 5000 --host 0.0.0.0
  ```
