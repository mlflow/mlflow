...
example_input = "Hi there, chatbot!"
example_output = conversational_pipeline(example_input)
...
example_input = data
example_output = model(example_input, **inference_params)
...
# Run the pipeline once to produce representative output
output_example = my_audio_pipeline(url)

# Infer the signature using the input and output example
signature = infer_signature(url, output_example)
