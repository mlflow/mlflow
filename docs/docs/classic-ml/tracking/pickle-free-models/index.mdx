...
## SHAP explainers remain pickle-based

<APILink fn="mlflow.shap.log_explainer" /> and <APILink fn="mlflow.shap.save_explainer" /> use pickle-based serialization for the SHAP explainer state, so the complete artifact is not pickle-free. For a scikit-learn-backed explainer, MLflow also uses cloudpickle for its extracted nested model. Using skops only for that nested model—or automatically trusting classes that skops rejects—would not make the complete SHAP artifact pickle-free or safe to load from an untrusted source.

Treat SHAP explainers as pickle-based artifacts and load them only from trusted sources. Setting [`MLFLOW_ALLOW_PICKLE_DESERIALIZATION=false`](#global-configuration-to-force-mlflow-pickle-free-model-loading) prevents them from loading outside Databricks Runtime and Databricks Model Serving. This SHAP-specific behavior does not change models saved directly with `mlflow.sklearn`: standalone sklearn models still use skops by default and can accept explicitly reviewed `skops_trusted_types`.
...
- `torch` >= 2.4 (`torch` >= 2.10 is required to load the model when `MLFLOW_ALLOW_PICKLE_DESERIALIZATION=false`, see [below](#global-configuration-to-force-mlflow-pickle-free-model-loading))
...
:::note[PyTorch `pt2` models]
`torch.export.load` internally deserializes parts of a `.pt2` archive with `torch.load(weights_only=False)`. When `MLFLOW_ALLOW_PICKLE_DESERIALIZATION=false`, MLflow therefore verifies before loading that every record in the archive is accepted by torch's restricted `weights_only` unpickler (tensors and primitive containers only), and refuses the model otherwise. This verification requires `torch` >= 2.10 (older versions have known `weights_only` unpickler vulnerabilities); on older versions, loading a `pt2` model with the flag set to `false` raises an error asking you to upgrade `torch` or enable the flag.
:::
...
| PyTorch             | `serialization_format="pt2"`   | Requires `input_example`, torch >= 2.4 (>= 2.10 to load with `MLFLOW_ALLOW_PICKLE_DESERIALIZATION=false`); not for ScriptModule. | **Breaking change** — `input_example` will be required; `torch.jit.ScriptModule` is not supported. |
