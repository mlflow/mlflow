...
- `torch` >= 2.4 (`torch` >= 2.6 is required to load the model when `MLFLOW_ALLOW_PICKLE_DESERIALIZATION=false`, see [below](#global-configuration-to-force-mlflow-pickle-free-model-loading))
...
`torch.export.load` internally deserializes parts of a `.pt2` archive with `torch.load(weights_only=False)`. When `MLFLOW_ALLOW_PICKLE_DESERIALIZATION=false`, MLflow therefore verifies before loading that every record in the archive is accepted by torch's restricted `weights_only` unpickler (tensors and primitive containers only), and refuses the model otherwise. This verification requires `torch` >= 2.6; on older versions, loading a `pt2` model with the flag set to `false` raises an error asking you to upgrade `torch` or enable the flag.
:::
...
| PyTorch             | `serialization_format="pt2"`   | Requires `input_example`, torch >= 2.4 (>= 2.6 to load with `MLFLOW_ALLOW_PICKLE_DESERIALIZATION=false`); not for ScriptModule. | **Breaking change** — `input_example` will be required; `torch.jit.ScriptModule` is not supported. |
