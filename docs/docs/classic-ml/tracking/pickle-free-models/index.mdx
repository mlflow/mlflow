...
:::note[Default serialization formats]
Outside Databricks Runtime, the `save_model` and `log_model` APIs default to pickle-free serialization for scikit-learn, PyTorch, and sklearn-style LightGBM models. In Databricks Runtime, these APIs retain the backward-compatible defaults: `cloudpickle` for scikit-learn and sklearn-style LightGBM models, and `pickle` for PyTorch models. Set `serialization_format` explicitly to override the environment-specific default.
:::
...
| PyTorch             | `serialization_format="pt2"`   | Requires `input_example`, torch >= 2.4 (>= 2.10 to load with `MLFLOW_ALLOW_PICKLE_DESERIALIZATION=false`); not for ScriptModule. |
