...
[Runs](/ml/tracking#runs) and experiments such as:
...
Note that large model artifacts such as model weight files are stored in [artifact store](/ml/tracking/artifacts-stores).
...
- Databricks workspace (specified as `databricks` or as `databricks://<profileName>`, a [Databricks CLI profile](https://github.com/databricks/databricks-cli#installation)).
  ...
  [[Azure]](http://docs.microsoft.com/azure/databricks/applications/mlflow/access-hosted-tracking-server), or [the quickstart](/ml/getting-started/intro-quickstart) to
  easily get started with hosted MLflow on the Databricks Free Trial.

:::warning important
In order to use [Model Registry](/ml/model-registry) functionality, you must run your server using a database-backed store. See [this FAQ](/ml/tracking#tracking-with-model-registry) for more information.
:::
