...
By default, MLflow server is configured for **secure localhost development**:

- Only accepts connections from localhost addresses (127.0.0.1, ::1, localhost)
- Blocks cross-origin requests except from common localhost ports
- Validates Host headers to prevent DNS rebinding
- Adds security headers to all responses

This means you can run `mlflow server` locally without any configuration, and it will be secure by default.
...
| ------------------------------- | -------------------------------------------- | ---------------------- | --------------------------------------------------------- |
| `MLFLOW_CORS_ALLOWED_ORIGINS`   | Comma-separated list of allowed CORS origins | localhost origins only | `"https://app.company.com,https://dashboard.company.com"` |
| `MLFLOW_ALLOWED_HOSTS`          | Comma-separated list of allowed Host headers | localhost variants     | `"mlflow.company.com,mlflow.internal:5000"`               |
