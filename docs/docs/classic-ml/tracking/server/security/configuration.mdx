...
| -------------------- | ------------------------------- | ------------------------------------ | ---------------------- |
| **Allowed Hosts**    | `--allowed-hosts`               | `MLFLOW_ALLOWED_HOSTS`               | localhost, private IPs |
| **CORS Origins**     | `--cors-allowed-origins`        | `MLFLOW_CORS_ALLOWED_ORIGINS`        | localhost:\*           |
| **X-Frame-Options**  | `--x-frame-options`             | `MLFLOW_X_FRAME_OPTIONS`             | SAMEORIGIN             |
| **Disable Security** | `--disable-security-middleware` | `MLFLOW_DISABLE_SECURITY_MIDDLEWARE` | false                  |
