...
- **Model Signature** - logs [Model signature](/ml/model/signatures#model-signature) instance, which describes input and output schema of the model
...
For flavors that automatically save models as an artifact, [additional files](/ml/models.html#storage-format) for dependency management are logged.
