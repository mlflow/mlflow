...
[MLServer project is no longer actively maintained](https://github.com/SeldonIO/MLServer/commits/master/). The guidance below is preserved for users on
existing MLServer-based deployments. For new Kubernetes deployments, build the Docker image without `--enable-mlserver` (which uses the default FastAPI
