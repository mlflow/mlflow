...
and could be unsuitable for production use cases. Flask is not designed for high performance and scale ([why?](/ml/deployment/deploy-model-locally#serving-frameworks)), and also
...
as a single place to manage the entire ML lifecycle. This includes [experiment tracking](/ml/tracking), [model packaging](/model),
[versioning](/ml/model-registry), [evaluation](/model-evaluation), and [deployment](/ml/deployment), which we will cover in this tutorial.
...
based on the `scoring` metric, which is negative mean absolute error in this example. You can also create parent and child runs manually, please refer to [Create Child Runs](/ml/tracking/tracking-api/#child_runs)
for more details.
...
:::note
To learn more about hyperparameter tuning with MLflow, please refer to [Hyperparameter Tuning with MLflow and Optuna](/ml/traditional-ml/hyperparameter-tuning-with-child-runs).
:::
