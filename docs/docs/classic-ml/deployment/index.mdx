...
<APILink fn="mlflow.server.cli" hash="mlflow-models-build-docker">`mlflow models build-docker`</APILink> CLI command or the
<APILink fn="mlflow.models.build_docker">`mlflow.models.build_docker`</APILink> Python API to build an image that serves your model.
...
<CardGroup>
  ...
  <LogoCard description="Modal is a serverless cloud platform for AI/ML with on-demand GPU access (T4 to H200), auto-scaling, and one-command deployment via the mlflow-modal-deploy plugin." link="/ml/deployment/deploy-model-to-modal">
    <span>![Modal Logo](/images/logos/modal-logo.png)</span>
  </LogoCard>
