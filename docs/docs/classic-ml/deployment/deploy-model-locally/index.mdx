...
Remember to note down the model URI, such as `runs:/<run_id>/<artifact_path>` (or `models:/<model_name>/<model_version>` if you registered the model in the [MLflow Model Registry](/ml/model-registry)).
...
a valid [Model Signature](/ml/model/signatures#model-signature) with `params` must be defined.
