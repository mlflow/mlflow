...
:::info Security Note
Starting with MLflow 3.5.0, the server includes built-in security features that protect against DNS rebinding and CORS attacks.
By default, it only accepts connections from localhost. If you need to expose your server to other machines,
see the [MLflow Server Security Guide](/ml/tracking/server-security) for proper configuration.
:::
