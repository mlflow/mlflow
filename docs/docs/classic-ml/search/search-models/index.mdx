...
2. **Supported Prefixes**: `metrics.`, `params.`, `tags.`, or no prefix for attributes
...
Tags allow you to categorize and filter models by custom metadata. Use the `tags.` prefix to search by tag values:

```python
# Find models tagged for classification tasks
models = mlflow.search_logged_models(filter_string="tags.task = 'classification'")

# Find models in production environment
models = mlflow.search_logged_models(
    filter_string="tags.environment IN ('production', 'staging')"
)
```

### 6 - Dataset-Specific Metric Filtering
