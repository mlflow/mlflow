...
[How MLflow Model Records Dependencies](/model/dependencies#how-mlflow-records-dependencies) for more details about these files.
...
See [Model Input Example](/model/signatures#input-example) for more details.
...
Please refer to [Managing Dependencies in MLflow Models](/model/dependencies) for more details.
...
[Model Signatures and Examples Guide](/model/signatures). If you would like to see signature enforcement in action,
see the [notebook tutorial on Model Signatures](/model/notebooks/signature_examples) to learn more.
...
To **learn more about the Models From Code feature**, please visit [the deep dive guide](/model/models-from-code)
...
see the [guide to models from code](/model/models-from-code) for the recommended approaches.
...
The <APILink fn="mlflow.pytorch">mlflow.pytorch</APILink> module defines utilities for saving and
loading MLflow Models with the `pytorch` flavor. You can use the <APILink fn="mlflow.pytorch.save_model" />
and <APILink fn="mlflow.pytorch.log_model" /> methods to save PyTorch models in MLflow format; both of these
functions use the [torch.save()](https://pytorch.org/docs/stable/torch.html#torch.save) method to
serialize PyTorch models. Additionally, you can use the <APILink fn="mlflow.pytorch.load_model" />
...
produced by <APILink fn="mlflow.pytorch.save_model" /> and <APILink fn="mlflow.pytorch.log_model" /> contain
the `python_function` flavor, allowing you to load them as generic Python functions for inference
via <APILink fn="mlflow.pyfunc.load_model" />.
...
the <APILink fn="mlflow.environment_variables.MLFLOW_DEFAULT_PREDICTION_DEVICE">MLFLOW_DEFAULT_PREDICTION_DEVICE</APILink>
...
models. The <APILink fn="mlflow.sklearn">mlflow.sklearn</APILink> module defines <APILink fn="mlflow.sklearn.save_model">`save_model()`</APILink>
and <APILink fn="mlflow.sklearn.log_model">`log_model()`</APILink> functions that save scikit-learn models in
...
be loaded as generic Python functions for inference via <APILink fn="mlflow.pyfunc.load_model" />.
This loaded PyFunc model can only be scored with DataFrame input. Finally, you can use
the <APILink fn="mlflow.sklearn.load_model"/ > method to load MLflow Models with the `sklearn` flavor as
...
For more information, see <APILink fn="mlflow.sklearn">mlflow.sklearn</APILink>.

### Spark MLlib (`spark`)

The `spark` model flavor enables exporting Spark MLlib models as MLflow Models.

The <APILink fn="mlflow.spark" /> module defines

- <APILink fn="mlflow.spark.save_model">`save_model()`</APILink> to save a Spark MLlib model to a DBFS path.
- <APILink fn="mlflow.spark.log_model">`log_model()`</APILink> to upload a Spark MLlib model to the tracking server.
- <APILink fn="mlflow.spark.load_model">`load_model()`</APILink> to load MLflow Models with the ``spark`` flavor as Spark MLlib pipelines.

MLflow Models produced by these functions contain the `python_function` flavor,
allowing you to load them as generic Python functions via <APILink fn="mlflow.pyfunc.load_model" />.
This loaded PyFunc model can only be scored with DataFrame input.
When a model with the `spark` flavor is loaded as a Python function via <APILink fn="mlflow.pyfunc.load_model" />
, a new [SparkContext](https://spark.apache.org/docs/latest/api/python/pyspark.html#pyspark.SparkContext)
...
[MLlib PipelineModel](http://spark.apache.org/docs/latest/api/python/pyspark.ml.html?highlight=pipelinemodel#pyspark.ml.Pipeline) to any production environment supported by MLflow
...
# Serialize the Model
with mlflow.start_run():
    model_info = mlflow.spark.log_model(lr_model, name="spark-model")
...
using low-level TensorFlow API. See [tf-keras-example](#tf-keras-example). for an example of mlflow and `tf.keras` models.
...
The `xgboost` model flavor enables logging of [XGBoost models](https://xgboost.readthedocs.io/en/latest/python/python_api.html#xgboost.Booster)
in MLflow format via the <APILink fn="mlflow.xgboost.save_model" /> and <APILink fn="mlflow.xgboost.log_model" />
methods in python and <Link to={useBaseUrl("/api_reference/R-api.html#mlflow-save-model-crate")} target="_blank">`mlflow_save_model`</Link>
and <Link to={useBaseUrl("/api_reference/R-api.html#mlflow-log-model")} target="_blank">`mlflow_log_model`</Link> in R respectively.
These methods also add the `python_function` flavor to the MLflow Models that they produce, allowing the
models to be interpreted as generic Python functions for inference via <APILink fn="mlflow.pyfunc.load_model" />.
This loaded PyFunc model can only be scored with DataFrame input. You can also use the <APILink fn="mlflow.xgboost.load_model" />
method to load MLflow Models with the `xgboost` model flavor in native XGBoost format.

Note that the `xgboost` model flavor only supports an instance of [xgboost.Booster](https://xgboost.readthedocs.io/en/latest/python/python_api.html#xgboost.Booster),
not models that implement the [scikit-learn API](https://xgboost.readthedocs.io/en/latest/python/python_api.html#module-xgboost.sklearn).
...
The `spaCy` model flavor enables logging of [spaCy models](https://spacy.io/models) in MLflow format via
the <APILink fn="mlflow.spacy.save_model" /> and <APILink fn="mlflow.spacy.log_model" /> methods. Additionally, these
...
also use the <APILink fn="mlflow.spacy.load_model" /> method to load MLflow Models with the `spacy` model flavor
in native spaCy format.

For more information, see <APILink fn="mlflow.spacy">`mlflow.spacy`</APILink>.
...
To set up the training job, we first need to generate a configuration file as described in the
[Spacy Documentation](https://spacy.io/usage/training#config) For simplicity, we will only use
a `TextCategorizer` in the pipeline.
...
Check the [spacy-loggers library](https://pypi.org/project/spacy-loggers/) for a more complete implementation.
...
The `prophet` model flavor enables logging of [Prophet models](https://facebook.github.io/prophet/) in MLflow format via
the <APILink fn="mlflow.prophet.save_model" /> and <APILink fn="mlflow.prophet.log_model" /> methods.
These methods also add the `python_function` flavor to the MLflow Models that they produce, allowing the
models to be interpreted as generic Python functions for inference via <APILink fn="mlflow.pyfunc.load_model" />.
This loaded PyFunc model can only be scored with DataFrame input. You can also use
the <APILink fn="mlflow.prophet.load_model" /> method to load MLflow Models with the `prophet` model
...
The full guide, including tutorials and detailed documentation for using
the `openai` flavor [can be viewed here](/llms/openai).

### LangChain (`langchain`) (Experimental)

The full guide, including tutorials and detailed documentation for using
the [langchain flavor can be viewed here](/llms/langchain).
...
The full guide, including tutorials and detailed documentation for using the `transformers` flavor is available at [this location](/llms/transformers/).

### SentenceTransformers (`sentence_transformers`)

The `sentence_transformers` model flavor enables logging of
[sentence-transformers models](https://www.sbert.net/docs/pretrained_models.html) in MLflow format via
the <APILink fn="mlflow.sentence_transformers.save_model" /> and <APILink fn="mlflow.sentence_transformers.log_model" /> functions.
...
You can also use the <APILink fn="mlflow.sentence_transformers.load_model" /> function to load a saved or logged MLflow
...
The `promptflow` model flavor is capable of packaging your flow in MLflow format via the <APILink fn="mlflow.promptflow.save_model" />
and <APILink fn="mlflow.promptflow.log_model" /> functions. Currently, a `flow.dag.yaml` file is required to be
present in the flow's directory. These functions also add the `python_function` flavor to the MLflow Models,
allowing the models to be interpreted as generic Python functions for inference via <APILink fn="mlflow.pyfunc.load_model" />.
You can also use the <APILink fn="mlflow.promptflow.load_model" /> method to load MLflow Models with the `promptflow`
...
Reach the flow source at [example from the MLflow GitHub Repository](https://github.com/mlflow/mlflow/blob/master/examples/promptflow/basic).
...
After building and training your MLflow Model, you can use the <APILink fn="mlflow.evaluate" /> API to
evaluate its performance on one or more datasets of your choosing. <APILink fn="mlflow.evaluate" />
currently supports evaluation of MLflow Models with the [python_function (pyfunc) model flavor](#pyfunc-model-flavor)
for classification, regression, and numerous language modeling tasks (see [Evaluating with LLMs](#model-evaluation-llms)),
computing a variety of task-specific performance metrics, model performance plots, and
model explanations. Evaluation results are logged to [MLflow Tracking](/tracking).

The following [example from the MLflow GitHub Repository](https://github.com/mlflow/mlflow/blob/master/examples/evaluation/evaluate_on_binary_classifier.py)
uses <APILink fn="mlflow.evaluate" /> to evaluate the performance of a classifier
on the [UCI Adult Data Set](https://archive.ics.uci.edu/ml/datasets/adult), logging a
...
<div style={{ display: "flex" }}>
  <span style={{ width: "15%" }}>![](/images/model_evaluation_metrics.png)</span>
  <span style={{ width: "84%" }}>![](/images/model_evaluation_feature_importance.png)</span>
</div>

### Evaluating with LLMs \{#model-evaluation-llms}

As of MLflow 2.4.0, <APILink fn="mlflow.evaluate" /> has built-in support for a variety of tasks with
LLMs, including text summarization, text classification, question answering, and text generation.
The following example uses <APILink fn="mlflow.evaluate" /> to evaluate a model that answers
...
(see the [MLflow OpenAI question answering full example](https://github.com/mlflow/mlflow/tree/master/examples/llms/question_answering/question_answering.py)),
you can navigate to the Artifact View to view the questions and compare the answers for
each model:

![](/images/artifact-view-ui.png)

For additional examples demonstrating the use of `mlflow.evaluate()` with LLMs, check out the
[MLflow LLMs example repository](https://github.com/mlflow/mlflow/tree/master/examples/llms).
...
to <APILink fn="mlflow.evaluate" /> to produce extra metrics and artifacts for the model(s) that you're evaluating.
...
The following [short example from the MLflow GitHub Repository](https://github.com/mlflow/mlflow/blob/master/examples/evaluation/evaluate_with_custom_metrics.py)
uses <APILink fn="mlflow.evaluate" /> with an extra metric function to evaluate the performance of a regressor on the
...
For a more comprehensive extra metrics usage example, refer to
[this example from the MLflow GitHub Repository](https://github.com/mlflow/mlflow/blob/master/examples/evaluation/evaluate_with_custom_metrics_comprehensive.py).
...
The following example uses <APILink fn="mlflow.evaluate" /> to evaluate a function:
...
As of MLflow 2.8.0, <APILink fn="mlflow.evaluate" /> supports evaluating a static dataset without specifying a model.
...
the `predictions` parameter in <APILink fn="mlflow.data.from_pandas" />, and specify `None` for the
`predictions` parameter in <APILink fn="mlflow.evaluate" />:
...
The following example uses <APILink fn="mlflow.evaluate" /> to evaluate a static dataset:
...
:::warning attention
MLflow 2.18.0 has moved the model validation functionality from the <APILink fn="mlflow.evaluate" /> API
...
using <APILink fn="mlflow.evaluate" /> (or load persisted evaluation results from local storage). Then, pass the results along
with a validation_thresholds dictionary that maps metric names to <APILink fn="mlflow.models.MetricThreshold">`mlflow.models.MetricThreshold`</APILink>
objects. If your model fails to meet the specified thresholds, <APILink fn="mlflow.validate_evaluation_results" /> will raise a
`ModelValidationFailedException` with details about the validation failure.

More information on model evaluation behavior and outputs can be found in the <APILink fn="mlflow.evaluate" /> API documentation.
...
Refer to <APILink fn="mlflow.models.MetricThreshold">`mlflow.models.MetricThreshold`</APILink>
...
![](/images/model_evaluation_compare_feature_importance.png)
...
The <APILink fn="mlflow.validate_evaluation_results" /> API can also be used to validate a candidate
model against static thresholds, instead of comparing it to a baseline model, by passing `None` to
the `baseline_result` parameter.
...
The <APILink fn="mlflow.models.EvaluationResult">`mlflow.models.EvaluationResult`</APILink> object
returned by <APILink fn="mlflow.evaluate" /> can be saved to and loaded from local storage.
...
:::note
There are plugins that support in-depth model validation with features that are not supported
directly in MLflow. To learn more, see:

- [Model Validation with Giskard’s plugin](#giskard_plugin)
- [Model Validation with Trubrics’ plugin](#trubrics_plugin)
...
[the documentation](https://scikit-learn.org/stable/modules/model_evaluation.html#precision-recall-f-measure-metrics).
...
To extend the validation capabilities of MLflow and anticipate issues before they go to production, a plugin has been built by
[Giskard](https://docs.giskard.ai/en/latest/integrations/mlflow/index.html) allowing users to:

    - scan a model in order to detect hidden vulnerabilities such as
      [Performance bias](https://docs.giskard.ai/en/latest/getting-started/key_vulnerabilities/performance_bias/index.html),
      ...
      [Ethical bias](https://docs.giskard.ai/en/latest/getting-started/key_vulnerabilities/ethics/index.html),
      [Data leakage](https://docs.giskard.ai/en/latest/getting-started/key_vulnerabilities/data_leakage/index.html),
      [Stochasticity](https://docs.giskard.ai/en/latest/getting-started/key_vulnerabilities/stochasticity/index.html),
      [Spurious correlation](https://docs.giskard.ai/en/latest/getting-started/key_vulnerabilities/spurious/index.html), and others
...
For more information on the plugin, see the [giskard-mlflow docs](https://docs.giskard.ai/en/latest/integrations/mlflow/index.html).

### Model Validation with Trubrics' plugin \{#trubrics_plugin}

To extend the validation capabilities of MLflow, a plugin has been built by [Trubrics](https://github.com/trubrics/trubrics-sdk) allowing users:
...
See the [plugin example notebook](https://github.com/trubrics/trubrics-sdk/blob/main/examples/mlflow/mlflow-trubrics.ipynb) for a demo.

For more information on the plugin, see the [trubrics-mlflow docs](https://trubrics.github.io/trubrics-sdk/mlflow/).
...
You can also create custom MLflow Models by writing a custom _flavor_.

As discussed in the [Model API](#model-api) and [Storage Format](#storage-format) sections, an MLflow Model
...
provided in the [Community Model Flavors](/community-model-flavors) page.

#### Example: Creating a custom "sktime" flavor

This example illustrates the creation of a custom flavor for
[sktime](https://github.com/sktime/sktime) time series library. The library provides a unified
...
predictions uses a _single-row_ `Pandas DataFrame` configuration argument to expose the parameters
of the `sktime` inference API. The complete code for this example is included in the
[flavor.py](https://github.com/mlflow/mlflow/tree/master/examples/sktime/flavor.py) module of the
...
is required to add the `pyfunc` specification to the MLflow model configuration. Note also the import
of the `flavor` module itself. This will be passed to the <APILink fn="mlflow.models.Model.log" />
...
model is saved to a specified output directory. Additionally, `save_model()` leverages
the <APILink fn="mlflow.models.Model.add_flavor" /> and <APILink fn="mlflow.models.Model.save" /> methods to
...
at <APILink fn="mlflow.sklearn.save_model">mlflow.sklearn.save_model</APILink>).
...
the <APILink fn="mlflow.models.infer_pip_requirements" /> method in the `save_model()` function will
...
Next, we add the `log_model()` function. This function is little more than a wrapper around
the <APILink fn="mlflow.models.Model.log" /> method to enable logging our custom model as an artifact to the
...
pass the `flavor` module to the <APILink fn="mlflow.models.Model.log" /> method which internally calls
...
The `_load_pyfunc()` function will be called by the <APILink fn="mlflow.pyfunc.load_model" /> method
...
method of the `sktime` wrapper class accepts a _single-row_ `Pandas DataFrame` configuration
...
provided in the [flavor.py](https://github.com/mlflow/mlflow/tree/master/examples/sktime/flavor.py) module
docstrings.
...
hyper-parameters, evaluation metrics and the trained model as an artifact. The _single-row_
...
![](/images/tracking_artifact_ui_custom_flavor.png)
...
[Deploy MLflow models](https://mlflow.org/docs/latest/models.html#deploy-mlflow-models) section):
...
- Model dependencies validation: The API helps ensure that the dependencies logged with the model are correct and sufficient by executing the model with an input example in a virtual environment.
  For more details, refer to [Validating Environment for Prediction](/model/dependencies/#validating-environment-for-prediction).
...
This information is moved to [MLflow Deployment](/deployment) page.
...
Go to the [Community Model Flavors](/community-model-flavors)
