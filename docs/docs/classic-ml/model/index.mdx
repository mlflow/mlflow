...
- `signature`: [model signature](/ml/model/signatures#model-signature) in JSON format.
- `input_example`: reference to an artifact with [input example](/ml/model/signatures#input-example).
...
See [Model Input Example](/ml/model/signatures#input-example) for more details.
...
The full guide, including tutorials and detailed documentation for using the `transformers` flavor is available at [this location](/genai/transformers/).
...
provided in the [Community Model Flavors](/ml/community-model-flavors) page.
...
known community-maintained plugins [here](/ml/plugins#deployment-plugins).
