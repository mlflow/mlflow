...
mlflow.sklearn.log_model(model, "model", input_example=single_record)
```
...
mlflow.tensorflow.log_model(model, "model", input_example=image_batch)
```
...
mlflow.langchain.log_model(model, "model", input_example=dict_example)
```
...
df_optional = pd.DataFrame({
    "required_col": [1, 2, 3],
    "optional_col": [1.0, None, 3.0]  # Contains None
})
...
mlflow.sklearn.log_model(model, "model", input_example=X_sample)

# ❌ Avoid: Logging without examples
mlflow.sklearn.log_model(model, "model")  # No signature or validation
```
...
mlflow.sklearn.log_model(model, "model", input_example=sample_df)
```
