...
:::warning
    ...
    The `prompts` parameter for associating prompts with models in only supported for GenAI flavors such as OpenAI, LangChain, LlamaIndex, DSPy, etc. Please refer to the [GenAI flavors](/llms#model-packaging) for the full list of supported flavors.
...
If you haven't already created a prompt, follow [this step](/llm/prompts#1-create-a-prompt) to create a new prompt.
...
prompt = mlflow.load_prompt("prompts:/summarization_prompt/2")
...
response = chain.invoke(
    {"num_sentences": 1, "sentences": "This is a test sentence."}
)
print(response)
```
...
[Models-from-Code](/model/models-from-code) is a feature that allows you to define and log models in code.
