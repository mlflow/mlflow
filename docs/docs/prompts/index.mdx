...
- **Lineage** - Seamlessly integrated with MLflow's Tracing, Evaluation, and Monitoring capabilities for agent observability and quality improvement.
...
After creating multiple prompt versions, you can compare them in the MLflow UI using the Compare tab on the prompt details page:
...
target_text = """
MLflow is the largest open source AI engineering platform for agents, LLMs, and ML models. It enables teams of all sizes
...
# Fluent API: returns a flat list of all matching prompts, ordered by name
prompts = mlflow.genai.search_prompts(filter_string="task='summarization'", order_by=["name ASC"])
