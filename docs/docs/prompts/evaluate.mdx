...
Run the <APILink fn="mlflow.evaluate" /> API to evaluate the model with the prepared data and prompt. In this example, we will the following two built-in metrics.
...
:::tip
    ...
    There are numbers of built-in metrics available in MLflow for evaluating LLMs. You can also define custom metrics including LLM-as-a-Judge. Refer to the [LLM Evaluation Metrics](https://mlflow.org/docs/latest/llms/llm-evaluate#llm-evaluation-metrics) for more details.
