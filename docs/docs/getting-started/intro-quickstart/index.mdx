...
[Getting Started with MLflow](/getting-started/logging-first-model) tutorial. We recommend that you start here first, though, as this quickstart
...
managed MLflow, [see the guide for tracking server options](/getting-started/running-notebooks/).
...
:::note
[MLflow 3.0](/mlflow-3) has made significant enhancements to the model tracking capabilities of MLflow, distinct <APILink fn="mlflow.entities.LoggedModel">LoggedModel</APILink> entity capturing its own metadata such as metrics and parameters. Follow the MLflow 3.0 version of the following step to experience these new features.
:::
...
<Tabs>
  <TabItem default label="MLflow 2.XX" value="mlflow-2xx">
    ```python
  ...
  <TabItem default label="MLflow 3.0" value="mlflow-30">
    ```python
    # Set our tracking server uri for logging
    mlflow.set_tracking_uri(uri="http://127.0.0.1:8080")
    ...
    with mlflow.start_run():
        ...
        # Log the model with hyperparameters
        model_info = mlflow.sklearn.log_model(
            sk_model=lr,
            name="iris_model",
            signature=signature,
        ...
        # Log the hyperparameters to the run as well
        mlflow.log_params(params)

        # Log the loss metric, specify model_id so that it's attached to the model as well
        mlflow.log_metric("accuracy", accuracy, model_id=model_info.model_id)

        # Set tags that we can use to remind ourselves what this run/model was for
        mlflow.set_tag("Training Info", "Basic LR model for iris data")
...
## Step 7 - View the Logged Model in the MLflow UI ([MLflow 3.0](/mlflow-3) only)
...
If you are interested in a more in-depth tutorial, please see the [Getting Started with MLflow](/getting-started/logging-first-model) tutorial as a
