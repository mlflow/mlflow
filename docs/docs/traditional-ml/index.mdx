...
pagination_next: traditional-ml/hyperparameter-tuning-with-child-runs/index
---
...
- **Models**: In [MLflow 3.0](/mlflow-3), each model is captured as a dedicated <APILink fn="mlflow.entities.LoggedModel">LoggedModel</APILink> entity along with its own metrics and parameters, enabling this data to be viewed in one central location.
...
- **Metrics and Parameters**: By allowing users to log parameters and metrics, MLflow makes it straightforward to compare different runs and models, facilitating model optimization.
