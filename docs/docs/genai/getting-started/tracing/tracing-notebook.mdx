...
This quickstart helps you integrate your GenAI app with [MLflow Tracing](/genai/tracing/index) if you use a Jupyter Notebook as your development environment. If you use a local IDE, please use the [IDE quickstart](/genai/getting-started/tracing/tracing-ide) instead.
...
:::note
Creating a Jupyter Notebook and setting up an MLflow Experiment will be the container for your GenAI application. Learn more about the Experiment and what it contains in the [data model](/genai/data-model/index) section.
:::
...
2. Create a cell in your notebook with the below code
   ...
   Here, we use the [`@mlflow.trace` decorator](/genai/tracing/app-instrumentation/manual-tracing/index) that makes it easy to trace any Python application combined with the [OpenAI automatic instrumentation](/genai/tracing/integrations/openai) to captures the details of the call to the OpenAI SDK.
...
- **Learn more about Tracing**: Learn about [tracing concepts](/genai/tracing/tracing-101), [manual tracing](/genai/tracing/app-instrumentation/manual-tracing/index) for more control, [tracking users & sessions](/genai/tracing/track-users-sessions/index), and [tracking environments & context](/genai/tracing/track-environments-context/index) to your traces.
- **Evaluate your application**: Use MLflow's [evaluation capabilities](/genai/eval-monitor/index) to measure the quality of your GenAI app.
