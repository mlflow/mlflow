...
<Tabs>
  <TabItem value="oss" label="OSS MLflow">
        ...
        MLflow supports different backend for tracking your experiment data. Choose one of the following options to get started. Refer to the [Self Hosting Guide](/self-hosting) for detailed setup and configurations.

        **Option A: Local Database (Recommended)**

        Set tracking URI to the local database URI (e.g., `sqlite:///mlflow.db`). This is recommended option for local development to get better performance and reliability.
