...
# Step 1: Create experiment and initial judge
...
# Step 2: Generate traces from your application (minimum 10 required)
traces = []
...
# Step 3: Run the judge on these traces to get initial assessments
...
# Step 4: Human reviews the JUDGE'S assessments and provides feedback
# This is the critical step - humans review what the judge decided
for trace_id in traces[:10]:  # Review at least 10 for alignment
    trace = mlflow.get_trace(trace_id)

    # Get the judge's assessment
    assessments = trace.search_assessments(name="product_quality")
    judge_assessment = next(
        (a for a in assessments if a.source.source_type == "LLM_JUDGE"), None
    )

    # Human expert reviews: "Did the judge rate this correctly?"
    # Example: Judge said "good" but human thinks it should be "excellent"
    human_rating = "excellent"  # What the human thinks is correct

    # Log human feedback on the same trace
    mlflow.log_feedback(
        trace_id=trace_id,
        name="product_quality",  # Must match judge name
        value=human_rating,  # Human's ground truth
        rationale=f"Judge said '{judge_assessment.value}' but should be '{human_rating}'",
        source=AssessmentSource(
            source_type=AssessmentSourceType.HUMAN, source_id="product_expert"
        ),
    )

# Step 5: Retrieve traces with both judge assessments and human feedback
traces_for_alignment = mlflow.search_traces(
    experiment_ids=[experiment_id], max_results=15, return_type="list"
)

# Step 6: Align the judge using human corrections (minimum 10 traces required)
if len(traces_for_alignment) >= 10:
    ...
    # Step 7: Register the aligned judge
...
optimizer = SIMBAAlignmentOptimizer(
    model="anthropic:/claude-3-opus-20240229"  # Model used for optimization
)
...
The quality of alignment depends on the quality and quantity of feedback:
