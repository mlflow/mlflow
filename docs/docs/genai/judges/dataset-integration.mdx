...
Evaluation datasets enable systematic testing and improvement of your custom LLM judges. By building datasets from production traces and adding ground truth labels, you can measure judge accuracy and identify areas for improvement.
  ...
  steps={[
    {
      ...
      detailedDescription: "Build evaluation datasets from production traces and add ground truth expectations for judge validation.",
      isFocus: true
    },
    {
      icon: Target,
...
Here's how to build a dataset from production traces, evaluate your judge, and improve accuracy:
...
# Step 1: Build dataset from production traces
...
# Get traces from production and add to dataset
...
print(f"Added {len(traces_df)} records from production")
...
1. **Start with Production Data**: Build datasets from real traces to ensure relevance
