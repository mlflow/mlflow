...
# Collect human feedback on the same traces
for trace_id in trace_ids:
    # In production, this would be from your review UI
    human_rating = "excellent"  # Human expert's assessment
    ...
    mlflow.log_feedback(
        ...
        source=AssessmentSource(
            source_type=AssessmentSourceType.HUMAN, source_id="expert_reviewer"
        ),
    )
```
...
# To see improvement, run another evaluation loop
# comparing the original and aligned judges
```
...
<TilesGrid>
  ...
  <TileCard
    ...
    link="/genai/judges/dataset-integration"
