...
The option is part of the model's Vertex AI configuration (`auth_config` when the model definition is created through the API), alongside `vertex_project` and `vertex_location`.
