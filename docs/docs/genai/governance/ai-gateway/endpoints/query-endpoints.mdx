...
AI Gateway endpoints can be used as the backing LLM for MLflow's [LLM-as-a-Judge scorers](/genai/eval-monitor/scorers/llm-judge/). This allows you to run judge evaluations through the gateway, benefiting from centralized API key management and cost tracking.
...
For more details on creating and using LLM judges, see the [LLM-as-a-Judge documentation](/genai/eval-monitor/scorers/llm-judge/).
