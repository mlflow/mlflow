...
The **Try it** tab lets you send a test request directly from the UI. Choose the API type (Unified or Passthrough) and, for passthrough, the provider (OpenAI, Anthropic, or Google Gemini). Edit the request body, click **Send request**, and view the response in the same modal—no code or external client required.
