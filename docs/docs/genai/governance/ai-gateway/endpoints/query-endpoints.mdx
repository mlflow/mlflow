...
When [workspaces](/self-hosting/workspaces/getting-started) are enabled on your MLflow server, you must specify the target workspace when querying gateway endpoints. This is done by including the `X-MLFLOW-WORKSPACE` header in your requests.

:::note
The workspace header is only required when the MLflow server is running with workspaces enabled (`--enable-workspaces` flag). If workspaces are not enabled, you can omit this header.
:::
...
# Set the workspace for your session
mlflow.set_workspace("team-a")

# The workspace is automatically included in requests
response = requests.post(
    "http://localhost:5000/gateway/my-endpoint/mlflow/invocations",
    json={"messages": [{"role": "user", "content": "Hello!"}], "temperature": 0.7},
...
- Passthrough APIs (`/gateway/openai/v1/*`, `/gateway/anthropic/*`, `/gateway/gemini/*`)
