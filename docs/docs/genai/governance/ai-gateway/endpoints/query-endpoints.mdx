...
description: Query MLflow AI Gateway endpoints using the MLflow unified API, OpenAI-compatible chat completions interface, or native passthrough APIs for OpenAI, Anthropic, and Google Gemini.
---
...
The following table summarizes the available gateway APIs:
...
| **Unified**     | OpenAI-compatible Models           | `GET /gateway/mlflow/v1/models`                                                                                             | List configured gateway endpoint names in the OpenAI models format.                                                                |
...
| **Passthrough** | TypeSafe                           | `POST /gateway/typesafe/v1/systemone`                                                                                       | Raw TypeSafe System One structured evaluation requests.                                                                            |
...
Use `GET /gateway/mlflow/v1/models` to discover named gateway endpoints in the active workspace. The response follows the OpenAI models list format. Each model's `id` is a gateway endpoint name that you can pass as the `model` parameter when querying the endpoint.

When MLflow authentication is enabled, users must authenticate and can list only endpoints they have `USE` permission on. Administrators can list all named endpoints in the active workspace.

The following examples assume the server does not require authentication:

<Tabs>
<TabItem value="curl" label="cURL" default>

```bash
curl http://localhost:5000/gateway/mlflow/v1/models
```
...
For detailed information on passthrough APIs for each provider, see [Model Providers](/genai/governance/ai-gateway/endpoints/model-providers).

### TypeSafe Passthrough

TypeSafe passthrough endpoints accept raw System One requests at `POST /gateway/typesafe/v1/systemone`. These endpoints are for structured evaluation with TypeSafe's native request schema. They do not support chat completions, embeddings, mixed TypeSafe/chat routing, or streaming.
...
curl -X POST http://localhost:5000/gateway/typesafe/v1/systemone \
  ...
  -d '{
    "model": "jev-evaluator",
    "state": {"answer": "Select Forgot password on the sign-in page."},
    "questions": {
      "helpful": {
        "type": "noul",
        "instructions": "Does state.answer help the user reset a password?"
      }
    }
  }'
```

The `model` value is the MLflow Gateway endpoint name. For MLflow judges, use direct TypeSafe model URIs such as `model="typesafe:/jev-latest"`; TypeSafe gateway endpoints are not `gateway:/` judge models for `make_judge()`.
...
When [workspaces](/self-hosting/workspaces/getting-started) are enabled on your MLflow server, you can specify the target workspace for a request by including the `X-MLFLOW-WORKSPACE` header.
...
- Passthrough APIs (`/gateway/openai/v1/*`, `/gateway/anthropic/v1/*`, `/gateway/gemini/v1beta/models/{endpoint_name}:*`, `/gateway/typesafe/v1/systemone`)
...
<Tabs>
<TabItem value="built-in" label="Built-in Judges" default>
...
<TabItem value="make-judge" label="Custom Judges">
...
For more details on creating and using LLM judges, see the [LLM Judges documentation](/genai/eval-monitor/scorers/#llms-as-judges).
