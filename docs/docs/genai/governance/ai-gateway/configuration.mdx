...
Databricks [Foundation Models APIs](https://docs.databricks.com/aws/en/machine-learning/foundation-model-apis/) are compatible with the OpenAI ChatCompletion API, so you can use them with `openai` provider in the AI Gateway. Specify the endpoint name (e.g., `databricks-claude-sonnet-4`) in the `name` field and set the host and token as OpenAI API key and base URL respectively.
  ...
  - name: databricks-chat
    ...
    model:
      ...
      config:
        ...
        openai_api_base: https://example.staging.cloud.databricks.com/serving-endpoints/
```
