...
:::tip
You can leave the LLM Connection empty so all users authenticate with their own Anthropic subscription. The gateway will use the credential provided by each request.
:::
