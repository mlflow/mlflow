...
When changing the API Base URL of a saved connection, re-enter the credentials before saving. If the authentication method stays the same, also re-enter any optional credential fields that were previously set, because the update replaces the stored credentials.

For REST API updates, adding, changing, or removing `auth_config.api_base` requires a non-empty `secret_value` in the same request. Otherwise, MLflow rejects the update with `INVALID_PARAMETER_VALUE`. Updates to other configuration fields do not require credential re-entry when `api_base` stays unchanged.
