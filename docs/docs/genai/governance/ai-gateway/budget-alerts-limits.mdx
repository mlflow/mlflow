...
| **On Exceeded** | `ALERT` sends a webhook notification; `REJECT` blocks requests with HTTP 429 |
...
|-------|------|----------|-------------|
| `budget_type` | string | Yes | Budget measurement type. Currently only `USD` |
| `budget_amount` | double | Yes | Spending limit in the specified currency |
| `duration_type` | string | Yes | Window type: `MINUTES`, `HOURS`, `DAYS`, or `MONTHS` |
| `duration_value` | int | Yes | Window length in units of `duration_type` |
| `target_type` | string | Yes | `GLOBAL` or `WORKSPACE` |
| `on_exceeded` | string | Yes | `ALERT` (webhook only) or `REJECT` (block requests) |
