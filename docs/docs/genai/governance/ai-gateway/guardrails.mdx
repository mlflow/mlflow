...
Guardrails let you enforce content policies on traffic flowing through your AI Gateway endpoints. Each guardrail evaluates requests or responses against a set of natural-language instructions, then either **blocks** the traffic or **sanitizes** (redacts) it before allowing it through.
...
<img
  src={useBaseUrl("/images/genai/governance/ai-gateway/guardrails-tab-empty.png")}
...
<img
  src={useBaseUrl("/images/genai/governance/ai-gateway/guardrails-create-type-picker.png")}
  alt="Create Guardrail modal showing type picker with Safety, PII Detection, and Custom Guardrail options"
  style={{ maxWidth: "100%" }}
/>

Selecting a built-in type pre-populates the name and instructions on the next step, saving you time. Choose "Custom Guardrail" if you want to write a guardrail prompt from scratch.

### Step 2 — Configure the guardrail

<img
  src={useBaseUrl("/images/genai/governance/ai-gateway/guardrails-create-config.png")}
  alt="Create Guardrail configuration step showing Name, Stage picker, Instructions, Guardrail Model, and Action fields"
  style={{ maxWidth: "100%" }}
/>
...
Click **Before Guardrails** or **After Guardrails** to select where this guardrail runs:

- **Before Guardrails**: evaluates the incoming request before it reaches the LLM. Use `{{ inputs }}` in your instructions to reference the request content.
- **After Guardrails**: evaluates the LLM's response before it is returned to the caller. Use `{{ outputs }}` in your instructions to reference the response content.
...
Instructions must contain the relevant variable (`{{ inputs }}` for Before, `{{ outputs }}` for After) so the judge receives the actual content to evaluate.
...
{
  "detail": "Guardrail 'pii-detection' blocked: The request contains an email address (user@example.com) which is personally identifiable information."
}
```

The `detail` field contains the guardrail name and the judge's rationale, giving clients actionable information about why the request failed.
...
The **Save** button is disabled until you make a change, and it remains disabled if instructions contain a validation error (for example, if you switch to the After stage but the instructions still reference `{{ inputs }}`).
...
Multiple guardrails on the same endpoint run in the order shown in the table. Before-stage guardrails all execute before the request reaches the LLM; after-stage guardrails execute before the response is returned to the caller. If any guardrail blocks the request, subsequent guardrails in the same stage are skipped.
