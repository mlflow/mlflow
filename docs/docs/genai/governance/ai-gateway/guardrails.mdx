...
:::note
Guardrails are only supported on **unified endpoints**. They are not available for passthrough or legacy endpoints.
:::
...
- **After Guardrails** — evaluates the LLM's response before it is returned to the caller. Use `{{ outputs }}` in your instructions to reference the response content. After guardrails are **not triggered for streaming requests** — only non-streaming responses are evaluated.
