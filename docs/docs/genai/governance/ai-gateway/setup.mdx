...
Navigate to the Endpoints tab and click Create Endpoint to begin configuring a new endpoint. Start by choosing a unique name for your endpoint, such as `my-chat-endpoint`. This name becomes part of the API path you'll use to call the endpoint.

![Create Endpoint](/images/genai/governance/ai-gateway/create-endpoint.png)

Next, select your provider from the comprehensive list of 100+ supported providers. Common providers like OpenAI, Anthropic, and Google Gemini appear at the top of the list, while clicking "View all providers" reveals the full catalog powered by LiteLLM.

After selecting a provider, choose your specific model. The model selector displays:

- Capability badges (Tools, Reasoning, Caching)
- Context window size and token costs
- Search function for quick filtering

For API key configuration, you have two options:
...
The interface provides a live preview of your configuration in the summary panel, so you can verify everything before creating the endpoint. Click Create Endpoint when you're satisfied with the configuration.
