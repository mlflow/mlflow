...
After completing this quickstart, explore the <APILink fn="mlflow.genai.judges.make_judge">make_judge</APILink> API for creating custom judges with template-based instructions and version control capabilities.
...
:::info[Pro Tip]
For more advanced custom judges with template-based instructions and version control, check out the <APILink fn="mlflow.genai.judges.make_judge">make_judge</APILink> API. It provides a unified interface for all judge types and seamless MLflow integration. [Learn more →](/genai/eval-monitor/scorers/llm-judge/make-judge)
:::
...
Want to create more sophisticated custom judges? The **make_judge API** (available in MLflow 3.4.0+) provides a unified, flexible interface for creating custom LLM judges with:
