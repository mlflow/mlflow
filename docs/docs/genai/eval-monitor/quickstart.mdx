...
:::tip Controlling Concurrency to Avoid Rate Limiting

When using LLM-as-a-Judge scorers, MLflow runs evaluations concurrently to speed up the process. If you're hitting rate limits from your LLM provider (especially with free tiers), you can control the concurrency using environment variables:
...
The total maximum concurrent API calls is approximately `MLFLOW_GENAI_EVAL_MAX_WORKERS × MLFLOW_GENAI_EVAL_MAX_SCORER_WORKERS`. Adjust these values based on your LLM provider's rate limits.
