...
- ✅ A simple GenAI app function
- ✅ A test dataset with 3 examples
...
```python
# Run the evaluation
result = mlflow.evaluate(data=test_data, model=my_chatbot, extra_metrics=[is_helpful])

print(f"✅ Evaluation complete!")
print(f"📊 Helpful responses: {result.metrics['is_helpful/mean']:.0%}")
print(f"🔍 View details: Run ID {result.run_id}")
```
...
print(f"✅ Evaluation complete!")
print(f"📊 Helpful responses: {result.metrics['is_helpful/mean']:.0%}")
print(f"🔍 View details in MLflow UI: {result.run_id}")
```
...
v1_result = mlflow.evaluate(
    data=test_data, model=my_chatbot, extra_metrics=[is_helpful]
)
v2_result = mlflow.evaluate(
    data=test_data, model=my_chatbot_v2, extra_metrics=[is_helpful]
)
...
**🎉 Congratulations!** You've successfully evaluated your first GenAI application with MLflow. You now have the foundation to systematically improve your app's quality.