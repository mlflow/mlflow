...
:::tip[Set it up with a coding agent]
Prefer to let a coding agent do it? Expand the prompt below and paste it into your coding agent of choice.

<details>
<summary>Show the coding-agent prompt</summary>

```text
Run an evaluation on my GenAI app in MLflow. If unsure, ask which experiment this is for.

1. Add `mlflow` as a dependency if needed.
2. Set the tracking URI to match the project (read `MLFLOW_TRACKING_URI`, check `~/.databrickscfg`, or ask me), then call `mlflow.set_experiment(experiment_id="<your-experiment-id>")`.
...
This quickstart guide will walk you through evaluating your LLM applications and AI agents with MLflow's comprehensive evaluation framework.
...
MLflow stores evaluation results in a [MLflow Tracking Server](/self-hosting/architecture/tracking-server/).
...
Create a file named `quickstart_eval.py`. This script will contain your mock agent, evaluation dataset, scorers, and the evaluation execution. Alternatively, you may run this in a [notebook](/genai/eval-monitor/notebooks/quickstart-eval).
...
<ImageBox src="/images/mlflow-3/eval-monitor/quickstart-eval-trace.png" alt="Detailed Score Rationale" width="90%" />
...
<ImageBox src="/images/mlflow-3/eval-monitor/quickstart-eval-runs-compare.png" alt="Compare Evaluation Runs" width="90%" />
