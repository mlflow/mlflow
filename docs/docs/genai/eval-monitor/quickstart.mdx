...
:::info[New in MLflow 3.4.0: make_judge API]

After completing this quickstart, explore the new <APILink fn="mlflow.genai.judges.make_judge">make_judge</APILink> API for creating custom judges with template-based instructions and version control capabilities.
...
:::info[Pro Tip]
For more advanced custom judges with template-based instructions and version control, check out the new <APILink fn="mlflow.genai.judges.make_judge">make_judge</APILink> API. It provides a unified interface for all judge types and seamless MLflow integration. [Learn more →](/genai/eval-monitor/scorers/llm-judge/make-judge)
:::
...
## Next Steps: Advanced Custom Judges

Want to create more sophisticated custom judges? The new **make_judge API** (MLflow 3.4.0+) provides a unified, flexible interface for creating custom LLM judges with:

- **Template-based instructions** with variable substitution
- **Version control** for registering and tracking judge iterations
- **Seamless integration** with MLflow evaluation framework
- **Flexible data organization** with inputs, outputs, and expectations

[Learn how to use make_judge →](/genai/eval-monitor/scorers/llm-judge/make-judge)
