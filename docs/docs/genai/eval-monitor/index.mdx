...
**Evaluation-Driven Development** is an emerging practice to tackle the challenge of building high-quality LLM/Agentic applications. MLflow is an **end-to-end** platform that is designed to support this practice and help you deploy AI applications with confidence.
...
```python
import mlflow
from mlflow.genai.evaluation import scorer
...
# Simple QA dataset
dataset = [
  {"inputs": { "question": "Can MLflow manage prompts?" }, "expectations": { "expected_answer": "Yes!" }},
  {"inputs": { "question": "Can MLflow create a taco for my lunch?" }, "expectations": { "expected_answer": "No, unfortunately, MLflow is not a taco maker." }}
]
...
results = mlflow.genai.evaluate(
  ...
  scorers=[Correctness()], # Built-in LLM judge scorer
)
```
...
<ImageBox src="/images/llms/tracing/genai-trace-evaluation.png" alt="Evaluation Results" />
...
<TilesGrid>
  <TileCard
    ...
    description="Understand the core concepts of evaluation in MLflow."
    href="/genai/eval-monitor/concepts"
    linkText="View the concepts →"
    containerHeight={64}
  />
  <TileCard
    icon={CheckCircle}
    ...
    description="Learn how to implement advanced scoring criteria for your applications."
