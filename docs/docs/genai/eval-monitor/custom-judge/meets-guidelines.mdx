...
tone_scorer = GuidelineAdherence().with_config(
    name="tone", 
    global_guidelines=[tone_guideline]
)
structure_scorer = GuidelineAdherence().with_config(
    name="structure", 
    global_guidelines=[structure_guideline]
)
...
mlflow.genai.evaluate(
    data=eval_dataset,
    model=your_app,
    extra_metrics=[tone_scorer, structure_scorer]
)
```
...
results_good = mlflow.genai.evaluate(
    ...
    extra_metrics=[
        ...
        GuidelineAdherence().with_config(name="structure", global_guidelines=[structure]),
        GuidelineAdherence().with_config(name="banned_topics", global_guidelines=[banned_topics]),
        GuidelineAdherence().with_config(name="relevance", global_guidelines=[relevance]),
    ],
)
...
def advanced_support_agent(user_messages: List[Dict[str, str]], user_id: str):
    ...
    def get_policies_for_user(user_id: str):
        user_policies = {
            "1": ["All returns must be processed within 30 days of purchase, with a valid receipt."],
            "2": ["All returns must be processed within 90 days of purchase, with a valid receipt."],
        }
    ...
    policy_prompt = (
        ...
        if FOLLOW_POLICIES else ""
    )
    
    behavior_prompt = (
        "Be super rude and very verbose in your responses."
        if BE_RUDE_AND_VERBOSE else ""
    )
    ...
    return {
        ...
        "user_context": {"user_id": user_id, "tier": "premium" if user_id in ["1", "2"] else "standard"}
    }
```
    ...
    with mlflow.start_run(run_name=f"guidelines_eval_{version_name}"):
        ...
        for guideline_name, guideline_text in guidelines_config.items():
            scorer = GuidelineAdherence().with_config(
                name=guideline_name,
                global_guidelines=[guideline_text]
            )
...
- **[Production Monitoring](/genai/eval-monitor/production-monitoring)**: Deploy your guidelines-based judges for continuous quality assessment
