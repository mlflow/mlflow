...
import { Target, BarChart3, Bot, AlertTriangle, Search, Lightbulb, FileText, Settings, CheckCircle } from "lucide-react";
...
MLflow organizes issue detection across six quality dimensions, forming the **CLEARS** framework. Choose which categories to focus on based on your application's requirements:
...
- Issues discovered so far (updates live)
...
## Working with Discovered Issues

### Issue Overview

Each discovered issue represents a cluster of related problems found across your traces. Issues provide:
...
<ImageBox src="/images/genai/issue-detection/issue-cards.png" alt="Discovered issues overview" width="90%"/>
...
To control costs at scale, connect via an [MLflow AI Gateway](/genai/gateway) endpoint and use its built-in budget controls to cap spending across detection runs.
...
|------------------------------|-------------------------|
| You want to discover unknown problems | You know the specific quality dimensions or hypotheses to test: use [MLflow Evaluation Judges](/genai/eval-monitor/scorers) |
