...
eval_dataset = mlflow.data.from_pandas(evaluation_data, name="app_eval_dataset", version="v1.0")
...
print(f"Successfully evaluated: {sum(1 for rid in evaluation_run_ids.values() if rid is not None)}/{len(apps_to_evaluate)} versions")
```
...
def analyze_version_performance(run_ids: dict):
    ...
    runs_df['version_name'] = runs_df['run_id'].map({v: k for k, v in valid_runs.items()})
    ...
    # Clean column names
    comparison_df.columns = [col.replace('metrics.', '') for col in comparison_df.columns]
    ...
    if comparison_df is not None:
        ...
        for metric in numeric_cols:
            ...
            if len(values) > 1:
                ...
                if cv < 0.1:
                    print(f"📊 {metric}: Low variance - differences may not be meaningful")
...
- **[LLM Evaluation](/genai/eval-monitor/llm-evaluation)**: Comprehensive guide to LLM evaluation metrics
