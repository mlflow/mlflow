...
- Automatic evaluation only supports [LLM judges](/genai/eval-monitor/scorers/#llms-as-judges). Code-based scorers (using the [`@scorer` decorator](/genai/eval-monitor/scorers/custom)) are not supported. Use [built-in LLM judges](/genai/eval-monitor/scorers/llm-judge/predefined) or create custom judges with [`make_judge()`](/genai/eval-monitor/scorers/llm-judge/custom-judges/create-custom-judge).
