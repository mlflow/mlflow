...
- [Jev (TypeSafe) scorers](/genai/eval-monitor/scorers/llm-judge/custom-judges/jev) also support automatic evaluation of individual traces using TypeSafe gateway endpoints.
...
- **Trace completion**: A trace becomes eligible for automatic evaluation 5 minutes after it starts by default. Configure the <APILink fn="mlflow.environment_variables.MLFLOW_ONLINE_SCORING_DEFAULT_TRACE_COMPLETION_BUFFER_SECONDS">trace completion buffer</APILink> to exceed the expected trace duration; traces that remain in progress longer than the buffer can be skipped
