...
- Online evaluations only support [LLM judges](/genai/eval-monitor/scorers/llm-judge). Code-based scorers (using the [`@scorer` decorator](/genai/eval-monitor/scorers/custom)) are not supported. Use [built-in LLM judges](/genai/eval-monitor/scorers/llm-judge/predefined) or create custom judges with [`make_judge()`](/genai/eval-monitor/scorers/llm-judge/make-judge).
...
**1. Specify the experiment for online evaluation**
...
**2. Start online evaluation for a trace-level judge**
...
**3. Start online evaluation for a multi-turn (session-level) judge**
...
**4. Update or disable online evaluation for an existing judge**
...
- **Re-evaluation**: If new traces are added to the session after evaluation, the session is re-evaluated and previous online evaluation results are replaced
