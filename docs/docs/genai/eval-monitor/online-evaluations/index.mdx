...
Assessments from online evaluation appear directly in the MLflow UI. For traces, assessments typically appear within a minute or two of logging. For multi-turn sessions, evaluation begins after 5 minutes of inactivity (no new traces) by default—this is [configurable](/api_reference/python_api/mlflow.environment_variables.html#mlflow.environment_variables.MLFLOW_ONLINE_SCORING_DEFAULT_SESSION_COMPLETION_BUFFER_SECONDS). Navigate to your experiment's **Traces** or **Sessions** tab to see results.
...
| **Evaluation errors** | Check server logs for details. Failed evaluations are not retried automatically |
