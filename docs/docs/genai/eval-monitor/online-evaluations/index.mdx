...
1. **The MLflow Server is running** → [Set Up MLflow Server](/genai/getting-started/connect-environment)
2. **MLflow Tracing is enabled** in your agent or LLM application → [Tracing Quickstart](/genai/tracing/quickstart)
   - **For multi-turn conversation evaluation**: Traces must include session IDs → [Track Users & Sessions](/genai/tracing/track-users-sessions)
3. **An AI Gateway endpoint is configured** for LLM judge execution → [Create and Manage Endpoints](/genai/governance/ai-gateway/endpoints/create-and-manage)
...
# Trace-level judge for tool call correctness
tool_judge = ToolCallCorrectness(model="gateway:/your-endpoint")
registered_judge = tool_judge.register(name="tool_call_correctness")
registered_judge.start(
    sampling_config=ScorerSamplingConfig(sample_rate=0.5),  # Evaluate 50% of traces
)

# Session-level judge for multi-turn conversations
...
Assessments from online evaluation appear directly on traces in the MLflow UI. Navigate to your experiment's **Traces** tab to see assessment scores for each trace.

![Online Evaluation Results](/images/genai/eval-monitor/online-evaluation-results.png)
