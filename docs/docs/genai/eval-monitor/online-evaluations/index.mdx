...
:::note
- Online evaluations only support [LLM judges](/genai/eval-monitor/scorers/llm-judge). Code-based scorers (using the [`@scorer` decorator](/genai/eval-monitor/scorers/custom-scorers)) are not supported. Use [built-in LLM judges](/genai/eval-monitor/scorers/builtin-scorers) or create custom judges with [`make_judge()`](/genai/eval-monitor/scorers/llm-judge).
...
<div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
  ...
  <div style={{ width: '80%', textAlign: 'right', marginTop: '8px', color: '#666', fontSize: '0.9em' }}>
    Assessments from online evaluation appear as columns in the Traces tab
  </div>
</div>
...
The server uses [AI Gateway](/genai/governance/ai-gateway/) endpoints to access LLMs for judge execution, ensuring secure and managed model access. Only the relevant trace or session data required by the judge (such as inputs, outputs, and context) is sent to the LLM.
