...
import { MessageSquare, Users, Target, Play, Code, Activity, BookOpen, Sparkles } from 'lucide-react';
...
<TabsWrapper>
<Tabs>
<TabItem value="static" label="Static Evaluation" default>
...
<TabItem value="dynamic" label="Dynamic Evaluation (Simulation)">
...
Static evaluation analyzes conversations that have already been traced. This is useful for evaluating production data or pre-recorded test conversations.
...
Dynamic evaluation generates new conversations by simulating user interactions. This enables testing different agent versions with consistent goals and personas.
...
| Scorer | Description | Return Values |
|--------|-------------|---------------|
| <APILink fn="mlflow.genai.scorers.ConversationCompleteness">ConversationCompleteness</APILink> | Evaluates whether the agent addressed all user questions | `"yes"` or `"no"` |
| <APILink fn="mlflow.genai.scorers.ConversationalGuidelines">ConversationalGuidelines</APILink> | Evaluates compliance with provided guidelines | `"yes"` or `"no"` |
| <APILink fn="mlflow.genai.scorers.ConversationalRoleAdherence">ConversationalRoleAdherence</APILink> | Evaluates if the assistant maintains its assigned role | `"yes"` or `"no"` |
| <APILink fn="mlflow.genai.scorers.ConversationalSafety">ConversationalSafety</APILink> | Evaluates if responses are safe and free of harmful content | `"yes"` or `"no"` |
| <APILink fn="mlflow.genai.scorers.ConversationalToolCallEfficiency">ConversationalToolCallEfficiency</APILink> | Evaluates if tool usage across the conversation was efficient | `"yes"` or `"no"` |
| <APILink fn="mlflow.genai.scorers.KnowledgeRetention">KnowledgeRetention</APILink> | Evaluates if the agent retains information from earlier turns | `"yes"` or `"no"` |
| <APILink fn="mlflow.genai.scorers.UserFrustration">UserFrustration</APILink> | Detects and tracks user frustration patterns | `"none"`, `"resolved"`, or `"unresolved"` |

See the [Built-in Judges](/genai/eval-monitor/scorers/llm-judge/predefined#multi-turn) page for detailed usage examples and API documentation.
...
To evaluate specific sessions, use <APILink fn="mlflow.search_sessions">`mlflow.search_sessions`</APILink> with a filter string:

```python
import mlflow
from mlflow.genai.scorers import ConversationCompleteness, UserFrustration

# Get traces for specific sessions using filter
traces = mlflow.search_sessions(
    locations=["<your-experiment-id>"],
    filter_string="metadata.`mlflow.trace.session` = '<your-session-id>'",
    return_type="list",
)

# Evaluate the filtered sessions
results = mlflow.genai.evaluate(
    data=traces,
    scorers=[ConversationCompleteness(), UserFrustration()],
)
...
<TilesGrid>
  ...
  <TileCard
    icon={Activity}
    iconSize={48}
    title="Built-in Multi-Turn Judges"
