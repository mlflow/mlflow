...
For reproducible testing, persist your test cases as an [MLflow Evaluation Dataset](/genai/datasets):
...
See [Conversation Simulation Datasets](/genai/datasets/conversation-simulation) for more details on managing test case datasets.
...
<TabItem value="stateful" label="Stateful Agent">
...
<TabItem value="remote" label="Remote Agent (A2A / OTLP)">

For agents that run out-of-process and push their own traces to MLflow via OpenTelemetry (e.g. an A2A server), MLflow cannot detect the trace they create in the local thread. Wrap your return value in `PredictResult` to pass the remote trace ID back to the simulator explicitly:
...
def predict_fn(input: list[dict], **kwargs) -> PredictResult:
    # Forward the session ID so the remote agent can tag its trace accordingly
    session_id = kwargs.get("mlflow_session_id")

    # Call your A2A agent server — it writes its own trace to the MLflow server via OTLP
    result = requests.post(
        "https://my-a2a-agent/v1/message:stream",
        json={"messages": input, "session_id": session_id},
    ).json()

    # The remote agent returns the MLflow trace ID it created
...
:::tip[When to use PredictResult]
Use `PredictResult` whenever your agent runs in a separate process and writes its own MLflow trace — for example, an [A2A protocol](https://google.github.io/A2A/) agent server exporting traces via OTLP, a Databricks Model Serving endpoint, or any HTTP-based agent. Without it, the simulator cannot find the remote trace to attach assessments to and falls back to creating a thin local trace that lacks internal spans such as tool calls.

If your agent runs in-process and is already decorated with `@mlflow.trace`, you do not need `PredictResult`.
:::
...
Supported model formats follow the pattern `"<provider>:/<model>"`. See [Supported Models](/genai/eval-monitor/scorers/llm-judge/custom-judges/supported-models) for the full list of supported providers.
