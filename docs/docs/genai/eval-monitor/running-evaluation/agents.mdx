...
Design test cases as a list of dictionaries, each with an `inputs`, `expectations`, and `tags` field. We would like to evaluate the correctness of the output, but also the tool calls used by the agent.
