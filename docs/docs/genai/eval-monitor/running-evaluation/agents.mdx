...
In MLflow 2.x, you can pass the model URI directly to the `model` argument of the legacy <APILink fn="mlflow.evaluate" /> API (deprecated). The new evaluation API in MLflow **3.x** still support evaluating MLflow Models, but the workflow is slightly different.
