...
```bash
uv pip install --upgrade 'mlflow>=3.3' openai
```
</TabItem>
...
These scorers automatically analyze traces to assess tool usage patterns. See the <ins>[Built-in Judges](/genai/eval-monitor/scorers/llm-judge/predefined)</ins> guide for more details.
...
In MLflow 2.x, you can pass the model URI directly to the `model` argument of the legacy <APILink fn="mlflow.evaluate" /> API (deprecated). The new evaluation API in MLflow **3.x** still supports evaluating MLflow Models, but the workflow is slightly different.
...
<TilesGrid>
  ...
  <TileCard
    ...
    description="Assess multi-turn conversations with specialized scorers for dialogue quality and user satisfaction."
    href="/genai/eval-monitor/running-evaluation/multi-turn"
    linkText="Evaluate conversations →"
    containerHeight={64}
  />
</TilesGrid>
