...
**Scorers** are Python functions that calculate quality metrics for your GenAI applications. When you run `mlflow.genai.evaluate()`, you provide a list of scorers that systematically assess different aspects of your application's performance.
...
**Result Generation**: Return one or more **Feedback** objects containing metric calculations, which `mlflow.genai.evaluate()` automatically logs as part of your evaluation results.

### Types of Scorers Available

<Tabs>
<TabItem value="builtin" label="🏗️ Built-in Scorers">
...
<TabItem value="custom" label="🛠️ Custom Scorers">
...
```mermaid
graph TD
    A[🎯 Your GenAI Application] --> B[📊 Evaluation Request]
    B --> C[🔧 Scorers Process Input/Output]
    C --> D{📝 Assessment Strategy}

    D -->|Built-in Logic| E[⚡ Direct Calculation]
    D -->|Need AI Assessment| F[🤖 Judges with LLM]
    D -->|Custom Logic| G[🛠️ Python Functions]

    E --> H[📋 Feedback Objects]
    F --> H
    G --> H

    H --> I[📈 Aggregated Results]
    H --> J[🔍 Individual Insights]

    I --> K[📊 Dashboard & Analytics]
    J --> L[🎯 Targeted Improvements]

    K --> M[🚀 Production Decisions]
    L --> N[🔄 Iterative Enhancement]
...
results = mlflow.genai.evaluate(
    ...
    predict_fn=your_genai_application,
    scorers=evaluation_suite,
)
```
