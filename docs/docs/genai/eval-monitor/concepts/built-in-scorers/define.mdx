...
1. For each row, `mlflow.genai.evaluate()` passes the Trace to the scorer
   - For some predefined scorers, the `expectations` key from the input data provided to the `data` argument
...
### Global guidelines, if 1 guideline, returns a single Feedback, otherwise List[Feedback]
