...
edge_cases = [
    ...
    {
        "inputs": {"question": "Tell me everything about machine learning"},  # Overly broad
        "expectations": {
            "guidelines": "Provide structured overview without overwhelming detail"
        }
    },
    {
        "inputs": {"question": "What's the best algorithm for my problem?"},  # Underspecified
        "expectations": {
...
mlflow.evaluate(
    data=dataset,
    model=my_app,
    extra_metrics=[relevance_scorer, completeness_scorer]
)
```
...
# Validate fixes
validation_results = mlflow.evaluate(data=problem_dataset, model=improved_app, extra_metrics=scorers)
