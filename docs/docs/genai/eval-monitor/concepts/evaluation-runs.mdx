...
Every call to `mlflow.genai.evaluate()` creates a new evaluation run within the specified or default experiment:
...
mlflow.genai.evaluate(
    ...
    predict_fn=development_version,
    scorers=basic_scorers,
...
mlflow.genai.evaluate(
    ...
    predict_fn=experimental_approach,
    scorers=comparison_scorers,
...
mlflow.genai.evaluate(
    ...
    predict_fn=updated_version,
    scorers=stability_scorers,
...
results = mlflow.genai.evaluate(
    data=test_dataset, predict_fn=my_application, scorers=quality_scorers
)
