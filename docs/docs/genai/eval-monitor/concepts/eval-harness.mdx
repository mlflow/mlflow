...
def evaluate_current_version():
    results = mlflow.genai.evaluate(
        data=focused_test_set, predict_fn=my_app_v2, scorers=standard_scorers
    )
    return results
...
def compare_versions():
    baseline_results = mlflow.genai.evaluate(
        data=comprehensive_dataset, predict_fn=my_app_v1, scorers=standard_scorers
    )

    current_results = mlflow.genai.evaluate(
        data=comprehensive_dataset, predict_fn=my_app_v2, scorers=standard_scorers
    )
...
production_results = mlflow.genai.evaluate(
    ...
    predict_fn=deployed_predict_fn,
