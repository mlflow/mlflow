...
[[poor]]: Incomplete, unprofessional, or misses key concerns""",
    numeric_values={"excellent": 1.0, "good": 0.7, "poor": 0.0},
)
...
[[poor]]: Cold, dismissive, or inappropriate emotional response""",
    numeric_values={"exceptional": 1.0, "good": 0.75, "adequate": 0.5, "poor": 0.0},
)
...
results = mlflow.genai.evaluate(data=eval_data, scorers=[customer_service_quality])
```
...
[[poor]]: Confusing, incomplete, or significantly flawed""",
    numeric_values={"excellent": 1.0, "good": 0.75, "fair": 0.4, "poor": 0.0},
)
...
[[minimal]]: Superficial or missing important clarity feedback""",
    numeric_values={"excellent": 1.0, "good": 0.7, "adequate": 0.4, "minimal": 0.0},
)
```
...
[[poor]]: Unclear, uses too much jargon, or lacks appropriate empathy""",
    numeric_values={"excellent": 1.0, "good": 0.75, "acceptable": 0.5, "poor": 0.0},
)
...
feedback = preference_judge(
    question=question, response_a=response_model_v1, response_b=response_model_v2
)
...
[[poor]]: Unhelpful or inappropriate""",
            numeric_values={"excellent": 1.0, "good": 0.7, "fair": 0.4, "poor": 0.0},
        )
...
quality_judge = create_prompt_judge(
    ..., numeric_values={"high": 1.0, "medium": 0.5, "low": 0.0}
)
accuracy_judge = create_prompt_judge(
    ..., numeric_values={"accurate": 1.0, "partial": 0.5, "wrong": 0.0}
)
```
...
academic_scale = {"A": 4.0, "B": 3.0, "C": 2.0, "D": 1.0, "F": 0.0}

# Net Promoter Score scale
nps_scale = {"promoter": 1.0, "passive": 0.0, "detractor": -1.0}  # 9-10  # 7-8  # 0-6
```
...
invalid_judge = create_prompt_judge(
    assessment_name="invalid", prompt_template="Rate the response: {{response}}"
)
...
```python
judge = create_prompt_judge(
    assessment_name="test", prompt_template="{{request}} {{response}} [[good]]: Good"
)
