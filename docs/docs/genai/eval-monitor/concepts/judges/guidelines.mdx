...
2. **[Prebuilt `ExpectationsGuidelines()` scorer](#2-prebuilt-expectationsguidelines-scorer)**: Apply per-row guidelines labled by domain experts in an evaluation dataset. Evaluates app inputs/outputs only. For offline evaluation only.
