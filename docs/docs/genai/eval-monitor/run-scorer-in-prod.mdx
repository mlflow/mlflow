...
:::warning
MLflow currently only supports using [predefined scorers](/genai/eval-monitor/predefined-judge-scorers) for production monitoring. Contact your Databricks account representative if you need to run custom code-based or LLM-based scorers in production.
:::
