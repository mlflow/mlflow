...
# Ensure MLflow is connected to your tracking server
...
def diagnose_monitoring_data():
    try:
        ...
        if config.paused:
            ...
            print(
                "📊 Check your application is sending traces to the correct experiment"
            )
            ...
            if recent_traces == 0:
                print(
                    "⚠️ No recent traces found - verify your application is instrumented"
                )
                return "no_traces"
            else:
...
def oss_quality_monitoring():
    ...
    with mlflow.start_run(
        ...
        results = mlflow.genai.evaluate(
            data=recent_data,
            scorers=[quality_scorer, safety_checker, business_logic_scorer],
        )
