...
def response_contains_keywords(inputs, outputs):
    ...
    if found_keywords:
        ...
        return {
            "score": "no", 
            "rationale": "No business-relevant keywords found"
        }
...
try:
    monitor = create_external_monitor(
        ...
        assessments_config=AssessmentsSuiteConfig(
            ...
            assessments=[
                ...
                BuiltinJudge(
                    name="safety", 
                    sample_rate=1.0  # Evaluate all traces for safety
                ),
                BuiltinJudge(
                    name="relevance_to_query",
                    sample_rate=0.2  # 20% of traffic
                ),
    ...
    print(f"📊 Monitoring dashboard: {monitor.monitoring_page_url}")
    print("⏱️ Allow 15-20 minutes for initial data to appear")

except Exception as e:
    print(f"❌ Error creating monitor: {e}")
    print("💡 Ensure you have CREATE TABLE permissions and the experiment exists")
```
...
try:
    ...
    print("✅ Current monitor configuration retrieved")
except Exception as e:
    print(f"❌ Error retrieving monitor: {e}")
    current_config = None
...
def response_sentiment_check(inputs, outputs):
    ...
    if negative_count > positive_count:
        ...
        return {"score": "yes", "rationale": "Response maintains neutral or positive sentiment"}

# Step 3: Create updated configuration
if current_config:
    # Preserve existing assessments and add new ones
    new_assessments = list(current_config.assessments) if current_config.assessments else []
    ...
    try:
        updated_monitor = update_external_monitor(
            experiment_id=EXPERIMENT_ID,
            assessments_config=updated_config
        )
        print("✅ Monitor updated successfully")
        print(f"📊 Updated dashboard: {updated_monitor.monitoring_page_url}")
    except Exception as e:
...
if current_config and current_config.assessments:
    ...
    retained_assessments = [
        assessment for assessment in current_config.assessments
    ...
    try:
        ...
        print(f"✅ Removed {len(scorers_to_remove)} scorer(s) from monitor")
        print(f"📊 Remaining assessments: {len(retained_assessments)}")
    except Exception as e:
        print(f"❌ Error removing scorers: {e}")
```
...
try:
    ...
    print("ℹ️ Trace data and archive tables are preserved")
except Exception as e:
...
try:
    ...
    if "permission" in str(e).lower():
        print("❌ Permission issue: Ensure you have CREATE TABLE permissions")
        print("💡 Contact your Databricks admin to grant Unity Catalog permissions")
        ...
        print("❌ Experiment issue: Verify experiment ID exists")
        print("💡 Check mlflow.get_experiment() to confirm experiment access")
        # Verify experiment exists
        try:
            exp = mlflow.get_experiment(EXPERIMENT_ID)
            print(f"✅ Experiment found: {exp.name}")
        except:
            print("❌ Experiment not found or not accessible")
    else:
...
def diagnose_monitoring_data():
    try:
        ...
        if config.paused:
            print("⚠️ Monitor is paused - unpause to resume data collection")
            return "paused"
        elif not config.assessments:
            print("⚠️ No assessments configured - add scorers to collect data")
            return "no_assessments"
        else:
            print(f"ℹ️ Monitor active with {len(config.assessments)} assessments")
            print("⏱️ Wait 15-20 minutes for initial data collection")
            print("📊 Check your application is sending traces to the correct experiment")
            
            # Check recent trace activity
            recent_traces = check_recent_trace_activity(EXPERIMENT_ID)
            if recent_traces == 0:
                print("⚠️ No recent traces found - verify your application is instrumented")
                return "no_traces"
            else:
                print(f"✅ Found {recent_traces} recent traces")
...
if optimized_config:
    update_external_monitor(
        experiment_id=EXPERIMENT_ID,
        assessments_config=optimized_config
    )
    print("✅ Monitor optimized for cost efficiency")
```
...
def oss_quality_monitoring():
    ...
    with mlflow.start_run(run_name=f"quality_check_{datetime.now().strftime('%Y%m%d_%H%M')}"):
        results = mlflow.evaluate(
            data=recent_data,
            extra_metrics=[
                quality_scorer,
                safety_checker,
                business_logic_scorer
            ]
        )
