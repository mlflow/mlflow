...
for issue in issues:
    with mlflow.start_run(experiment_id=experiment_id):
        ...
        trace_id = mlflow.get_last_active_trace().info.trace_id
...
# Collect human feedback based on your scenario:

# Scenario 1: You have ground truth labels
ground_truth = {
...
# Scenario 2 (RECOMMENDED): Use the MLflow UI for feedback collection
# The MLflow UI provides an intuitive interface for reviewing and rating traces:
# 1. Navigate to the Traces tab in your experiment
# 2. Click on individual traces to review inputs/outputs
# 3. Add feedback ratings directly through the UI
# 4. The UI automatically logs feedback in the correct format
```
...
if len(aligned_traces) >= 10:
    aligned_judge = support_judge.align(
        traces=aligned_traces, optimizer="simba"  # Default optimizer
    )
...
aligned_judge.register(experiment_id)
