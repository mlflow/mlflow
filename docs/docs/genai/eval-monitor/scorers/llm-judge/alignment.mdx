...
MLflow provides the SIMBA (Simplified Multi-Bootstrap Aggregation) optimizer as the **default optimizer** for aligning judges with human feedback. When you call `align()` without specifying an optimizer, SIMBA is used automatically:
...
When using `align()` without an optimizer parameter, MLflow automatically uses the SIMBA optimizer with its default configuration. This simplifies the alignment process while still allowing customization when needed.
:::
...
- **SIMBAAlignmentOptimizer** (default): Uses Simplified Multi-Bootstrap Aggregation for robust alignment
