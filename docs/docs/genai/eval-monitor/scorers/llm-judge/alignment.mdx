...
# Step 4: Collect human feedback through the MLflow UI
# IMPORTANT: Human feedback is typically provided through the MLflow UI's trace view,
# not programmatically. The UI provides an intuitive interface for reviewing traces
# and adding feedback ratings.

# To provide feedback via UI:
# 1. Open MLflow UI and navigate to your experiment
# 2. Click on a trace to view details
# 3. Review the judge's assessment
# 4. Click "Add Feedback" to provide your rating
# 5. Select the same assessment name as the judge uses
# 6. Provide your ground truth rating

# The API is available but rarely used in practice:
# mlflow.log_feedback(trace_id, name="product_quality", value="excellent", ...)
