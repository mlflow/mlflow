...
**Note**: Alignment is currently supported only for field-based evaluation using `{{ inputs }}` and `{{ outputs }}` templates. Support for agentic evaluation (`{{ trace }}`) and expectations (`{{ expectations }}`) is not yet available.
:::
...
<img src="/images/assessments/add_feedback_ui.png" alt="MLflow UI Feedback Interface" style={{maxWidth: '800px', display: 'block', margin: '20px 0'}} />
