...
For any models that are not supported natively, it is also possible to use LiteLLM. Since LiteLLM is not a dependency of MLflow, you'll need to install it separately by running `uv pip install litellm` or `pip install litellm`. After this, simply specify the provider and model name in the same format as natively supported providers.
