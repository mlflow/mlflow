...
- OpenAI / Azure OpenAI
- Anthropic
- Amazon Bedrock (including Anthropic Claude and Amazon Nova models)
- Cohere
- Together AI
...
Amazon Bedrock supports multiple model families. Use the `bedrock:/` prefix with the Bedrock model ID:
...
:::note
Amazon Bedrock requires AWS credentials to be configured in your environment (e.g., `AWS_ACCESS_KEY_ID`, `AWS_SECRET_ACCESS_KEY`, and `AWS_DEFAULT_REGION`). For more details, see the [Amazon Nova documentation](https://docs.aws.amazon.com/nova/latest/userguide/what-is-nova.html).
:::
