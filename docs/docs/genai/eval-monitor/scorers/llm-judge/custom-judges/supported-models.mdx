...
## TypeSafe Jev Evaluation Models

Jev uses a structured evaluation API. Create a [native Jev scorer](/genai/eval-monitor/scorers/llm-judge/custom-judges/jev) with `make_jev_scorer()` and `model="typesafe:/jev-latest"` for local evaluation with `TYPESAFE_API_KEY`. Use a TypeSafe `gateway:/` endpoint to save the scorer or run it from the UI. The chat-based `make_judge()` API does not support Jev models.
