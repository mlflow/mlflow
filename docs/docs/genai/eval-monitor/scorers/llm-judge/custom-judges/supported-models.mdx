...
:::warning
Judges configured with direct model providers require credentials to be available locally (typically via environment variables) and **cannot be run from the UI**. For supported chat-completion providers, use AI Gateway endpoints if you want to run judges from the UI. TypeSafe Gateway passthrough endpoints cannot back MLflow judges and are not available in judge model selectors.
:::

TypeSafe judge models use TypeSafe's System One evaluation API rather than a chat-completion API. Use `typesafe:/` model URIs only with the supported `bool` and finite `Literal[...]` outputs described in [TypeSafe Judge Models](/genai/eval-monitor/scorers/llm-judge/custom-judges/typesafe). TypeSafe AI Gateway passthrough endpoints expose raw System One requests and are not `gateway:/` judge models for `make_judge()`.
