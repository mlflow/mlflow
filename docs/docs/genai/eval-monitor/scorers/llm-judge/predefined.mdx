...
The Guidelines, RelevanceToQuery, Safety, Correctness, and ExpectationsGuidelines scorers can extract inputs and outputs directly from traces:
...
When the scorer cannot extract the required fields from a trace (e.g., complex multi-step workflows), it automatically falls back to submitting the serialized trace info to an LLM judge. This ensures evaluation can still proceed even with traces that don't follow the standard input/output pattern.

The LLM judge receives the same evaluation instructions along with the trace info JSON (metadata, inputs, outputs, but not individual spans), allowing it to make a best-effort evaluation by analyzing the available execution context.
