...
The `make_judge` API requires **MLflow >= 3.4.0**. For earlier versions, use the <ins>[legacy judge functions](#bring-your-own-prompt-deprecated)</ins>.
:::
...
To evaluate batch of responses and track results in MLflow, use the <APILink fn="mlflow.genai.evaluate">mlflow.genai.evaluate</APILink> function.
...
#### TODO: Add migration guide from legacy APIs
