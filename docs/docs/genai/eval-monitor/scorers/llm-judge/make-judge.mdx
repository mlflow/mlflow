...
4. **Configure the judge**:
   ...
   - **Model**: Select an endpoint from the dropdown (recommended) or click "enter model manually" to specify a custom model URI. Endpoints can be configured using [AI Gateway](/genai/governance/ai-gateway/), which centralizes API key management. Judges using manually entered model URIs require local API keys and cannot be run directly from the UI. See [Supported Models](/genai/eval-monitor/scorers/llm-judge#supported-models) for details.
...
6. **Configure evaluation settings** (optional):
...
- **[AI Gateway](/genai/governance/ai-gateway/) endpoints** (Recommended for UI) - Enables running judges directly from the UI with centralized API key management. Select an endpoint from the dropdown or use the `gateway:/` prefix in the SDK.
- **Direct model providers** - Call providers like OpenAI, Anthropic, Google, etc. directly. Requires local API keys and cannot be run from the UI.
