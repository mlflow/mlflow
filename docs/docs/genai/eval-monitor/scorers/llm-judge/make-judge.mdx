...
You can control the type of the judge result using the `result_type` argument. The <APILink fn="mlflow.genai.judges.make_judge">make_judge</APILink> API natively supports common types like `bool`, `int`, `float`, and `Literal` for categorical outcomes. This ensures judge LLMs produce structured outputs, making results reliable and easy to use.
