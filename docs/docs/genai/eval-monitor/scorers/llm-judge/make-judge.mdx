...
**Note on `conversation` variable:** The `{{ conversation }}` template variable can only be used with `{{ expectations }}`. It cannot be combined with `{{ inputs }}`, `{{ outputs }}`, or `{{ trace }}` variables. This is because conversation history provides complete context, making individual turn data redundant.
:::
