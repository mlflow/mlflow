...
LLM-as-a-Judge is an evaluation approach that uses Large Language Models to assess the quality of AI-generated responses. LLM judges can evaluate subjective qualities like helpfulness, safety, which are hard to be measured by heuristic metrics. On the the hand, LLM-as-a-Judge scorers are more scalable and cost-effective than human evaluation. See the [Evaluation Approach Comparison](/genai/eval-monitor/scorers#evaluation-approaches-comparison) for more detailed comparison and the guides of how to use other approaches in MLflow.

## Approaches for using LLM

MLflow offers different approaches to use LLM-as-a-Judge, with different levels of simplicity and control. We recommends starting with simpler approach and evolve to more complex ones as needed.
...
- **Best for**: Quickly try MLflow's LLM evaluation capabilities with a few lines of code.
- **How it works**: Select from a list of built-in classes such as <APILink fn="mlflow.genai.scorers.Correctness">Correctness</APILink>. MLflow constructs inputs for the judge using predefined prompt templates.
...
[Get started with bring your own prompt »](/genai/eval-monitor/scorers/llm-judge/prompt)

## Selecting Judge Models

By default, MLflow will use **OpenAI's GPT 4.1 mini** model as the judge model. You can change the judge model by passing an override to the `model` argument within the scorer definition. The model must be specified in the format of `<provider>:/<model-name>`.

```python
from mlflow.genai.scorers import Correctness

Correctness(model="openai:/o3-mini")
Correctness(model="anthropic:/claude-4-opus")
Correctness(model="google:/gemini-2.5-flash")
```
...
To use LiteLLM integrated models, you need to install LiteLLM and specify the provider and model name in the same format as natively supported providers, e.g., `gemini/gemini-2.5-flash`.
