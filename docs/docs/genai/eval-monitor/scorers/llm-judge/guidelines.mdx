...
:::info[New: make_judge API for unified judge creation]

The <APILink fn="mlflow.genai.judges.make_judge">make_judge</APILink> API (MLflow 3.4.0+) offers a unified approach for creating custom judges:

- **Single API** - Replace Guidelines, ExpectationsGuidelines, and custom_prompt_judge with one interface
- **Template-based** - Natural language templates with variable substitution
- **Version control** - Register and track judge iterations
- **Flexible data handling** - Clean separation of inputs, outputs, and expectations
