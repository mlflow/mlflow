...
**Scorers** are key components of the MLflow GenAI evaluation framework. It is a unified interface to define evaluation criteria for your model, agents, and applications.
...
2. MLflow runs your `predict_fn` to generate <div className="inline-flex rounded-sm bg-orange-100 px-2 py-1 text-sm text-orange-900">outputs</div> and <div className="inline-flex rounded-sm bg-cyan-100 px-2 py-1 text-sm text-cyan-900">traces</div> for each row in the dataset. Alternatively, you can provide outputs and traces directly in the dataset and omit predict function.
3. Scorers receive the <div className="inline-flex rounded-sm bg-green-100 px-2 py-1 text-sm text-green-900">inputs</div>, <div className="inline-flex rounded-sm bg-orange-100 px-2 py-1 text-sm text-orange-900">outputs</div>, <div className="inline-flex rounded-sm bg-yellow-100 px-2 py-1 text-sm text-yellow-900">expectations</div>, and <div className="inline-flex rounded-sm bg-cyan-100 px-2 py-1 text-sm text-cyan-900">traces</div> (or subset of them) and produce scores and metadata such as explanations, source.
4. MLflow aggregates the scorers results and save them. You can analyze the results in the UI.
...
|--------|------------------|-------------------|------------------|
| **Description** | Deterministic metrics such as `exact_match`, `BLEU`. | Let LLM judge subjective qualities such as `Correctness` | Ask domain experts or users to provide feedbacks. |
