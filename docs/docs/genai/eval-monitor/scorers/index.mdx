...
Start with [built-in judges](/genai/eval-monitor/scorers/llm-judge/predefined/#available-judges) for quick evaluation. As your needs evolve, build [custom LLM judges](/genai/eval-monitor/scorers/llm-judge/custom-judges) for domain-specific criteria and create [custom code-based scorers](#) for programmatic business logic.
