...
| **Agent-as-a-Judge Scorers** | ❌ No               | ✅ Yes                        | Scorers analyzing execution flow, tool usage patterns, span attributes      |
| **Retrieval Scorers**        | ❌ No               | ✅ Yes (with RETRIEVER spans) | `RetrievalGroundedness`, `RetrievalRelevance`, `RetrievalSufficiency`       |
| **Agent-as-a-Judge**         | ❌ No               | ✅ Yes                        | Custom judges analyzing multi-step workflows, agent reasoning, performance  |

### Key Points:

- **Static datasets** (pandas DataFrames with inputs/outputs/expectations) work only with field-based scorers
- **Agent-as-a-Judge scorers** require actual execution traces from your application
- If you see errors about missing traces when using retrieval or Agent-as-a-Judge scorers, you need to:
