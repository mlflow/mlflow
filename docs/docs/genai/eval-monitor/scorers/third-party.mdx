...
<Tabs>
  <TabItem value="deepeval" label="DeepEval" default>
...
You can call RAGAS scorers directly using <APILink fn="mlflow.genai.scorers.ragas.get_scorer">get_scorer</APILink>:
...
feedback = scorer(
    inputs="What is MLflow?",
    outputs="MLflow is an open-source platform for managing machine learning workflows.",
    trace=trace,  # Trace containing retrieval context
)
...
eval_dataset = [
    {
        "inputs": {"query": "What is MLflow?"},
        "outputs": "MLflow is an open-source platform for managing machine learning workflows.",
        "trace": trace,  # Trace with retrieval context
    },
]
...
| -------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- | -------------------------------------------------------------------------------- |
| <APILink fn="mlflow.genai.scorers.ragas.ContextPrecision">ContextPrecision</APILink>                                       | Are relevant retrieved documents ranked higher than irrelevant ones?       | [Link](https://docs.ragas.io/en/latest/concepts/metrics/context_precision)       |
| <APILink fn="mlflow.genai.scorers.ragas.NonLLMContextPrecisionWithReference">NonLLMContextPrecisionWithReference</APILink> | Non-LLM version of context precision using reference answers               | [Link](https://docs.ragas.io/en/latest/concepts/metrics/context_precision)       |
| <APILink fn="mlflow.genai.scorers.ragas.ContextRecall">ContextRecall</APILink>                                             | Does retrieval context contain all information needed to answer the query? | [Link](https://docs.ragas.io/en/latest/concepts/metrics/context_recall)          |
| <APILink fn="mlflow.genai.scorers.ragas.NonLLMContextRecall">NonLLMContextRecall</APILink>                                 | Non-LLM version of context recall using reference answers                  | [Link](https://docs.ragas.io/en/latest/concepts/metrics/context_recall)          |
| <APILink fn="mlflow.genai.scorers.ragas.ContextEntityRecall">ContextEntityRecall</APILink>                                 | Are entities from the expected answer present in the retrieved context?    | [Link](https://docs.ragas.io/en/latest/concepts/metrics/context_entities_recall) |
| <APILink fn="mlflow.genai.scorers.ragas.NoiseSensitivity">NoiseSensitivity</APILink>                                       | How sensitive is the model to irrelevant information in the context?       | [Link](https://docs.ragas.io/en/latest/concepts/metrics/noise_sensitivity)       |
| <APILink fn="mlflow.genai.scorers.ragas.Faithfulness">Faithfulness</APILink>                                               | Is the output factually consistent with the retrieval context?             | [Link](https://docs.ragas.io/en/latest/concepts/metrics/faithfulness)            |
...
| Scorer                                                                                           | What does it evaluate?                                       | RAGAS Docs                                                                   |
| ------------------------------------------------------------------------------------------------ | ------------------------------------------------------------ | ---------------------------------------------------------------------------- |
| <APILink fn="mlflow.genai.scorers.ragas.FactualCorrectness">FactualCorrectness</APILink>         | Is the output factually correct compared to expected answer? | [Link](https://docs.ragas.io/en/latest/concepts/metrics/factual_correctness) |
| <APILink fn="mlflow.genai.scorers.ragas.NonLLMStringSimilarity">NonLLMStringSimilarity</APILink> | String similarity between output and expected answer         | [Link](https://docs.ragas.io/en/latest/concepts/metrics/string_based)        |
| <APILink fn="mlflow.genai.scorers.ragas.BleuScore">BleuScore</APILink>                           | BLEU score for text comparison                               | [Link](https://docs.ragas.io/en/latest/concepts/metrics/string_based)        |
| <APILink fn="mlflow.genai.scorers.ragas.ChrfScore">ChrfScore</APILink>                           | CHRF score for text comparison                               | [Link](https://docs.ragas.io/en/latest/concepts/metrics/string_based)        |
| <APILink fn="mlflow.genai.scorers.ragas.RougeScore">RougeScore</APILink>                         | ROUGE score for text comparison                              | [Link](https://docs.ragas.io/en/latest/concepts/metrics/string_based)        |
| <APILink fn="mlflow.genai.scorers.ragas.StringPresence">StringPresence</APILink>                 | Is a specific string present in the output?                  | [Link](https://docs.ragas.io/en/latest/concepts/metrics/string_based)        |
| <APILink fn="mlflow.genai.scorers.ragas.ExactMatch">ExactMatch</APILink>                         | Does output exactly match expected output?                   | [Link](https://docs.ragas.io/en/latest/concepts/metrics/string_based)        |
...
| Scorer                                                                             | What does it evaluate?                             | RAGAS Docs                                                        |
| ---------------------------------------------------------------------------------- | -------------------------------------------------- | ----------------------------------------------------------------- |
| <APILink fn="mlflow.genai.scorers.ragas.AspectCritic">AspectCritic</APILink>       | Evaluates specific aspects of the output using LLM | [Link](https://docs.ragas.io/en/latest/concepts/metrics/critique) |
| <APILink fn="mlflow.genai.scorers.ragas.RubricsScore">RubricsScore</APILink>       | Scores output based on predefined rubrics          | [Link](https://docs.ragas.io/en/latest/concepts/metrics/rubrics)  |
| <APILink fn="mlflow.genai.scorers.ragas.InstanceRubrics">InstanceRubrics</APILink> | Scores output based on instance-specific rubrics   | [Link](https://docs.ragas.io/en/latest/concepts/metrics/rubrics)  |
...
| Scorer                                                                                   | What does it evaluate?        | RAGAS Docs                                                             |
| ---------------------------------------------------------------------------------------- | ----------------------------- | ---------------------------------------------------------------------- |
| <APILink fn="mlflow.genai.scorers.ragas.SummarizationScore">SummarizationScore</APILink> | Quality of text summarization | [Link](https://docs.ragas.io/en/latest/concepts/metrics/summarization) |
...
### Using RAGAS with Traces

RAGAS metrics work seamlessly with MLflow traces that contain retrieval context:

```python
import mlflow
from mlflow.genai.scorers.ragas import get_scorer

# Create trace with retrieval context
with mlflow.start_span(name="retrieval") as span:
    # Simulate retrieval
    span.set_attribute("mlflow.span.type", "retrieval")
    span.set_outputs(
        {
            "retrieved_contexts": [
                "MLflow is an open-source platform for managing ML workflows.",
                "It provides experiment tracking and model deployment capabilities.",
            ]
        }
    )
...
feedback = scorer(
    inputs="What is MLflow?",
    outputs="MLflow is a platform for machine learning.",
