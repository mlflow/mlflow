...
results = mlflow.genai.evaluate(
    ...
    scorers=[
        ...
        ContextualRecall(threshold=0.7),
    ],
)
```
...
results = mlflow.genai.evaluate(
    data=agent_traces,
    scorers=[
        TaskCompletion(threshold=0.8),
        ToolCorrectness(threshold=0.9),
        StepEfficiency(threshold=0.7),
    ],
)
```
...
results = mlflow.genai.evaluate(
    data=conversation_traces,
    scorers=[
        TurnRelevancy(threshold=0.7),
        KnowledgeRetention(threshold=0.8),
        ConversationCompleteness(threshold=0.8),
    ],
)
```
...
```python
from mlflow.genai.scorers.deepeval import Bias, Toxicity, PIILeakage
...
results = mlflow.genai.evaluate(
    ...
    scorers=[
        Bias(threshold=0.3),
        Toxicity(threshold=0.2),
        PIILeakage(threshold=0.1),
    ],
)
```
...
```python
from mlflow.genai.scorers.deepeval import Hallucination, Summarization
...
```python
from mlflow.genai.scorers.deepeval import ExactMatch, PatternMatch
...
results = mlflow.genai.evaluate(
    ...
    scorers=[
        ExactMatch(),
        PatternMatch(pattern=r"\d{3}-\d{3}-\d{4}"),  # Phone number format
    ],
)
```
...
### Alternative API: get_judge()

For dynamic scorer creation, use the <APILink fn="mlflow.genai.scorers.deepeval.get_judge">get_judge</APILink> function:
...
This is useful when:
- Dynamically selecting metrics based on configuration
- Building generic evaluation pipelines
- Iterating over multiple metric types
