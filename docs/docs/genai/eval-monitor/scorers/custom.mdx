...
:::warning[Important: Trace-based Scorers Require Active Traces]

Scorers that accept a `trace` parameter **cannot be used with static pandas DataFrames**. They require actual execution traces from your application.

If you need to evaluate static data (e.g., a CSV file with pre-generated responses), use scorers that work with `inputs`, `outputs`, and `expectations` parameters only.
