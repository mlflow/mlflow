...
LLM judge scorers take a Gemini model ID, a pass/fail threshold, and a sample count for majority voting:

```python
from mlflow.genai.scorers.google_adk import ResponseEvaluation, Safety, Hallucination
...
safety = Safety(model="gemini-2.5-flash", threshold=0.5)
