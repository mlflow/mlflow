...
2. **🌐 HTTP Server**: Flask-based server handles incoming requests
  ...
  <TabItem value="ab-testing" label="A/B Testing">
    ...
    def predict():
        ...
        try:
            ...
            # Log for analysis
            logging.info(
                f"Model: {model_version}, Request: {request.json}, Response: {result}"
            )

            return jsonify({"prediction": result, "model_version": model_version})
