...
## Agent Metadata with AgentInfo

Use <APILink fn="mlflow.types.agent_info.AgentInfo">`AgentInfo`</APILink> to attach descriptive metadata to your server. This metadata is served at the `GET /agent/info` endpoint, enabling clients and orchestration platforms to discover what your agent does at runtime.

Pass an `AgentInfo` instance to the `AgentServer` constructor:
...
If no `AgentInfo` is provided, a default is created automatically with the server name, use case, and MLflow version. See the [AgentInfo](/genai/serving/agent-info) page for the full fields reference, custom schema declaration, subclassing, and more examples.
