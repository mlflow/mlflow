...
- Agent metadata via [`AgentInfo`](/genai/serving/agent-info) for runtime discovery through the `GET /agent/info` endpoint
...
2. Define a `start_server.py` file to start the `AgentServer`:
   ...
   agent_server = AgentServer(
       ...
       agent_info=AgentInfo(
           name="MathTutor",
           description="A math tutoring agent that explains its reasoning",
           version="1.0.0",
       ),
   )
...
The `GET /agent/info` endpoint returns metadata about the running agent. See [Agent Info](/genai/serving/agent-info) for full details on configuring `AgentInfo`.

```bash
curl http://localhost:8000/agent/info
```

With the example above, the response is:
