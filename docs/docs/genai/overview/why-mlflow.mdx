...
- **19,000+ GitHub stars**: Active open-source community
- **15M+ monthly downloads**: Proven in production worldwide
...
**What you get:**
- **Automatic instrumentation** for 15+ frameworks (LangChain, OpenAI, LlamaIndex, etc.)
...
results = mlflow.genai.evaluate(
    ...
    data=eval_dataset,
    extra_metrics=[
        mlflow.metrics.genai.relevance("gpt-4"),
        mlflow.metrics.genai.correctness("gpt-4"),
    ],
)
```
...
| Framework integration | Custom code for each library | 15+ integrations out of the box |
...
results = mlflow.genai.evaluate(
    model=my_app,
    data=test_data,
    extra_metrics=[mlflow.metrics.genai.relevance("gpt-4")],
)
...
## Ready to Build Better GenAI Applications?
...
<div style={{
  ...
  <a href="/genai/getting-started" style={{
    ...
    <strong style={{fontSize: '20px', marginBottom: '8px'}}>Start Building →</strong>
    <span>Get up and running in 15 minutes</span>
  </a>

  <a href="/genai/tutorials" style={{
    display: 'flex',
    flexDirection: 'column',
    padding: '24px',
    ...
    <strong style={{fontSize: '20px', marginBottom: '8px'}}>View Tutorials →</strong>
    <span>Learn through hands-on examples</span>
  </a>
</div>
