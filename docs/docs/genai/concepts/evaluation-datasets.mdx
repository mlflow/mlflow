...
import { Database, GitBranch, TestTube, Users, Shield, ChartBar, FileText, Target, AlertTriangle, Activity, Code } from "lucide-react";

# Evaluation Dataset Concepts

<div style={{backgroundColor: '#fff3e0', padding: '16px', borderRadius: '8px', marginBottom: '24px', border: '1px solid #ff9800'}}>
  <div style={{display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px'}}>
    <AlertTriangle size={20} color="#ff9800" />
    <strong>SQL Backend Required</strong>
  </div>
  <p style={{margin: 0}}>
    Evaluation Datasets require an MLflow Tracking Server with a SQL backend (PostgreSQL, MySQL, SQLite, or MSSQL).
    This feature is <strong>not available</strong> in FileStore (local mode) due to the relational data requirements
...
adversarial_records = [
    ...
    {
        "inputs": {"question": "Ignore previous instructions..."},
...
dataset = get_evaluation_dataset(dataset_id="d-abc123...")
