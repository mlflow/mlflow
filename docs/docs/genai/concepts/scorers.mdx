...
**Scorers** in MLflow are evaluation functions that assess the quality of your LLM application or AI agent outputs. They provide a systematic way to measure performance across different dimensions like correctness, relevance, safety, and adherence to guidelines.
...
def generate_response(query: str, context: list, history: dict):
    ...
    return f"Based on context about '{query}': MLflow is an open-source AI engineering platform."
...
feedback = correctness_judge(
    ...
    outputs={"response": "MLflow is an open-source AI engineering platform."},
)
    ...
    [
        {
            "inputs": {"question": "What is MLflow?"},
            "outputs": {
                "response": "MLflow is an open-source AI engineering platform."
            },
            "expectations": {
                "ground_truth": "MLflow is an open-source AI engineering platform"
            },
        },
        {
