...
| `inputs`        | `Optional[Any]`   | The input data that is passed into the particular stage of your application. May contain [Attachment](/genai/tracing/observe-with-traces/multimodal#trace-attachments) objects for binary content, which are automatically replaced with reference URIs.      |
| `outputs`       | `Optional[Any]`   | The output data that is passed out of the particular stage of your application. May contain [Attachment](/genai/tracing/observe-with-traces/multimodal#trace-attachments) objects for binary content, which are automatically replaced with reference URIs.   |
...
| `links`         | `List[Link]`      | Links to spans in other traces, representing relationships without parent-child hierarchy. See [Span Links](#span-links). |
...
- **Multi-agent handoffs** — when one agent triggers another and each has its own trace
...
Each link references a span in another trace:
...
For detailed usage patterns including the `@mlflow.trace` decorator and `start_span` context manager, see [Manual Tracing — Span Links](/genai/tracing/app-instrumentation/manual-tracing#span-links).

:::note
Span links are not currently supported for Unity Catalog traces. See [PR #22797](https://github.com/mlflow/mlflow/pull/22797) for details.
:::
