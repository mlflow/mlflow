...
<FeatureHighlights features={[
  {
    icon: Search,
    title: "Debugging",
    description: "Tracing provides deep insights into what happens beneath the abstractions of LLM and AI agent frameworks, helping you precisely identify where issues occur."
  },
  {
    icon: CheckCircle,
...
When a trace contains [Attachments](/genai/tracing/observe-with-traces/multimodal#trace-attachments) (binary content like images, audio, or PDFs), a directory is created in artifact storage for that trace under an `attachments/` path. The binary content is stored there as separate files, while the span data contains only lightweight `mlflow-attachment://` reference URIs. This keeps the span data small while supporting rich media content.

:::note
Prior to MLflow 3.3.0, spans were stored in artifact storage (e.g., S3, local filesystem) rather than in the database. Starting from MLflow 3.3.0, spans are stored directly in the database for improved query performance and transactional consistency. Attachments continue to use artifact storage.
:::
