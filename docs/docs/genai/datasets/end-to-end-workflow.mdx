...
```bash
uv pip install --upgrade 'mlflow>=3.4' openai
```
</TabItem>
...
Start with a traced LLM application. This example shows a customer support bot, but the pattern applies to any LLM application. You can use the <APILink fn="mlflow.trace">mlflow.trace decorator</APILink> for manual instrumentation or <APILink fn="mlflow.openai.autolog">enable automatic tracing for OpenAI</APILink> as shown below.
