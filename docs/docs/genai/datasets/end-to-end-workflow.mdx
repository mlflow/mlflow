...
```bash
pip install --upgrade mlflow>=3.3 openai pandas
```

## Step 1: Build & Trace Your Application

Start with a traced GenAI application. This example shows a customer support bot, but the pattern applies to any LLM application.

<APILink fn="mlflow.trace">mlflow.trace</APILink> | <APILink fn="mlflow.openai.autolog">mlflow.openai.autolog</APILink>
...
Run your application with real or test scenarios to capture traces.

<APILink fn="mlflow.search_traces">search_traces</APILink>
...
Add expectations to your traces to define what constitutes correct behavior.

<APILink fn="mlflow.log_expectation">log_expectation</APILink>
...
for trace in traces:
    # Determine expected behavior based on the question
    question = trace.info.tags.get("mlflow.spanInputs", "")
...
Transform your annotated traces into a reusable evaluation dataset.

<APILink fn="mlflow.genai.datasets.create_dataset">create_dataset</APILink> | <APILink fn="mlflow.entities.EvaluationDataset.merge_records">merge_records</APILink>
...
# Get traces with expectations using fluent API
annotated_traces = mlflow.search_traces(
    filter_string="expectations.key_information IS NOT NULL",
    max_results=100,
...
Evaluate your application against the dataset using built-in and custom scorers.

<APILink fn="mlflow.genai.evaluate">mlflow.genai.evaluate</APILink> | <APILink fn="mlflow.genai.scorers.Correctness">Correctness</APILink> | <APILink fn="mlflow.genai.scorers.scorer">scorer</APILink>
...
if not low_scores.empty:
    ...
    # Update knowledge base or prompts based on failures
    # ... make improvements ...
