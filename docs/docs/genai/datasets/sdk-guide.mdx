...
Remove specific records from a dataset using <APILink fn="mlflow.entities.EvaluationDataset.delete_records" text="delete_records()" />:

```python
import mlflow

# Get the dataset and its records
dataset = mlflow.get_dataset(dataset_id="d-1a2b3c4d5e6f7890")
df = dataset.to_df()
