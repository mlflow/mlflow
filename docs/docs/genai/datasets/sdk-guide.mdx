...
Remove specific records from a dataset using <APILink fn="mlflow.client.MlflowClient.delete_dataset_records" text="delete_dataset_records()" />:
...
deleted_count = client.delete_dataset_records(
    dataset_id=dataset.dataset_id,
    dataset_record_ids=record_ids_to_delete,
)
