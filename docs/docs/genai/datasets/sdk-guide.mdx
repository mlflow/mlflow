...
```python
# Add records with inputs and expectations
new_records = [
    {
    ...
    },
    {
        ...
        "expectations": {"includes_steps": True, "helpful": True},
    },
]
...
df = pd.DataFrame(
    {
        "inputs.question": ["What is MLflow?", "How to track experiments?"],
        "inputs.context": ["general", "technical"],
        "expectations.accuracy": [0.9, 0.95],
        "expectations.relevance": [1.0, 1.0],
        "tags.priority": ["high", "medium"],
    }
)
...
# Update with same inputs but different expectations
dataset.merge_records(
    [
        {
            ...
            "expectations": {
                ...
                # Note: "mentions_tracking": True is preserved
            },
        }
    ]
)
...
Use <APILink fn="mlflow.entities.EvaluationDataset.delete_records" text="delete_records()" /> to remove specific records by their IDs:

```python
# Get the dataset
from mlflow.genai.datasets import get_dataset

dataset = get_dataset(dataset_id="d-1a2b3c4d5e6f7890")

# View current records
print(f"Current records: {len(dataset.records)}")
for record in dataset.records[:3]:
    print(f"  - {record.record_id}: {record.inputs}")
...
dataset.delete_records(record_ids_to_delete)

print(f"After deletion: {len(dataset.records)} records remaining")
```
...
human_curated = [
    {
        ...
        # Automatically inferred as HUMAN source due to the presence of expectations
    }
]
