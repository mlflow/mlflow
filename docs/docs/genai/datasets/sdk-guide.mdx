...
import { Sparkles, Layers, Search, Tag, RefreshCw, Users, Database, FileText, GitBranch, BarChart3, Code, Activity } from "lucide-react";
...
Start by creating a new evaluation dataset with meaningful metadata:
...
dataset = create_evaluation_dataset(
    ...
    experiment_ids=["0"],  # Link to experiments ("0" is default)
    tags={
        "version": "1.0",
...
# merge_records() accepts both dict and pandas.DataFrame formats
test_cases = [
    {
        "inputs": {
...
# Add to your dataset (accepts list[dict] or pandas.DataFrame)
...
As you discover edge cases and improve your understanding, continuously update your dataset:
...
Use tags to track dataset evolution and enable powerful searches:
...
Find datasets using powerful search capabilities:
...
datasets = search_evaluation_datasets(
    experiment_ids=["0", "1"]  # Search in multiple experiments
)

# Search by name pattern
regression_datasets = search_evaluation_datasets(
    filter_string="name LIKE '%regression%'"
)

# Complex search with tags
production_ready = search_evaluation_datasets(
...
for dataset in production_ready:
    # Access dataset properties - pagination handled automatically
    pass
```
...
new_records = [{"inputs": {...}, "expectations": {...}}]
...
# Re-merge the same traces - existing records are updated with expectations
dataset.merge_records(traces[:20])
```
...
```python
# Get a dataset by ID
dataset = client.get_evaluation_dataset(dataset_id="d-abc123...")
...
# Delete a tag
client.delete_evaluation_dataset_tag(dataset_id=dataset.dataset_id, key="deprecated")
```
...
```python
# Delete a dataset
client.delete_evaluation_dataset(dataset_id=dataset.dataset_id)
```
...
- [Evaluation Dataset Concepts](/genai/concepts/evaluation-datasets) - Understand the conceptual model
- [Expectations Guide](/genai/assessments/expectations) - Define ground truth for your tests
- [MLflow Evaluation](/genai/eval-monitor) - Run evaluations with your datasets
- [Tracing Integration](/genai/tracing) - Capture production data for datasets