...
API. Let's evaluate our model on some [pre-defined metrics](/genai/eval-monitor/predefined-judge-scorers)
...
The <APILink fn="mlflow.genai.evaluate" /> API also supports [custom metrics](/genai/eval-monitor/custom-scorers),
[static dataset evaluation](/genai/eval-monitor/concepts/eval-datasets), and much more. For a
more in-depth guide, see [MLflow LLM Evaluation](/genai/eval-monitor).
