...
### Detailed Trace Comparison Analysis

Perform comprehensive side-by-side analysis to understand version differences:

```python
# Compare versions with detailed metrics
def detailed_version_comparison(v1_traces, v2_traces):
    """Comprehensive comparison of two versions using trace data."""

    print("🔍 Detailed Version Comparison")
    print(f"V1 Traces: {len(v1_traces)} | V2 Traces: {len(v2_traces)}\n")

    if not v1_traces or not v2_traces:
        print("⚠️ Need traces from both versions for comparison")
        return

    # Extract comprehensive metrics
    def extract_detailed_metrics(traces, version):
        metrics = {
            "version": version,
            "execution_times": [],
            "response_lengths": [],
        ...
        for trace in traces:
            try:
                ...
                # Execution time
                exec_time = info.get("execution_time_ms", 0)
                ...
                if spans:
                    ...
                    inputs = root_span.get("inputs", {})
                    outputs = root_span.get("outputs", "")
                    ...
                    metrics["input_lengths"].append(len(input_text))
                    metrics["response_lengths"].append(len(output_text))

                    # Approximate token count (rough estimate: 1 token ≈ 4 characters)
                    approx_tokens = (len(input_text) + len(output_text)) / 4
                    metrics["token_counts"].append(approx_tokens)
    ...
    v1_detailed = extract_detailed_metrics(v1_traces, "V1")
    v2_detailed = extract_detailed_metrics(v2_traces, "V2")
    ...
    def compare_metric(name, v1_values, v2_values, lower_is_better=True):
        v1_avg = safe_avg(v1_values)
        v2_avg = safe_avg(v2_values)

        if v1_avg > 0:
            change_pct = ((v2_avg - v1_avg) / v1_avg) * 100
            ...
            print(f"  {name}:")
            print(
                f"    V1: {v1_avg:.1f} | V2: {v2_avg:.1f} | Change: {change_pct:+.1f}% {direction}"
            )

    compare_metric(
        "Execution Time (ms)",
        v1_detailed["execution_times"],
        v2_detailed["execution_times"],
        lower_is_better=True,
    )
    compare_metric(
        "Response Length (chars)",
        v1_detailed["response_lengths"],
        v2_detailed["response_lengths"],
        lower_is_better=False,
    )
    compare_metric(
        "Approx Token Usage",
        v1_detailed["token_counts"],
        v2_detailed["token_counts"],
        lower_is_better=True,
    )

    print(f"\n🔧 Reliability:")
    print(
        f"  V1 Errors: {v1_detailed['error_count']}/{len(v1_traces)} ({v1_detailed['error_count']/len(v1_traces)*100:.1f}%)"
    )
    print(
        f"  V2 Errors: {v2_detailed['error_count']}/{len(v2_traces)} ({v2_detailed['error_count']/len(v2_traces)*100:.1f}%)"
    )

    return v1_detailed, v2_detailed


# Run detailed comparison
if v1_traces and v2_traces:
    detailed_comparison = detailed_version_comparison(v1_traces, v2_traces)
else:
    print("⚠️ Generate traces for both versions to see detailed comparison")
```
...
<TilesGrid>
  <TileCard
    href="/genai/prompt-version-mgmt/version-tracking"
