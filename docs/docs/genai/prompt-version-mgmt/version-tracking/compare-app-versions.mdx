...
improved_model_name = f"customer_support_agent-v2-{git_commit}"
...
print(
    f"V2 Average relevance: {evaluation_results_v2.metrics.get('relevance_to_query_mean', 'N/A'):.2f}"
)
print(
    f"V2 Support guidelines: {evaluation_results_v2.metrics.get('support_guidelines_mean', 'N/A'):.2f}"
)
```
...
- Model selection in experiment view
- Compare view diff, including trace comparison
  :::
...
all_versions = search_logged_models(
    filter_string="name LIKE 'customer_support_agent%'",
    order_by=[
        {"field_name": "creation_timestamp", "ascending": False}
    ],  # Most recent first
    output_format="list",
)
...
for model in all_versions[:2]:  # Compare latest 2 versions
    ...
    for metric_key, metric_value in model.metrics.items():
        if "mean" in metric_key:  # Focus on average scores
...
best_by_guidelines = max(
    all_versions, key=lambda m: m.metrics.get("support_guidelines_mean", 0)
)
print(f"Best version for support guidelines: {best_by_guidelines.name}")
print(f"  Score: {best_by_guidelines.metrics.get('support_guidelines_mean', 0):.3f}")
```
...
print("\nMetric Improvements:")
metric_keys = ["relevance_to_query_mean", "support_guidelines_mean"]
for metric in metric_keys:
    v1_score = v1.metrics.get(metric, 0)
    v2_score = v2.metrics.get(metric, 0)
...
min_relevance_threshold = 0.90
min_guidelines_threshold = 0.85

if (
    v2.metrics.get("relevance_to_query_mean", 0) >= min_relevance_threshold
    and v2.metrics.get("support_guidelines_mean", 0) >= min_guidelines_threshold
    and v2.metrics.get("support_guidelines_mean", 0)
    > v1.metrics.get("support_guidelines_mean", 0)
):
...
- **[Deploy your best version](/genai/prompt-version-mgmt/version-tracking/optionally-package-app-code-and-files-for-databricks-model-serving)** to Databricks Model Serving
