...
Combining [MLflow Prompt Registry](/prompts) with [MLflow LLM Evaluation](/llms/llm-evaluate) enables you to evaluate prompt performance across different models and datasets, and track the evaluation results in a centralized registry. You can also inspect model outputs from the **traces** logged during evaluation to understand how the model responds to different prompts.
