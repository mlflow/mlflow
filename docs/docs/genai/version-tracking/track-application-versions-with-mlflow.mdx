...
- This feature doesn't work in [Databricks Git Folders](https://docs.databricks.com/aws/en/repos/) yet due to limitations in accessing Git metadata.
...
- Creates or reuses a LoggedModel based on your current Git branch, commit, and dirty state
...
# Log params to the active Git-based version
mlflow.log_model_params(model_id=context.active_model.model_id, params=app_params)
```

## Step 4: Test version tracking with code changes
...
- Different uncommitted changes (captured as diffs)
...
traces = mlflow.search_traces(
    filter_string=f"metadata.`mlflow.modelId` = '{context.active_model.model_id}'"
)
print(traces)
```

You can use `get_logged_model()` to get details of the LoggedModel including Git metadata:
...
# Access the parameters if any were logged
if logged_model.params:
    print(f"\n=== Model Parameters ===")
