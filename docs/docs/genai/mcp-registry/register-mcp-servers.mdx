...
The most direct way to register an MCP server is by providing a `server_json` dictionary containing the server's configuration. Use <APILink fn="mlflow.genai.register_mcp_server" />:
...
| `icons` | No | Object with `light` and/or `dark` icon URLs |
...
Tool auto-discovery requires the `mcp` extra: `pip install 'mlflow[mcp]'`. Discovery failure (network issues, auth requirements, timeouts) does **not** abort registration — the version is created with `tools=None`.
