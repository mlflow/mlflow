...
Access endpoints define how agents and applications connect to MCP servers registered in the registry. This guide covers creating endpoints, generating connection instructions, and refreshing tool definitions.
...
for ep in endpoints:
    print(f"  {ep.url} ({ep.transport_type}) → version {ep.resolved_version}")
...
Tool discovery requires the `mcp` extra: `pip install 'mlflow[mcp]'`. Discovered tools are a point-in-time snapshot — they are not automatically kept in sync with the live server.
