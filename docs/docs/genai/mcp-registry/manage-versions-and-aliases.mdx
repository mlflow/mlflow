...
MCP Registry uses semantic versioning and aliases to give you precise control over which server configurations your agents use. This guide covers version management, status transitions, aliases, and tags.

## Semantic Versioning

Every MCP server version is identified by a [semver](https://semver.org/) string (e.g., `1.0.0`, `2.0.0-beta.1`). MLflow parses and stores major, minor, and patch components for correct ordering, so `2.0.0` is always considered newer than `1.9.0`.
