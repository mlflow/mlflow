...
import { Target, Zap, Database, Shield, Settings, TrendingUp, Users, FileText, MessageSquare, Book } from "lucide-react";

# Annotating Ground Truth with Expectations
...
mlflow.log_expectation(
    ...
    source=AssessmentSource(
        source_type=AssessmentSourceType.HUMAN,
        source_id="business_analyst@company.com"
    ),
)
```
...
## How to Add Expectations

### Within the MLflow UI

The MLflow UI provides an intuitive way to add expectations directly to traces. From any trace detail page, click "Add Assessment" and select "Expectation" to define your ground truth.

![Add Expectation](/images/assessments/add_expectation_ui.png)

The UI approach is ideal for domain experts who need to define ground truth without writing code, and for collaborative annotation workflows where multiple stakeholders contribute different perspectives.

### Via API

Use the programmatic API when you need to automate expectation creation, integrate with existing annotation tools, or build custom ground truth collection workflows.

## Step-by-Step Guides

### Annotate Human Ground Truth via UI

Follow these steps to define ground truth expectations using the MLflow UI:

1. **Navigate to your experiment** and select the trace containing the interaction you want to annotate
2. **Click "Add Assessment"** button on the trace detail page
3. **Select "Expectation"** from the Assessment Type dropdown
4. **Enter a descriptive name** (e.g., "expected_answer", "correct_classification", "expected_documents")
5. **Choose the appropriate data type** (String for text, JSON for structured data, Boolean for binary expectations)
6. **Enter the ground truth value** - what the AI should have produced for this input
7. **Add rationale** explaining why this is the correct/expected output
8. **Click "Create"** to record your expectation
...
mlflow.log_expectation(
    ...
    source=AssessmentSource(
        source_type=AssessmentSourceType.HUMAN,
        source_id="endocrinologist@hospital.org"
    ),
)
```

### Consider Multiple Valid Outputs
Some questions have multiple correct answers. Capture this flexibility in your expectations:

```python
mlflow.log_expectation(
    trace_id=trace_id,
    name="acceptable_responses",
    value={
        "valid_answers": [
        ...
        "must_include": ["Amsterdam", "Netherlands", "capital"],
        "must_not_include": ["The Hague"]  # Common misconception
    },
    source=AssessmentSource(
        source_type=AssessmentSourceType.HUMAN,
        source_id="geography_teacher@school.edu"
    ),
)
```
...
<TilesGrid>
  <TileCard
    ...
    description="Complete reference for the expectations API with examples"
