...
This guide walks you through the complete MLflow Assessments API following the natural workflow: from initial setup through advanced evaluation strategies. Each section builds on the previous, showing you how to implement the assessment patterns described in the [main assessments overview](/genai/assessments).
...
### Assessment Types and Sources

MLflow supports two fundamental assessment types that work together to create a complete evaluation framework:

**Expectations** define the ground truth - what your AI should produce for a given input. These always come from human experts who understand the correct answer.

**Feedback** evaluates how well your AI actually performed. This can come from three sources:
...
### Core API: `log_assessment`

The `log_assessment` function is the foundational API that accepts any assessment object. The convenience functions `log_expectation` and `log_feedback` use this internally:

```python
from mlflow.entities import Feedback

# Using log_assessment directly
feedback = Feedback(
    name="faithfulness",
    value=0.9,
...
assessment = mlflow.log_assessment(trace_id="tr-123", assessment=feedback)
```

The `log_assessment` API requires the input of either a `Feedback` or `Expectation` object, making it more verbose and lower-level to use than the direct `log_expectation` and `log_feedback` APIs.
It is recommended to only use `log_assessment` for integration purposes.

## Assessment Logging

This section covers the core workflow of adding assessments to your traces. Start with expectations to establish ground truth, then add feedback from various sources to evaluate performance.
...
Once you have assessments on your traces, you need to retrieve, update, and sometimes delete them. This section covers the core management operations.
...
Retrieve specific assessments to analyze evaluation results:
...
Update existing assessments when you need to correct or refine them:
...
Delete assessments that were logged incorrectly:
...
:::note
If deleting a `Feedback` assessment that has been marked as a replacement to an existing assessment using the `override_feedback` API, the original assessment that was marked as invalid will return to a valid state.
:::
...
The `override_feedback` function allows you to correct automated assessments while preserving the original for audit trails. This is essential for human-in-the-loop evaluation workflows.
...
The process of overriding an existing `Feedback` assessment will mark the original assessment as invalid, but preserve the history of the original assessment.

### Override vs Update

Note the difference between override and update:

- **`override_feedback`**: Creates a new assessment that marks the original as invalid but preserves it for analysis
- **`update_assessment`**: Modifies the existing assessment in-place

Use override when you want to preserve the original automated evaluation for training data or analysis. Use update when you want to simply correct an assessment.


## Best Practices

Following these practices ensures your assessment data is reliable, scalable, and valuable for improving your AI systems.

<Tabs>
  <TabItem value="naming" label="📝 Naming" default>

### Consistent Assessment Names

Use clear, descriptive naming conventions that make assessment data easy to analyze:
...
- [MLflow Tracing Guide](/genai/tracing) - Learn how assessments integrate with trace data
- [Evaluation Workflows](/genai/eval-monitor) - Discover systematic evaluation strategies using assessments