...
graph TD
    subgraph PROMPT[📝 MLflow Prompt Entity]
        ...
        subgraph METADATA[📋 Metadata]
            ...
            VERSION_META[🏷️ Version Metadata]
            TAGS[🔖 Tags]
        end
    ...
    class METADATA,COMMIT,VERSION_META,TAGS metaStyle
...
graph TB
    ...
    subgraph CONTENT[📄 Content Properties]
        ...
        VARIABLES[Variables: Extracted parameters]
...
## Prompt Metadata and Annotations

### Version Metadata

Each prompt version includes comprehensive metadata for tracking and collaboration:
...
graph TB
    ...
    subgraph CONTEXT[🏷️ Contextual Metadata]
        VERSION_META[Version Metadata: Custom key-value pairs]
    ...
    class CONTEXT,VERSION_META,PURPOSE,PERFORMANCE contextStyle
...
<Tabs>
  <TabItem value="version-meta" label="Version Metadata" default>
    ```python
    # Register prompt with comprehensive version metadata
  ...
  <TabItem value="tags" label="Cross-Version Tags">
    ```python
    # Tags apply to all versions of the prompt
    prompt = mlflow.register_prompt(
        name="content-analyzer",
        template=template,
        tags={
...
<Tabs>
  ...
  <TabItem value="search" label="Discovery and Search">
    ...
    prompts = mlflow.search_prompts(
        filter_string="tags.domain='content-analysis' AND tags.language='english'"
    )

    # Load discovered prompt
    prompt = mlflow.load_prompt(f"prompts:/{prompts[0].name}/{prompts[0].version}")
    ```
  </TabItem>
</Tabs>
...
mlflow_prompt = mlflow.load_prompt("prompts:/qa-prompt/1")
...
**Register your first prompt** using <APILink fn="mlflow.register_prompt" /> with comprehensive metadata and clear commit messages. **Establish version workflow** by creating new versions for significant changes and using descriptive commit messages. **Implement alias strategy** by setting up aliases for different environments and use cases. **Integrate with applications** by loading prompts via <APILink fn="mlflow.load_prompt" /> and formatting with dynamic variables.
...
production_prompt = mlflow.load_prompt("prompts:/customer-support@production")
...
**[Prompt Management Guide](/genai/prompt-version-mgmt/)** provides comprehensive guidance on creating, versioning, and managing prompts effectively. **[MLflow UI Navigation](/genai/tracing/observe-with-traces/ui)** helps you master the interface for exploring and managing prompts.
