...
When migrating to a new language model, you often discover that your carefully crafted prompts don't work as well with the new model. MLflow's <APILink fn="mlflow.genai.optimize_prompts" /> API helps you **automatically adapt prompts** to maintain output quality when switching models, using your existing application's outputs as training data.
...
- **Provider Switching**: Migrate between providers (OpenAI ↔ Anthropic ↔ others)
...
### Step 3: Test Target Model (Optional)

Verify that the target model needs optimization:
...
# Test target model with same inputs
with mlflow.start_run() as run:
    for record in inputs:
        predict_fn(**record["inputs"])

# Review traces to see format differences
...
### Scenario 1: Cost Reduction (GPT-4o → GPT-4o-mini)

```python
# Collect outputs from expensive model
@mlflow.trace
def expensive_model(text: str) -> str:
    return openai_call(model="gpt-4o", prompt=prompt.format(text=text))


# Optimize for cheaper model
result = mlflow.genai.optimize_prompts(
    predict_fn=lambda text: openai_call(
        model="gpt-4o-mini", prompt=prompt.format(text=text)
    ),
    train_data=dataset,
    target_prompt_uris=[prompt.uri],
    optimizer_lm_params=LLMParams(model_name="openai:/gpt-4o-mini"),
...
### Scenario 2: Provider Switch (OpenAI → Anthropic)

```python
# Source: OpenAI GPT-4
@mlflow.trace
def openai_predict(question: str) -> str:
    return openai_call(model="gpt-4", prompt=prompt.format(question=question))


# Target: Anthropic Claude
@mlflow.trace
def anthropic_predict(question: str) -> str:
    return anthropic_call(
        model="claude-3-opus", prompt=prompt.format(question=question)
    )


# Optimize for Claude
result = mlflow.genai.optimize_prompts(
    predict_fn=anthropic_predict,
    train_data=dataset_from_gpt4_traces,
    target_prompt_uris=[prompt.uri],
    optimizer_lm_params=LLMParams(model_name="anthropic:/claude-3-opus"),
)
```

### Scenario 3: Speed Optimization (Large → Small Model)

```python
# Optimize for faster model while maintaining quality
result = mlflow.genai.optimize_prompts(
    predict_fn=fast_model_predict,
    train_data=slow_model_outputs,
    target_prompt_uris=[prompt.uri],
    optimizer_lm_params=LLMParams(model_name="openai:/gpt-4o-mini"),
)
```
...
Always test optimized prompts before production deployment:

```python
# Evaluate on held-out test set
test_inputs = [...]  # New inputs not used for optimization

for test_input in test_inputs:
    result = predict_fn(**test_input["inputs"])
    print(f"Expected: {test_input['expected']}, Got: {result}")
```
