...
Any registered prompt can be loaded directly into the
[**LLM Playground**](/genai/prompt-registry/playground) for interactive testing against an
[AI Gateway](/genai/governance/ai-gateway) endpoint. Chat-typed prompts preserve
their full message structure, and any stored **model config** is applied
automatically.
