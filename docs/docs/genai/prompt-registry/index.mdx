...
anthropic_config = PromptModelConfig(
...
### Model Config with Chat Prompts

Model configuration works seamlessly with both text and chat prompts:
...
# Use the prompt with its configured model
prompt = mlflow.genai.load_prompt("chat-assistant")
formatted_messages = prompt.format(style="helpful", question="What is MLflow?")

# Access the model config when making LLM calls
import openai
...
### Combining with Response Format

Model configuration can be combined with response format specifications for complete prompt management:
...
prompt = mlflow.genai.load_prompt("structured-analysis")
# Access both model config and response format
print(f"Model: {prompt.model_config['model_name']}")
print(f"Schema: {prompt.response_format}")
```

### Version-Specific Configuration

Model configuration is version-specific, allowing you to evolve both your prompt template and model settings together:

```python
# Version 1: Conservative settings for production
mlflow.genai.register_prompt(
    name="content-generator",
    template="Generate content about: {{topic}}",
    model_config={"model_name": "gpt-4", "temperature": 0.5},
    commit_message="Conservative settings for production",
)

# Version 2: More creative settings
mlflow.genai.register_prompt(
    name="content-generator",
    template="Generate creative content about: {{topic}}",
    model_config={"model_name": "gpt-4-turbo", "temperature": 0.9},
    commit_message="Higher temperature for more creativity",
)

# Version 3: Different model entirely
mlflow.genai.register_prompt(
    name="content-generator",
    template="Generate creative content about: {{topic}}",
    model_config={"model_name": "claude-3-opus", "temperature": 0.8},
    commit_message="Switching to Claude",
)

# Each version maintains its own configuration
v1 = mlflow.genai.load_prompt("content-generator", version=1)
v2 = mlflow.genai.load_prompt("content-generator", version=2)
v3 = mlflow.genai.load_prompt("content-generator", version=3)

print(f"V1: {v1.model_config['model_name']} @ {v1.model_config['temperature']}")
print(f"V2: {v2.model_config['model_name']} @ {v2.model_config['temperature']}")
print(f"V3: {v3.model_config['model_name']} @ {v3.model_config['temperature']}")
```

### Best Practices

1. **Version Together**: When you change your prompt template in a way that requires different model parameters, create a new version with updated model config.

2. **Use Validation**: Prefer `PromptModelConfig` over plain dictionaries to catch configuration errors early during development.

3. **Document Decisions**: Use commit messages to explain why specific model settings were chosen for a particular prompt version.

4. **Start Conservative**: Begin with conservative settings (lower temperature, reasonable token limits) and iterate based on evaluation results.

5. **Provider Consistency**: Keep provider-specific parameters in `extra_params` to maintain clarity about which settings are standard vs. provider-specific.
