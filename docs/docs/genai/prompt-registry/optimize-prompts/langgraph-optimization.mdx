...
Here's a complete example of optimizing a customer support agent built with LangGraph. The example shows how easy it is to optimize prompts in a LangGraph workflow with minimal changes.
...
from angchain_core.messages import SystemMessage, HumanMessage
