...
## Automatic Prompt Linking with MLflow Tracing

When you use prompts from the registry within traced functions or runs, MLflow automatically tracks which prompts are being used and creates a linkage that is visible in the MLflow UI. This provides immediate visibility into prompt usage across your experiments without requiring explicit model logging.

### How It Works

When you load a prompt using `mlflow.genai.load_prompt()` inside a function decorated with `@mlflow.trace` or within an active MLflow run, the prompt usage is automatically recorded. The linked experiments will then appear on the prompt's detail page in the UI, allowing you to see which experiments are using which prompt versions.

### Example: Linking Prompts in Traced Functions
...
@mlflow.trace
def question(name):
    # Load the prompt - this creates an automatic link
...
# Use the traced function within a run
...
1. The `question` function is decorated with `@mlflow.trace`, enabling tracing for that function
2. Inside the function, `mlflow.genai.load_prompt()` loads a prompt from the registry
3. MLflow automatically detects this prompt usage and creates a link between the experiment/run and the prompt
4. In the MLflow UI, when you navigate to the prompt's detail page, you'll see which experiments and traces have used this prompt

### Benefits of Automatic Linking

- **Zero-effort tracking**: No need to manually log prompt dependencies - they're tracked automatically
- **Experiment visibility**: Easily see which experiments are using which prompts from the prompt detail page
- **Version awareness**: Track usage of specific prompt versions or aliases across different experiments
- **Debugging and analysis**: Quickly identify all places where a particular prompt is being used in your experimentation workflow
