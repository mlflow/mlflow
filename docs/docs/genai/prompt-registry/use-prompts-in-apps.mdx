...
## Automatic Prompt Linking with Trace

When you load prompts from the registry within traced functions (annotated with `@mlflow.trace`), MLflow automatically tracks which prompts are being used and creates a linkage with the active trace that is visible in the MLflow UI. This provides immediate visibility into prompt usage and enables you to compare your agent's behavior across different prompt versions.

### How It Works

When you load a prompt using `mlflow.genai.load_prompt()` inside an active trace, the prompt usage is automatically recorded. The linked traces will then appear on the prompt's detail page.
...
![UI for Prompt-trace linkage](/images/llms/prompt-registry/prompt-trace-linkage)
