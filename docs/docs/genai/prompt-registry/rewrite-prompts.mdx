...
- **Downgrading Models**: Moving from `gpt-5` → `gpt-5-mini` to reduce costs
...
Here's a complete example of migrating from `gpt-5` to `gpt-5-mini` while maintaining output consistency:
...
- `outputs`: The actual outputs from your source model (`gpt-5`)
...
The optimized prompt will include additional instructions to help `gpt-5-mini` match the behavior of `gpt-5`:
