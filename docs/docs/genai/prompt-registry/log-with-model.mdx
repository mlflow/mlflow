...
:::warning
    ...
    The `prompts` parameter for associating prompts with models is only supported for LLM and AI agent flavors such as OpenAI, LangChain, LlamaIndex, DSPy, etc. Please refer to the [supported flavors](/genai/flavors) for the full list.
