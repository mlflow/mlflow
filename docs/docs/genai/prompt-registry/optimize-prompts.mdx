...
MLflow's [prompt optimization](https://mlflow.org/prompt-optimization) lets you systematically enhance your AI applications with minimal code changes. Whether you're building with LangChain, OpenAI Agent, CrewAI, or your own custom implementation, MLflow provides a universal path from initial prototyping to steady improvement.
...
**Best For:** Tasks where you want quick improvements based on [prompt engineering](https://mlflow.org/prompt-optimization) best practices, or when you have limited training data but want to leverage it for targeted improvements.
...
Provide a dataset with `inputs` and `expectations`. Both columns should have dictionary values. `inputs` values will be passed to the predict function as keyword arguments. Please refer to [Built-in Judges](/genai/eval-monitor/scorers/llm-judge/predefined/) for the expected format of each built in scorers.
