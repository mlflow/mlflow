...
The system supports multiple feedback collection approaches to accommodate different workflows and user types. **In-application UI** enables thumbs up/down or free-text feedback captured via the `log_feedback` API, making it easy for users to provide quick quality assessments without leaving your application. **MLflow Trace UI** allows direct annotation of traces during development, perfect for developers who want to mark specific interactions for further analysis. **Review App** provides a built-in chat UI for Databricks deployments with automatic feedback integration, streamlining the review process for stakeholders.
...
During development, you often encounter situations where you need authoritative input on correct answers for queries outside your expertise, validation of whether negative user feedback is justified, priority assessment for different types of issues, and business stakeholder input on acceptable response quality. Domain experts need an accessible way to provide structured feedback without requiring technical complexity or deep familiarity with your development tools.
...
**Review App Features** include a user-friendly interface that domain experts can use without technical knowledge, custom questions tailored to your specific review needs, automatic integration with original traces for unified feedback view, and structured collection of expert assessments and recommendations.

The workflow begins with developers selecting specific traces that need expert validation, often focusing on cases where user feedback was negative or where the correct response is unclear. **Custom questions** can be configured to gather specific insights relevant to your domain, whether that's medical accuracy, legal compliance, customer service appropriateness, or technical correctness.

**Expert Integration** happens seamlessly through the Review App's intuitive interface, where domain experts can review the original user query, see the application's response, and provide structured feedback without needing to understand the underlying technical implementation. This feedback automatically enriches the original traces, creating a comprehensive record that combines technical execution details with authoritative quality assessment.
