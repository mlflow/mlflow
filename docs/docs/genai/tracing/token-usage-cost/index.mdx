...
:::note
Cost tracking requires [LiteLLM](https://www.litellm.ai/) to be installed. Install it with:

```bash
pip install litellm
```

Cost is calculated based on token usage and [model pricing data from LiteLLM](https://models.litellm.ai/). Cost may not be available for all models, particularly custom or self-hosted models.
:::
...
# Access token usage for each LLM call
print("== Token usage for each LLM call: ==")
...
# Access cost for each LLM call
print("\n== Cost for each LLM call: ==")
for span in trace.data.spans:
    if cost := span.llm_cost:
        print(f"{span.name}:")
...
The `token_usage` field returns a dictionary with the following keys:
