...
The `[genai]` extra enables other powerful features such as <ins>[AI Gateway](/genai/gateway)</ins>, <ins>[Online Evaluation](/genai/eval-monitor/scoring/online)</ins>, and <ins>[Prompt Optimization](/genai/prompts/optimize-prompt)</ins>. It is recommended to start your MLflow server with the extra installed, even if you don't need cost tracking.

:::

:::info

When using Databricks managed MLflow, the cost computation requires the client application to install <ins>[LiteLLM](https://www.litellm.ai/)</ins> or <ins>[manually set the cost](../#manually-setting-token-and-cost-information) attributes on spans</ins>. This is not required for self-hosted MLflow.
