...
MLflow also provides a comprehensive command-line interface for trace management. The `mlflow traces` CLI offers the same search capabilities as the Python API, along with additional operations for trace management, tagging, and assessment handling.

### Available CLI Commands

```bash
# Search traces with filtering and field selection
mlflow traces search --experiment-ids 1,2,3 --max-results 50

# Filter traces with advanced queries
mlflow traces search --experiment-ids 1 \
    --filter-string "status = 'OK' AND execution_time_ms > 1000"
...
# Delete traces by timestamp or ID
...
mlflow traces update-assessment --trace-id tr-abc123 --assessment-id asmt-def456 \
    --value 0.95 --rationale "Updated after review"
mlflow traces delete-assessment --trace-id tr-abc123 --assessment-id asmt-def456
```

### CLI Field Selection

The CLI supports the same JSONPath-like field selection as the Python API:

```bash
# Basic field selection
mlflow traces search --experiment-ids 1 \
    --fields "info.trace_id,info.state,info.execution_duration_ms"

# Wildcard expansion for assessments
mlflow traces search --experiment-ids 1 \
    --fields "info.assessments.*" --output json

# Span data extraction
mlflow traces search --experiment-ids 1 \
    --fields "data.spans.*.name,data.spans.*.attributes.mlflow.spanType"

# Complex queries with multiple wildcards
mlflow traces search --experiment-ids 1 \
    --fields "info.trace_id,info.assessments.*.feedback.value,data.spans.*.name"
```

### CLI Output Formats

The CLI supports both table and JSON output formats:

- **Table format** (default): Human-readable tabular output for quick inspection
- **JSON format**: Machine-readable JSON for scripting and data processing

```bash
# Table output (default) - great for quick inspection
mlflow traces search --experiment-ids 1 --max-results 10

# JSON output - perfect for scripting and automation
mlflow traces search --experiment-ids 1 --output json | jq '.traces[0].info.trace_id'
```

### CLI Performance Tips

1. **Use specific experiment IDs** to limit search scope
2. **Apply filters** to reduce result set size
3. **Select specific fields** when you don't need full trace data
...
The CLI provides the same powerful search capabilities as the Python API while offering convenient command-line access for automation, scripting, and interactive trace management.
