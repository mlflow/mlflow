...
<DatabricksCallout docsPage="/mlflow3/genai/tracing/search-traces" />
...
<Tabs>
<TabItem value="ui" label="UI Search" default>

## Filtering Traces in the UI

Use the search box in the MLflow Trace UI to filter traces by various criteria using the same syntax described in the other tabs.
...
<TabItem value="syntax" label="Search Syntax">
...
<TabItem value="examples" label="Query Examples">
...
traces_df = mlflow.search_traces(experiment_ids=["1"], filter_string="status = 'OK'")

# Return as list of Trace objects
traces_list = mlflow.search_traces(
    experiment_ids=["1"], filter_string="status = 'OK'", return_type="list"
)
```
...
The DataFrame format depends on your MLflow version:
...
traces = mlflow.search_traces(experiment_ids=["1"], order_by=["timestamp_ms DESC"])

# Multiple ordering criteria
traces = mlflow.search_traces(
    experiment_ids=["1"], order_by=["timestamp_ms DESC", "status ASC"]
)
```
...
### Using the Client API for Pagination
...
results = mlflow.evaluate(
    model=your_model, data=eval_dataset, targets="ground_truth", evaluators=["default"]
)
```
...
for env in environments:
    total = mlflow.search_traces(
        experiment_ids=["1"], filter_string=f"tags.environment = '{env}'"
    )
...
<TabItem value="setup" label="Setup Examples">
...
- **Local File Store**: Full functionality, may be slower with large datasets
