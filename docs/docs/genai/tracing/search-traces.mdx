...
await client.searchTraces({
  experimentIds: ['1'],
  filter: "trace.text LIKE '%authentication error%'"
});

// Search for multiple terms
await client.searchTraces({
  experimentIds: ['1'],
  filter: "trace.text LIKE '%timeout%'"
});
```
...
await client.searchTraces({
  experimentIds: ['1'],
  filter: "trace.name = 'predict'"
});

// Pattern matching with LIKE
await client.searchTraces({
  experimentIds: ['1'],
  filter: "trace.name LIKE '%inference%'"
});

// Case-insensitive pattern matching with ILIKE
await client.searchTraces({
  experimentIds: ['1'],
  filter: "trace.name ILIKE '%PREDICT%'"
});
```
...
await client.searchTraces({
  experimentIds: ['1'],
  filter: "trace.execution_time_ms > 1000"
});

// Performance range
await client.searchTraces({
  experimentIds: ['1'],
  filter: "trace.execution_time_ms >= 200 AND trace.execution_time_ms <= 800"
});
```
...
await client.searchTraces({
  experimentIds: ['1'],
  filter: `trace.timestamp_ms > ${timestamp - 3600000}`
});

// Timestamp range
await client.searchTraces({
  experimentIds: ['1'],
  filter: `trace.timestamp_ms >= ${timestamp - 7200000} AND trace.timestamp_ms <= ${timestamp - 3600000}`
});
```
...
await client.searchTraces({
  experimentIds: ['1'],
  filter: "tag.model_name = 'gpt-4'"
});

// Pattern matching with LIKE (case-sensitive)
await client.searchTraces({
  experimentIds: ['1'],
  filter: "tag.model_name LIKE 'gpt-%'"
});

// Case-insensitive pattern matching with ILIKE
await client.searchTraces({
  experimentIds: ['1'],
  filter: "tag.environment ILIKE '%prod%'"
});
```
...
await client.searchTraces({
  experimentIds: ['1'],
  filter: "trace.run_id = 'run_id_123456'"
});
```
...
await client.searchTraces({
  experimentIds: ['1'],
  filter: 'prompt = "qa-agent-system-prompt/4"'
});
```
...
await client.searchTraces({
  experimentIds: ['1'],
  filter: "span.name = 'llm_call'"
});

// Pattern matching on span name
await client.searchTraces({
  experimentIds: ['1'],
  filter: "span.name LIKE '%embedding%'"
});

// Filter by span type
await client.searchTraces({
  experimentIds: ['1'],
  filter: "span.type = 'LLM'"
});
```
...
await client.searchTraces({
  experimentIds: ['1'],
  filter: "feedback.user_comment LIKE '%helpful%'"
});
```
...
await client.searchTraces({
  experimentIds: ['1'],
  filter: "expectation.accuracy = 'high'"
});

// Pattern matching on expectations
await client.searchTraces({
  experimentIds: ['1'],
  filter: "expectation.label ILIKE '%success%'"
});
```
...
await client.searchTraces({
  experimentIds: ['1'],
  filter: `trace.end_time_ms > ${endTime - 3600000}`
});

// Find traces that ended within a specific time range
await client.searchTraces({
  experimentIds: ['1'],
  filter: `trace.end_time_ms >= ${endTime - 7200000} AND trace.end_time_ms <= ${endTime - 3600000}`
});
```
...
await client.searchTraces({
  experimentIds: ['1'],
  filter: "trace.status = 'OK' AND tag.importance = 'high'"
});

// Production error analysis with execution time
await client.searchTraces({
  experimentIds: ['1'],
  filter: "tag.environment = 'production' AND trace.status = 'ERROR' AND trace.execution_time_ms > 500"
});

// Advanced query with span attributes and feedback
await client.searchTraces({
  experimentIds: ['1'],
  filter: "span.name LIKE '%llm%' AND feedback.rating = 'positive' AND trace.execution_time_ms < 1000"
});

// Search with pattern matching and time range
await client.searchTraces({
  experimentIds: ['1'],
  filter: "trace.name ILIKE '%inference%' AND trace.timestamp_ms > 1700000000000"
});
```
