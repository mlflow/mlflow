...
| **Linked Prompts**   | `prompts`                                                            | `=` (format: `"name/version"`)      | prompts = "qa-system-prompt/4"            |
...
mlflow.search_traces(filter_string='prompts = "qa-agent-system-prompt/4"')
```

:::note
The `prompts` filter only supports exact match (`=`) operator with the format `"name/version"`.
:::
