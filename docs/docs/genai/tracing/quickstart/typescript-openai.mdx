...
MLflow's official TypeScript SDK only contains tracing functionality. If you want to use the full experiment tracking experience, you can combine it with <ins>[mlflow.js](https://github.com/open-source-labs/mlflow-js)</ins>, a **community-maintained** package that provides experiment tracking APIs compatible with Python SDK, built on top of MLflow's REST APIs.
...
import { init } from "mlflow-tracing";
    ...
    If [authentication](/self-hosting/security/basic-http-auth) is enabled on the tracking server, you can pass the credential through `trackingServerUsername` and `trackingServerPassword`.
