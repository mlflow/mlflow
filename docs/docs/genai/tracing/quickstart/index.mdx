...
Once you add session IDs to your traces, you can filter and group them in the MLflow UI using the search query `metadata.`mlflow.trace.session` = 'session-123'`.

For more details on tracking users and sessions, including web application examples and programmatic analysis, see the [Track Users & Sessions guide](/genai/tracing/track-users-sessions).
