...
If you already have an existing `mlruns/` directory from older MLflow experiments, the MLflow UI may continue using the file-store backend while GenAI traces are written to SQLite.

To view new traces correctly, start the UI with:

```bash
mlflow ui --backend-store-uri sqlite:///mlflow.db
```

You may also consider migrating from file-store storage to a SQL backend.
