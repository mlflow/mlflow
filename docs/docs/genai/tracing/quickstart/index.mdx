...
:::tip[Set up Tracing with the MLflow CLI]
...
**Your agent code runs locally.** The Python file you write, including your prompt
templates, lives and executes on your machine. `mlflow.set_tracking_uri(...)` does not
move or run your code. It sets where the MLflow client sends the trace it records. To
change agent behavior (prompt wording, model, logic), edit your local files and re-run
the script. The UI shows the recorded trace. It is not where you edit the agent.

The trace holds more than timing. Tracing records the inputs and outputs of the calls it
wraps, so your prompt text and the model's responses are uploaded to the tracking server
and shown in the MLflow UI. Treat the tracking server as the destination for that
content, and do not trace data you cannot send there.
...
:::note[On Databricks: trace storage limit]

When using Databricks as the tracking server, MLflow stores traces in the experiment
backend by default, which caps at 100,000 traces. For production workloads with no cap,
bind the experiment to a Unity Catalog location:
...
See the [Databricks UC trace storage guide](https://docs.databricks.com/aws/en/mlflow3/genai/tracing/trace-unity-catalog) for details.
