...
**Your agent code runs locally.** The Python file you write, including any prompts,
lives on your machine. `mlflow.set_tracking_uri(...)` controls only *where trace data
is sent*. It does not move your code. When a trace appears in the MLflow UI, you are
viewing telemetry uploaded from your local process.

To change agent behavior (prompt wording, model, logic), edit your local files and
re-run the script.
