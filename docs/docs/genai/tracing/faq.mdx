...
### Q: Do I need to explicitly enable MLflow Tracing?

No. MLflow Tracing does **not** require an explicit `enable()` call to start recording traces.

Tracing starts automatically when you:
- Use an MLflow auto-tracing integration (for example, `mlflow.openai.autolog()`), or
- Decorate a function with `@mlflow.trace`

When the first traced operation runs, MLflow automatically creates a trace and records spans under it.
...
### Q: Does the MLflow TypeScript SDK work in frontend (browser) environments?
