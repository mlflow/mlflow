...
### Viewing Trace Archival Status

When trace archival is enabled for an experiment, MLflow shows an `Archive after: ...` badge on
trace-focused experiment views so you can see the effective retention at a glance.

Archived traces remain accessible, but searches that depend on span payloads stop working after
those payloads move out of the tracking store. See
[Archive Traces](/genai/tracing/observe-with-traces/archive-traces) for the retention model,
scheduler behavior, and setup options.
