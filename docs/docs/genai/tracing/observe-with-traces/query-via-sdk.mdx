...
:::important
The `search_traces` API returns a pandas DataFrame with trace data that you can analyze using standard pandas operations.

**Syntax Differences**: The column names used in `filter_string` differ from those in the returned DataFrame:
...
print(f"Average execution time: {error_traces['execution_duration'].mean():.1f}ms")
```
