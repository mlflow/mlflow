...
def analyze_recent_errors(experiment_ids: list, hours_back: int = 24):
            ...
            if span.status.status_code == "ERROR":
                ...
                for event in span.events:
                    if "exception" in event.name.lower():
                        ...
                        error_message = event.attributes.get("exception.message", "No details")
                        break
    ...
    # Report findings
    print("\nError Patterns:")
    for pattern, details in sorted(error_patterns.items(), key=lambda x: x[1]["count"], reverse=True):
...
```python
def analyze_performance_bottlenecks(experiment_ids: list, percentile_threshold: float = 95):
    ...
    for _, trace_row in slow_traces.head(10).iterrows():  # Analyze top 10 slow traces
        ...
        for span in trace.data.spans:
            duration = (span.end_time_ns - span.start_time_ns) / 1_000_000  # Convert to ms
    ...
    for span_name, durations in sorted(span_durations.items(), 
...
```python
def analyze_user_session(experiment_ids: list, user_id: str = None, session_id: str = None):
    ...
    print(f"Time range: {session_traces['timestamp_ms'].min()} to {session_traces['timestamp_ms'].max()}")
    ...
    for _, trace in session_traces.iterrows():
        ...
        print(f"  {status_icon} {trace['timestamp_ms']}: {trace['execution_time_ms']:.1f}ms")
        if hasattr(trace, 'request') and trace['request']:
            request_preview = str(trace['request'])[:50] + "..." if len(str(trace['request'])) > 50 else str(trace['request'])
...
def collect_trace_metrics(experiment_ids: list, time_window_minutes: int = 60) -> TraceMetrics:
    ...
    avg_time = success_traces['execution_time_ms'].mean() if not success_traces.empty else 0
    p95_time = success_traces['execution_time_ms'].quantile(0.95) if not success_traces.empty else 0
...
```python
def create_evaluation_dataset(experiment_ids: list, task_type: str, sample_size: int = 1000):
    ...
    if task_type == "chat":
        eval_dataset = dataset_df.rename(columns={
        ...
        eval_dataset['context'] = dataset_df['retrieval.outputs'].apply(
            lambda x: [doc.get('page_content', '') for doc in x] if isinstance(x, list) else []
        )
