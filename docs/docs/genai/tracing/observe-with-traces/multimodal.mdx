...
{/* TODO: add screenshot of multimodal content in the trace viewer */}
...
MLflow provides Pydantic models for constructing multimodal chat messages programmatically:

- <APILink fn="mlflow.types.chat.ChatMessage" /> — a chat message with text or multimodal content
- <APILink fn="mlflow.types.chat.TextContentPart" /> — a text content part (`type: "text"`)
- <APILink fn="mlflow.types.chat.ImageContentPart" /> — an image content part (`type: "image_url"`)
- <APILink fn="mlflow.types.chat.AudioContentPart" /> — an audio content part (`type: "input_audio"`)
- <APILink fn="mlflow.types.chat.ImageUrl" /> — image URL with optional `detail` parameter
- <APILink fn="mlflow.types.chat.InputAudio" /> — base64-encoded audio data with format
...
- **Gemini multimodal** is not yet captured — only text content is traced for Google Gemini models
