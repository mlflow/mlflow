...
After extraction, the base64 data in the span is replaced with a lightweight `mlflow-attachment://` reference URI. The MLflow UI resolves these URIs and renders the content inline.
...
MLflow stores binary content as separate artifact files alongside trace data, keeping the trace JSON lightweight while supporting rich media. There are two ways attachments are created:
...
2. **Explicit `Attachment` objects** -- for binary content that is not base64-encoded in a recognized pattern (e.g., raw bytes from an API, file uploads, PDFs).
