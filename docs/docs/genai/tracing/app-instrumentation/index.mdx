...
<Tabs>
  ...
  <TabItem value="databricks" label="Databricks">
    ...
    If you're working on tracing with Databricks, configure your authentication with Databricks CLI or the following environment variables:
    ...
    Then set the tracking URI and the target experiment to store the traces in your application.
...
4. When using self-hosted MLflow, make sure to use the **SQL Backend** with scalable database like PostgreSQL. The default file-based backend has scalability limitation and is not recommended for production use.
...
Multi-threading is a common strategy for parallelizing IO-bound operations in your application. MLflow Tracing supports [multi-threaded applications](/genai/tracing/app-instrumentation/manual-tracing#multi-threading) using context propagation.
