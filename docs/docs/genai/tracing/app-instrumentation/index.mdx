...
<DatabricksCallout docsPath="/mlflow3/genai/tracing/app-instrumentation" />
...
<Tabs>
  ...
  <TabItem value="local" label="Local MLflow">
...
**[Automatic Tracing](#automatic-tracing)**: Just add 1 line of code `mlflow.<library>.autolog()` to automatically capture your app's logic. Automatic tracing works with 20+ [supported libraries and frameworks](/genai/tracing/integrations/index) out of the box

**[Manual Tracing](#manual-tracing)**: Designed for custom logic and complex workflows, manual tracing gives you full control over what gets traced and how using [high-level APIs (decorators and fluent context managers)](manual-tracing/fluent-apis) or [low-level APIs](/mlflow3/genai/tracing/app-instrumentation/manual-tracing/log-level-api).
...
Choose the appropriate tracing approach based on your application architecture:

<Tabs>
  <TabItem value="single-library" label="Single GenAI Library" default>
    **Using a single GenAI Authoring Library** (e.g., LangGraph, CrewAI, OpenAI Agents, Bedrock Agents, etc)
    
    Use **automatic tracing** by adding a one-line integration from our [integrations page](/genai/tracing/integrations/index) for your selected library: `mlflow.<library>.autolog()`
    
    This provides complete observability with minimal code changes and is perfect for getting started quickly with comprehensive tracing.
  </TabItem>
  <TabItem value="llm-sdks" label="LLM Provider SDKs">
    **Using an LLM provider's SDKs Directly** (e.g., OpenAI SDK, Anthropic SDK, Bedrock SDK, etc)
    
    Enable [automatic tracing](/genai/tracing/app-instrumentation/automatic) for the API library
    
    Add manual tracing decorators to [combine](/genai/tracing/app-instrumentation/manual-tracing/fluent-apis#combining-with-auto-tracing) multiple LLM calls into a single trace
  </TabItem>
  <TabItem value="multiple-frameworks" label="Multiple Frameworks">
    **Using Multiple Authoring Frameworks or combining an authoring framework with a LLM provider's SDK** (e.g., LangGraph AND OpenAI SDK, etc)
    
    Enable [automatic tracing](/genai/tracing/app-instrumentation/automatic) for each framework / SDK
    
    Add manual tracing decorators to [combine](/genai/tracing/app-instrumentation/manual-tracing/fluent-apis#combining-with-auto-tracing) calls to multiple frameworks or SDKs into a single trace
  </TabItem>
  <TabItem value="custom-control" label="Custom Control">
    **All other approaches or you have a need for fine-grained control**
    
    Use [**Manual Tracing**](/genai/tracing/app-instrumentation/manual-tracing/index)
    
    Start with the [high-level APIs](/genai/tracing/app-instrumentation/manual-tracing/fluent-apis) (`@mlflow.trace` decorator and fluent context managers) which provide a balance of control and ease of use
    
    Use the [low-level APIs](/genai/tracing/app-instrumentation/manual-tracing/log-level-api) only if the high-level APIs don't give you enough control
  </TabItem>
</Tabs>

Generally, we recommend starting with [automatic tracing](/genai/tracing/app-instrumentation/automatic) and only moving to [manual tracing](/genai/tracing/app-instrumentation/manual-tracing/index) if your application's logic is not accurately captured or you need more control.
...
MLflow supports automatic tracing for 20+ popular GenAI libraries including LLM providers (OpenAI, Anthropic, Google Gemini, AWS Bedrock), frameworks (LangChain, LangGraph, LlamaIndex, DSPy), and tools (LiteLLM, Instructor, CrewAI). See the complete list at [Automatic Tracing Integrations](/genai/tracing/integrations/index).
...
<Tabs>
  ...
  <TabItem value="combining" label="Combining with Auto Tracing">
    ...
    def rag_pipeline(question: str) -> str:
        ...
        with mlflow.start_span(name="Document Retrieval", span_type=SpanType.RETRIEVER) as span:
...
Learn more about manual tracing techniques, decorators, context managers, and advanced patterns in the [Manual Tracing Guide](/tracing/app-instrumentation/manual-tracing).
...
<Tabs>
  <TabItem value="mlflow-ui" label="MLflow UI" default>
    ...
    **Search**: Using the search bar in the UI, you can easily filter your traces based on name, tags, or other metadata. Check out the [search docs](/tracing/api/search/) for details about the query string format.
  ...
  <TabItem value="jupyter" label="Jupyter Notebooks">
    ...
    This feature requires using an [MLflow Tracking Server](/tracking/server/), as this is where the UI assets are fetched from. To get started, simply ensure that the MLflow Tracking URI is set to your tracking server (e.g. `mlflow.set_tracking_uri("http://localhost:5000")`).

    By default, the trace UI will automatically be displayed for the following events:

    **When the cell code generates a trace** (e.g. via [automatic tracing](/tracing/#automatic-tracing), or by running a manually traced function)
...
**Naming Convention**: Use clear, descriptive names for your spans like "PDF Document Parser" or "Customer Intent Classification" rather than generic names like "Process" or "Function".

**Useful Attributes**: Add meaningful attributes to capture input characteristics, processing parameters, and output metrics. This metadata is valuable for debugging and analysis.

**Error Handling**: Properly handle and trace errors by setting error-related attributes when exceptions occur.

**Performance Considerations**: For production environments, configure sampling rates and trace size limits to manage performance impact.

## Troubleshooting

<Tabs>
  <TabItem value="missing-traces" label="Missing Traces" default>
    **Problem**: Traces are not appearing in the UI

    **Solutions**: 
    
    Verify MLflow tracking URI is set correctly with `print(mlflow.get_tracking_uri())`
    
    Check if tracing is enabled with `print(mlflow.tracing.is_tracing_enabled())`
    
    Ensure autolog is called before library usage
    
    Verify experiment exists and is accessible
  </TabItem>
  <TabItem value="performance-issues" label="Performance Issues">
    **Problem**: Tracing is slowing down your application

    **Solutions**: 
    
    Reduce sampling rate with `mlflow.tracing.set_global_config(sampling_rate=0.1)`
    
    Limit trace size with size restrictions in the global config
    
    Use async tracing for high-throughput applications
  </TabItem>
  <TabItem value="large-inputs" label="Large Input/Output Data">
    **Problem**: Traces contain too much data or fail due to size limits

    **Solutions**: 
    
    Configure automatic truncation in global settings
    
    Manually truncate sensitive or large data before logging
    
    Use references instead of full data content
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem value="learn-more" label="Learn More" default>
    ...
    **[Manual Tracing](/genai/tracing/app-instrumentation/manual-tracing/index)**: Learn advanced techniques for custom instrumentation and complex workflows
  ...
  <TabItem value="integrations" label="Integration Examples">
    ...
    **[OpenAI Integration](/genai/tracing/integrations/openai)**: Detailed OpenAI tracing examples and best practices

    **[LangChain Integration](/genai/tracing/integrations/langchain)**: Comprehensive LangChain application tracing

    **[LlamaIndex Integration](/genai/tracing/integrations/llamaindex)**: Trace LlamaIndex workflows and components

    **[All Integrations](/genai/tracing/integrations/index)**: Browse all supported libraries and frameworks
  </TabItem>
  <TabItem value="working-with-traces" label="Working with Traces">
    ...
    **[Search and Filter](/genai/tracing/api/search)**: Advanced search techniques and filtering options

    **[Production Monitoring](/genai/tracing/production)**: Scale tracing for production workloads
  </TabItem>
</Tabs>
