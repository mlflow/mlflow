...
:::note
Decorator requires TypeScript version 5.0+.
:::
...
class MyClass {
    ...
    summarizeDocument(documentContent: string, userInstructions: string) {
        // Construct a custom preview for the request column
        // For example, show beginning of document and user instructions
        const requestP = `Doc: ${documentContent.slice(0, 30)}... Instr: ${userInstructions.slice(0, 30)}...`;
        mlflow.updateCurrentTrace({ requestPreview: requestP });
        ...
        // Customize the response preview
        const responseP = `Summary: ${summary.slice(0, 50)}...`;
        mlflow.updateCurrentTrace({ responsePreview: responseP });
...
const result = await mlflow.withSpan(
    ...
    // Pass name, span type, and inputs as options.
    {
        ...
        inputs: { prompt: "What's the weather like in Seattle?" },
    }
);
```
...
The below documentation applies only to the Python SDK.
:::
