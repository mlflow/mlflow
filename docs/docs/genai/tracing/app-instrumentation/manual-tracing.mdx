...
See [Multimodal Content and Attachments in Traces](/genai/tracing/observe-with-traces/multimodal#manual-tracing) for more examples.
:::
...
[Span Links](/genai/concepts/span#span-links) connect related spans — within the same trace or across different traces — without establishing a parent-child relationship. This is useful for tracking causal relationships in multi-agent systems, async workflows, and retry chains.

### Attaching Links at Span Creation

You can pass links when creating a span using either the decorator or context manager:
...
with mlflow.start_span(
    name="process-task",
    links=[
        Link(trace_id="tr-abc123def456", span_id="aabbccddeeff0011")
    ],
) as span:
...
You can also add links to an active span dynamically using `add_link()`:
...
When the link target is known upfront, prefer attaching links at span creation time via the `links` parameter. Use `add_link()` for cases where the target isn't known until runtime — for example, when it arrives in a message payload from another agent.
...
<TilesGrid>
  <TileCard
    icon={Zap}
    title="Combining with Auto-Tracing"
    description="Mix automatic and manual tracing for optimal observability"
    href="/genai/tracing/app-instrumentation/automatic#combining-manual-and-automatic-tracing"
    linkText="Learn more →"
  />
  <TileCard
    icon={BookOpen}
    title="Trace Concepts"
    description="Understand the structure and components of MLflow traces"
    href="/genai/concepts/trace"
    linkText="Explore concepts →"
  />
  <TileCard
    icon={Search}
    title="Querying Traces"
    description="Programmatically search and analyze your traces"
