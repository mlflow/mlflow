...
:::tip Multimodal Content
You can attach images and audio to spans using content parts lists in `set_inputs()` and `set_outputs()`. See [Multimodal Content in Traces](/genai/tracing/observe-with-traces/multimodal#manual-tracing) for examples.
:::
