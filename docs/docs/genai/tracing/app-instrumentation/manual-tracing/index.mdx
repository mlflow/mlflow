...
<Tabs>
  <TabItem value="decorator" label="Decorator" default>
    ...
    **Key Benefits**:
    ...
    [Learn more about decorators →](/tracing/app-instrumentation/manual-tracing/fluent-apis#decorator)
  </TabItem>
  <TabItem value="context-manager" label="Context Manager">
    ...
    **Key Benefits**:
    ...
    [Learn more about context managers →](/tracing/app-instrumentation/manual-tracing/fluent-apis#context-manager)
  </TabItem>
</Tabs>
...
[Learn more about client APIs →](/tracing/app-instrumentation/manual-tracing/log-level-api)
...
- `SpanType.CHAIN` - For multi-step workflows
- `SpanType.LLM` - For language model calls
- `SpanType.RETRIEVER` - For document/data retrieval
- `SpanType.EMBEDDING` - For embedding generation
- `SpanType.PARSER` - For data parsing operations
- `SpanType.TOOL` - For tool/function calls
- `SpanType.AGENT` - For agent operations
- `SpanType.UNKNOWN` - For general operations
...
def predict(messages: list[dict]) -> str:
    ...
    mlflow.update_current_trace(response_preview=f"Generated {len(response)} characters")
...
**Use descriptive span names**: Choose names that clearly describe what the span does, like "Extract PDF Text" rather than "Step 1".

**Add meaningful attributes**: Include metadata that helps with debugging and analysis, such as input sizes, processing times, and configuration parameters.

**Organize spans hierarchically**: Use parent-child relationships to show the logical structure of your workflow.

**Handle errors gracefully**: Use try-catch blocks to capture error information in span attributes.

**Combine with automatic tracing**: Use manual tracing to add context around automatically traced operations.

## Next Steps

Ready to start instrumenting your code? Choose your approach:

**[Decorators & Fluent APIs](/tracing/app-instrumentation/manual-tracing/fluent-apis)** - Start here for most use cases

**[Low-Level Client APIs](/tracing/app-instrumentation/manual-tracing/log-level-api)** - For advanced scenarios requiring full control

**[Automatic Tracing](/tracing/app-instrumentation/automatic)** - Combine manual and automatic tracing
