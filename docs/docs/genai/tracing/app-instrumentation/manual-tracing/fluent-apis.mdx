...
class PerformantTracer:
    ...
    def should_trace(self, operation_type: str, user_tier: str = None, **context) -> bool:
...
**Set meaningful attributes**: Add custom attributes that help with debugging and analysis, such as model names, processing parameters, or business context.

**Handle errors gracefully**: The decorator automatically captures exceptions, but you can add custom error handling and additional context as needed.

**Choose the right approach**: Use decorators for simple cases, context managers for complex blocks, and client APIs only when you need full control.
...
**[Context Managers](/tracing/app-instrumentation/manual-tracing/fluent-apis#context-manager)**: Learn about using `mlflow.start_span()` for more flexible tracing

**[Combining with Auto-Tracing](/tracing/app-instrumentation/automatic#combining-manual-and-automatic-tracing)**: Mix automatic and manual tracing for optimal observability

**[Trace Data Model](/tracing/data-model)**: Understand the structure and components of MLflow traces
