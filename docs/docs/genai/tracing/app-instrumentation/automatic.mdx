...
<DatabricksCallout docsPath="/mlflow3/genai/tracing/app-instrumentation/automatic" />

This guide requires the following packages:

**mlflow>=3.1**: Core MLflow functionality with GenAI features and Databricks connectivity.

**openai>=1.0.0**: Only required to run the [Basic Automatic Tracing Example](#basic-automatic-tracing-example) on this page (if using other LLM providers, install their respective SDKs instead)
...
<Tabs>
  <TabItem value="openai" label="OpenAI" default>
    ...
    [Full OpenAI Integration Guide](/genai/tracing/integrations/openai)
  </TabItem>
  <TabItem value="langchain" label="LangChain">
    ...
    [Full LangChain Integration Guide](/genai/tracing/integrations/langchain)
  </TabItem>
  <TabItem value="langgraph" label="LangGraph">
    ...
    [Full LangGraph Integration Guide](/genai/tracing/integrations/langgraph)
  </TabItem>
  <TabItem value="anthropic" label="Anthropic">
    ...
    [Full Anthropic Integration Guide](/genai/tracing/integrations/anthropic)
  </TabItem>
  <TabItem value="dspy" label="DSPy">
    ...
    [Full DSPy Integration Guide](/genai/tracing/integrations/dspy)
  </TabItem>
  <TabItem value="bedrock" label="Amazon Bedrock">
    ...
    [Full Bedrock Integration Guide](/genai/tracing/integrations/bedrock)
  </TabItem>
  <TabItem value="autogen" label="AutoGen">
    ...
    [Full AutoGen Integration Guide](/genai/tracing/integrations/autogen)
  </TabItem>
</Tabs>
...
def process_user_query(query: str):
    ...
    analysis = openai.OpenAI().chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "Analyze the user's query and determine if it requires factual information or creative writing.",
            },
    ...
    if "factual" in analysis_result.lower():
        ...
        response = openai.OpenAI().chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Provide a factual, well-researched response.",
                },
                {"role": "user", "content": query},
            ],
        )
...
**A parent span for the entire `process_user_query` function**

**Two child spans automatically created by the OpenAI autologging**:
...
Most autolog functions accept configuration parameters to customize tracing behavior:
...
# Autolog with custom configuration
mlflow.openai.autolog(
    log_traces=True,  # Enable/disable tracing
    log_models=False,  # Disable model logging
    log_inputs_outputs=True,  # Log input/output data
    silent=False,  # Enable logging messages
)
```

You can enable autolog for multiple libraries simultaneously by calling each autolog function:
...
**Enable autolog early**: Call autolog functions before importing or using the target libraries to ensure all operations are captured.

**Combine with manual tracing**: Use the `@mlflow.trace` decorator to group related operations and add context to your traces.

**Use appropriate configuration**: Customize autolog settings based on your needs (e.g., disable model logging in production for performance).

**Monitor trace volume**: In high-throughput applications, consider sampling or filtering to manage the volume of traces generated.

## Next Steps

**[Manual Tracing](/tracing/app-instrumentation/manual-tracing)**: Learn how to add custom tracing to your application logic

**[Integration Guides](/genai/tracing/integrations/index)**: Explore detailed guides for specific libraries and frameworks

**[Viewing Traces](/tracing/ui)**: Learn how to explore and analyze your traces in the MLflow UI

**[Querying Traces](/tracing/api/search)**: Programmatically search and retrieve trace data for analysis