...
MLflow is OpenTelemetry-compatible, so trace context is propagated through the HTTP headers following the [W3C TraceContext](https://www.w3.org/TR/trace-context/) specification. MLflow provides APIs to simplify handling the headers at the client and server sides:
...
**TypeScript:**
- Use `getTracingContextHeadersForHttpRequest()` in the **client side** to get headers containing the current trace context.
- Use `withTracingContextFromHeaders()` or `withTracingContextFromHeadersAsync()` in the **server side** to extract the trace context from the incoming request headers.
