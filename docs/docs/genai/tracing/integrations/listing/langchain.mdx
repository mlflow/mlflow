...
Once you have defined your custom tracer, you need to integrate it into your LangChain workflow. There are two main approaches:

#### Approach 1: Using with Callbacks

You can pass the custom tracer instance directly to the `callbacks` parameter when invoking your chain:
...
custom_tracer = CustomLangchainTracer()
...
# Use the custom tracer with callbacks
...
#### Approach 2: Using with Autolog's Inheritable Handlers

You can also register your custom tracer globally using MLflow's inheritable handlers. This is useful when you want the tracer to be automatically applied to all LangChain invocations without explicitly passing it each time:
...
# Enable autolog with your custom tracer
mlflow.langchain.autolog(inheritable_handlers=[CustomLangchainTracer()])
...
# The custom tracer is automatically applied
response = chain.invoke({"input": "What is MLflow?"})
```

With autolog enabled, all LangChain operations will automatically use the custom tracer without needing to pass it explicitly in the callbacks parameter.
