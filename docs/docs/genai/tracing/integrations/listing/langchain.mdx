...
for LangChain by calling the <APILink fn="mlflow.langchain.autolog" /> function, and nested traces are automatically logged to the active MLflow Experiment upon invocation of chains. In TypeScript, you can use the `@mlflow/langchain` package to wrap LangChain chat models with automatic tracing.
...
MLflow supports tracing for LangChain in both Python and TypeScript/JavaScript. Please select the appropriate tab below to get started.

:::tip Choosing a TypeScript tracing approach
The **JS / TS (MLflow)** tab uses the `@mlflow/langchain` package, which exports traces in a format optimized for the MLflow Trace UI — input/output messages, token usage, and span types render cleanly in the Chat and Trace views. This is the recommended approach for most users.

The **OpenTelemetry** tabs use the Arize OpenInference instrumentation and still work well — traces are fully functional for programmatic access and agent-based evaluation — but the raw OpenTelemetry span format is harder to read as a human in the MLflow UI.
:::
...
Visit `http://localhost:5000` (or your custom MLflow tracking server URL) to view the trace in the MLflow UI.
  ...
  <TabItem value="typescript" label="JS / TS (MLflow)">
...
```bash
npm install @mlflow/langchain @mlflow/core @langchain/core @langchain/anthropic
```

### 3. Enable tracing and make API calls

Wrap a LangChain chat model with the `tracedModel` function and make API calls as usual. Both `invoke()` and `stream()` are automatically traced.
...
// Wrap the LangChain model with tracedModel
const model = tracedModel(
  new ChatAnthropic({ model: "claude-sonnet-4-5-20250514" })
);

// invoke() and stream() calls are automatically traced
const result = await model.invoke([
  { role: "user", content: "Hello, Claude!" },
]);
```

`tracedModel` works with any `BaseChatModel` subclass, including `ChatOpenAI`, `ChatXAI`, and others.
...
Visit `http://localhost:5000` (or your custom MLflow tracking server URL) to view the trace in the MLflow UI.
  ...
  <TabItem value="typescript-v1" label="JS / TS (OpenTelemetry v1)">
...
Visit `http://localhost:5000` (or your custom MLflow tracking server URL) to view the trace in the MLflow UI.
  ...
  <TabItem value="typescript-v0" label="JS / TS (OpenTelemetry v0)">
