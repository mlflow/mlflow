...
Pass `--project` (or `-p`) to write to `./.codex/` (project-level) instead of your user-level config. Pass `--non-interactive` to skip the prompts and accept defaults, or supply `--tracking-uri <url>` and `--experiment-id <id>` to bypass individual prompts.
...
```json title="~/.codex/mlflow-tracing.json"
{
...
Setup refuses to overwrite a pre-existing `notify = ...` entry so your manual Codex config is never mangled — edit it yourself if a different notifier is already registered.
...
Write these values via `mlflow-codex setup` or by editing `~/.codex/mlflow-tracing.json` directly:
