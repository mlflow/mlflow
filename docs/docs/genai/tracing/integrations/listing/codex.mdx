...
Find your global `node_modules` path so you can reference the bundled hook script in the Codex config:
...
Add the `notify` hook to your Codex configuration, replacing `<npm-root>` with the path above. Codex calls this after every agent turn:

```toml title="~/.codex/config.toml"
notify = ["node", "<npm-root>/@mlflow/codex/bundle/stop.js"]
```
