...
### Step 3: Enable the Plugin in Hermes
...
```text
> Use the terminal tool to print the working directory and list 3 files.
```
...
Tracing captures the full agent workflow. Pairing it with the [MLflow AI Gateway](/genai/governance/ai-gateway/coding-agents/hermes-agent) gives you centralized model routing for the same Hermes runs, plus:
