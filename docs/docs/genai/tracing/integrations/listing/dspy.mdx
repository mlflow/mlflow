...
MLflow >= 3.2.0 supports token usage tracking for dspy. The token usage for each LLM call will be logged in the `mlflow.chat.tokenUsage` attribute. The total token usage throughout the trace will be
...
Currently, dspy token usage doesn't support token usage tracking for Audio transcription and Audio translation.
