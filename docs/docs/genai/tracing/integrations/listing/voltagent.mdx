...
Since this integration is built on OpenTelemetry, you can combine the automatically generated traces with the [MLflow Tracing SDK](/genai/tracing) to add custom spans, set tags, and log assessments within the same trace. If your application includes a Python backend, you can use both the OpenTelemetry SDK and the MLflow Tracing SDK together to enrich traces with additional context.
...
exp_id = mlflow.set_experiment("VoltAgent").experiment_id
