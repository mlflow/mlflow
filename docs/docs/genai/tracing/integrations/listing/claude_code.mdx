...
[MLflow Tracing](/genai/tracing) provides automatic tracing capability for Claude Code in two ways:

1. **CLI tracing**: Automatically trace interactive Claude Code CLI conversations
2. **SDK tracing**: Trace Claude Code SDK usage in Python applications

By setting up auto tracing, MLflow will automatically capture traces of your Claude Code conversations and log them to the active MLflow experiment. The trace automatically captures information such as:
...
Use SDK tracing when building applications that programmatically use the Claude Code SDK.
...
# Enable automatic tracing for Claude Code SDK
mlflow.claude_code.autolog()  # clint: disable=unknown-mlflow-function
```

Once enabled, all Claude Code SDK interactions will be automatically traced.
...
async def run_agent(query: str) -> str:
    """Run Claude Code SDK and return response"""
    async with ClaudeSDKClient() as client:
        await client.query(query)

        response_text = ""
        async for message in client.receive_response():
            # Process messages and accumulate response
...
evaluate(data=eval_data, predict_fn=predict_fn, scorers=[response_length])
```
...
- Verify `mlflow.claude_code.autolog()` is called before creating the `ClaudeSDKClient`
...
```python
mlflow.claude_code.autolog(disable=True)
```
