...
MLflow automatically tracks the token usage for each LLM call within Claude Code conversations. The token usage for each LLM call is logged in the `mlflow.chat.tokenUsage` attribute. The total token usage throughout the trace is available in the `token_usage` field of the trace info object.

```python
import mlflow
from mlflow.claude_code.tracing import process_transcript

# Process a Claude Code transcript
trace = process_transcript("/path/to/transcript.jsonl", "session-id")
...
:::note
Token usage tracking requires the Claude Code transcript to include usage metadata in the assistant message responses. This is automatically included when using Claude Code CLI version 1.0.0 and above.
:::
