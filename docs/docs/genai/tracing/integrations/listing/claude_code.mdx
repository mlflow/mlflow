...
description: Trace Claude Code CLI conversations and Claude Agent SDK usage with MLflow for full observability into AI-assisted software development workflows.
sidebar_position: 9
...
1. **Setup Phase**: The `mlflow autolog claude` command installs the MLflow Claude plugin from the MLflow marketplace and writes tracing config into `.claude/settings.json` in your project directory
2. **Automatic Tracing**: When you use the `claude` command in the configured directory, the plugin automatically traces your conversations
...
- Re-run `mlflow autolog claude` if the project still points at an older Python hook setup
...
This removes the MLflow tracing configuration from `.claude/settings.json` and cleans up any legacy Python hook entries, but preserves existing traces.
