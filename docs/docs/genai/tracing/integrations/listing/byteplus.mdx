...
[MLflow Tracing](../../) provides automatic tracing capability for BytePlus models through the OpenAI SDK integration. Since BytePlus uses an OpenAI-compatible API format, you can use `mlflow.openai.autolog()` to trace interactions with BytePlus models.
...
<StepHeader number={3} title="Enable tracing and call BytePlus" />
...
<ImageBox src="/images/llms/tracing/basic-openai-trace.png" alt="BytePlus Tracing" />
...
MLflow supports tracing for streaming and async BytePlus APIs. Visit the [OpenAI Tracing documentation](../openai) for example code snippets for tracing streaming and async calls through OpenAI SDK.
...
<ImageBox src="/images/llms/tracing/openai-trace-with-manual-span.png" alt="BytePlus Tracing with Manual Tracing" />
