...
<StepHeader number={2} title="Start MLflow Server (Optional)" />

If you're running outside of Databricks, start a local MLflow server:

<ServerSetup />

:::tip Using Databricks Managed MLflow
If you're running on Databricks, MLflow is already integrated. Traces are automatically captured and viewable in the Databricks workspace without additional setup.
:::
...
# Set tracking URI and experiment (skip if running on Databricks)
mlflow.set_tracking_uri("http://localhost:5000")
mlflow.set_experiment("Databricks AI Gateway")
...
// Initialize MLflow tracing (skip if running on Databricks)
init({
