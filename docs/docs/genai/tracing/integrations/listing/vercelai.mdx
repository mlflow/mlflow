...
<ImageBox src="/images/llms/tracing/vercel-ai-tracing.gif" alt="Vercel AI SDK tracing via MLflow" />
...
When the integration is enabled, MLflow let you record the following information for Vercel AI SDK calls:
...
Alternatively, you can use [Docker Compose](/self-hosting/architecture/docker-compose) to start the server without setting up Python environment. See [Self-Hosting Guide](/self-hosting/architecture/backend-store) for more details.
