...
![Agno Tracing via autolog](/images/llms/agno/agno-tracing-basic.png)
...
MLflow now offers native support for tracing agent-to-agent workflows when using the Agno API’s non-streaming endpoints. Under the hood, every handoff between agents
is automatically instrumented and recorded as part of the overall execution trace, ensuring end-to-end visibility into the entire multi-agent conversation. In addition
to logging each inter-agent message exchange, MLflow captures and associates detailed metadata for every function invocation and tool call performed by each agent,
including input parameters, return values, and execution duration. This comprehensive tracing not only simplifies debugging complex agent pipelines but also enables
fine-grained performance analysis and reproducibility of multi-agent orchestrations.
...
mlflow.set_experiment("Multi-Agent Testing")
