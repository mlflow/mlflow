...
The thread ID will be recorded in the trace metadata and can be accessed in the `Attributes` tab in the MLflow UI.

![LangGraph Thread ID](/images/llms/tracing/langgraph-thread-id.png)

By clicking the session ID, you will be redirected to the session page where you can view all the traces in the session.
