---
title: Tracing AutoGen
description: Trace AutoGen AgentChat multi-agent conversations and tool usage with MLflow for full observability into conversational AI agent applications.
sidebar_position: 6
sidebar_label: AutoGen
keywords: [autogen, tracing, mlflow, multi-agent, agent chat, microsoft]
image: /images/llms/autogen/autogen-trace.png
---
...
MLflow supports automatic token usage and cost tracking for the following AutoGen APIs.
It does not support tracking for asynchronous generators. Asynchronous streaming APIs such as `run_stream` or `on_messages_stream` are not tracked.
