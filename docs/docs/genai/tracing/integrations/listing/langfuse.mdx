...
By default, `mlflow.otel.autolog()` exports each trace synchronously as it completes. For high-throughput applications, you can enable batched export to buffer spans and send them in the background:

```python
mlflow.otel.autolog(batch=True)
```

Batched export uses OpenTelemetry's `BatchSpanProcessor`, which accumulates spans and flushes them periodically. This reduces per-request latency at the cost of a short delay before traces appear in the MLflow UI.
