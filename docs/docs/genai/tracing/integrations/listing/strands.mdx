...
![Strands Agents SDK Tracing via autolog](/images/llms/strands/strands-tracing.png)
...
MLflow >= 3.3.0 supports token usage tracking for Strand Agent SDK. The token usage for each Agent call will be logged in the `mlflow.chat.tokenUsage` attribute. The total token usage throughout the trace will be
