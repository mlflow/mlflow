...
Welcome to the MLflow Tracing contribution guide! This step-by-step resource will assist you in implementing additional LLM and AI agent framework integrations for tracing into MLflow.
...
From a tracing perspective, LLM and AI agent libraries can be categorized into two types:
...
2. **Expose Instrumentation Helpers**: Implement the tracing entry points in `src/`—for example, a `traced<Client>()` wrapper or `register()` function that wires the integration into `@mlflow/core`.
