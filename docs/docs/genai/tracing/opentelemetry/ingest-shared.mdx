...
When [workspaces](/self-hosting/workspaces/getting-started) are enabled on your MLflow server, OTLP clients can also send the `X-MLFLOW-WORKSPACE` header to override the default workspace for a request.
