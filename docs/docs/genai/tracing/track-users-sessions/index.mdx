...
<Tabs>
  <TabItem value="user-analysis" label="User Behavior Analysis" default>
    ...
    def analyze_user_behavior(user_id: str, experiment_id: str):
        ...
        unique_sessions = user_traces['tags'].apply(
            lambda x: x.get('mlflow.trace.session') if isinstance(x, dict) else None
        ).nunique()
        
        # Performance metrics
        successful_traces = user_traces[user_traces['status'] == 'OK']
        avg_response_time = successful_traces['execution_time_ms'].mean() if not successful_traces.empty else 0
    ...
    print(f"User has {user_stats['total_interactions']} interactions across {user_stats['unique_sessions']} sessions")
    ```
  </TabItem>
  <TabItem value="session-analysis" label="Session Flow Analysis">
    ...
    def analyze_session_flow(session_id: str, experiment_id: str):
        ...
        for i, (_, trace) in enumerate(session_traces.iterrows()):
            conversation_turns.append({
                ...
                "status": trace['status'],
                "request_preview": str(trace.get('request', ''))[:100] + "..." if trace.get('request') else None,
                "response_preview": str(trace.get('response', ''))[:100] + "..." if trace.get('response') else None
            })

        # Session-level metrics
        session_duration = session_traces['timestamp_ms'].max() - session_traces['timestamp_ms'].min()
    ...
    print(f"Session had {session_analysis['total_turns']} turns over {session_analysis['session_duration_ms']}ms")
    ```
  </TabItem>
  <TabItem value="comparative-analysis" label="Comparative Analysis">
    ...
    def compare_user_segments(experiment_id: str, segment_tag: str = "user_tier"):
        ...
        all_traces = mlflow.search_traces(
            experiment_ids=[experiment_id],
            filter_string=f"tags.{segment_tag} != ''"
        )
        ...
        for segment_value in all_traces['tags'].apply(
            lambda x: x.get(segment_tag) if isinstance(x, dict) else None
        ).unique():
            ...
            segment_traces = all_traces[
                all_traces['tags'].apply(
                    lambda x: x.get(segment_tag) == segment_value if isinstance(x, dict) else False
                )
            ]
            ...
            if not segment_traces.empty:
                ...
                segment_stats[segment_value] = {
                    ...
                    "unique_users": segment_traces['tags'].apply(
                        lambda x: x.get('mlflow.trace.user') if isinstance(x, dict) else None
                    ).nunique(),
                    "avg_latency_ms": successful_traces['execution_time_ms'].mean() if not successful_traces.empty else 0,
                    "error_rate": ((len(segment_traces) - len(successful_traces)) / len(segment_traces)) * 100
                }
    ...
    for segment, stats in segment_comparison.items():
        print(f"{segment}: {stats['unique_users']} users, {stats['avg_latency_ms']:.1f}ms avg latency")
    ```
  </TabItem>
</Tabs>
...
**[MLflow Tracing UI](/genai/tracing/observe-with-tracing/ui)**: Use the web interface for interactive user and session exploration