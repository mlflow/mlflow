...
<DatabricksCallout docsPath="/mlflow3/genai/tracing/attach-tags/index" />
...
def intelligent_routing(query: str):
    # Initial tags
    mlflow.update_current_trace(tags={"query_type": "unknown"})
    
    if "weather" in query.lower():
        mlflow.update_current_trace(tags={
            "query_type": "weather",
            "routing": "weather_service",
            "complexity": "simple"
        })
        return handle_weather_query(query)
    elif "complex calculation" in query.lower():
        mlflow.update_current_trace(tags={
            "query_type": "calculation", 
            "routing": "math_service",
            "complexity": "high"
        })
...
user_traces = mlflow.search_traces(
    experiment_ids=["1"],
    filter_string="tags.user_id = 'user_123'"
)
...
control_group = mlflow.search_traces(
    experiment_ids=["1"],
    filter_string="tags.experiment_variant = 'control'"
)

treatment_group = mlflow.search_traces(
    experiment_ids=["1"],
    filter_string="tags.experiment_variant = 'treatment'"
)
...
model_v1_traces = mlflow.search_traces(
    experiment_ids=["1"],
    filter_string="tags.model_version = 'v1.0.0'"
)

model_v2_traces = mlflow.search_traces(
    experiment_ids=["1"],
    filter_string="tags.model_version = 'v2.0.0'"
)

# Compare performance
v1_avg_time = sum(t.info.execution_time_ms for t in model_v1_traces) / len(model_v1_traces)
v2_avg_time = sum(t.info.execution_time_ms for t in model_v2_traces) / len(model_v2_traces)
