...
- **`mlflow.trace.span.duration`**: A histogram measuring span execution duration in milliseconds
  ...
  - **Labels/Attributes**:
    ...
    - `tag_*`: Trace tags (e.g., `tag_trace_name`, `tag_eval_request_id`) - only for root spans
    - `meta_*`: Select trace metadata (e.g., `meta_source_run`, `meta_model_id`) - only for root spans
