...
- **`mlflow.trace.span.duration`**: A histogram measuring span execution duration in milliseconds
  ...
  - **Labels/Attributes**:
    ...
    - `tags.*`: All trace tags (e.g., `tags.mlflow.traceName`, `tags.mlflow.evalRequestId`) - available for all spans
    - `metadata.*`: All trace metadata (e.g., `metadata.source_run`, `metadata.model_id`, `metadata.token_usage`) - available for all spans
...
- Performance analysis by model ID or source run (e.g., metadata.model_id, metadata.source_run)
