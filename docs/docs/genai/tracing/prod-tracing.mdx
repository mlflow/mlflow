...
| `MLFLOW_ENABLE_INCREMENTAL_SPAN_EXPORT`     | Whether to export spans incrementally as they complete via `log_spans`, in addition to exporting the full trace. Disabling this reduces DB round-trips under high concurrency but removes real-time span visibility for in-progress traces. Remote/distributed trace spans are still exported when the backend supports `log_spans`. | `True` (`False` when running `mlflow server`)                                    |
