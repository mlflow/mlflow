...
- `sampling_ratio` only applies at the trace level (root spans). Nested calls within an existing trace are always included, and their `sampling_ratio` settings are ignored.
