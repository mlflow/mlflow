...
- **`mlflow.trace.span.duration`**: A histogram measuring span execution duration in milliseconds
  ...
  - **Labels/Attributes**:
    ...
    - `tag_*`: All trace tags (e.g., `tag_mlflow.traceName`, `tag_mlflow.evalRequestId`) - available for all spans
    - `meta_*`: All trace metadata (e.g., `meta_source_run`, `meta_model_id`, `meta_token_usage`) - available for all spans
...
- Metrics segmented by trace tags (e.g., trace name, evaluation request ID)
- Performance analysis by model ID or source run
