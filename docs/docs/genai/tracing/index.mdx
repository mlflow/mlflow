...
- Use <APILink fn="mlflow.tracing.get_tracing_context_headers_for_http_request" /> on the client to inject headers.

- Use <APILink fn="mlflow.tracing.set_tracing_context_from_http_request_headers" /> on the server to extract and set the current context for the duration of the request.
