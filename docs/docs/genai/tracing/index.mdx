...
- Use `mlflow.tracing.distributed.get_tracing_context_headers_for_http_request()` on the client to inject headers.
- Use `mlflow.tracing.distributed.set_tracing_context_from_http_request_headers(headers)` on the server to extract and set the current context for the duration of the request.
- See full API reference: [/api_reference/python_api/mlflow.tracing.distributed](/api_reference/python_api/mlflow.tracing.distributed)
