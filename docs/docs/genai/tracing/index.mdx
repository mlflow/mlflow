---
title: LLM Tracing and Agent Observability | MLflow AI Platform
...
MLflow tracing does **not** require an explicit `enable()` call to start recording traces.

Tracing starts automatically when you:
- Use an MLflow auto-tracing integration (for example `mlflow.openai.autolog()`)
- Decorate a function with `@mlflow.trace`

When the first traced operation runs, MLflow automatically creates a trace
and records spans under it.
