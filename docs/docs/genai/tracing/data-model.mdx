...
<DatabricksCallout docsPath="/mlflow3/genai/tracing/data-model" />
...
<Tabs>
  ...
  <TabItem value="usage" label="Usage Examples">
    To set a span type, you can pass the `span_type` parameter to the <APILink fn="mlflow.trace" /> decorator or <APILink fn="mlflow.start_span" /> context manager. When you are using [automatic tracing](/tracing/app-instrumentation/automatic), the span type is automatically set by MLflow.
...
**[Tracing Concepts](/tracing/concepts)**: Understand the fundamental concepts behind tracing

**[Querying Traces](/tracing/api/search)**: Learn how to search and retrieve trace data programmatically

**[Automatic Tracing](/tracing/app-instrumentation/automatic)**: Get started with one-line tracing for supported libraries

**[Manual Tracing](/tracing/app-instrumentation/manual-tracing)**: Create custom spans for detailed instrumentation

**[Advanced Patterns](/tracing/app-instrumentation/manual-tracing#advanced-patterns)**: Learn about distributed tracing, performance optimization, and custom trace management
