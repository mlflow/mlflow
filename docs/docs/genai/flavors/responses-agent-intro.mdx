...
- <APILink fn="mlflow.types.responses_utils.prep_msgs_for_cc_llm" /> (use for input)
- <APILink fn="mlflow.types.responses_utils.output_to_responses_items_stream" /> (use for output)
  - Can be used to parse and aggregate:
    - a Chat Completion stream into a Responses Stream
    - a LangChain message stream (from a LangChain or LangGraph agent) into a ResponsesAgent Stream
