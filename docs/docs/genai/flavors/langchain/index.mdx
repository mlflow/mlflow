...
- [LLMChain](https://python.langchain.com/docs/modules/chains/foundational/llm_chain) (deprecated, only support for `langchain<0.3.0`)
...
The [mlflow.genai.evaluate](https://mlflow.org/docs/latest/model-evaluation/index.html) function provides
...
inference call. Given `mlflow.genai.evaluate` performs naive comparisons between raw predictions and a specified
ground truth value, it is the user's responsibility to reconcile potential differences prediction output
and ground truth.

Often, the best approach is to use a [custom function](/genai/eval-monitor/custom-scorers)
to process the response. Below we provide an example of a custom function that extracts the last chat
message from a LangGraph model. This function is then used in mlflow.genai.evaluate to return a single
...
with mlflow.start_run() as run:
    results = mlflow.genai.evaluate(
        predict_fn=custom_langgraph_wrapper,  # Pass our function defined above
        data=eval_data,
