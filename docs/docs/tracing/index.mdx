  ...
  <TabItem value="online-monitoring" label="Production Monitoring">
    <div class="flex-column">
      <div class="flex-row">
        ...
        Refer to [Export Traces to Other Services](production#opentelemetry-collector) for more details.
...
1. **MLflow UI**: The MLflow UI provides a rich interface for exploring traces. You can view traces for a specific experiment, run, [search and filter traces](/api/search) based on various criteria. You can start the UI by running `mlflow ui` in your terminal and navigating to `http://localhost:5000`.

2. **Jupyter Notebook**: The trace UI is also available within Jupyter notebooks! The trace UI will automatically be displayed when a cell generates a trace, eliminating the need to switch between the notebook and web browser. See the [Jupyter Notebook Integration](/tracing/sdk/how-to#render-trace-inside-jupyter-notebook) for more details.
