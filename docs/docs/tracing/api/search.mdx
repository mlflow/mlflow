...
When using `return_type="pandas"` (default), MLflow returns the retrieved traces as a Pandas DataFrame, where
each row corresponds to a single trace. The DataFrame format has changed between MLflow 2.x and 3.x.

<Tabs>
  <TabItem value="3.x" label="MLflow 3.x">
    - **trace_id**: A primary identifier of a trace
    - **trace**: A trace object
    - **client_request_id**: Client supplied request ID associated with the trace.
...
:::note
The format of the DataFrame depends on the MLflow version when you call the `search_traces` API,
**not** the version used for logging the traces. For example, if you log traces using MLflow 2.x
