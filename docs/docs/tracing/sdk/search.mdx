...
- **<APILink fn="mlflow.search_traces" />**: A higher-level fluent API that supports same filtering functionalities, but returns a pre-formatted **Pandas DataFrame**, with each row representing a trace, aiding with building an evaluation dataset for [MLflow LLM Evaluation](../../llms/llm-evaluate.mdx).
...
The fluent API <APILink fn="mlflow.search_traces" /> allows you to extract specific fields from spans with `extract_fields` argument. The span fields can be specifie in the format of `span_name.[inputs|outputs]` or `span_name.[inputs|outputs].field_name`:
...
This feature is particularly useful for building an evaluation dataset for your application. You can convert the returned Pandas DataFrame into an MLflow LLM evaluation dataset format by renaming fields, and evaluate your language model using the [mlflow.evaluate()](../../llms/llm-evaluate.mdx) API.
