...
- **<APILink fn="mlflow.search_traces" />**: A higher-level fluent API that supports same filtering functionalities, but returns a pre-formatted **Pandas DataFrame**, with each row representing a trace, aiding with building an evaluation dataset for [MLflow LLM Evaluation](llms/llm-evaluate).
