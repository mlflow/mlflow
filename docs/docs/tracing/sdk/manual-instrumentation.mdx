...
In addition to the [Auto Tracing](..#automatic-tracing) integrations, you can instrument your Python code using the MLflow Tracing SDK. This is especially useful when you need to instrument your custom Python code.
...
* `span_type` parameter to set the type of span. Set either one of built-in [Span Types](../tracing-schema#span-types) or a string.
...
Tags can be added to traces to provide additional metadata at the trace level. There are a few different ways to set tags on a trace. Please refer to the [how-to guide](sdk/how-to#setting-trace-tags) for the other methods.
