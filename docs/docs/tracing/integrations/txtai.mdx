...
[MLflow Tracing](../) provides automatic tracing capability for txtai. By enabling auto tracing
for txtai by calling the `mlflow.txtai.autolog` function, MLflow will capture traces for LLM invocation, embeddings, vector search, and log them to the active MLflow Experiment.
