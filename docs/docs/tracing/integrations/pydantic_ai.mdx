...
[MLflow Tracing](../) provides automatic tracing capability for [​PydanticAI](https://ai.pydantic.dev/), an open source framework for building multi-agent applications. By enabling auto tracing for ​PydanticAI by calling the <APILink fn="mlflow.pydanticai.autolog" /> function, , MLflow will capture nested traces for ​PydanticAI workflow execution and logged them to the active MLflow Experiment.

```python
import mlflow

mlflow.pydanticai.autolog()
```
...
# Turn on auto tracing by calling mlflow.pydanticai.autolog()
mlflow.pydanticai.autolog()
...
mlflow.pydanticai.autolog(log_traces=True)
...
Auto tracing for PydanticAI can be disabled globally by calling `mlflow.pydanticai.autolog(disable=True)` or `mlflow.autolog(disable=True)`.