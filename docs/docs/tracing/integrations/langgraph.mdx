...
[MLflow Tracing](/tracing/) provides automatic tracing capability for LangGraph, as a extension of its LangChain integration. By enabling auto-tracing for LangChain by calling the <APILink fn="mlflow.langchain.autolog" /> function, MLflow will
...
MLflow LangGraph integration is not only about tracing. MLflow offers full tracking experience for LangGraph, including model tracking, dependency management, and evaluation. Please checkout the **[MLflow LangChain Flavor](/llms/langchain)** to learn more!
...
for span in trace.data.spans:
    if usage := span.attributes.get("mlflow.chat.tokenUsage"):
        print(f"{span.name}:")
...
By combining auto-tracing with the [manual tracing APIs](/tracing/api/manual-instrumentation/), you can add child spans inside a node or tool, to get more detailed insights for the step.
