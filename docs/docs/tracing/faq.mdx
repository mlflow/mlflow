...
### Q: Can I use MLflow Tracing for production application?

Yes, MLflow Tracing is stable and designed to be used in production environments.

When using MLflow Tracing for production environment, we recommend using the [MLflow Tracing SDK](https://pypi.org/project/mlflow-tracing/) (`mlflow-tracing`) to instrument your code/models/agents with minimal dependencies and footprint. The SDK is designed to be a perfect fit for production environments where you want efficient and lightweight tracing solution. Please refer to the [Production Monitoring](/tracing/production) section for more details.
