...
This feature removes the need to query Runs for traces from different environments (batch jobs, separate inference tables etc.) to gain a full understanding of the agent’s performance.
...
- Model artifacts storage location change: In MLflow 2.x, [model artifacts](../model/#storage-format) are stored as run artifacts. Since MLflow 3.0, those artifacts will be stored into models artifacts location.
