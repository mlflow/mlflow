...
### Manul control over traces grouping

To manually control traces grouping, you can use <APILink fn="mlflow.set_active_model">mlflow.set_active_model</APILink> API to **set an active LoggedModel in the current context**.
...
First, we create a prompt template and register it with [MLflow Prompt Registry](../prompts) capability.
...
At this step, we create an OpenAI client, set an active mode for grouping traces.
Then after turning on autologging, all traces generated during requests are linked to the active model.
...
Finally, we evaluate the response based on different metrics, and record the evaluation result to a run and the current active model.
