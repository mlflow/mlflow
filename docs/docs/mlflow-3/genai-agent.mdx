...
In this section, we manually test the agent with example queries and leverage MLflow Tracing to analyze the outputs and debug potential issues. 
...
```python
# Enable autologging so that interactive traces from the chain are automatically linked to a LoggedModel
...
for chain_input in chain_inputs:
    outputs.append(chain.invoke({"messages": chain_input}).content)
