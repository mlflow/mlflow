...
The artifact store is a core component in [MLflow Tracking](/tracking) where MLflow stores (typically large) artifacts
for each run such as model weights (e.g. a pickled scikit-learn model), images (e.g. ONGs), model and data files (e.g. [Parquet](https://parquet.apache.org/) file).
Note that metadata like parameters, metrics, and tags are stored in a [backend store](/tracking/backend-stores) (e.g., PostGres, MySQL, or MSSQL Database), the other component of the MLflow Tracking.
...
See [tracking server setup](/tracking/server#tracking-server-artifact-store) and the specific section for your storage in [supported storages](/tracking/artifacts-stores#artifacts-store-supported-storages) for guidance on
...
MLflow automatically records the `artifact_uri` property as a part of <APILink fn="mlflow.entities.RunInfo">`mlflow.entities.RunInfo`</APILink>, so you can
...
You can also run this cleanup automatically by starting the tracking server with the
`--gc-interval` option or by setting the `MLFLOW_GC_INTERVAL_SECONDS` environment
variable. Configure `MLFLOW_GC_OLDER_THAN` to delete only runs and experiments older
