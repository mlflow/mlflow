...
Models represent the trained machine learning artifacts that are produced during your runs. In [MLflow 3.0](/mlflow-3), models have become first class citizens with the introduction of a standalone <APILink fn="mlflow.entities.LoggedModel">LoggedModel</APILink> entity. Logged Models contain their own metadata and artifacts similar to runs.
...
[MLflow 3.0](/mlflow-3) introduces a rich set of APIs for tracking <APILink fn="mlflow.entities.LoggedModel">LoggedModel</APILink> objects. You can create a logged model by calling `mlflow.<flavor>.log_model` (with or without an active run), then log metadata using functions such as <APILink fn="mlflow.log_metric" />, <APILink fn="mlflow.log_model_params" />, and <APILink fn="mlflow.set_logged_model_tags" />. For more information about these APIs, please see the <APILink fn="mlflow.get_logged_model">API reference</APILink>. For code examples, please visit [MLflow 3.0 Showcases](/mlflow-3/#mlflow-30-showcases).
...
These features are available for models as well, provided you are using [MLflow 3.0](/mlflow-3), as shown below.
<figure>
...
## Querying Runs and Models Programmatically \{#tracking_query_api}
...
For example, the following code snippet searches for runs that has the best validation loss among all runs in the experiment.
...
This code snippet searches for the logged model that has the best accuracy among all logged models in the experiment ([MLflow 3.0](/mlflow-3) only).

```python
client = mlflow.tracking.MlflowClient()
best_model = client.search_logged_models(
    experiment_ids=["0"],
    order_by=[{"field_name": "metrics.accuracy", "ascending": False}],
)[0]
print(best_model)
# LoggedModel(model_id='...', name='...', ...)
```
