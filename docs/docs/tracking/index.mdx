...
To learn more, please visit the [MLflow data documentation](/tracking/data-api) to see the features available in this API.
...
<Tabs>
  ...
  <TabItem label="Artifacts-only Mode" value="artifacs-only-mode">
    ...
    [Optionally using a Tracking Server instance exclusively for artifact handling](/tracking/server#optionally-using-a-tracking-server-instance-exclusively-for-artifact-handling)
  ...
  <TabItem label="Direct Access to Artifacts" value="direct-access-to-artifacts">
    ...
    Refer to [Use Tracking Server without Proxying Artifacts Access](/tracking/server#use-tracking-server-w-o-proxying-artifacts-access)
...
- [Organize runs into experiments](/tracking/tracking-api#organizing_runs_in_experiments) - Experiments are logical containers for your runs. You can create an experiment using the CLI, API, or UI.
...
- [Add tags to runs](/tracking/tracking-api#add-tags-to-runs) - You can associate arbitrary tags with each run, which allows you to filter and search runs based on tags.
