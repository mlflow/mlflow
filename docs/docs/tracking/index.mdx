...
<figure>
  ...
  <figcaption>
    A screenshot of the MLflow Tracking UI, showing a plot of validation loss metrics during model
    training.
  </figcaption>
</figure>
...
The MLflow API and UI also let you create and search for experiments. See [Organizing Runs into Experiments](tracking-api#organizing-runs-in-experiments)
...
debugging, or analysis. See the [MLflow Tracing Guide](/llms/tracing) for more details.

## Tracking Runs \{#start-logging}

[MLflow Tracking APIs](tracking-api) provide a set of functions to track your runs. For example, you can call <APILink fn="mlflow.start_run" /> to start a new run,
then call [Logging Functions](tracking-api#tracking_logging_functions) such as <APILink fn="mlflow.log_param" /> and <APILink fn="mlflow.log_metric" /> to log a parameters and metrics respectively.
Please visit the [Tracking API documentation](tracking-api) for more details about using these APIs.
...
Alternatively, [Auto-logging](autolog) offers the ultra-quick setup for starting MLflow tracking.
...
libraries such as [Scikit-learn](autolog#autolog-sklearn), [XGBoost](autolog#autolog-xgboost), [PyTorch](autolog#autolog-pytorch),
[Keras](autolog#autolog-keras), [Spark](autolog#autolog-spark), and more. See [Automatic Logging Documentation](autolog)
...
To learn more, please visit the [MLflow data documentation](data-api) to see the features available in this API.
...
#### [MLflow Tracking APIs](tracking-api)
...
#### [Artifact Store](artifacts-stores) \{#artifact-stores}
...
#### [MLflow Tracking Server](server) (Optional) \{#tracking_server}
...
[MLflow Tracking Server documentation](server) for more details.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <th>Use Case</th>
      <td>
        By default, MLflow records metadata and artifacts for each run to a local directory,
        `mlruns`. This is the simplest way to get started with MLflow Tracking, without setting up
        any external server, database, and storage.
      </td>
      <td>
        The MLflow client can interface with a SQLAlchemy-compatible database (e.g., SQLite,
        PostgreSQL, MySQL) for the [backend](backend-stores). Saving metadata to a database allows
        you cleaner management of your experiment data while skipping the effort of setting up a
        server.
      </td>
      <td>
        MLflow Tracking Server can be configured with an artifacts HTTP proxy, passing artifact
        requests through the tracking server to store and retrieve artifacts without having to
        interact with underlying object store services. This is particularly useful for team
        development scenarios where you want to store artifacts and experiment metadata in a shared
        location with proper access control.
      </td>
    </tr>
    <tr>
      <th>Tutorial</th>
      <td>[QuickStart](/getting-started/intro-quickstart)</td>
      <td>[Tracking Experiments using a Local Database](tutorials/local-database)</td>
      <td>[Remote Experiment Tracking with MLflow Tracking Server](tutorials/remote-server)</td>
    </tr>
  </tbody>
</table>
...
[Remote Experiment Tracking with MLflow Tracking Server](tutorials/remote-server) for learning
...
<Tabs>
  ...
  <TabItem label="Artifacts-only Mode" value="artifacs-only-mode">
    ...
    [Optionally using a Tracking Server instance exclusively for artifact handling](server#optionally-using-a-tracking-server-instance-exclusively-for-artifact-handling)
  ...
  <TabItem label="Direct Access to Artifacts" value="direct-access-to-artifacts">
    ...
    Refer to [Use Tracking Server without Proxying Artifacts Access](server#use-tracking-server-w-o-proxying-artifacts-access)
...
See [Launching Multiple Runs in One Program](tracking-api#launching-multiple-runs) for more details.

### How can I organize many MLflow Runs neatly?

MLflow provides a few ways to organize your runs:

- [Organize runs into experiments](tracking-api#organizing_runs_in_experiments) - Experiments are logical containers for your runs. You can create an experiment using the CLI, API, or UI.
- [Create child runs](tracking-api#child_runs) - You can create child runs under a single parent run to group them together. For example, you can create a child run for each fold in a cross-validation experiment.
- [Add tags to runs](tracking-api#add-tags-to-runs) - You can associate arbitrary tags with each run, which allows you to filter and search runs based on tags.
...
1. Set up artifacts configuration such as credentials and endpoints, just like you would for the MLflow Tracking Server.
   See [configure artifact storage](artifacts-stores#artifacts-store-supported-storages) for more details.
...
See [Backend Stores](backend-stores) and [Common Setups](#tracking_setup) for how to configures backend store properly for your workflow.

#### How to include additional decription texts about the run?

A system tag `mlflow.note.content` can be used to add descriptive note about this run. While the other [system tags](tracking-api#system_tags) are set automatically,
