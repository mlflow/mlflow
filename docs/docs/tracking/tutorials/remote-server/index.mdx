...
In this tutorial, you will learn how to set up MLflow Tracking environment for team development using the [MLflow Tracking Server](/tracking#tracking_server).
...
- **Sharing Results**: The tracking server also serves a [Tracking UI](/tracking#tracking_ui) endpoint, where team members can easily explore each other's results.
...
:::note
You can find the list of supported data stores in the [artifact stores](/tracking/artifacts-stores) and [backend stores](/tracking/backend-stores) documentation guides.
:::
...
You can find the instructions for how to configure credentials for other storages in [Supported Storage](/tracking/artifacts-stores#artifacts-store-supported-storages).
...
- **Other configurations for the Tracking Server**: By default, MLflow Tracking Server serves both backend store and artifact store.
  ...
  traffic or security concerns. See [other use cases](/tracking#other-tracking-setup) for how to customize the Tracking Server for these use cases.
- **Secure the Tracking Server**: The `--host` option exposes the service on all interfaces. If running a server in production, we
  would recommend not exposing the built-in server broadly (as it is unauthenticated and unencrypted). Read [Secure Tracking Server](/tracking/server#tracking-auth)
  for the best practices to secure the Tracking Server in production.
- **New Features**: The MLflow team and a host of community contributors constantly develops new features to support broader use cases. See [New Features](/new-features) to catch up with the latest features!
