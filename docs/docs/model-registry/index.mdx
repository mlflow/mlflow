...
<TOCInline toc={toc} maxHeadingLevel={4} minHeadingLevel={2} />
...
<dl>
  ...
  <a name="using-registered-model-aliases" />
  <dt>Model Alias</dt>
  <dd>
  ...
  <dt>Annotations and Descriptions</dt>
  <dd>
...
In this page, you can inspect model version details like the model signature, MLflow source run, and creation timestamp. You can also view and configure the verion's aliases,
...
If the model name exists, this method will throw an <Link to={"/api_reference/python_api/exceptions/mlflow.exceptions.html#mlflow.exceptions.ExecutionException"} target="\_blank">MLflowException</Link>
