...
use [Lightning](https://lightning.ai) to train your models. When Lightning is being used, you can turned
on autologging by calling <APILink fn="mlflow.tensorflow.autolog" /> or <APILink fn="mlflow.autolog" />. For more
...
- <APILink fn="mlflow.log_metric" /> / <APILink fn="mlflow.log_metrics" />: log metrics such as
  accuracy and loss during training.
- <APILink fn="mlflow.log_param" /> / <APILink fn="mlflow.log_params" />: log parameters such as
  learning rate and batch size during training.
- <APILink fn="mlflow.pytorch.log_model" />: save your PyTorch model to MLflow, which is usually
  called at the end of training.
- <APILink fn="mlflow.log_artifact" />: log artifacts such as model checkpoints and plots during
  training.
...
- Log your model and training parameters via <APILink fn="mlflow.log_params" /> at the beginning of training
  loop, such as learning rate, batch size, etc. <APILink fn="mlflow.log_params" /> is the
  batched logging version of <APILink fn="mlflow.log_param" />, which is more efficient than the latter.
- Log your model architecture at the beginning of training via <APILink fn="mlflow.log_artifact" />. You can use
  `torchinfo` package to get the model summary.
- Log training and validation metrics via <APILink fn="mlflow.log_metric" /> inside your training loop, such as
  loss and accuracy for classification tasks. If you have multiple metrics per logging step, you can
  use <APILink fn="mlflow.log_metrics" /> to log them together.
- Log your trained/finetuned model to MLflow via `mlflow.pytorch.log_model()` at the end of your training.
- [Optional] You can also log your model checkpoints to MLflow via <APILink fn="mlflow.log_artifact" /> during
...
[tracking server overview](/getting-started/tracking-server-overview#method-1-start-your-own-mlflow-server)),
you will see results on MLflow UI similar to the screenshot below:

<div className="center-div" style={{ width: "90%" }}>
  ![Basic Example of PyTorch with
  MLflow](/images/deep-learning/pytorch/guide/pytorch-guide-basic-example-ui.png)
</div>

## Saving Your PyTorch Model to MLflow

As we mentioned in the previous section, you can save your PyTorch model to MLflow via <APILink fn="mlflow.pytorch.log_model" />.
...
can either use the <APILink fn="mlflow.models.infer_signature"/> API or manually set the signature.

<APILink fn="mlflow.models.infer_signature" /> takes your input data and model outputs to automatically
...
After setting the signature, you can include it when calling <APILink fn="mlflow.pytorch.log_model" />:
