...
- **Models**: In [MLflow 3.0](/mlflow-3), each deep learning checkpoint is stored as a distinct <APILink fn="mlflow.entities.LoggedModel">LoggedModel</APILink> entity capturing its own metrics and parameters, allowing for easy comparison across model checkpoints. For a code example, see [Deep Learning with MLflow 3.0](/mlflow-3/deep-learning).
...
<Tabs>
  ...
  <TabItem label="Model Comparison" value="model-comparison">
    #### Model Comparison ([MLflow 3.0](/mlflow-3) only)
