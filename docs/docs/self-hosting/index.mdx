...
By default, MLflow uses SQLite (`sqlite:///mlflow.db`) as the backend store. For production deployments or custom configurations, you can specify a different backend with `--backend-store-uri`. Check out <ins>[Backend Store](./architecture/backend-store)</ins> for more options.
