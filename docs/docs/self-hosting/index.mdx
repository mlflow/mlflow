...
MLflow support [username/password login](./authentication/basic-http-auth) via basic HTTP authentication, [SSO (Single Sign-On)](./authentication/sso), and [custom authentication plugins](./authentication/custom).
