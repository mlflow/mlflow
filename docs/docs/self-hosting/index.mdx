...
The MLflow repository includes a ready-to-run Compose project under `docker-compose/` that provisions different samples with S3-compatible storage: [Garage](), [SeaweedFS](), [RustFS](). All the example starts MLflow, PostgreSQL, and the S3 Storage.

```bash
git clone https://github.com/mlflow/mlflow.git
cd docker-compose/<S3_STORAGE_FOLDER>
