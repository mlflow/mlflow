...
| `--verify`          | After migration, verify that DB row counts match source data.    |
...
Each experiment is committed independently — if a failure occurs, previously migrated experiments are preserved in the database. Entity types that don't exist in your FileStore (e.g., traces if you never used tracing) are automatically skipped.
...
- **Per-experiment batching.** Each experiment (with all its runs, traces, datasets, etc.) is committed independently. If a failure occurs, previously committed experiments are preserved. Model registry entries are also committed individually.
