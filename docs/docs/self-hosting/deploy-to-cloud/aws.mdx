...
## Step 4: Create ECS (Elastic Container Service) to host the MLflow server

Create ECS (Elastic Container Service) to host the MLflow web server and the UI. Choose "Express mode" to configure most settings including VPC and ALB automatically, but you need to fill the following settings correctly:
...
MLflow supports [basic authentication](/self-hosting/security/basic-http-auth/) and [authentication with OIDC plugin](/self-hosting/security/sso/#using-oidc-plugin), the 2 kinds of authentication settings require:
...
Use [MLflow demo](/api_reference/cli.html#mlflow-demo) CLI to validate the deployment. Run the command from your own laptop as follows:
