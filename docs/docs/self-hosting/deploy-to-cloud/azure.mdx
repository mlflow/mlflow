...
- Container Apps environment: Create a new environment, and in the environment networking setting page, enable public network access, and select the select the VNet and the "aca-infra-subnet" subnet.
Container configrations:
- Image source: Docker hub
- Registry login server: ghcr.io
- Image and tag: ghcr.io/mlflow/mlflow:v3.10.0-full
- Command override: /bin/bash
- Arguments override: `-c, pip install azure-storage-blob && mlflow server --backend-store-uri <database-URL> --artifacts-destination wasbs://<container-name>@<storage-account-name>.blob.core.windows.net --host 0.0.0.0 --port 5000 --disable-security-middleware`, note that a space must follow the comma.
