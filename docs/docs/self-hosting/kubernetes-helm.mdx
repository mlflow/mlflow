...
Released chart versions are published to GitHub Container Registry as OCI artifacts, so the chart can be installed directly from the registry:
...
The examples on this page install from a local checkout. To use the registry instead, replace `./charts` with `oci://ghcr.io/mlflow/charts/mlflow --version <version>` in any command.
