...
Each stable MLflow release publishes a chart after its Docker images succeed;
release candidates do not publish charts. Chart version and `appVersion` both
match the MLflow version, so chart `X.Y.Z` defaults to image
`ghcr.io/mlflow/mlflow:vX.Y.Z-full`.

Release-aligned versions replace the original `0.1.x` chart numbering. Existing
`0.1.x` artifacts remain available, but update any `0.1.x` version constraints
to receive new releases. Check the published versions before installing; older
MLflow releases may not have a corresponding chart.
