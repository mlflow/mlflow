...
- Either admin privileges to manage permissions, or `MANAGE` permission on the specific resource being granted
...
Get workspace permissions for a specific user (includes any wildcard grants like `username="*"`):

```bash
curl "http://localhost:5000/api/2.0/mlflow/workspaces/permissions/search?username=alice"
```
