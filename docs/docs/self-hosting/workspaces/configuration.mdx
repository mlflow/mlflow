...
runs keep using the pre-move storage path. To repoint experiments that still use the layout
derived from the server's default artifact root, pass `--artifact-policy retarget`
(experiments only):
...
`--default-artifact-root` must match the value the tracking server is started with. It is used to
recognize server-derived layouts and to resolve the target workspace root. Both the pre-workspace
`<root>/<experiment_id>` layout and the source workspace's `<root>/workspaces/<name>/<experiment_id>`
layout are recognized. A workspace-level `default_artifact_root` takes precedence and is used
without the `workspaces/<name>` suffix.
...
target workspace's artifact root. Experiments with custom artifact locations are reported and
left unchanged. Physically relocating artifact objects may be supported by a future artifact
policy.
