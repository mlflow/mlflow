...
3. **Container READ** - the caller must be able to read the container. This is a
   separate check from the tier, and an absent container grant can still satisfy
   it through `default_permission` (see the last table row).
...
Editing a run in experiment 42. The first four rows hold in any configuration,
...
| `(experiment, 42, MANAGE)` + `(run, *, DENY)` | no        | no        | `DENY` on the tier is not rescued      |
| `(run, *, EDIT)` alone                        | see below | see below | step 3 falls to `default_permission`   |
