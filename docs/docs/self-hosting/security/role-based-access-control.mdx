...
3. **Container READ** - the caller must be able to read the container. This is a
   separate check from the tier, and it is what stops a sub-resource grant from
   becoming an existence oracle: a successful child operation would otherwise
   reveal that the container exists. Experiment ids are small sequential
   integers, so a caller holding only `(run, *, READ)` could sweep
   `experiment_ids=[1..N]` on `SearchRuns` and map every experiment in the
   workspace without holding a single experiment grant. Requiring `READ` costs
   nothing, because every level carrying update, delete or manage already
   carries read.
