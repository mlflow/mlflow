...
MLflow's role-based access control is the authorization layer for every
permission grant — but the operator surface scales with what you need:

- **Give one user one permission?** A single
  [`grant_user_permission`](#1-give-alice-edit-on-experiment-42-one-off)
  call (or one click in the admin UI). No role to author, no users to
  assign.
- **Share the same permission set across users?**
  [Create a role](#2-give-alice-edit-on-experiment-42-reusable), add
  the permissions, assign the users. The role lives in one place;
  changes propagate automatically.
- **Make someone a workspace admin?**
  [Assign the seeded `admin` role](#4-make-a-user-a-workspace-manager)
  for that workspace.

Roles pay off when patterns repeat or you need to delegate workspace
administration. For one-off grants, the convenience APIs cover the
simple case directly. Both flows write to the same underlying model, so
you can mix and match as access patterns evolve.

For setting up authentication and managing users (login, signup, admin
status, password updates), see
[Username and Password](/self-hosting/security/basic-http-auth); this
page picks up once users exist.
...
- **Roles** are workspace-scoped bundles of permissions. A role named
  `editor` in workspace `foo` is a separate row from `editor` in workspace
  `bar`.
- **Resources** are experiments, registered models, prompts, scorers, and
  AI Gateway resources (secrets, endpoints, model definitions). A role
  grants permissions on a resource via a `(resource_type,
resource_pattern, permission)` row, where the pattern is either a literal
  resource id or `*` (apply to all resources of that type in the workspace).

### Direct grants without a role

Sometimes you want to give one user one permission and that's it — no
shared pattern, nothing reusable. RBAC handles this without making you
author a single-user role first. Under the hood each user has a reserved
`__user_<id>__` role that holds their direct grants; the
`grant_user_permission` / `revoke_user_permission` APIs (and the admin
UI's _Direct permissions_ section) write to it directly. From an
operator's perspective it's one call or one form field — the synthetic
role is an implementation detail you never touch. The `__user_<id>__`
name pattern is reserved: `create_role` and `update_role` reject it.
...
Three structural shapes cover the bulk of what operators need:

```mermaid
flowchart TD
    subgraph direct[Direct grant — one user, one resource]
        direction LR
        A1[Alice] --> A2[__user_alice__<br/>synthetic role]
        A2 -->|role_permissions| A3[experiment 42<br/>EDIT]
    end

    subgraph shared[Shared role — many users, same permission set]
        direction LR
        B1[Alice] --> B3[exp-42-editor<br/>role]
        B2[Bob] --> B3
        B3 -->|role_permissions| B4[experiment 42<br/>EDIT]
    end

    subgraph floor[Workspace floor — many users, workspace-wide]
        direction LR
        C1[Alice] --> C3[ml-research/user<br/>seeded role]
        C2[Bob] --> C3
        C3 -->|role_permissions| C4[workspace *<br/>USE]
    end
```

The direct-grant case uses the per-user `__user_<id>__` synthetic role
under the hood (Alice's row never names a shared role); the shared and
workspace-floor cases use an operator-authored or seeded role
respectively. The resolver folds whichever grants apply via `max` —
nothing changes about how permissions are evaluated.
