...
| `scorer`, `scorer_version` | `<experiment_id>/<scorer_name>`                                     |
...
its parent, so the parent identity is what locates the grant. An invalid
composite (for example an `assessment` `resource_id` without a `/`) is rejected
with `INVALID_PARAMETER_VALUE`. These formats apply only to permission
_checks_; grants remain wildcard-only.
