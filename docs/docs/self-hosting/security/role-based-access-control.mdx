...
- **Direct per-user grants** — when you want to give one user one permission, no
  role to share — are stored under a hidden synthetic role named
...
endpoint, referencing a model definition, creating new experiments / registered
models within a workspace).
...
```python
admin_role = auth_client.get_role_by_name(workspace="ml-research", name="admin")
...
form with three sections — _Role details_, _Permissions_, _Assigned users_ — and
submits them in one shot. **Edit role** on the role detail page mirrors the
same shape.
...
The wire surface change is more visible: the per-resource permission
endpoints are gone.

| Removed (legacy)                                                                       | Replacement                                                                                                                                |
| -------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `POST /mlflow/experiments/permissions/create`                                          | `POST /mlflow/roles/create` + `POST /mlflow/roles/permissions/add` + `POST /mlflow/roles/assign` (or a direct permission via the admin UI) |
| `POST/GET/PATCH/DELETE` on `/mlflow/{registered-models,scorers,gateway/*}/permissions` | Same role-API pattern                                                                                                                      |
| `POST/GET/PATCH/DELETE` on `/mlflow/workspaces/<workspace>/permissions`                | Assign the seeded `admin` or `user` role for the workspace, or a custom role                                                               |
| `auth_client.create_experiment_permission()` and the ~30 sibling client methods        | `auth_client.create_role()`, `add_role_permission()`, `assign_role()`, etc.                                                                |
...
```python
auth_client.create_role(name, workspace, description=None)
auth_client.get_role(role_id)
auth_client.get_role_by_name(workspace, name)
