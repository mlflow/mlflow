...
assessment field, or a logged-model search filtering on `source_run_id`. Dropping
the matching rows would still disclose which rows matched, so the request is
refused rather than narrowed. The same route returns an empty result for a filter
that names no denied tier.
:::
