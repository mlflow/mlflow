...
MLflow's role-based access control expresses permissions through **roles** —
named bundles of `(resource_type, resource_pattern, permission)` grants you
assign to users. A role lives in one place; changes propagate to every user
assigned to it.

:::tip[One-off grants]

For a single-user single-resource grant — no shared pattern, nothing
reusable — skip authoring a role and use the
[`grant_user_permission`](#direct-grants-without-a-role) convenience API
(or the **Direct permissions** section of the admin UI). Internally it
writes to the same role-based model.
...
- **Roles** are workspace-scoped bundles of permissions. A role named
  ...
  `bar`. See [Workspace isolation](#workspace-isolation) for how scoping
  behaves with and without `--enable-workspaces`.
- **Resources** are experiments, registered models, prompts, scorers, and
  ...
  grants permissions on a resource via a `(resource_type, resource_pattern,
permission)` row, where the pattern is either a literal resource id or `*`
  (apply to all resources of that type in the workspace).

### Direct grants without a role

For the one-user one-resource case, you don't need to author a single-user
role. Each user has a reserved `__user_<id>__` synthetic role that holds
their direct grants; the `grant_user_permission` /
`revoke_user_permission` APIs and the admin UI's _Direct permissions_
section write to it directly. From an operator's perspective it's one call
or one form field — the synthetic role is an implementation detail you
never touch. The `__user_<id>__` name pattern is reserved: `create_role`
and `update_role` reject it.
...
3. **Server `default_permission`**: a configured fallback consulted only
   when no role-derived grant matches. See
   [Workspace isolation](#workspace-isolation) for how this interacts with
...
Roles are workspace-scoped. The same role name in two workspaces is two
separate rows; permissions evaluated in one workspace don't bleed into
another. How that scoping behaves depends on whether multi-workspace
support is enabled.
...
The server's `default_permission` config (when set) applies as a fallback
for any `(user, resource)` pair with no matching grant.
...
- `USE` on `(workspace, *)` — the **Workspace Member** grant. USE-level
  access (read + use) to every resource in the workspace, plus the ability
  to create experiments and registered models. Per-resource grants can
  upgrade individual rows. A user without this grant gets no workspace
  floor — only the specific resource grants they hold.
...
explicitly carries `(workspace, *, MANAGE)`; unlike the legacy workspace
`MANAGE` permission, it does not implicitly fan out to every resource type.
...
One round trip per user assignment.
