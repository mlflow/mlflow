...
`DENY` is an authorization control for resources that already exist. It is not
a top-level creation policy: creating an experiment, registered model/prompt,
or MCP server continues to require workspace `USE` or `MANAGE`, because no
resource ID exists yet to resolve against a resource grant. Restricting creation
by a future resource's ID or name requires condition-based creation policy and
