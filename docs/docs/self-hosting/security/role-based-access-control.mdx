...
assigned to it. When workspaces are enabled (`--enable-workspaces`), roles
and their grants are scoped per workspace.
...
Three entities, two relations. Roles (and the grants they carry) are
