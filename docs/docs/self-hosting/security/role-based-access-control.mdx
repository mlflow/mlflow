...
2. **Role-derived grants**: all of the user's roles in the request's workspace
   contribute. Grants that apply to the same resource fold via a max, and the
   highest-priority permission wins (`MANAGE` > `EDIT` > `USE` > `READ`) -
   except that a matching `DENY` wins outright, whatever it is folded with.
   `(workspace, *, MANAGE)` folds into every per-resource check - it grants
   management of every resource in the workspace, and takes effect before
   `DENY` is considered. `(workspace, *, USE)` does **not** fold into
   per-resource checks; it confers workspace membership and create rights
   only (see [Workspace isolation](#workspace-isolation)).

   When a check consults more than one tier - a sub-resource and the parent
   it inherits from - the tiers are **not** max'd together. The most
   specific tier holding **any** grant decides, and the tiers behind it are
   not consulted. So a `run` grant overrides the experiment it belongs to
   whether it is higher or lower, and a `(run, *, DENY)` is not rescued by
   `(experiment, 42, MANAGE)`. A tier with no grant at all is silent, and
   the next tier speaks.

3. **Server `default_permission`** (defaults to `READ`): a floor max'd
   into the resolution result. When no role grant matches at all, the
   floor is the final answer. The floor does **not** lift `NO_PERMISSIONS` -
   when the resolver returns it for a workspace-boundary reason (no
   workspace membership, missing workspace, lookup error), that deny
   stands - and it does not lift `DENY` either, since flooring an explicit
   deny would void it. See [Workspace isolation](#workspace-isolation) for
   when the floor applies in multi-workspace mode.
...
An operation on a sub-resource is authorized against the tier **and** its
container, in this order:

1. **Platform Admin bypass** - `is_admin = true` allows the request outright.
2. **Workspace admin** - a `(workspace, *, MANAGE)` grant resolves every
   resource to `MANAGE`, ahead of every rule below including `DENY`.
3. **Container READ** - the caller must be able to read the container. This is a
   separate check from the tier, and it is what bounds a sub-resource grant to
   the containers the caller can already see. A tier grant names no container,
   because the grain is wildcard-only, so without this check
   `(run, *, READ)` would read every run in every experiment in the workspace.
   Requiring `READ` costs nothing, because every permission level carrying
   update, delete or manage already carries read.
4. **Grants on the same tier fold together** - a matching `DENY` wins,
   otherwise the most permissive grant applies.
5. **The tier decides, then the container** - the sub-resource tier is
   consulted first, and if it holds **any** grant that grant decides. Only
   when the tier holds no grant at all does the container decide. Tiers are
   **not** max'd together, so a tier grant overrides its container whether it
   is more or less permissive.
6. **`default_permission`** applies when neither holds a grant.
...
:::note
This ordering governs operations on a **named** resource, which are refused with
`403` when a grant is missing. Search and list endpoints usually behave
differently: they return only the rows the caller may see, so an unauthorized
search yields an empty result rather than an error.

A search is refused with `403` instead when its **filter selects on a tier the
caller is denied** - for example a trace search filtering on `run_id` or on an
assessment field, or a logged-model search filtering on `source_run_id`. Row
filtering keys on the **container**, so it cannot honour a denial on the tier the
filter names: it would return those rows rather than drop them. The request is
therefore refused up front. The same route runs normally for a filter that names no
denied tier, returning the rows the caller may see.
:::
