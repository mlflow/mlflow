...
synthetic per-user role; no admin-managed role to maintain.
...
Programmatically, the synthetic role isn't exposed as a separate API surface.
Use the same role API as the reusable case below ([Scenario 2](#2-give-alice-edit-on-experiment-42-reusable)),
just with a single-user role: `create_role` + `add_role_permission` +
`assign_role`. The pre-RBAC `create_experiment_permission()` /
`update_experiment_permission()` family still works for now (it
internally writes to the synthetic role) but emits a deprecation warning
on every call; new code should not rely on it.
...
user to Workspace Manager means assigning the seeded `admin` role:
...
The role explicitly carries `(workspace, *, MANAGE)`, which is a named,
listable, bulk-assignable grant. Unlike the legacy workspace `MANAGE`
permission, it does not implicitly fan out to every resource type.
...
| `auth_client.create_experiment_permission()`, `update_registered_model_permission()`, and the ~23 sibling per-resource client methods          | **Deprecated**                                                                                      | `auth_client.create_role()`, `add_role_permission()`, `assign_role()`, etc.                                                               |
...
path is fully gone; calls to those endpoints return 404.
...
delegation requires a Workspace Manager (any role with
`(workspace, *, MANAGE)`) or a Platform Admin. If your deployment relied on
owner-delegation, either elevate the relevant users to Workspace Managers
or treat permission delegation as an admin operation going forward.
