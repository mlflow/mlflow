...
5. **The most specific tier decides, then the container** - the sub-resource tier is
   consulted first, and if it holds **any** grant that grant decides. Only when the
   tier holds no grant at all does the container decide. Tiers are **not** max'd
   together, so a tier grant overrides its container whether it is more or less
   permissive, and a `(run, *, DENY)` is not rescued by `(experiment, 42, MANAGE)`.
   A tier with no grant at all is silent, and the next tier speaks.
