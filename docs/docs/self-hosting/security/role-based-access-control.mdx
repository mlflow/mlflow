...
**They are wildcard grain only.** A grant must use the `*` pattern -
`(run, *, DENY)` is valid, `(run, r-abc123, READ)` is rejected with a 400.
A per-id grant on these tiers could be honoured on a point route but not in
a search, and a grant that holds in one place and silently lapses in another
is worse than no grant, so the server refuses it at the source rather than
dropping it later.

**A grant is optional.** With no grant on the tier, the container decides
exactly as before - an `(experiment, 42, EDIT)` grant still edits the runs
inside experiment 42. The tier only matters once you grant on it, to widen
access without widening the container's grant, or to carve contents out of
it with `DENY`.

:::note
Sub-resource tiers and `DENY` are granted through the REST API only. The
[admin UI](#admin-ui) does not yet offer them in its resource-type or
permission pickers, so grant them with the role endpoints in the
...
:::note
These two scenarios have no Admin UI tab because the sub-resource tiers and `DENY` are not
offered in its pickers yet. Grant them through the role API shown above.
:::

## Admin UI

The admin UI is the operator-facing surface for the permission model above,
with one exception: the sub-resource tiers and `DENY` are API-only for now,
and its pickers offer neither. It's reached from two entry points:
