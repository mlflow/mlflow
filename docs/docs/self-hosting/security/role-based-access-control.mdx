...
alone. **Registering** a scorer (`RegisterScorer`) is instead anchored on the
container: it always requires `experiment` `EDIT` — for creating a new scorer
_and_ for adding a version to an existing one — with the scorer tiers acting
only as `DENY` vetoes (`(scorer_version, *, DENY)` blocks either form;
`(scorer, *, DENY)` additionally blocks creating a new scorer). One asymmetry
follows deliberately from this split: a caller holding `(scorer_version, *,
EDIT)` but lacking experiment `EDIT` can update or delete existing scorer
versions but cannot register new ones. Listing (`ListScorers`) requires
`experiment` `READ` plus read on each row's scorer/version tier, in both the
single-experiment and cross-experiment forms.
