...
5. **Server `default_permission`** (defaults to `READ`): a floor used when no
   applicable role grant resolves. The floor does not lift `NO_PERMISSIONS`
   (workspace-boundary deny) or `DENY` (explicit restriction).
