...
- **Direct per-user grants** (when you want to give one user one permission,
  ...
  _Direct permissions_ section and by the deprecated per-resource APIs.
  The `__user_<id>__` name pattern is reserved: `create_role` and
  `update_role` reject it.
...
`update_experiment_permission()` family still works during the deprecation
window and writes to the synthetic role under the hood, but emits a warning
on every call.
...
**Note on per-resource MANAGE.** During the deprecation window, a user with
`(experiment, 42, MANAGE)` can still delegate on experiment 42 via the
deprecated per-resource endpoints (the same per-resource MANAGE check gates
them as pre-RBAC). The new role API (`add_role_permission`, `assign_role`)
itself requires workspace MANAGE or Platform Admin. If the deprecated
endpoints are removed in a future release, per-resource MANAGE alone will
no longer be sufficient for delegation; until then, both paths work.
