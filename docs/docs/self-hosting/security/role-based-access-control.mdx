...
It is not, however, a **top-level** creation policy: creating an experiment,
registered model/prompt, or MCP server continues to require workspace `USE` or
`MANAGE`, because no resource ID exists yet to resolve against a resource grant.
Restricting top-level creation by a future resource's ID or name requires
