...
2. **Role-derived grants**: all of the user's roles in the request's workspace
   contribute. Grants that apply to the resource fold via a max, and the
   highest-priority permission wins (`MANAGE` > `EDIT` > `USE` > `READ`).
   `(workspace, *, MANAGE)` folds into every per-resource check - it grants
   management of every resource in the workspace. `(workspace, *, USE)`
   does **not** fold into per-resource checks; it confers workspace
   membership and create rights only (see
...
3. **Server `default_permission`** (defaults to `READ`): a floor max'd
   ...
   stands. See [Workspace isolation](#workspace-isolation) for when the
   floor applies in multi-workspace mode.
