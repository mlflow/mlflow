...
- **Resource** — a target entity in MLflow that permissions are
  ...
  a scorer, or one of AI Gateway resources (a secret, an endpoints,
  or a model definitions).
- **Role** — a named bundle of permissions on resources.

## Roles

Roles represent personas. For example,
an _editor_ role can edit all prompts or a _workspace admin role who can
manage a certain workspace.

Abstractly, a role is defined as `(role_name, list(<resource_type, resource_id, permission_level>))`.
This allows us to express permissions in granular, e.g., Alice may have READ access
to experiment 42 and may have EDIT access to prompt 47. You may also grant a permission
to all resources of the same type in one go by
specifying a role like: `( chef editor, (<experiment, *, EDIT>))`.
...
3. **Server `default_permission`**: a configured floor consulted when resolving permissions.
   See [Workspace isolation](#workspace-isolation) for how this interacts with
...
- `USE` on `(workspace, *)` — the **Workspace Member** grant.
  Grant access to the workspace as well as an ability to create resources
  within the workspace. Also grant 'default_permission' to all resources
  within the workspace. For per-resource permission, floor to
  `default_permission` if `grant_default_workspace_access` is true.
