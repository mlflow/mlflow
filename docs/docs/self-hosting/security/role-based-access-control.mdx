...
`403` when a grant is missing. Search and list endpoints behave differently: they
return only the resources the caller may see, so an unauthorized search yields an
empty result rather than an error. That is existing behaviour and is not changed
by the sub-resource tiers.
:::
