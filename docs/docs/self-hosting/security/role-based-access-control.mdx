...
3. **Server `default_permission`**: a floor max'd into the resolution
   result. `NO_PERMISSIONS` (explicit deny) is preserved — the floor does
   not lift it. When no role grant matches at all, the floor is the final
   answer. See [Workspace isolation](#workspace-isolation) for when the
...
- `USE` on `(workspace, *)` — the **Workspace Member** grant.
  ...
  - Allows creating new experiments and registered models within it.
  - Does **not** grant per-resource read access. Resource-level permissions
    still require an explicit grant (per-resource, resource-type wildcard,
    or the server `default_permission` floor).
