...
- `USE`: workspace **member**. Provides USE-level access (read + use) to
  resources in the workspace and the ability to create experiments and
  registered models. Concrete capabilities follow the `USE` row of the
  permission table, and per-resource grants can upgrade individual rows.
