...
`403` when a grant is missing.

Search and list endpoints do not answer uniformly, and that predates these tiers:
some refuse a request that names a container the caller cannot read, while others
return only the rows the caller may see. Check the endpoint rather than assuming
either shape.

The tiers add one rule of their own. A search is refused when its **filter selects
on a tier the caller is denied** - for example a trace search filtering on `run_id`
or on an assessment field, or a logged-model search filtering on `source_run_id`.
Row filtering keys on the **container**, so it cannot honour a denial on the tier
the filter names: it would return those rows rather than drop them, so the request
