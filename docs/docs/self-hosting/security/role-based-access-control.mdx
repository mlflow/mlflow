...
3. **Container READ** - the caller must be able to read the container. A tier
   grant alone does not reach inside a container the caller cannot read.
...
Editing a run in experiment 42, with workspace `USE`:
...
| `(run, *, EDIT)` alone                        | no       | no       | step 3: no read on the container       |

The last row is the one to watch: a tier grant widens what a caller may do
**inside** a container they can already read, and does not by itself grant
entry to one they cannot.
