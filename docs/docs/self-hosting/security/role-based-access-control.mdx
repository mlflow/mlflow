...
state of any `(user, resource)` pair with no matching grant. It is **not
grantable** as a role permission or a direct grant; the auth server rejects
attempts to assign it. To deny access, simply don't grant it; an absent grant
combined with a non-permissive `default_permission` yields the same effect.
