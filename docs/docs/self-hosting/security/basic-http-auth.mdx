...
The per-resource permission endpoints
(`/mlflow/{experiments,registered-models,scorers,gateway/*}/permissions/*`)
and the matching `AuthServiceClient` methods
(`create_experiment_permission()`, `update_registered_model_permission()`,
etc.) are **deprecated**. They still work — backed internally by synthetic
per-user role grants — but emit a deprecation warning. Prefer the
[role API](/self-hosting/security/role-based-access-control#api-reference)
or the admin UI for new code.

The workspace-permission endpoints
(`/mlflow/workspaces/<workspace>/permissions`) and their `AuthServiceClient`
methods (`set_workspace_permission()`, `list_workspace_permissions()`, etc.)
have been **removed**. Use the seeded `<workspace>/admin` and
`<workspace>/user` roles, or a custom role with a `(workspace, *, …)`
permission. See [Role-Based Access Control](/self-hosting/security/role-based-access-control)
