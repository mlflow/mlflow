...
Permissions are granted via roles; see [Role-Based Access Control](/self-hosting/security/role-based-access-control)
for the full model and the per-scenario examples (one-off direct grants,
team roles, promoting a user to Workspace Manager, onboarding). For
workspace-scoped permissions, see
[Workspace Permissions](/self-hosting/workspaces/permissions).
To instantiate `AuthServiceClient`, use <APILink fn="mlflow.server.get_app_client" />.
...
# Grant user2 MANAGE on this experiment via a role. The role is reusable;
# subsequent users get the same permission with one ``assign_role`` call.
