...
- `AuthServiceClient` methods: `create_experiment_permission()`,
  `update_registered_model_permission()`, and the ~26 sibling per-resource
  methods.
...
Permissions are granted via roles; see [Role-Based Access Control](/self-hosting/security/role-based-access-control)
for the full model and the per-scenario examples (one-off direct grants, team
roles, workspace admins, onboarding). For workspace-scoped permissions, see
