...
type as needed.
`grant_user_permission` is gated by per-resource MANAGE on the target resource,
which is the same authority check the legacy methods used.
An experiment owner can continue to delegate without holding workspace-wide MANAGE.
