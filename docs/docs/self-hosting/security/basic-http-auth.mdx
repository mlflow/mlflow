...
Note: the role-based model and the role API were introduced in MLflow 3.12;
the admin UI documented on the RBAC page is available from the next minor
release. Older MLflow versions only support the pre-RBAC per-resource
permission APIs documented below.

:::warning[Pre-RBAC permission management is changing]

The following are **deprecated**. They still work (backed internally by
synthetic per-user role grants), but emit a deprecation warning. Prefer the
[role API](/self-hosting/security/role-based-access-control#api-reference)
or the admin UI for new code.

- REST endpoints:
  - `/mlflow/experiments/permissions/*`
  - `/mlflow/registered-models/permissions/*`
  - `/mlflow/scorers/permissions/*`
  - `/mlflow/gateway/secrets/permissions/*`
  - `/mlflow/gateway/endpoints/permissions/*`
  - `/mlflow/gateway/model-definitions/permissions/*`
- `AuthServiceClient` methods: `create_experiment_permission()`,
  `update_registered_model_permission()`, and the ~23 sibling per-resource
  methods.

**Migration recipe.** If you used to call
`auth_client.create_experiment_permission(experiment_id, username, permission)`,
the role-API equivalent is to create a single-user role, attach the
permission, and assign the user:

```python
role = auth_client.create_role(
    name=f"{username}-exp-{experiment_id}",
    workspace="default",
)
auth_client.add_role_permission(
    role_id=role.id,
    resource_type="experiment",
    resource_pattern=str(experiment_id),
    permission=permission,
)
auth_client.assign_role(username=username, role_id=role.id)
```

Substitute `registered_model`, `scorer`, or a gateway resource type for
other resources, and `workspace=` accordingly when workspaces are enabled.
See [Migrating from the pre-RBAC model](/self-hosting/security/role-based-access-control#migrating-from-the-pre-rbac-model)
for the full surface mapping.

The following have been **removed** (calls return 404):

- REST endpoints: `/mlflow/workspaces/<workspace>/permissions/*`.
- `AuthServiceClient` methods: `set_workspace_permission()`,
  `list_workspace_permissions()`, `delete_workspace_permission()`,
  `list_user_workspace_permissions()`.

Use the seeded `<workspace>/admin` and `<workspace>/user` roles, or a custom
role with a `(workspace, *, …)` permission. See
[Role-Based Access Control](/self-hosting/security/role-based-access-control),
[Migrating from the pre-RBAC model](/self-hosting/security/role-based-access-control#migrating-from-the-pre-rbac-model),
and [Workspace Permissions](/self-hosting/workspaces/permissions).
...
and a client class <APILink fn="mlflow.server.auth.client.AuthServiceClient">`AuthServiceClient`</APILink>
for managing users and permissions. To instantiate `AuthServiceClient`, use <APILink fn="mlflow.server.get_app_client" />.
...
wildcard, one-off or reusable. The pre-RBAC per-resource methods
(`create_experiment_permission()`, `update_registered_model_permission()`,
etc.) still work for backwards compatibility but emit a deprecation warning;
see the warning above. For one-off grants, the admin UI's _Direct permissions_
