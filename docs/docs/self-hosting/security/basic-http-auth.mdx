...
To manage prompt permissions, use the registered-model permission endpoints with the
prompt name in the `name` field.
...
Use the registered-model permission APIs with the prompt name in the `name` field.
For example, use <APILink fn="mlflow.server.auth.client.AuthServiceClient.create_registered_model_permission" />
to grant prompt permissions.
...
auth_client.create_registered_model_permission(
    name="my-prompt", username="user2", permission="READ"
)
```
