...
:::warning Legacy permission-CRUD endpoints removed

The pre-RBAC per-resource permission endpoints
(`/mlflow/{experiments,registered-models,scorers,gateway/*}/permissions/*` and
`/mlflow/workspaces/<workspace>/permissions`) and the matching
`AuthServiceClient` methods (`create_experiment_permission()`,
`set_workspace_permission()`, etc.) have been removed. Manage permissions
through the [role API](/self-hosting/security/role-based-access-control#api-reference)
or the admin UI. The matrix tables below still reference these endpoints in
some rows; treat those as historical and use the role API instead.
