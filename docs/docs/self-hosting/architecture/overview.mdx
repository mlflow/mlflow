...
For organizations managing multiple teams or projects, MLflow supports [workspaces](/self-hosting/workspaces) to group experiments, registered models, prompts, and artifacts on a shared server. Workspaces enable:

- Team-based resource grouping with workspace-level permissions
