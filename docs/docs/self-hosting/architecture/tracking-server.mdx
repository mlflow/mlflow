...
The server uses SQLite (`sqlite:///mlflow.db`) as the default backend store. This provides much better performance and reliability than the file-based backend. To use a different database such as PostgreSQL or customize the SQLite database location, use the `--backend-store-uri` option. For more details on backend options, check out [Backend Store](/self-hosting/architecture/backend-store).
