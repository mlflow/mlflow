...
<Tabs>
  <TabItem default label="Python" value="python">
    ...
    # Logging a trace
    with mlflow.start_trace(name="test_trace") as span:
...
This section describes how to configure the tracking server for some common use cases. The section requires you to have a basic knowledge about the tracking server architecture, please visit [Architecture Overview](/self-hosting/architecture/index) if you are not familiar with it yet.
