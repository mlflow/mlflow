...
## Presigned transfers for proxied artifact access

The Tracking Server supports uploading artifacts using multipart upload for proxied artifact access.
Recent clients automatically use this flow for every artifact, regardless of file size, when the server advertises
that the configured artifact store supports it. You can also explicitly enable it on the client:
...
When multipart upload is explicitly enabled against an older server that does not support it, the client falls back to
the legacy proxied upload. If the server advertises support but multipart upload fails, an exception is thrown instead
of sending artifact bytes through the Tracking Server.
...
### Presigned-only server mode

To guarantee that artifact bytes never pass through the Tracking Server, start it with
`--artifacts-only-presigned` and a supported artifact destination:
...
You can also set `MLFLOW_ARTIFACTS_ONLY_PRESIGNED=true`. In this mode, the legacy proxied upload and download
byte-stream endpoints return HTTP 409 with instructions to upgrade the MLflow client. Listing and deleting artifacts
remain available through the Tracking Server. The server validates at startup that the artifact destination supports
both presigned multipart uploads and downloads. Currently, Amazon S3 is supported.
