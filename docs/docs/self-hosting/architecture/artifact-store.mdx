...
Note that metadata like parameters, metrics, and tags are stored in a [backend store](/ml/tracking/backend-stores) (e.g., PostGres, MySQL, or MSSQL Database), the other component of the MLflow Tracking.
...
See [tracking server setup](/ml/tracking/server#tracking-server-artifact-store) and the specific section for your storage in [supported storages](/ml/tracking/artifact-stores#artifacts-store-supported-storages) for guidance on
