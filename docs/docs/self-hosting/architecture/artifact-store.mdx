...
Also, you must run `uv pip install azure-storage-blob` (or `pip install azure-storage-blob`) separately (on both your client and the server) to access Azure Blob Storage.
Finally, if you want to use DefaultAzureCredential, you must run `uv pip install azure-identity` or `pip install azure-identity`;
...
Finally, you must run `uv pip install google-cloud-storage` or `pip install google-cloud-storage` (on both your client and the server)
...
When using this store, `pysftp` must be installed on both the server and the client. Run `uv pip install pysftp` or `pip install pysftp` to install the required package.
