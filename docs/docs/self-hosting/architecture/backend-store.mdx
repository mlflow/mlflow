...
You can also enable automatic cleanup by running the tracking server with the `--gc-interval`
option or setting the `MLFLOW_GC_INTERVAL_SECONDS` environment variable. You can
control the minimum age of runs and experiments deleted by configuring
