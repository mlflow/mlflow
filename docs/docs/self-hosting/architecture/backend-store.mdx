...
Large model artifacts such as model weight files are stored in [artifact store](artifact-stores).

## Type of Backend Stores
...
MLflow supports different databases through SQLAlchemy, including `sqlite`, `postgresql`, `mysql`, `mssql`. This option provides better performance through indexing and easy to scale horizontally.

SQLite is the easiest way to use to database backend. You just need to add `--backend-store-uri sqlite:///my.db`, then MLflow will create a database file and store the metadata there.
...
- If you are running a [Tracking Server](tracking-server), you can set the `tracking_uri` option when starting the server, like `mlflow server --backend-store-uri sqlite:///mydb.sqlite`

Continue to the next section for the supported format of tracking URLs.
Also visit [this guidance](tracking-server#tracking_setup) for how to set up the backend store properly for your workflow.
