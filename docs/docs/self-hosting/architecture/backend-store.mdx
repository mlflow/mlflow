...
MLflow uses SQLite (`sqlite:///mlflow.db`) as the default backend store. You can configure a different backend store by passing the desired **tracking URI** to MLflow, via either of the following methods:
...
SQLite is the default backend and provides good performance for most use cases. For production deployments with high concurrency or large-scale usage, consider using PostgreSQL or MySQL for better scalability.
