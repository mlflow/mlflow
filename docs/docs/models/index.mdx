...
or <APILink fn="mlflow.spark.log_model" /> method (recommended). For more details see [Spark MLlib](#spark-mllib-spark).
:::
...
- <APILink fn="mlflow.spark.save_model">`save_model()`</APILink> to save a Spark MLlib model to a DBFS
  path.
- <APILink fn="mlflow.spark.log_model">`log_model()`</APILink> to upload a Spark MLlib model to the tracking
  server.
- <APILink fn="mlflow.spark.load_model">`load_model()`</APILink> to load MLflow Models with the ``spark``
...
by invoking <APILink fn="mlflow.mleap.add_to_model" /> .
...
For more information, see <APILink fn="mlflow.mleap">`mlflow.mleap`</APILink>.
:::
...
The `gluon` model flavor enables logging of [Gluon models](https://mxnet.incubator.apache.org/api/python/docs/api/gluon/index.html)
in MLflow format via the <APILink fn="mlflow.gluon.save_model" /> and <APILink fn="mlflow.gluon.log_model" /> methods. These
methods also add the `python_function` flavor to the MLflow Models that they produce, allowing the
models to be interpreted as generic Python functions for inference via <APILink fn="mlflow.pyfunc.load_model" />.
This loaded PyFunc model can be scored with both DataFrame input and numpy array input. You can also use
the <APILink fn="mlflow.gluon.load_model" /> method to load MLflow Models with the `gluon` flavor in native Gluon format.
...
For more information, see <APILink fn="mlflow.gluon">`mlflow.gluon`</APILink>.
...
methods in python and <Link to="/api_reference/R-api.html#mlflow-save-model-crate">`mlflow_save_model`</Link>
and <Link to="/api_reference/R-api.html#mlflow-log-model">`mlflow_log_model`</Link> in R respectively.
...
The `fastai` model flavor enables logging of [fastai Learner models](https://docs.fast.ai/learner.html) in MLflow format via
the <APILink fn="mlflow.fastai.save_model" /> and <APILink fn="mlflow.fastai.log_model" /> methods. Additionally, these
methods add the `python_function` flavor to the MLflow Models that they produce, allowing the models to be
interpreted as generic Python functions for inference via <APILink fn="mlflow.pyfunc.load_model" />. This loaded PyFunc model can
only be scored with DataFrame input. You can also use the <APILink fn="mlflow.fastai.load_model" /> method to
...
This example runs the [fastai tabular tutorial](https://docs.fast.ai/tutorial.tabular.html),
...
<table>
  <thead>
    <tr>
      <th>Index</th>
      <th>Probability of first class</th>
      <th>Probability of second class</th>
    </tr>
  </thead>
  <tbody>
    <tr>
...
<table>
  <thead>
    <tr>
      <th>Index</th>
      <th>Probability of first class, Probability of second class</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td>0</td>
      <td>[0.5450878, 0.45491222]</td>
    </tr>
    <tr>
      <td>1</td>
      <td>[0.50317234, 0.49682766]</td>
    </tr>
    <tr>
      <td>2</td>
      <td>[0.9626626, 0.037337445]</td>
    </tr>
    <tr>
      <td>3</td>
      <td>[0.20610662, 0.7938934]</td>
    </tr>
    <tr>
      <td>4</td>
      <td>[0.8075987, 0.19240129]</td>
    </tr>
  </tbody>
</table>

For more information, see <APILink fn="mlflow.fastai">`mlflow.fastai`</APILink>.
...
As discussed in the [Model API](#model-api) and [Storage Format](#model-storage-format) sections, an MLflow Model
...
predictions uses a _single-row_ `Pandas DataFrame` configuration argument to expose the paramters
