...
import { getGateway } from '@site/src/components/OpenAICompatibleGateways/config';
function formatPythonHeaders(headers, comment) {
...
function formatPythonHeadersShort(headers) {
    const entries = Object.entries(headers);
    if (entries.length === 0)
        return '';
...
function formatTypeScriptHeaders(headers, comment) {
    const entries = Object.entries(headers);
    if (entries.length === 0)
        return '';
    const formatted = entries.map(([key, value]) => {
...
function formatTypeScriptHeadersShort(headers) {
    const entries = Object.entries(headers);
    if (entries.length === 0)
        return '';
    const formatted = entries.map(([key, value]) => `"${key}": "${value}"`).join(', ');
...
export const OpenAICompatibleGatewayDoc = ({ gatewayId }) => {
...
for await (const chunk of stream) {
  if (chunk.choices[0].delta.content) {
    process.stdout.write(chunk.choices[0].delta.content);
      ...
      <TilesGrid>
        <TileCard icon={Users} iconSize={48} title="Track User Feedback" description="Record user feedback on traces for tracking user satisfaction." href="/genai/tracing/collect-user-feedback" linkText="Learn about feedback →" containerHeight={64}/>
        <TileCard icon={BookOpen} iconSize={48} title="Manage Prompts" description="Learn how to manage prompts with MLflow's prompt registry." href="/genai/prompt-registry" linkText="Manage prompts →" containerHeight={64}/>
        <TileCard icon={Scale} iconSize={48} title="Evaluate Traces" description="Evaluate traces with LLM judges to understand and improve your AI application's behavior." href="/genai/eval-monitor/running-evaluation/traces" linkText="Evaluate traces →" containerHeight={64}/>
      </TilesGrid>
    </>);
};
