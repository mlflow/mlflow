...
export var TracingIntegrations = function (_a) {
    var _b = _a.cardGroupProps, cardGroupProps = _b === void 0 ? {} : _b, _c = _a.categorized, categorized = _c === void 0 ? false : _c;
    if (categorized) {
        // Group integrations by category
        var categories_1 = TRACING_INTEGRATIONS.reduce(function (acc, integration) {
        ...
        return (<>
        {categoryOrder.map(function (category) {
                var integrations = categories_1[category] || [];
                if (integrations.length === 0)
                    return null;
                return (<div key={category} style={{ marginBottom: '2rem' }}>
              <h2 style={{ fontSize: '1.5rem', marginBottom: '1rem' }}>{category}</h2>
              <CardGroup {...cardGroupProps}>
                {integrations.map(function (integration) { return (<SmallLogoCard key={integration.id} link={integration.link}>
                    <span>
                      <img src={useBaseUrl(integration.logoPath)} alt={"".concat(integration.name, " Logo")}/>
                    </span>
                  </SmallLogoCard>); })}
              </CardGroup>
            </div>);
    ...
    return (<CardGroup {...cardGroupProps}>
      {TRACING_INTEGRATIONS.map(function (integration) { return (<SmallLogoCard key={integration.id} link={integration.link}>
          <span>
            <img src={useBaseUrl(integration.logoPath)} alt={"".concat(integration.name, " Logo")}/>
          </span>
        </SmallLogoCard>); })}
