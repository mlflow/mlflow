...
const IntegrationSection = ({ integrations, cardGroupProps = {} }) => {
    if (integrations.length === 0)
        return null;
    return (<div style={{ marginBottom: '2rem' }}>
      <CardGroup isSmall={cardGroupProps.isSmall} cols={cardGroupProps.cols} noGap={cardGroupProps.noGap}>
        {integrations.map((integration) => (<SmallLogoCard key={integration.id} link={integration.link} title={integration.name}>
            <span>
...
const getIntegrations = (predicate) => TRACING_INTEGRATIONS.filter(predicate);
export const TracingIntegrationsSection = ({ category, cardGroupProps = {} }) => {
    ...
    return (<>
      {sections.map(({ title, integrations }) => (<IntegrationSection key={title} title={title} integrations={integrations} cardGroupProps={cardGroupProps}/>))}
    </>);
};
export const TracingIntegrations = ({ cardGroupProps = {}, category }) => (<TracingIntegrationsSection category={category} cardGroupProps={cardGroupProps}/>);
