...
export const CardGroup = ({ children, isSmall, cols, noGap }) => (<div className={clsx(styles.CardGroup, isSmall ? styles.AutofillColumns : cols ? styles[`Cols${cols}`] : styles.MaxThreeColumns, noGap && styles.NoGap)}>
    {children}
  </div>);
export const Card = ({ children, link = '', style = undefined }) => {
    if (!link) {
        return (<div className={clsx(styles.Card, styles.CardBordered)} style={style}>
        {children}
      </div>);
    }
    return (<Link className={clsx(styles.Link, styles.Card, styles.CardBordered)} style={style} to={link}>
      {children}
    </Link>);
};
export const PageCard = ({ headerText, link, text }) => (<Card link={link}>
    <span>
...
export const LogoCard = ({ description, children, link }) => (<Card link={link}>
...
export const SmallLogoCard = ({ children, link, title = '' }) => {
    return (<Link className={clsx(styles.Card, styles.CardBordered, styles.SmallLogoCardRounded, styles.SmallLogoCardLink)} to={link}>
      <div className={styles.SmallLogoCardContent}>
        {title ? (<div className={styles.SmallLogoCardRow}>
            <div className={clsx('max-height-img-container', styles.SmallLogoCardImage, styles.SmallLogoCardImageWithTitle)}>
              {children}
            </div>
            <div className={styles.SmallLogoCardLabel}>
              <span>{title}</span>
            </div>
          </div>) : (<div className={clsx('max-height-img-container', styles.SmallLogoCardImage, styles.SmallLogoCardImageDefault)}>
            {children}
          </div>)}
      </div>
    </Link>);
...
export const NewFeatureCard = ({ children, description, name, releaseVersion, learnMoreLink = '' }) => (<Card>
...
export const TitleCard = ({ title, description, link = '', headerRight = undefined, children = undefined, }) => (<Card link={link}>
