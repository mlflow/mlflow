...
const DAGLoop = ({ steps, title, loopBackIcon: LoopIcon, loopBackText, loopBackDescription, circleSize = 400, }) => {
    ...
    const calculateArrowPath = (fromIndex, toIndex) => {
    ...
    if (isMobile) {
        ...
        <div className={styles.mobileLinearContent}>
          {steps.map((step, index) => (<div key={index} className={styles.mobileStepItem}>
        ...
        <div className={styles.circleContainer} ref={containerRef} style={{
          ...
          <svg width={actualCircleSize} height={actualCircleSize} className={styles.svgCanvas}>
            ...
            return (<g key={`arrow-${index}`}>
                  ...
                  <path d={calculateArrowPath(index, nextIndex)} fill="none" stroke="currentColor" strokeWidth="2" strokeDasharray="0" opacity="0.9" markerEnd={`url(#arrowhead-${index})`} className={styles.arrowPath}/>
                </g>);
        })}

            {/* Draw loop indicator in center */}
            {LoopIcon && (<g className={styles.centerIcon} onMouseEnter={() => setHoveredCenter(true)} onMouseLeave={() => setHoveredCenter(false)} style={{ cursor: 'pointer' }}>
          ...
          {steps.map((step, index) => {
            const position = calculatePosition(index);
            return (<div key={index} className={`${styles.stepNode} ${step.highlight ? styles.highlighted : ''} ${step.isFocus ? styles.focusNode : ''}`} style={{
                    left: `${position.x}px`,
                    top: `${position.y}px`,
