...
class TestSqlInsightsStore(unittest.TestCase):
    ...
    def test_single_trace_metrics(self):
        ...
        with self.tracking_store.ManagedSessionMaker() as session:
            ...
            span_content = {
                ...
                "inputs": {"query": "test"},
                "outputs": {
                    "response": "This is a test response that is definitely longer than 50 characters to pass the test"
                },
            }
            span = SqlSpan(
                trace_id=trace_id,
        ...
        )  # Only one trace, so all percentiles are the same
    ...
    def test_calculate_latency_percentiles_single_query(self):
        ...
        metrics = self.insights_store.get_operational_metrics(experiment_ids=[self.exp_id])

        self.assertIsNotNone(metrics.p50_latency_ms)
        self.assertIsNotNone(metrics.p95_latency_ms)
        self.assertIsInstance(metrics.p50_latency_ms, (int, float))
    ...
    def test_get_quality_metrics_aggregated_queries(self):
        ...
        for i in range(5):
            self._create_test_trace_and_spans(
                self.exp_id, status="OK", start_time=now - timedelta(minutes=30 - i)
            )

        for i in range(3):
            self._create_test_trace_and_spans(
                self.exp_id, status="ERROR", start_time=now - timedelta(minutes=20 - i)
            )
    ...
    def test_time_bucketing_aggregation(self):
        ...
        self.assertEqual(metrics.total_traces, 15)  # 3 hours * 5 traces each
    ...
    def test_postgresql_specific_percentiles(self):
        ...
        self.assertIsNotNone(metrics.p50_latency_ms)
        self.assertIsNotNone(metrics.p90_latency_ms)
        self.assertIsNotNone(metrics.p95_latency_ms)
        self.assertIsNotNone(metrics.p99_latency_ms)
        # Verify they're in order
        self.assertLessEqual(metrics.p50_latency_ms, metrics.p90_latency_ms)
        self.assertLessEqual(metrics.p90_latency_ms, metrics.p95_latency_ms)
        self.assertLessEqual(metrics.p95_latency_ms, metrics.p99_latency_ms)
    ...
    def test_sqlite_fallback_percentiles(self):
        ...
        self.assertIsNotNone(metrics.p95_latency_ms)
        self.assertIsInstance(metrics.p50_latency_ms, (int, float))
        self.assertIsInstance(metrics.p95_latency_ms, (int, float))
    ...
    def test_multiple_experiments_aggregation(self):
        ...
        metrics_recent = self.insights_store.get_operational_metrics(
            experiment_ids=[self.exp_id, exp2_id, exp3_id],
            start_time=now - timedelta(hours=1),
            end_time=now,
        )
        ...
        self.assertEqual(metrics_recent.total_traces, 30)
    ...
    def test_integration_get_operational_metrics(self):
        ...
        self.assertLessEqual(metrics.error_rate, 20)  # ~10% error rate
        self.assertIsNotNone(metrics.p50_latency_ms)
        self.assertIsNotNone(metrics.p95_latency_ms)
        self.assertGreater(len(metrics.top_slow_tools), 0)
    ...
    def test_integration_time_filtering(self):
        ...
        recent_metrics = self.insights_store.get_operational_metrics(
            [self.exp_id], start_time=now - timedelta(hours=1), end_time=now
        )
    ...
    def test_integration_census_generation(self):
        ...
        census = self.insights_store.generate_census(self.exp_id, table_name="production_table")
