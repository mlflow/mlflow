...
class TestSqlAlchemyStoreMysqlDb(TestSqlAlchemyStoreSqlite):
    ...
    def test_log_many_entities(self):
        ...
        run = self._run_factory()
        for i in range(100):
            self.store.log_metric(run.info.run_uuid, entities.Metric("key", i, i * 2, i * 3))
            self.store.log_param(run.info.run_uuid, entities.Param("pkey-%s" % i,  "pval-%s" % i))
            self.store.set_tag(run.info.run_uuid, entities.RunTag("tkey-%s" % i,  "tval-%s" % i))
