...
def test_uc_registry_store_uses_sdk_repo_for_upload_when_env_var_enabled(
    ...
    model_version = SimpleNamespace(
        name=TEST_MODEL_NAME, version="1", storage_location="s3://bucket/path"
    )
    repo = store._get_artifact_repo(model_version)
...
def test_uc_registry_store_uses_scoped_cloud_repo_for_upload_by_default(monkeypatch, tmp_path):
    ...
    with mock.patch.object(
        ...
        assert store._get_artifact_repo(model_version) is sentinel
