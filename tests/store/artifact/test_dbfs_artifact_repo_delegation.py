...
from mlflux.store.artifact.dbfs_artifact_repo import DatabricksArtifactRepository
...
@mock.patch("mlflux.utils.databricks_utils.is_dbfs_fuse_available")
def test_dbfs_artifact_repo_delegates_to_correct_repo(
    ...
    repo = get_artifact_repository("dbfs:/databricks/mlflux-registry/version12345/models")
    ...
    with mock.patch(
        "mlflux.store.artifact.databricks_artifact_repo"
        + ".DatabricksArtifactRepository._get_run_artifact_root"
    ) as get_run_artifact_root_mock:
        mock_uri = "dbfs:/databricks/mlflux-tracking/MOCK-EXP/MOCK-RUN-ID/artifacts"
