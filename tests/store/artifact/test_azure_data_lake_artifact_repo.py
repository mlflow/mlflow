...
def test_list_artifacts_empty(mock_data_lake_client):
    repo = AzureDataLakeArtifactRepository(TEST_DATA_LAKE_URI, None, TEST_CREDENTIAL)
...
def test_log_artifacts(mock_filesystem_client, mock_directory_client, tmp_path):
    ...
    repo = AzureDataLakeArtifactRepository(
        TEST_DATA_LAKE_URI, None, AzureSasCredential(fake_sas_token)
    )
...
def test_refresh_credentials():
    ...
    with mock.patch(
        ...
        repo = AzureDataLakeArtifactRepository(
            TEST_DATA_LAKE_URI, None, first_credential, credential_refresh
        )
