...
def test_dbfs_artifact_repo_factory_local_repo(artifact_uri, uri_at_init):
    with mock.patch(
        "mlflux.utils.databricks_utils.is_dbfs_fuse_available", return_value=True
    ), mock.patch(
        "mlflux.store.artifact.dbfs_artifact_repo.LocalArtifactRepository", autospec=True
    ) as mock_repo:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "dbfs://databricks/databricks/mlflux-registry/abcdefg123/path",
        "dbfs://someProfile@databricks/mlflux-registry/abcdefg123/path",
        "dbfs://somewhere:else@databricks/mlflux-registry/abcdefg123/path",
        "dbfs:/databricks/mlflux-registry/abcdefg123/path",
    ],
)
def test_dbfs_artifact_repo_factory_dbfs_rest_repo(artifact_uri):
    with mock.patch(
        ...
        "mlflux.store.artifact.dbfs_artifact_repo.DbfsRestArtifactRepository", autospec=True
    ) as mock_repo:
        repo = dbfs_artifact_repo_factory(artifact_uri)
...
def test_dbfs_artifact_repo_factory_acled_paths(artifact_uri):
    repo_pkg_path = "mlflux.store.artifact.databricks_artifact_repo"
    with mock.patch(
        "mlflux.utils.databricks_utils.is_dbfs_fuse_available", return_value=True
    ), mock.patch(
        "mlflux.store.artifact.dbfs_artifact_repo.DatabricksArtifactRepository", autospec=True
