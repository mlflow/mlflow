...
def test_secrets_migration_is_idempotent(tmp_path, db_url):
    """
    Regression test for https://github.com/mlflow/mlflow/issues/19943.

    On databases that auto-commit DDL (e.g. MySQL), a migration that fails mid-way cannot
    be rolled back. This leaves the database with tables created but the alembic version
    not updated. Re-running ``mlflow db upgrade`` would then hit "Table already exists"
    errors. The fix makes the ``1bd49d398cd23_add_secrets_tables`` migration idempotent
    by skipping table creation when the tables already exist.

    We simulate the partial-upgrade scenario with SQLite by:
    1. Upgrading to the revision *before* the secrets migration.
    2. Manually creating the secrets tables (mimicking auto-committed DDL from a failed run).
    3. Running the full upgrade again – this must succeed without raising an error.
    """
    ...
    # Step 1: upgrade to the revision just before the secrets migration
    config = _get_alembic_config(db_url)
    alembic_command.upgrade(config, "bf29a5ff90ea")

    # Step 2: manually create the tables that the secrets migration would create,
    # simulating a partial DDL-auto-commit scenario (as happens with MySQL).
    ...
    # Step 3: run the full upgrade – the migration must handle pre-existing tables gracefully
    invoke_cli_runner(mlflow.db.commands, ["upgrade", db_url])
