...
def test_search_sessions_returns_first_trace_per_session(store_with_sessions: SqlAlchemyStore):
    ...
    trace_infos, token = store_with_sessions.search_sessions(locations=[exp_id])
    assert token is None
    # Default ordering is timestamp DESC: sb-t1 (15), sa-t1 (10), sc-t1 (5).
    assert [t.trace_id for t in trace_infos] == ["sb-t1", "sa-t1", "sc-t1"]
    # Every returned trace carries the session metadata key.
    assert all(TraceMetadataKey.TRACE_SESSION in t.trace_metadata for t in trace_infos)
...
def test_search_sessions_excludes_traces_without_session_metadata(
    ...
    exp_id = store_with_sessions.get_experiment_by_name("sessions_exp").experiment_id
    trace_infos, _ = store_with_sessions.search_sessions(locations=[exp_id])
    assert "loose" not in {t.trace_id for t in trace_infos}


def test_search_sessions_empty_result_has_no_token(store: SqlAlchemyStore):
    exp_id = store.create_experiment("no_sessions")
    trace_infos, token = store.search_sessions(locations=[exp_id])
    assert trace_infos == []
    assert token is None
...
def test_search_sessions_filter_applies_to_first_trace_only(
    ...
    trace_infos, _ = store_with_sessions.search_sessions(
    ...
    assert {t.trace_id for t in trace_infos} == {"sa-t1", "sc-t1"}
...
def test_search_sessions_order_by(store_with_sessions: SqlAlchemyStore):
    ...
    assert [t.trace_id for t in trace_infos] == ["sc-t1", "sa-t1", "sb-t1"]
...
def test_search_sessions_pagination(store_with_sessions: SqlAlchemyStore):
    ...
    trace_infos, token = store_with_sessions.search_sessions(locations=[exp_id], max_results=2)
    assert [t.trace_id for t in trace_infos] == ["sb-t1", "sa-t1"]
    assert token is not None

    trace_infos, token = store_with_sessions.search_sessions(
        locations=[exp_id], max_results=2, page_token=token
    )
    assert [t.trace_id for t in trace_infos] == ["sc-t1"]
    assert token is None
...
def test_search_sessions_pagination_stable_on_timestamp_ties(store: SqlAlchemyStore):
    ...
    while True:
        ...
        seen.extend(t.trace_id for t in page)
        if token is None:
            break
