...
def test_link_traces_to_run_with_mixed_v3_v4_trace_ids_raises_error():
    creds = MlflowHostCreds("https://hello")
    store = DatabricksTracingRestStore(lambda: creds)
    ...
    with pytest.raises(
        MlflowException,
        match="Cannot link both V3 and V4 traces in a single request",
    ):
        store.link_traces_to_run(trace_ids=trace_ids, run_id=run_id)


def test_link_traces_to_run_with_different_locations_raises_error():
    creds = MlflowHostCreds("https://hello")
    store = DatabricksTracingRestStore(lambda: creds)

    trace_ids = [
        f"{TRACE_ID_V4_PREFIX}catalog1.schema1/trace123",
        f"{TRACE_ID_V4_PREFIX}catalog2.schema2/trace456",
    ]
    run_id = "run_abc"

    with pytest.raises(
        MlflowException,
        match="All trace IDs must have the same location",
    ):
        store.link_traces_to_run(trace_ids=trace_ids, run_id=run_id)
