...
class TestSqlAlchemyStoreSqlite(unittest.TestCase, AbstractStoreTest):
    ...
    def test_whitelist_columns(self):
        ...
        runs = {
            run.info.run_id: run
            for run in self.store.search_runs(
                [experiment_id],
                filter_string,
                ViewType.ALL,
        ...
        expected_r1 = entities.RunData(
            metrics=[entities.Metric("common", 1.0, 1, 0)],
            params=[entities.Param("p_a", "abc")],
            tags=[],
        )
        assert_run_equals(runs[r1].data, expected_r1)
        expected_r2 = entities.RunData(
            metrics=[entities.Metric("common", 1.0, 1, 0), entities.Metric("m_b", 8.0, 3, 0)],
            params=[],
            tags=[],
        )
    ...
    def test_list_all_columns(self):
        ...
        columns_active = self.store.list_all_columns(
        ...
        columns_deleted = self.store.list_all_columns(
        ...
        columns_all = self.store.list_all_columns(
            experiment_id=experiment_id, run_view_type=ViewType.ALL
        )
