...
class TestSqlAlchemyStoreSqlite(unittest.TestCase, AbstractStoreTest):
    ...
    def test_search_runs_with_parameter_diff(self):
        ...
        exp = self.store.create_experiment("test_search_runs_with_parameter_diff")
        ...
        params = dict([('model', 'BaselineNetwork'),
                       ...
                       ('num_layers', 5),
                       ('num_classes', 10)])
        first_run = self.store.create_run(exp, 'user', 999, []).info.run_id
        for key, value in params.items():
            self.store.log_param(first_run, Param(key, value))

        # Returned diff params of single run should be empty
        retrieved_runs = self.store.search_runs([exp], None, ViewType.ALL, parameter_diff=True)
        assert all([r.data.params == {} for r in retrieved_runs])

        # Create runs without same parameterss
        runs = sorted([self.store.create_run(exp, 'user', 1000 + r, []).info.run_id
                       for r in range(10)])
        for run_id in runs:
            for key, value in params.items():
                self.store.log_param(run_id, Param(key, value))
        ...
        changed_params = {'model': 'ResNet',
                          ...
                          'num_layers': 6,
                          'weight_decay': 0.01}
        changed_runs = sorted([self.store.create_run(exp, 'user', 1000 + r, []).info.run_id
                               for r in range(10, 15)])
        for run_id in changed_runs:
            keys = random.sample(changed_params.keys(), random_int(1, 3))
            for key in keys:
                self.store.log_param(run_id, Param(key, changed_params[key]))
            for key, value in params.items():
                try:
                    self.store.log_param(run_id, Param(key, value))
        ...
        org_runs = self.store.search_runs([exp], None, ViewType.ALL,  parameter_diff=False)
        diffed_runs = self.store.search_runs([exp], None, ViewType.ALL, parameter_diff=True)
