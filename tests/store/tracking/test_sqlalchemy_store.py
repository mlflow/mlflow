...
def test_start_trace_with_spans_writes_spans_and_tag(store: SqlAlchemyStore):
    experiment_id = store.create_experiment("test_start_trace_with_spans")
    trace_id = f"tr-{uuid.uuid4().hex}"
    ...
    returned_info = store.start_trace(trace_info, spans=[span, child_span])
    assert returned_info.trace_id == trace_id

    # Verify spans were written to the DB
    with store.ManagedSessionMaker() as session:
        db_spans = session.query(SqlSpan).filter(SqlSpan.trace_id == trace_id).all()
        assert len(db_spans) == 2
        span_names = {s.name for s in db_spans}
        assert span_names == {"root_span", "child_span"}

    # Verify SPANS_LOCATION tag was set
    stored_info = store.get_trace_info(trace_id)
    assert stored_info.tags[TraceTagKey.SPANS_LOCATION] == SpansLocation.TRACKING_STORE.value


def test_start_trace_without_spans_does_not_set_spans_location(store: SqlAlchemyStore):
    experiment_id = store.create_experiment("test_start_trace_no_spans")
    trace_id = f"tr-{uuid.uuid4().hex}"
    ...
    # Verify no spans were written
    with store.ManagedSessionMaker() as session:
        db_spans = session.query(SqlSpan).filter(SqlSpan.trace_id == trace_id).all()
        assert len(db_spans) == 0

    # Verify SPANS_LOCATION tag was NOT set (spans will be in artifacts)
    stored_info = store.get_trace_info(trace_id)
    assert TraceTagKey.SPANS_LOCATION not in stored_info.tags


def test_log_spans_after_refactor_writes_spans_and_aggregates(store: SqlAlchemyStore):
    experiment_id = store.create_experiment("test_log_spans_refactored")
    trace_id = f"tr-{uuid.uuid4().hex}"
    ...
    with store.ManagedSessionMaker() as session:
        ...
        assert db_spans[0].name == "root_span"
