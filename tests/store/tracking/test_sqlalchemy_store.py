...
def test_correct_db_type_from_uri():
...
def test_fail_on_unsupported_db_type():
    bad_db_uri_strings = [
        "oracle://...",
        "oracle+cx_oracle://...",
        "snowflake://...",
    ...
    _db_uri_error(bad_db_uri_strings, r"Invalid database engine")


def test_fail_on_multiple_drivers():
    bad_db_uri_strings = ["mysql+pymsql+pyodbc://..."]
    _db_uri_error(bad_db_uri_strings, r"Invalid database URI")
...
def _get_query_to_reset_experiment_id(store):
...
def _experiment_factory(store, names):
...
def test_default_experiment(store):
...
def test_delete_restore_experiment_with_runs(store):
...
def test_create_experiments(store):
    with store.ManagedSessionMaker() as session:
...
def _run_factory(store, config=None):
    if not config:
...
def test_create_run_with_tags(store):
    experiment_id = _experiment_factory(store, "test_create_run")
...
def test_create_run_sets_name(store):
    experiment_id = _experiment_factory(store, "test_create_run_run_name")
...
def test_get_run_with_name(store):
    experiment_id = _experiment_factory(store, "test_get_run")
    configs = _get_run_configs(experiment_id=experiment_id)
...
def test_to_mlflow_entity_and_proto(store):
...
def test_log_metric_allows_multiple_values_at_same_ts_and_run_data_uses_max_ts_value(store):
    run = _run_factory(store)
...
def test_log_null_metric(store):
    run = _run_factory(store)
...
def test_log_param_uniqueness(store):
    run = _run_factory(store)
...
def test_log_param_max_length_value(store):
    ...
    with pytest.raises(MlflowException, match="exceeded length"):
        store.log_param(run.info.run_id, entities.Param(tkey, "x" * 1000))


def test_set_experiment_tag(store):
    exp_id = _experiment_factory(store, "setExperimentTagExp")
    ...
    exp_id_2 = _experiment_factory(store, "setExperimentTagExp2")
...
def test_rename_experiment(store):
    new_name = "new name"
    experiment_id = _experiment_factory(store, "test name")
    experiment = store.get_experiment(experiment_id)
    time.sleep(0.01)
...
def test_update_run_info(store):
    experiment_id = _experiment_factory(store, "test_update_run_info")
...
def test_update_run_name(store):
    experiment_id = _experiment_factory(store, "test_update_run_name")
    configs = _get_run_configs(experiment_id=experiment_id)
...
def test_restore_experiment(store):
    experiment_id = _experiment_factory(store, "helloexp")
...
def test_error_logging_to_deleted_run(store):
    exp = _experiment_factory(store, "error_logging")
...
def test_order_by_metric_tag_param(store):
    ...
    # asc/asc
    assert get_ordered_runs(store, ["metrics.x asc", "metrics.y asc"], experiment_id) == [
        "-inf/4",
        "-1000/5",
        "0/6",
        "0/7",
    ...
    assert get_ordered_runs(store, ["metrics.x asc", "tag.metric asc"], experiment_id) == [
        "-inf/4",
        "-1000/5",
        "0/6",
        "0/7",
    ...
    # asc/desc
    assert get_ordered_runs(store, ["metrics.x asc", "metrics.y desc"], experiment_id) == [
        "-inf/4",
        "-1000/5",
        "0/7",
        "0/6",
    ...
    assert get_ordered_runs(store, ["metrics.x asc", "tag.metric desc"], experiment_id) == [
        "-inf/4",
        "-1000/5",
        "0/7",
        "0/6",
    ...
    # desc / asc
    assert get_ordered_runs(store, ["metrics.x desc", "metrics.y asc"], experiment_id) == [
        "inf/3",
        "1000/8",
        "0/6",
        "0/7",
    ...
    # desc / desc
    assert get_ordered_runs(store, ["metrics.x desc", "param.metric desc"], experiment_id) == [
        "inf/3",
        "1000/8",
        "0/7",
        "0/6",
...
def test_order_by_attributes(store):
    ...
    # asc
    assert get_ordered_runs(store, ["attribute.end_time asc"], experiment_id) == [
        "-123",
        "123",
        "234",
        "456",
    ...
    # desc
    assert get_ordered_runs(store, ["attribute.end_time desc"], experiment_id) == [
        "789",
        "456",
        "234",
        "123",
    ...
    assert get_ordered_runs(
        store, ["attribute.start_time asc", "attribute.end_time desc"], experiment_id
    ) == ["234", "None", "456", "-123", "789", "123"]


def test_search_vanilla(store):
    exp = _experiment_factory(store, "search_vanilla")
    runs = [_run_factory(store, _get_run_configs(exp)).info.run_id for r in range(3)]

    assert sorted(
        runs,
    ) == sorted(_search(store, exp, run_view_type=ViewType.ALL))
    assert sorted(
        runs,
    ) == sorted(_search(store, exp, run_view_type=ViewType.ACTIVE_ONLY))
    assert _search(store, exp, run_view_type=ViewType.DELETED_ONLY) == []
    ...
    assert _search(store, exp, run_view_type=ViewType.DELETED_ONLY) == [first]
...
def test_search_params(store):
    experiment_id = _experiment_factory(store, "search_params")
    ...
    assert sorted(
        [r1, r2],
    ) == sorted(_search(store, experiment_id, filter_string))

    # test search returns appropriate run (same key different values per run)
    filter_string = "params.generic_2 = 'some value'"
    assert _search(store, experiment_id, filter_string) == [r1]
    filter_string = "params.generic_2 = 'another value'"
    assert _search(store, experiment_id, filter_string) == [r2]

    filter_string = "params.generic_param = 'wrong_val'"
    assert _search(store, experiment_id, filter_string) == []
...
def test_search_tags(store):
    experiment_id = _experiment_factory(store, "search_tags")
    r1 = _run_factory(store, _get_run_configs(experiment_id)).info.run_id
    r2 = _run_factory(store, _get_run_configs(experiment_id)).info.run_id
    ...
    assert sorted(
        [r1, r2],
    ) == sorted(_search(store, experiment_id, filter_string="tags.generic_tag = 'p_val'"))
    assert _search(store, experiment_id, filter_string="tags.generic_tag = 'P_VAL'") == []
    assert sorted(
        [r1, r2],
    ) == sorted(_search(store, experiment_id, filter_string="tags.generic_tag != 'P_VAL'"))
    # test search returns appropriate run (same key different values per run)
    assert _search(store, experiment_id, filter_string="tags.generic_2 = 'some value'") == [r1]
    assert _search(store, experiment_id, filter_string="tags.generic_2 = 'another value'") == [r2]
    assert _search(store, experiment_id, filter_string="tags.generic_tag = 'wrong_val'") == []
    assert _search(store, experiment_id, filter_string="tags.generic_tag != 'p_val'") == []
    assert sorted(
        [r1, r2],
    ) == sorted(
        _search(store, experiment_id, filter_string="tags.generic_tag != 'wrong_val'"),
    )
    assert sorted(
        [r1, r2],
    ) == sorted(
        _search(store, experiment_id, filter_string="tags.generic_2 != 'wrong_val'"),
    )
    assert _search(store, experiment_id, filter_string="tags.p_a = 'abc'") == [r1]
    assert _search(store, experiment_id, filter_string="tags.p_b = 'ABC'") == [r2]
    assert _search(store, experiment_id, filter_string="tags.generic_2 LIKE '%other%'") == [r2]
    assert _search(store, experiment_id, filter_string="tags.generic_2 LIKE '%Other%'") == []
    assert _search(store, experiment_id, filter_string="tags.generic_2 LIKE 'other%'") == []
    assert _search(store, experiment_id, filter_string="tags.generic_2 LIKE '%other'") == []
    assert _search(store, experiment_id, filter_string="tags.generic_2 LIKE 'other'") == []
    assert _search(store, experiment_id, filter_string="tags.generic_2 ILIKE '%Other%'") == [r2]
    assert _search(
        store,
        experiment_id,
...
def test_search_metrics(store):
    experiment_id = _experiment_factory(store, "search_metric")
    r1 = _run_factory(store, _get_run_configs(experiment_id)).info.run_id
    r2 = _run_factory(store, _get_run_configs(experiment_id)).info.run_id
...
def test_search_attrs(store):
    e1 = _experiment_factory(store, "search_attributes_1")
    r1 = _run_factory(store, _get_run_configs(experiment_id=e1)).info.run_id

    e2 = _experiment_factory(store, "search_attrs_2")
    r2 = _run_factory(store, _get_run_configs(experiment_id=e2)).info.run_id

    filter_string = ""
    assert sorted(
        [r1, r2],
    ) == sorted(_search(store, [e1, e2], filter_string))
    ...
    assert _search(store, [e1, e2], filter_string) == [r1]

    filter_string = "attribute.status = 'FAILED'"
    assert _search(store, [e1, e2], filter_string) == [r2]
    ...
    filter_string = "attribute.status = 'SCHEDULED'"
    assert _search(store, [e1, e2], filter_string) == []
    ...
    for k, v in {"experiment_id": e1, "lifecycle_stage": "ACTIVE"}.items():
        with pytest.raises(MlflowException, match=r"Invalid attribute key '.+' specified"):
            _search(store, [e1, e2], f"attribute.{k} = '{v}'")


def test_search_full(store):
    experiment_id = _experiment_factory(store, "search_params")
    r1 = _run_factory(store, _get_run_configs(experiment_id)).info.run_id
    r2 = _run_factory(store, _get_run_configs(experiment_id)).info.run_id
...
def test_search_with_max_results(store):
    exp = _experiment_factory(store, "search_with_max_results")
    runs = [
    ...
    assert runs[:1000] == _search(store, exp)
    ...
    with pytest.raises(MlflowException, match=r"Invalid value for request parameter max_results"):
        _search(store, exp, max_results=int(1e10))


def test_search_with_deterministic_max_results(store):
    exp = _experiment_factory(store, "test_search_with_deterministic_max_results")
...
def test_search_runs_pagination(store):
    exp = _experiment_factory(store, "test_search_runs_pagination")
...
def test_search_runs_run_name(store):
    exp_id = _experiment_factory(store, "test_search_runs_pagination")
...
def test_search_runs_run_id(store):
    exp_id = _experiment_factory(store, "test_search_runs_run_id")
...
def test_search_runs_start_time_alias(store):
    exp_id = _experiment_factory(store, "test_search_runs_start_time_alias")
    # Set start_time to ensure the search result is deterministic
    run1 = _run_factory(store, dict(_get_run_configs(exp_id), start_time=1))
    run2 = _run_factory(store, dict(_get_run_configs(exp_id), start_time=2))
    run_id1 = run1.info.run_id
...
def test_log_batch(store):
    experiment_id = _experiment_factory(store, "log_batch")
...
def test_log_batch_limits(store):
    # Test that log batch at the maximum allowed request size succeeds (i.e doesn't hit
    # SQL limitations, etc)
    experiment_id = _experiment_factory(store, "log_batch_limits")
    run_id = _run_factory(store, _get_run_configs(experiment_id)).info.run_id
...
def test_log_batch_param_overwrite_disallowed(store):
...
def test_log_batch_with_unchanged_and_new_params(store):
    """
...
def test_log_batch_param_overwrite_disallowed_single_req(store):
...
def test_log_batch_metrics(store):
    run = _run_factory(store)
...
def test_log_batch_same_metrics_repeated_multiple_reqs(store):
    run = _run_factory(store)
    metric0 = Metric(key="metric-key", value=1, timestamp=2, step=0)
    metric1 = Metric(key="metric-key", value=2, timestamp=3, step=0)
    store.log_batch(run.info.run_id, params=[], metrics=[metric0, metric1], tags=[])
...
def test_log_batch_null_metrics(store):
    run = _run_factory(store)
...
def test_log_batch_params_max_length_value(store):
    run = _run_factory(store)
...
def test_search_runs_returns_expected_results_with_large_experiment(store):
    """
...
def test_search_runs_correctly_filters_large_data(store):
...
def test_search_runs_keep_all_runs_when_sorting(store):
...
def test_try_get_run_tag(store):
    run = _run_factory(store)
...
def test_get_metric_history_on_non_existent_metric_key(store):
    experiment_id = _experiment_factory(store, "test_exp")[0]
