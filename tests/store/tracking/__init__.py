...
class AbstractStoreTest:
    ...
    def _verify_logged(store, run_id, metrics, params, tags):
        ...
        assert {logged_metrics} == {(m.key, m.value, m.timestamp, m.step) for m in metrics}
        logged_tags = {run.data.tags.items()}
        assert {(tag.key, tag.value) for tag in tags} <= logged_tags
        assert len(run.data.params) == len(params)
        assert {run.data.params.items()} == {(param.key, param.value) for param in params}
