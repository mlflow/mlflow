...
def test_log_spans_prefixes_workspace_endpoint(monkeypatch):
    spans = _create_test_spans()
    ...
    monkeypatch.setattr(
        "mlflow.store.tracking.rest_store.rest_utils.http_request",
        _fake_http_request,
    )
    monkeypatch.setattr(
        "mlflow.store.tracking.rest_store.verify_rest_response",
        lambda response, endpoint: response,
    )

    result = store.log_spans("exp-1", spans)
    assert result == spans
    assert captured["endpoint"] == "/v1/workspaces/team-space/traces"
