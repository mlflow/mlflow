...
def test_process_ephemeral_base_key_unique_per_instance():
    crypto1 = EphemeralCacheEncryption(ttl_seconds=60)
    crypto2 = EphemeralCacheEncryption(ttl_seconds=60)
    assert crypto1._base_key != crypto2._base_key
...
def test_cache_hit_returns_value(cache):
    ...
    secret = json.dumps({"api_key": "secret123"})
...
def test_cache_stores_multiple_entries(cache):
    cache.set("SCORER_JOB:job_1", json.dumps({"key": "secret1"}))
    cache.set("SCORER_JOB:job_2", json.dumps({"key": "secret2"}))
    cache.set("GLOBAL:workspace_1", json.dumps({"key": "secret3"}))
    assert json.loads(cache.get("SCORER_JOB:job_1"))["key"] == "secret1"
    assert json.loads(cache.get("SCORER_JOB:job_2"))["key"] == "secret2"
    assert json.loads(cache.get("GLOBAL:workspace_1"))["key"] == "secret3"


def test_lru_eviction_when_max_size_exceeded():
    cache = SecretCache(ttl_seconds=60, max_size=3)
    cache.set("key_1", json.dumps({"value": "1"}))
    cache.set("key_2", json.dumps({"value": "2"}))
    cache.set("key_3", json.dumps({"value": "3"}))
    assert cache.size() == 3
    cache.set("key_4", json.dumps({"value": "4"}))
    assert cache.size() == 3
    assert cache.get("key_1") is None
    assert cache.get("key_4") == json.dumps({"value": "4"})
...
def test_lru_promotion_on_access():
    ...
    _ = cache.get("key_1")
    cache.set("key_4", json.dumps({"value": "4"}))
    assert cache.get("key_1") == json.dumps({"value": "1"})
...
def test_ttl_validation(ttl, should_raise):
    if should_raise:
        with pytest.raises(ValueError, match=f"Cache TTL must be between {MIN_TTL} and {MAX_TTL}"):
...
def test_thread_safety_concurrent_reads(cache):
    cache.set("key_1", json.dumps({"value": "secret"}))

    def read_cache():
        for _ in range(100):
            result = cache.get("key_1")
            assert result == json.dumps({"value": "secret"})
...
def test_thread_safety_concurrent_writes():
    ...
    def write_cache(thread_id):
        for i in range(50):
            cache.set(f"key_{thread_id}_{i}", json.dumps({"value": f"{thread_id}_{i}"}))
...
def test_thread_safety_concurrent_clear():
    cache = SecretCache(ttl_seconds=60, max_size=1000)
    for i in range(100):
        cache.set(f"key_{i}", json.dumps({"value": str(i)}))
...
def test_cache_roundtrip_with_complex_secret(cache):
    ...
    assert json.loads(result) == json.loads(complex_secret)
