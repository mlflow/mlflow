...
from mlflow.store.model_registry.sqlalchemy_workspace_store import WorkspaceAwareSqlAlchemyStore
...
def test_model_version_allows_workspace_scoped_proxied_artifacts(
    workspace_registry_store, monkeypatch
):
    monkeypatch.setenv("_MLFLOW_SERVER_SERVE_ARTIFACTS", "true")
    with WorkspaceContext("team-a"):
        workspace_registry_store.create_registered_model("alpha")
        mv = workspace_registry_store.create_model_version(
            "alpha",
            "mlflow-artifacts:/workspaces/team-a/models/model-a",
