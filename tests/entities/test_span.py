...
def test_adding_chat_messages_and_tools():
    request_id = "tr-12345"
    ...
    with tracer.start_as_current_span("parent") as parent_span:
        live_span = LiveSpan(parent_span, request_id=request_id, span_type=SpanType.LLM)
        live_span.set_chat_messages(messages)
        live_span.set_chat_tools(tools)

    span = live_span.to_immutable_span()
    assert span.messages == messages
    assert span.tools == tools
