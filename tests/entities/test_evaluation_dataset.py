...
def test_evaluation_dataset_merge_records_from_traces(tracking_uri):
    ...
    expectation1 = Expectation(
        ...
        value="MLflow is an ML platform",
        source=assessment_source
    )
    mlflow.log_expectation(trace_id=trace_id1, name=expectation1.name, value=expectation1.value, source=expectation1.source)
    ...
    mlflow.log_expectation(trace_id=trace_id1, name=expectation2.name, value=expectation2.value, source=expectation2.source)
    ...
    expectation3 = Expectation(
        name="expected_answer",
        value="Use MLflow for ML lifecycle",
        source=assessment_source
    )
    mlflow.log_expectation(trace_id=trace_id2, name=expectation3.name, value=expectation3.value, source=expectation3.source)
...
def test_evaluation_dataset_merge_records_with_tracking_store():
    ...
    mock_store = mock.Mock()
    mock_store.upsert_evaluation_dataset_records.return_value = {
        "inserted": 2,
        "updated": 0
    }
    dataset._tracking_store = mock_store
...
def test_evaluation_dataset_merge_records_from_traces_edge_cases():
    ...
    mock_assessment.name = "expected_response"
    mock_assessment.expectation = mock.Mock()
    mock_assessment.expectation.value = "Expected value"
    
    mock_trace1 = mock.Mock(spec=Trace)
    mock_trace1.info = mock.Mock()
    mock_trace1.info.trace_id = "trace1"
    mock_trace1.info.assessments = [mock_assessment]
    ...
    mock_assessment_none = mock.Mock()
    mock_assessment_none.name = "score"
    mock_assessment_none.expectation = None
    
    mock_trace3 = mock.Mock(spec=Trace)
    mock_trace3.info = mock.Mock()
    mock_trace3.info.trace_id = "trace3"
    mock_trace3.info.assessments = [mock_assessment_none]
    mock_trace3.data = mock.Mock()
    mock_trace3.data.spans = [mock_span3]
    
    mock_assessment_direct = mock.Mock()
    mock_assessment_direct.name = "direct_value"
    mock_assessment_direct.expectation = "Direct expectation value"
    
    mock_trace4 = mock.Mock(spec=Trace)
    mock_trace4.info = mock.Mock()
    mock_trace4.info.trace_id = "trace4"
    mock_trace4.info.assessments = [mock_assessment_direct]
...
def test_evaluation_dataset_merge_records_from_traces_with_duplicates(tracking_uri):
    ...
    mlflow.log_expectation(trace_id=trace_id2, name=expectation2.name, value=expectation2.value, source=expectation2.source)
    mlflow.log_expectation(trace_id=trace_id2, name=expectation3.name, value=expectation3.value, source=expectation3.source)
    ...
    mlflow.log_expectation(trace_id=trace_id3, name=expectation4.name, value=expectation4.value, source=expectation4.source)
    ...
    spark_record = records_by_question["What is Spark?"]
    assert spark_record.expectations == {"expected_answer": "Apache Spark is a unified analytics engine"}
...
def test_evaluation_dataset_merge_records_single_call_deduplication():
    ...
    dataset.merge_records([
        ...
        {
            "inputs": {"question": "What is MLflow?", "context": "ML platforms"},  # Same inputs
