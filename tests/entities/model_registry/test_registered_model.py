...
class TestRegisteredModel(unittest.TestCase):
    ...
    def test_with_latest_model_versions(self):
        name = random_str()
        mvd_1 = ModelVersion(name, "1", 1000, 2000, "version 1", "user 1", "Production", "source 1",
                             "run ID 1", "PENDING_REGISTRATION", "Model version is in production!")
        mvd_2 = ModelVersion(name, "4", 1300, 2002, "version 4", "user 2", "Staging", "source 4",
                             "run ID 12", "READY", "Model copied over!")
        as_dict = {
            "name": name,
            "creation_timestamp": 1,
        ...
        self.assertEqual(set([mvd.name for mvd in proto.latest_versions]), set([
            name,
        ]))
        self.assertEqual(
