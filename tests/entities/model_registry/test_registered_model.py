...
class TestRegisteredModel(unittest.TestCase):
    ...
    def test_with_latest_model_versions(self):
        ...
        self.assertEqual(set([mvd.version for mvd in proto.latest_versions]), {"1", "4"})
        self.assertEqual(set([mvd.name for mvd in proto.latest_versions]), {name})
        self.assertEqual(
            set([mvd.current_stage for mvd in proto.latest_versions]),
            {"Production", "Staging"},
        )
        self.assertEqual(
            set([mvd.last_updated_timestamp for mvd in proto.latest_versions]), {2000, 2002}
        )
        self.assertEqual(
            set([mvd.creation_timestamp for mvd in proto.latest_versions]), {1300, 1000}
        )
