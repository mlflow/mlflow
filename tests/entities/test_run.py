...
class TestRun(TestRunInfo, TestRunData):
    ...
    def test_creation_and_hydration(self):
        ...
        as_dict = {"info": {"run_uuid": run_uuid,
                   ...
                   "data": {"metrics": {m.key: dict(m) for m in metrics.values()},
                            "params": params,
                            "tags": tags}}
    ...
    def test_string_repr(self):
        ...
        metrics = {"key-%s" % i: Metric("key-%s" % i, i, 0) for i in range(3)}
        run_data = RunData(metrics=metrics, params=[], tags=[])
        run1 = Run(run_info, run_data)
        expected = ("<Run: data=<RunData: metrics={'key-0': <Metric: key='key-0', timestamp=0, "
                    "value=0>,\n 'key-1': <Metric: key='key-1', timestamp=0, value=1>,\n "
                    "'key-2': <Metric: key='key-2', timestamp=0, value=2>}, params={}, tags={}>, "
