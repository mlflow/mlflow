...
def _check_run(run, ri, rd_metrics, rd_params, rd_tags):
    TestRunInfo._check(run.info, ri.run_uuid, ri.experiment_id, ri.name,
    ...
    TestRunData._check(run.data, rd_metrics, rd_params, rd_tags)
...
def test_creation_and_hydration(self):
    ...
    _check_run(run1, run_info, metrics, params, tags)
    ...
    assert run1.to_dictionary() == as_dict

    proto = run1.to_proto()
    run2 = Run.from_proto(proto)
    _check_run(run2, run_info, metrics, params, tags)
