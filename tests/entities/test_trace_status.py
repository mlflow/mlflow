...
def test_trace_status_from_proto():
    ...
    assert (
        TraceStatus.from_proto(ProtoTraceStatus.TRACE_STATUS_UNSPECIFIED)
        == TraceStatus.UNSPECIFIED
    )
    assert TraceStatus.from_proto(ProtoTraceStatus.ERROR) == TraceStatus.ERROR
    assert (
        TraceStatus.from_proto(ProtoTraceStatus.IN_PROGRESS) == TraceStatus.IN_PROGRESS
    )
...
def test_trace_status_to_proto():
    ...
    assert (
        TraceStatus.UNSPECIFIED.to_proto() == ProtoTraceStatus.TRACE_STATUS_UNSPECIFIED
    )
