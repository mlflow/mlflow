...
def test_safe_patch_keeps_session_active_for_non_fatal_original_exception(
    test_autologging_integration,
):
    class ExpectedControlFlow(Exception):
        pass
    ...
    class Destination:
        def outer(self):
            try:
                self.inner()
            except ExpectedControlFlow:
                return self.after()

        def inner(self):
            raise ExpectedControlFlow("retry")

        def after(self):
            return "retried"

    after_patch_called = False

    def passthrough(original, *args, **kwargs):
        return original(*args, **kwargs)

    def track_after(original, *args, **kwargs):
        nonlocal after_patch_called
        after_patch_called = True
        return original(*args, **kwargs)

    safe_patch(test_autologging_integration, Destination, "outer", passthrough)
    safe_patch(
        test_autologging_integration,
        Destination,
        "inner",
        passthrough,
        non_fatal_original_exceptions=(ExpectedControlFlow,),
    )
    safe_patch(test_autologging_integration, Destination, "after", track_after)

    assert Destination().outer() == "retried"
    assert after_patch_called


@pytest.mark.asyncio
async def test_async_safe_patch_keeps_session_active_for_non_fatal_original_exception(
    test_autologging_integration,
):
    class ExpectedControlFlow(Exception):
        pass
    ...
    async def passthrough(original, *args, **kwargs):
        return await original(*args, **kwargs)

    async def track_after(original, *args, **kwargs):
        nonlocal after_patch_called
        after_patch_called = True
        return await original(*args, **kwargs)
    ...
    assert await Destination().outer() == "retried"
    assert after_patch_called
