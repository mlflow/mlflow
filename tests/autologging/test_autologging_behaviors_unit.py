...
from mlflux.utils.autologging_utils import autologging_integration, safe_patch
...
@pytest.fixture
def logger():
    return logging.getLogger(mlflux.__name__)
...
def autolog_function(patch_destination, logger):
    ...
    def patch_impl(original):
        ...
        warnings.warn_explicit(
            "preamble mlflux warning", category=Warning, filename=mlflux.__file__, lineno=5
        )
        warnings.warn_explicit(
        ...
        original()
        warnings.warn_explicit(
            "postamble mlflux warning", category=Warning, filename=mlflux.__file__, lineno=10
        )
        warnings.warn_explicit(
    ...
    def test_autolog(disable=False, silent=False):
        ...
        warnings.warn_explicit(
            "enablement warning mlflux", category=Warning, filename=mlflux.__file__, lineno=15
        )
        warnings.warn_explicit(
...
def test_autologging_warnings_are_redirected_as_expected(
    ...
    # The following types of warnings are rerouted to mlflux's event loggers:
    # 1. All mlflux warnings emitted during patch function execution
    ...
    #    function)
    # 3. non-mlflux warnings emitted during autologging setup / enablement
    #
    # Accordingly, we expect the following warnings to have been emitted normally: 1. mlflux
    # warnings emitted during autologging enablement, 2. non-mlflux warnings emitted during original
    # / underlying function execution
    warning_messages = set([str(w.message) for w in warnings_record])
    assert warning_messages == set(["enablement warning mlflux", "Test warning from OG function"])

    # Further, We expect mlflux's logging stream to contain content from all warnings emitted during
    # the autologging preamble and postamble and non-mlflux warnings emitted during autologging
    # enablement
    for item in [
        'mlflux autologging encountered a warning: "%s:5: Warning: preamble mlflux warning"',
        'mlflux autologging encountered a warning: "%s:10: Warning: postamble mlflux warning"',
    ]:
        assert (item % mlflux.__file__) in stream.getvalue()
    for item in [
        'mlflux autologging encountered a warning: "%s:7: UserWarning: preamble numpy warning"',
        'mlflux autologging encountered a warning: "%s:14: Warning: postamble numpy warning"',
        'mlflux autologging encountered a warning: "%s:30: Warning: enablement warning numpy"',
    ]:
...
def test_autologging_event_logging_and_warnings_respect_silent_mode(
    ...
    assert "enablement warning mlflux" in warning_messages
...
def test_silent_mode_is_respected_in_multithreaded_environments(
    ...
    # Verify that all warnings and log events from mlflux autologging code were silenced
...
def test_silent_mode_operates_independently_across_integrations(patch_destination, logger):
    ...
    def autolog2(disable=False, silent=False):
        warnings.warn_explicit(
            "warn_autolog2", category=Warning, filename=mlflux.__file__, lineno=5
        )
...
def test_silent_mode_and_warning_rerouting_respect_disabled_flag(
    ...
    # Verify that nothing is printed to the stderr-backed mlflux event logger, which would indicate
