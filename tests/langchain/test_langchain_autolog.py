...
def test_autolog_manage_run(is_async):
...
def test_autolog_record_exception(is_async):
    from langchain.schema.runnable import RunnableLambda
...
def test_llmchain_autolog(is_async):
...
def test_loaded_llmchain_within_model_evaluation(mock_get_display, tmp_path, is_async):
...
def test_agent_autolog(is_async):
    mlflow.langchain.autolog(log_models=True)
...
def test_runnable_sequence_autolog(is_async):
    mlflow.langchain.autolog(log_models=True)
...
def test_retriever_autolog(tmp_path, is_async):
    mlflow.langchain.autolog(log_models=True)
...
def test_langchain_autolog_callback_injection_in_invoke(invoke_arg, config, is_async):
    mlflow.langchain.autolog()
...
@pytest.mark.asyncio
async def test_langchain_autolog_callback_injection_in_ainvoke(invoke_arg, config, is_async):
    mlflow.langchain.autolog()
...
def test_langchain_autolog_callback_injection_in_batch(invoke_arg, config, is_async):
    mlflow.langchain.autolog()
...
@pytest.mark.asyncio
async def test_langchain_autolog_callback_injection_in_abatch(invoke_arg, config, is_async):
    mlflow.langchain.autolog()
...
@pytest.mark.parametrize("invoke_arg", ["args", "kwargs", None])
@pytest.mark.parametrize("config", _CONFIG_PATTERNS)
def test_langchain_autolog_callback_injection_in_stream(invoke_arg, config, is_async):
    mlflow.langchain.autolog()
...
@pytest.mark.parametrize("config", _CONFIG_PATTERNS + _ASYNC_CONFIG_PATTERNS)
@pytest.mark.asyncio
async def test_langchain_autolog_callback_injection_in_astream(invoke_arg, config, is_async):
    mlflow.langchain.autolog()
...
def test_langchain_autolog_produces_expected_traces_with_streaming(tmp_path, is_async):
    from langchain.prompts import ChatPromptTemplate
    from langchain.schema.output_parser import StrOutputParser
    from langchain.schema.runnable import RunnablePassthrough
...
def test_langchain_tracer_injection_for_arbitrary_runnables(log_traces, is_async):
    ...
    if is_async and should_log_traces:
        mlflow.flush_trace_async_logging(terminate=True)
