...
def test_model_save_load(spacy_model_with_data, model_path):
    ...
    assert all(
        _predict(spacy_model, spacy_model_with_data.inference_data) == pyfunc_loaded.predict(
...
def test_predict_df_with_wrong_shape(spacy_model_with_data, model_path):
    ...
    with pytest.raises(MlflowException):
        pyfunc_loaded.predict(
            pd.concat([spacy_model_with_data.inference_data, spacy_model_with_data.inference_data],
                      axis=1))
    ...
    for should_start_run in [False, True]:
        with TempDir(chdr=True, remove_on_exit=True):
            try:
                ...
                assert all(
                    _predict(spacy_model, spacy_model_with_data.inference_data) == _predict(
                        spacy_model_loaded, spacy_model_with_data.inference_data))
            finally:
                mlflow.end_run()
