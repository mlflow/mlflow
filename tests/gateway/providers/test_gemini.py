...
async def test_chat_with_structured_output():
    ...
    # Expected Gemini schema with uppercase types
    ...
    with mock.patch(
        ...
        # Verify that the schema is converted to Gemini format with uppercase types
        assert call_kwargs["json"]["generationConfig"]["responseSchema"] == expected_gemini_schema
        assert call_kwargs["json"]["generationConfig"]["responseMimeType"] == "application/json"


@pytest.mark.asyncio
async def test_chat_with_nested_structured_output():
    config = {
        "name": "chat",
        "endpoint_type": "llm/v1/chat",
        "model": {
    ...
    # OpenAI-style JSON Schema with nested array and object types
    json_schema = {
        "type": "object",
        "properties": {
            "recipes": {
    ...
    resp = {
        "candidates": [
            {
                "content": {
                    "parts": [{"text": '{"recipes": [{"name": "Salad", "calories": 200}]}'}],
                    "role": "model",
                },
                "finishReason": "STOP",
            }
...
async def test_chat_with_complex_schema_defs_and_refs():
    ...
    # OpenAI-style JSON Schema with $defs and $ref (from Pydantic)
    json_schema = {
        "$defs": {
            "ProgrammingLanguage": {
                "properties": {
        ...
        "properties": {
            "languages": {
                "items": {"$ref": "#/$defs/ProgrammingLanguage"},
                "title": "Languages",
                "type": "array",
            }
        },
    ...
    # Expected Gemini schema: $defs removed, $ref resolved and inlined, types uppercase
    expected_gemini_schema = {
        "properties": {
            "languages": {
                "items": {
        ...
        "candidates": [
            {
                "content": {
                    "parts": [
                        {"text": '{"languages": [{"name": "Python", "type": "interpreted"}]}'}
                    ],
                    "role": "model",
                },
                "finishReason": "STOP",
