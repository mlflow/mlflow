...
def test_model_save_load(gluon_model, model_data, model_path):
    ...
    mlflux.gluon.save_model(gluon_model, model_path)
    # Loading Gluon model
    model_loaded = mlflux.gluon.load_model(model_path, ctx.cpu())
...
def test_model_log_load(gluon_model, model_data, model_path):
    ...
    with mlflux.start_run():
        mlflux.gluon.log_model(gluon_model, artifact_path=artifact_path)
        model_uri = "runs:/{run_id}/{artifact_path}".format(
            run_id=mlflux.active_run().info.run_id, artifact_path=artifact_path
        )

    # Loading Gluon model
    model_loaded = mlflux.gluon.load_model(model_uri, ctx.cpu())
    actual = nd.argmax(model_loaded(test_data), axis=1)
    assert all(expected == actual)
    # Loading pyfunc model
    pyfunc_loaded = mlflux.pyfunc.load_model(model_uri)
    test_pyfunc_data = pd.DataFrame(test_data.asnumpy())
    pyfunc_preds = pyfunc_loaded.predict(test_pyfunc_data)
    assert all(np.argmax(pyfunc_preds.values, axis=1) == expected.asnumpy())
...
def test_model_save_persists_specified_conda_env_in_mlflow_model_directory(
    ...
    mlflux.gluon.save_model(gluon_model=gluon_model, path=model_path, conda_env=gluon_custom_env)
...
def test_model_save_accepts_conda_env_as_dict(gluon_model, model_path):
    conda_env = dict(mlflux.gluon.get_default_conda_env())
    conda_env["dependencies"].append("pytest")
    mlflux.gluon.save_model(gluon_model=gluon_model, path=model_path, conda_env=conda_env)
