...
def test_generate_creates_prompts(tracking_uri):
    generator = PromptsDemoGenerator()
...
def test_generate_creates_expected_prompts(tracking_uri):
    generator = PromptsDemoGenerator()
    generator.generate()
...
def test_prompts_have_multiple_versions(tracking_uri):
    generator = PromptsDemoGenerator()
    generator.generate()
...
def test_prompts_have_version_aliases(tracking_uri):
    generator = PromptsDemoGenerator()
    generator.generate()

    for prompt_def in DEMO_PROMPTS:
        for version_num, version_def in enumerate(prompt_def.versions, start=1):
            if version_def.alias:
                prompt = load_prompt(f"prompts:/{prompt_def.name}@{version_def.alias}")
...
def test_delete_demo_removes_prompts(tracking_uri):
    generator = PromptsDemoGenerator()
    generator.generate()
    assert generator._data_exists() is True
...
def test_prompts_have_demo_tag(tracking_uri):
    generator = PromptsDemoGenerator()
    generator.generate()
...
def test_prompt_templates_are_valid(tracking_uri):
    generator = PromptsDemoGenerator()
    generator.generate()
