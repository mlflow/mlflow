...
def test_model_save_load(lgb_model, model_path):
    ...
    mlflux.lightgbm.save_model(lgb_model=model, path=model_path)
    reloaded_model = mlflux.lightgbm.load_model(model_uri=model_path)
...
def test_model_load_from_remote_uri_succeeds(lgb_model, model_path, mock_s3_bucket):
    mlflux.lightgbm.save_model(lgb_model=lgb_model.model, path=model_path)
    ...
    reloaded_model = mlflux.lightgbm.load_model(model_uri=model_uri)
    np.testing.assert_array_almost_equal(
...
def test_model_save_persists_specified_conda_env_in_mlflow_model_directory(
    ...
    mlflux.lightgbm.save_model(lgb_model=lgb_model.model, path=model_path, conda_env=lgb_custom_env)
...
def test_model_save_accepts_conda_env_as_dict(lgb_model, model_path):
    conda_env = dict(mlflux.lightgbm.get_default_conda_env())
    conda_env["dependencies"].append("pytest")
    mlflux.lightgbm.save_model(lgb_model=lgb_model.model, path=model_path, conda_env=conda_env)
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    mlflux.lightgbm.save_model(lgb_model=lgb_model.model, path=model_path, conda_env=None)
    ...
    assert conda_env == mlflux.lightgbm.get_default_conda_env()
