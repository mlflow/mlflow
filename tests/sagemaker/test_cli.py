...
@pytest.mark.parametrize(
    "env_manager, skip_java", [("conda", False), ("virtualenv", False), ("virtualenv", True)]
)
def test_build_and_push_container(tmp_path, env_manager, skip_java):
    ...
    with mock.patch(
        ...
        res = CliRunner().invoke(
            ...
            + (["--skip-java"] if skip_java else []),
            catch_exceptions=False,
        )
        assert res.exit_code == 0

    actual = dst_dir / "Dockerfile"
    expected = Path(_RESOURCE_DIR) / f"Dockerfile_sagemaker_{env_manager}{'_no_java' if skip_java else ''}"
