...
@pytest.mark.asyncio
async def test_litellm_provider_chat(store: SqlAlchemyStore):
    secret = store.create_gateway_secret(
        secret_name="litellm-chat-key",
    ...
    store.create_gateway_endpoint(
        name="litellm-chat-endpoint", model_definition_ids=[model_def.model_definition_id]
    )

    provider = _create_provider_from_endpoint_name(
        store, "litellm-chat-endpoint", EndpointType.LLM_V1_CHAT
    )

    # Mock litellm.acompletion
    mock_response = MagicMock()
    mock_response.id = "litellm-chat-id"
    ...
    with patch("litellm.acompletion", return_value=mock_response):
        ...
        assert response.choices[0].message.content == "Hello from LiteLLM!"
...
@pytest.mark.asyncio
async def test_litellm_provider_chat_stream(store: SqlAlchemyStore):
    secret = store.create_gateway_secret(
        secret_name="litellm-stream-key",
        secret_value={"api_key": "test-key"},
        provider="litellm",
    )
    model_def = store.create_gateway_model_definition(
        name="litellm-stream-model",
        secret_id=secret.secret_id,
        provider="litellm",
        model_name="claude-3-5-sonnet-20241022",
    )
    store.create_gateway_endpoint(
        name="litellm-stream-endpoint", model_definition_ids=[model_def.model_definition_id]
    )

    provider = _create_provider_from_endpoint_name(
        store, "litellm-stream-endpoint", EndpointType.LLM_V1_CHAT
    )
    ...
    async def mock_stream():
        ...
        choice1 = MagicMock()
        choice1.index = 0
        choice1.delta = MagicMock(spec=["role", "content"])
        ...
        choice2 = MagicMock()
        choice2.index = 0
        choice2.delta = MagicMock(spec=["content"])
    ...
    with patch("litellm.acompletion", return_value=mock_stream()):
        payload = chat.RequestPayload(
            messages=[chat.RequestMessage(role="user", content="Hello")], stream=True
        )
...
@pytest.mark.asyncio
async def test_litellm_provider_embeddings(store: SqlAlchemyStore):
    secret = store.create_gateway_secret(
        secret_name="litellm-embed-key",
        secret_value={"api_key": "test-key"},
        provider="litellm",
    )
    model_def = store.create_gateway_model_definition(
        name="litellm-embed-model",
        secret_id=secret.secret_id,
        provider="litellm",
        model_name="text-embedding-3-small",
    )
    store.create_gateway_endpoint(
        name="litellm-embed-endpoint", model_definition_ids=[model_def.model_definition_id]
    )

    provider = _create_provider_from_endpoint_name(
        store, "litellm-embed-endpoint", EndpointType.LLM_V1_EMBEDDINGS
    )

    # Mock litellm.aembedding
    mock_response = MagicMock()
    mock_data = MagicMock()
    mock_data.__getitem__ = lambda self, key: [0.1, 0.2, 0.3] if key == "embedding" else None
    ...
    with patch("litellm.aembedding", return_value=mock_response):
        payload = embeddings.RequestPayload(input=["Hello world"])
