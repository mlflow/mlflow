...
def test_create_workspace_seeds_default_roles(workspace_client, monkeypatch):
    ...
    assert role_names == ["contributor", "editor", "viewer", "workspace-admin"]
    ...
    with User(ADMIN_USERNAME, ADMIN_PASSWORD, monkeypatch):
        ...
        contributor_perms = client.list_role_permissions(by_name["contributor"].id)
        viewer_perms = client.list_role_permissions(by_name["viewer"].id)

    # All four roles use resource_type='workspace' (the workspace-wide grant form).
    # MANAGE additionally grants workspace-admin capability; EDIT / USE / READ are
    # pure workspace-wide resource access at progressively narrower scopes.
    ...
    assert [(p.resource_type, p.resource_pattern, p.permission) for p in contributor_perms] == [
        ("workspace", "*", "USE")
    ]
