...
from mlflow.server.assistant.api import assistant_router, _sessions, _require_localhost
...
def test_message(client):
    ...
    assert data["stream_url"] == f"/api/assistant/stream/{data['session_id']}"
...
def test_stream_not_found_for_invalid_session(client):
    response = client.get("/api/assistant/stream/invalid-session-id")
...
def test_stream_bad_request_when_no_pending_message(client):
    ...
    client.get(f"/api/assistant/stream/{session_id}")

    # Try to stream again without a new message
    response = client.get(f"/api/assistant/stream/{session_id}")
