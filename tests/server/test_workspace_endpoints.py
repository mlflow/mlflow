...
def app():
    ...
    for http_path, handler, methods in get_endpoints():
        ...
        fastapi_app.add_api_route(fastapi_path, handler, methods=methods, response_class=Response)
    return fastapi_app
...
def test_list_workspaces_endpoint(client, mock_workspace_store):
...
def test_create_workspace_endpoint(client, mock_workspace_store, mock_tracking_store):
    created = Workspace(
        name="team-b",
        description="Team B",
...
def test_get_workspace_endpoint(client, mock_workspace_store):
...
def test_update_workspace_endpoint(client, mock_workspace_store):
    updated = Workspace(
        name="team-d",
        description="Updated",
...
def test_update_default_workspace_allows_reserved_name(client, mock_workspace_store):
...
def test_update_workspace_can_clear_default_artifact_root(
    client, mock_workspace_store, mock_tracking_store
):
...
def test_update_workspace_can_clear_trace_archival_location(client, mock_workspace_store):
...
def test_update_workspace_can_clear_trace_archival_retention(client, mock_workspace_store):
...
def test_update_workspace_with_only_trace_archival_location(client, mock_workspace_store):
...
def test_update_workspace_with_only_trace_archival_retention(client, mock_workspace_store):
...
def test_update_workspace_rejects_invalid_trace_archival_retention(client, mock_workspace_store):
    response = client.patch(
...
def test_delete_workspace_endpoint(client, mock_workspace_store):
...
def test_delete_default_workspace_rejected_by_validation(client, mock_workspace_store):
...
def test_create_workspace_fails_without_artifact_root(client, mock_workspace_store, monkeypatch):
...
def test_create_workspace_with_artifact_root_succeeds_without_server_default(
    client, mock_workspace_store, monkeypatch
):
    tracking_store = mock.Mock()
    tracking_store.artifact_root_uri = None
    monkeypatch.setattr(
...
def test_create_default_workspace_rejected(client, mock_workspace_store, mock_tracking_store):
    response = client.post(
        "/api/3.0/mlflow/workspaces",
        json={"name": "default"},
    )
