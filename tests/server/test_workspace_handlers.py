...
def test_workspace_scoped_repo_path_prefixes_workspace(monkeypatch):
    ...
    with workspace_context.WorkspaceContext("team-a"):
        ...
        deep_path = "projects/foo/workspaces/team-a/runs/123/artifacts"
        assert handlers._workspace_scoped_repo_path(deep_path) == deep_path
        proxied = "/api/2.0/mlflow-artifacts/artifacts/projects/foo/workspaces/team-a/runs/123"
        assert handlers._workspace_scoped_repo_path(proxied) == proxied.lstrip("/")

    workspace_context.clear_workspace()
    with pytest.raises(MlflowException, match="Active workspace is required"):
        handlers._workspace_scoped_repo_path("foo/bar")
...
def test_workspace_scoped_repo_path_blocks_cross_workspace_access(monkeypatch):
    ...
    with workspace_context.WorkspaceContext("team-a"):
        ...
        with pytest.raises(MlflowException, match="team-b"):
            handlers._workspace_scoped_repo_path("foo/workspaces/team-b/bar")

        with pytest.raises(MlflowException, match="include a workspace name"):
            handlers._workspace_scoped_repo_path("workspaces/")
        with pytest.raises(MlflowException, match="include a workspace name"):
            handlers._workspace_scoped_repo_path("foo/workspaces/")
