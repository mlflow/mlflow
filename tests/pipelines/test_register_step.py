...
""".format(
    ...
    experiment_id = pipeline_config["steps"]["evaluate"]["mlflow_experiment_id"]

    assert f"<a href={workspace_url}#mlflow/models/demo_model/1" in step_card_content
    assert (
        f"<a href={workspace_url}#mlflow/experiments/{experiment_id}/runs/{run_id}/"
        "artifactPath/train/model>" in step_card_content
    )
