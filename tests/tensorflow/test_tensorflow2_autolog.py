...
def test_tf_keras_autolog_log_models_configuration(
    ...
    client = MlflowClient()
    run_id = client.search_runs(["0"])[0].run_id
...
def tf_keras_random_data_run(random_train_data, random_one_hot_labels, initial_epoch):
    ...
    client = MlflowClient()
    return client.get_run(client.search_runs(["0"])[0].run_id), history
...
def get_tf_keras_random_data_run_with_callback(
    ...
    client = MlflowClient()
    return client.get_run(client.search_runs(["0"])[0].run_id), history, callback
