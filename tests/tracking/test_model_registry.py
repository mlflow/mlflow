...
def test_create_and_query_registered_model_flow(mlflow_client, backend_store_uri):
    ...
    assert [name] == [
        rm.name for rm in mlflow_client.search_registered_models(filter_string="")
        if rm.name == name
    ]
    assert [name] == [
        rm.name for rm in mlflow_client.search_registered_models("name = 'CreateRMTest'")
        if rm.name == name
    ]
    # clean up test
    ...
    try:
        _verify_pagination(
            lambda tok: mlflow_client.list_registered_models(
                max_results=max_results, page_token=tok), rms)
...
def test_search_registered_model_flow_paginated(mlflow_client, backend_store_uri, max_results,
