...
def cached_db(tmp_path_factory: pytest.TempPathFactory) -> Path:
    ...
    os.environ["MLFLOW_SECRETS_KEK_PASSPHRASE"] = "test-cached-db-passphrase-32chars-min"
...
@pytest.fixture
def mlflow_client_with_secrets(store_type: str, tmp_path: Path, monkeypatch):
    if store_type != "sqlalchemy":
        pytest.skip("Secrets are only supported with sqlalchemy backend")

    # Set encryption passphrase before creating database (needed for secrets tables migration)
    monkeypatch.setenv("MLFLOW_SECRETS_KEK_PASSPHRASE", "test-secrets-passphrase-32chars-min")

    # Create a fresh database for each test (don't use cached_db to avoid conflicts)
    db_path = tmp_path / "mlflow.db"
    backend_uri = f"sqlite:///{db_path}"
...
def test_rest_store_secrets_end_to_end(mlflow_client_with_secrets, store_type):
    ...
    assert binding.resource_id == "job-1"
    assert binding.field_name == "OPENAI_API_KEY"
    assert binding.endpoint_id == endpoint.endpoint_id

    fetched_secret = store._get_secret_info(secret.secret_id)
    assert fetched_secret.secret_name == "openai_key"
    assert fetched_secret.is_shared is True
    assert fetched_secret.provider == "openai"

    updated_secret = store._update_secret(secret.secret_id, "sk-test-updated")
    assert updated_secret.secret_id == secret.secret_id
    ...
    assert binding2.resource_id == "job-2"

    bindings = store._list_endpoint_bindings(secret_id=secret.secret_id)
    assert len(bindings) == 2
    assert bindings[0].models[0].secret_name == "openai_key"
    assert bindings[0].models[0].provider == "openai"

    job2_binding = next(b for b in bindings if b.resource_id == "job-2")
    store._delete_endpoint_binding(binding_id=job2_binding.binding_id)

    bindings_after_unbind = store._list_endpoint_bindings(secret_id=secret.secret_id)
    assert len(bindings_after_unbind) == 1

    store._delete_secret(secret.secret_id)

    anthropic_secret = store._create_secret(
    ...
    production_endpoint = store._create_endpoint(
        models=[
            {
        ...
        description="High-quality model for production workloads",
        tags={
            "environment": "production",
            "team": "ml-platform",
        },
    )
    assert production_endpoint.name == "Production Claude"
    assert production_endpoint.description == "High-quality model for production workloads"
    assert len(production_endpoint.tags) == 2
    tag_dict = {tag.key: tag.value for tag in production_endpoint.tags}
    assert tag_dict["environment"] == "production"
    assert tag_dict["team"] == "ml-platform"

    binding3 = store._bind_endpoint(
        endpoint_id=production_endpoint.endpoint_id,
        resource_type="SCORER_JOB",
        resource_id="job-3",
        field_name="ANTHROPIC_API_KEY",
    )
    assert binding3.resource_id == "job-3"

    dev_endpoint = store._create_endpoint(
        models=[
            {
        ...
        description="Fast and cost-efficient model for development",
        tags={
            "environment": "development",
            "cost": "low",
        },
    )
    assert dev_endpoint.endpoint_id != production_endpoint.endpoint_id
    assert dev_endpoint.models[0].model_name == "claude-3-haiku-20240307"
    assert dev_endpoint.name == "Development Claude"
    assert dev_endpoint.description == "Fast and cost-efficient model for development"
    assert len(dev_endpoint.tags) == 2
    dev_tag_dict = {tag.key: tag.value for tag in dev_endpoint.tags}
    assert dev_tag_dict["environment"] == "development"
    assert dev_tag_dict["cost"] == "low"

    binding4 = store._bind_endpoint(
        endpoint_id=dev_endpoint.endpoint_id,
        resource_type="SCORER_JOB",
        resource_id="job-4",
        field_name="ANTHROPIC_API_KEY",
    )
    assert binding4.resource_id == "job-4"

    endpoints = store._list_endpoints(secret_id=anthropic_secret.secret_id)
    assert len(endpoints) == 2
    model_names = {e.models[0].model_name for e in endpoints}
    assert model_names == {"claude-3-5-sonnet-20241022", "claude-3-haiku-20240307"}

    anthropic_endpoints = store._list_endpoints(provider="anthropic")
    assert len(anthropic_endpoints) == 2
    assert all(e.models[0].secret_id == anthropic_secret.secret_id for e in anthropic_endpoints)

    all_bindings = store._list_endpoint_bindings(secret_id=anthropic_secret.secret_id)
    assert len(all_bindings) == 2
    assert all_bindings[0].models[0].secret_name == "anthropic_key"
    assert all_bindings[0].models[0].provider == "anthropic"

    vertex_auth_config = {
        "project_id": "my-gcp-project",
        "location": "us-central1",
    }
    vertex_secret = store._create_secret(
    ...
    assert vertex_secret.provider == "google"

    vertex_endpoint = store._create_endpoint(
        models=[
            {
                "model_name": "gemini-2.0-flash-exp",
                "secret_id": vertex_secret.secret_id,
            }
        ],
        name="Vertex AI Production",
    )
    assert vertex_endpoint.models[0].model_name == "gemini-2.0-flash-exp"
    assert vertex_endpoint.name == "Vertex AI Production"

    binding5 = store._bind_endpoint(
        endpoint_id=vertex_endpoint.endpoint_id,
        resource_type="SCORER_JOB",
        resource_id="job-5",
        field_name="VERTEX_CREDENTIALS",
    )
    assert binding5.resource_id == "job-5"


@pytest.mark.parametrize("store_type", ["sqlalchemy"], indirect=True)
def test_rest_store_secrets_create_and_get(mlflow_client_with_secrets, store_type):
    store = mlflow_client_with_secrets._tracking_client.store
    ...
    assert secret.secret_name == "test_key"
    assert secret.is_shared is True
    assert secret.provider == "openai"

    fetched = store._get_secret_info(secret.secret_id)
    assert fetched.secret_name == "test_key"
...
@pytest.mark.parametrize("store_type", ["sqlalchemy"], indirect=True)
def test_rest_store_endpoint_create_and_list(mlflow_client_with_secrets, store_type):
    store = mlflow_client_with_secrets._tracking_client.store
    ...
    endpoints = store._list_endpoints(secret_id=secret.secret_id)
...
@pytest.mark.parametrize("store_type", ["sqlalchemy"], indirect=True)
def test_rest_store_endpoint_binding(mlflow_client_with_secrets, store_type):
    store = mlflow_client_with_secrets._tracking_client.store
    ...
    assert binding.endpoint_id == endpoint.endpoint_id
    assert binding.resource_type == "SCORER_JOB"
    assert binding.resource_id == "job-test"
