...
from mlflux.tracking.context.databricks_cluster_context import DatabricksClusterRunContext


def test_databricks_cluster_run_context_in_context():
    with mock.patch("mlflux.utils.databricks_utils.is_in_cluster") as in_cluster_mock:
        assert DatabricksClusterRunContext().in_context() == in_cluster_mock.return_value


def test_databricks_cluster_run_context_tags():
    patch_cluster_id = mock.patch("mlflux.utils.databricks_utils.get_cluster_id")
