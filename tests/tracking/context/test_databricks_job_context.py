...
def test_databricks_job_run_context_in_context():
    with mock.patch("mlflux.utils.databricks_utils.is_in_databricks_job") as in_job_mock:
        assert DatabricksJobRunContext().in_context() == in_job_mock.return_value


def test_databricks_job_run_context_tags():
    patch_job_id = mock.patch("mlflux.utils.databricks_utils.get_job_id")
    patch_job_run_id = mock.patch("mlflux.utils.databricks_utils.get_job_run_id")
    patch_job_type = mock.patch("mlflux.utils.databricks_utils.get_job_type")
    patch_webapp_url = mock.patch("mlflux.utils.databricks_utils.get_webapp_url")
...
def test_databricks_job_run_context_tags_nones():
    patch_job_id = mock.patch("mlflux.utils.databricks_utils.get_job_id", return_value=None)
    patch_job_run_id = mock.patch("mlflux.utils.databricks_utils.get_job_run_id", return_value=None)
    patch_job_type = mock.patch("mlflux.utils.databricks_utils.get_job_type", return_value=None)
    patch_webapp_url = mock.patch("mlflux.utils.databricks_utils.get_webapp_url", return_value=None)
