...
from mlflux.pytorch._pytorch_autolog import _get_optimizer_name
...
def test_pytorch_autolog_log_models_configuration(log_models):
    mlflux.pytorch.autolog(log_models=log_models)
    model = IrisClassification()
    dm = IrisDataModule()
    dm.setup(stage="fit")
    trainer = pl.Trainer(max_epochs=NUM_EPOCHS)
...
def test_pytorch_autolog_persists_manually_created_run():
    with mlflux.start_run() as manual_run:
        mlflux.pytorch.autolog()
        model = IrisClassification()
        dm = IrisDataModule()
        dm.setup(stage="fit")
        ...
        assert mlflux.active_run().info.run_id == manual_run.info.run_id
...
def test_pytorch_autolog_model_can_load_from_artifact(pytorch_model_with_callback):
    ...
    model = mlflux.pytorch.load_model("runs:/" + run_id + "/model")
