...
from mlflux.pytorch import get_default_conda_env
...
from mlflux.pytorch import pickle_module as mlflow_pytorch_pickle_module
...
from mlflux.utils.environment import _mlflow_conda_env, _mlflow_additional_pip_env
    ...
    for should_start_run in [False, True]:
        with TempDir(chdr=True, remove_on_exit=True) as tmp:
            try:
                ...
                mlflux.pytorch.log_model(sequential_model, artifact_path=artifact_path)
                model_uri = "runs:/{run_id}/{artifact_path}".format(
                    run_id=mlflux.active_run().info.run_id, artifact_path=artifact_path
                )

                # Load model
                sequential_model_loaded = mlflux.pytorch.load_model(model_uri=model_uri)
...
def test_raise_exception(sequential_model):
    with TempDir(chdr=True, remove_on_exit=True) as tmp:
        ...
        with pytest.raises(IOError):
            mlflux.pytorch.load_model(path)

        with pytest.raises(TypeError):
            mlflux.pytorch.save_model([1, 2, 3], path)

        mlflux.pytorch.save_model(sequential_model, path)
        with pytest.raises(RuntimeError):
            mlflux.pytorch.save_model(sequential_model, path)

        from mlflux import sklearn
...
def test_save_and_load_model(sequential_model, model_path, data, sequential_predicted):
    mlflux.pytorch.save_model(sequential_model, model_path)

    # Loading pytorch model
    sequential_model_loaded = mlflux.pytorch.load_model(model_path)
...
def test_model_save_accepts_conda_env_as_dict(sequential_model, model_path):
    conda_env = dict(mlflux.pytorch.get_default_conda_env())
    conda_env["dependencies"].append("pytest")
    mlflux.pytorch.save_model(pytorch_model=sequential_model, path=model_path, conda_env=conda_env)
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    mlflux.pytorch.save_model(pytorch_model=sequential_model, path=model_path, conda_env=None)
    ...
    assert conda_env == mlflux.pytorch.get_default_conda_env()
...
def test_load_model_with_differing_pytorch_version_logs_warning(sequential_model, model_path):
    mlflux.pytorch.save_model(pytorch_model=sequential_model, path=model_path)
    saver_pytorch_version = "1.0"
    model_config_path = os.path.join(model_path, "MLmodel")
    model_config = Model.load(model_config_path)
    model_config.flavors[mlflux.pytorch.FLAVOR_NAME]["pytorch_version"] = saver_pytorch_version
    ...
    with mock.patch("mlflux.pytorch._logger.warning") as warn_mock, mock.patch(
        "torch.__version__", loader_pytorch_version
    ):
        warn_mock.side_effect = custom_warn
        mlflux.pytorch.load_model(model_uri=model_path)
...
def test_load_model_succeeds_with_dependencies_specified_via_code_paths(
    ...
    # `mlflux.pytorch.load_model`
    class TorchValidatorModel(pyfunc.PythonModel):
        def load_context(self, context):
            # pylint: disable=attribute-defined-outside-init
            self.pytorch_model = mlflux.pytorch.load_model(context.artifacts["pytorch_model"])
    ...
    # constituent PyTorch model via `mlflux.pytorch.load_model`
    scoring_response = pyfunc_serve_and_score_model(
...
def test_load_pyfunc_succeeds_when_data_is_model_file_instead_of_directory(
    ...
    """
    This test verifies that PyTorch models saved in older versions of mlflux are loaded successfully
    by ``mlflux.pytorch.load_model``. The ``data`` path associated with these older models is
    ...
    assert mlflux.pytorch._SERIALIZED_TORCH_MODEL_FILE_NAME in os.listdir(model_data_path)
    pyfunc_conf[pyfunc.DATA] = os.path.join(
        model_data_path, mlflux.pytorch._SERIALIZED_TORCH_MODEL_FILE_NAME
    )
...
def test_load_model_allows_user_to_override_pickle_module_via_keyword_argument(
    ...
    with mock.patch("torch.load") as torch_load_mock, mock.patch(
        "mlflux.pytorch._logger.warning"
    ) as warn_mock:
        mlflux.pytorch.load_model(model_uri=model_path, pickle_module=mlflow_pytorch_pickle_module)
...
def test_load_model_raises_exception_when_pickle_module_cannot_be_imported(
    ...
    model_data_path = os.path.join(model_path, pyfunc_conf[pyfunc.DATA])
    assert os.path.exists(model_data_path)
    assert mlflux.pytorch._PICKLE_MODULE_INFO_FILE_NAME in os.listdir(model_data_path)
    with open(
        os.path.join(model_data_path, mlflux.pytorch._PICKLE_MODULE_INFO_FILE_NAME), "w"
    ) as f:
...
def test_sagemaker_docker_model_scoring_with_sequential_model_and_default_conda_env(
    ...
    mlflux.pytorch.save_model(pytorch_model=model, path=model_path, conda_env=None)
...
def test_requirements_file_log_model(create_requirements_file, sequential_model):
    ...
    with mlflux.start_run():
        ...
        model_uri = "runs:/{run_id}/{model_path}".format(
            run_id=mlflux.active_run().info.run_id, model_path="models"
        )
...
def test_log_model_invalid_requirement_file_path(sequential_model):
    with mlflux.start_run(), pytest.raises(FileNotFoundError):
        mlflux.pytorch.log_model(
            pytorch_model=sequential_model,
            artifact_path="models",
            conda_env=None,
...
def test_log_model_invalid_requirement_file_type(sequential_model):
    with mlflux.start_run(), pytest.raises(
...
def test_save_state_dict(sequential_model, model_path, data):
    ...
    mlflux.pytorch.save_state_dict(state_dict, model_path)

    loaded_state_dict = mlflux.pytorch.load_state_dict(model_path)
...
def test_save_state_dict_throws_for_invalid_object_type(not_state_dict, model_path):
    with pytest.raises(TypeError, match="Invalid object type for `state_dict`"):
        mlflux.pytorch.save_state_dict(not_state_dict, model_path)
...
def test_log_state_dict(sequential_model, data):
    ...
    state_dict = sequential_model.state_dict()
    with mlflux.start_run():
        mlflux.pytorch.log_state_dict(state_dict, artifact_path)
        state_dict_uri = mlflux.get_artifact_uri(artifact_path)

    loaded_state_dict = mlflux.pytorch.load_state_dict(state_dict_uri)
    assert state_dict_equal(loaded_state_dict, state_dict)
    model = get_sequential_model()
    model.load_state_dict(loaded_state_dict)
    np.testing.assert_array_almost_equal(
