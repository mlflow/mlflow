...
from mlflow.pytorch_lightning._pytorch_lightning_autolog import _get_optimizer_name
...
def test_pytorch_autolog_log_models_configuration(log_models):
    mlflow.pytorch_lightning.autolog(log_models=log_models)
    model = IrisClassification()
    dm = IrisDataModule()
    dm.setup(stage="fit")
    trainer = pl.Trainer(max_epochs=NUM_EPOCHS)
...
def test_pytorch_autolog_raises_error_when_step_logging_unsupported():
    mlflow.pytorch_lightning.autolog(log_every_n_step=1)
    model = IrisClassification()
    dm = IrisDataModule()
    trainer = pl.Trainer(max_epochs=NUM_EPOCHS)
    with pytest.raises(
...
def test_autolog_registering_model():
    ...
    mlflow.pytorch_lightning.autolog(registered_model_name=registered_model_name)
    model = IrisClassification()
    dm = IrisDataModule()
    dm.setup(stage="fit")
    trainer = pl.Trainer(max_epochs=NUM_EPOCHS)
