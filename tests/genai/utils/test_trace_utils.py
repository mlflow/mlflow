...
def test_extract_available_tools_from_genai_attributes():
    """Test extraction of tools from OpenTelemetry GenAI semantic convention attributes."""
    with mlflow.start_span(name="execute_tool calculator", span_type=SpanType.TOOL) as span:
        ...
        span.set_inputs([{"role": "tool", "content": {"expression": "25 + 17"}}])
        span.set_outputs([{"text": "42"}])

    trace = mlflow.get_trace(span.trace_id)
    extracted_tools = extract_available_tools_from_trace(trace)

    assert len(extracted_tools) == 1
    assert extracted_tools[0].function.name == "calculator"
    assert extracted_tools[0].function.description == (
        "Useful for performing mathematical calculations"
    )
    assert extracted_tools[0].function.parameters == {
        "properties": {"expression": {"type": "string"}},
        "required": ["expression"],
        "type": "object",
    }
...
def test_extract_available_tools_from_genai_attributes_with_quoted_values():
    """Test extraction handles JSON-encoded string values (with quotes)."""
    with mlflow.start_span(name="execute_tool get_time", span_type=SpanType.TOOL) as span:
        span.set_attribute("gen_ai.tool.name", '"get_current_time"')
        span.set_attribute("gen_ai.tool.description", '"Get the current time"')
        span.set_attribute("gen_ai.tool.json_schema", '{"properties": {}, "type": "object"}')
        span.set_inputs([{"role": "tool", "content": {}}])
        span.set_outputs([{"text": "2026-03-07T12:00:00"}])

    trace = mlflow.get_trace(span.trace_id)
    extracted_tools = extract_available_tools_from_trace(trace)

    assert len(extracted_tools) == 1
    assert extracted_tools[0].function.name == "get_current_time"
    assert extracted_tools[0].function.description == "Get the current time"


def test_extract_available_tools_from_genai_attributes_multiple_tools():
    """Test extraction of multiple tools from TOOL spans."""
    with mlflow.start_span(name="agent_run", span_type=SpanType.AGENT) as parent:
        with mlflow.start_span(name="execute_tool calculator", span_type=SpanType.TOOL) as tool1:
            tool1.set_attribute("gen_ai.tool.name", "calculator")
            tool1.set_attribute("gen_ai.tool.description", "Math calculations")
            tool1.set_attribute("gen_ai.tool.json_schema", '{"type": "object"}')
            tool1.set_inputs([{"role": "tool", "content": {"expression": "5+5"}}])
            tool1.set_outputs([{"text": "10"}])

        with mlflow.start_span(name="execute_tool get_time", span_type=SpanType.TOOL) as tool2:
            tool2.set_attribute("gen_ai.tool.name", "get_time")
            tool2.set_attribute("gen_ai.tool.description", "Get current time")
            tool2.set_attribute("gen_ai.tool.json_schema", '{"type": "object"}')
            tool2.set_inputs([{"role": "tool", "content": {}}])
            tool2.set_outputs([{"text": "12:00"}])

    trace = mlflow.get_trace(parent.trace_id)
    extracted_tools = extract_available_tools_from_trace(trace)

    assert len(extracted_tools) == 2
    tool_names = {tool.function.name for tool in extracted_tools}
    assert tool_names == {"calculator", "get_time"}


def test_extract_available_tools_genai_fallback_after_mlflow_format():
    """Test that GenAI attributes are checked as fallback after MLflow format."""
    tools_mlflow_format = [
        {
            "type": "function",
            "function": {
    ...
    with mlflow.start_span(name="chat", span_type=SpanType.CHAT_MODEL) as chat_span:
        set_span_chat_tools(chat_span, tools_mlflow_format)
        chat_span.set_inputs({"messages": [{"role": "user", "content": "test"}]})
        chat_span.set_outputs({"response": "result"})

    with mlflow.start_span(name="execute_tool calculator", span_type=SpanType.TOOL) as tool_span:
        tool_span.set_attribute("gen_ai.tool.name", "calculator")
        tool_span.set_attribute("gen_ai.tool.description", "Math tool")
        tool_span.set_attribute("gen_ai.tool.json_schema", '{"type": "object"}')
        tool_span.set_inputs([{"role": "tool", "content": {}}])
        tool_span.set_outputs([{"text": "42"}])

    trace = mlflow.get_trace(chat_span.trace_id)
    extracted_tools = extract_available_tools_from_trace(trace)
...
def test_extract_available_tools_genai_missing_name():
    """Test that spans without gen_ai.tool.name are skipped."""
    with mlflow.start_span(name="execute_tool unknown", span_type=SpanType.TOOL) as span:
        span.set_attribute("gen_ai.tool.description", "Some tool")
        span.set_inputs([{"role": "tool", "content": {}}])
        span.set_outputs([{"text": "result"}])
...
def test_normalize_tool_arguments_langchain_format():
    """Test normalization preserves LangChain dict format."""
    langchain_input = {"expression": "25 + 17"}
    result = _normalize_tool_arguments(langchain_input)
    assert result == {"expression": "25 + 17"}


def test_normalize_tool_arguments_strands_format():
    """Test normalization unwraps Strands list/role/content format."""
    strands_input = [{"role": "tool", "content": {"expression": "25 + 17"}}]
    result = _normalize_tool_arguments(strands_input)
    assert result == {"expression": "25 + 17"}


def test_normalize_tool_arguments_list_without_content():
    """Test normalization handles list with dict but no content key."""
    input_data = [{"expression": "25 + 17"}]
    result = _normalize_tool_arguments(input_data)
    assert result == {"expression": "25 + 17"}


def test_normalize_tool_arguments_none():
    """Test normalization handles None input."""
    result = _normalize_tool_arguments(None)
    assert result is None


def test_normalize_tool_arguments_string():
    """Test normalization preserves string format."""
    string_input = "some_string_input"
    result = _normalize_tool_arguments(string_input)
    assert result == "some_string_input"


def test_normalize_tool_outputs_langchain_format():
    """Test normalization preserves LangChain string format."""
    langchain_output = "42"
    result = _normalize_tool_outputs(langchain_output)
    assert result == "42"


def test_normalize_tool_outputs_strands_format():
    """Test normalization unwraps Strands list/text format."""
    strands_output = [{"text": "42"}]
    result = _normalize_tool_outputs(strands_output)
    assert result == "42"


def test_normalize_tool_outputs_list_without_text():
    """Test normalization handles list with dict but no text key."""
    output_data = [{"result": "42"}]
    result = _normalize_tool_outputs(output_data)
    assert result == [{"result": "42"}]


def test_normalize_tool_outputs_none():
    """Test normalization handles None output."""
    result = _normalize_tool_outputs(None)
    assert result is None


def test_normalize_tool_outputs_dict():
    """Test normalization preserves dict format."""
    dict_output = {"result": "42"}
    result = _normalize_tool_outputs(dict_output)
    assert result == {"result": "42"}
