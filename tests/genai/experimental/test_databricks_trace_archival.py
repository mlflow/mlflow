...
def test_databricks_agents_import_error():
    ...
    with pytest.raises(ImportError, match=r"The `mlflow\[databricks\]` package is required to use databricks trace archival"):
        enable_databricks_trace_archival("12345", "catalog", "schema")
...
def test_backend_returns_unsupported_spans_schema(mock_mlflow_client, mock_create_view, mock_get_creds, mock_call_endpoint):
...
@patch("mlflow.genai.experimental.databricks_trace_archival._create_genai_trace_view")
@patch("mlflow.tracking.MlflowClient")
def test_backend_returns_unsupported_events_schema(mock_mlflow_client, mock_create_view, mock_get_creds, mock_call_endpoint):
...
def test_successful_archival_with_custom_prefix(mock_mlflow_client, mock_create_view, mock_get_creds, mock_call_endpoint):
    ...
    with patch("sys.modules", {"databricks.agents": Mock()}):
        result = enable_databricks_trace_archival("12345", "catalog", "schema", table_prefix="custom")
