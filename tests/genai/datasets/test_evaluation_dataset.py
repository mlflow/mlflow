...
def test_evaluation_dataset_to_df_with_mlflow_dataset():
    """Test that genai EvaluationDataset.to_df() includes outputs column when using MLflow dataset."""
    # Create an MLflow entity dataset
    entity_dataset = EntityEvaluationDataset(
    ...
    # Add some records to the entity dataset
    entity_dataset._records = [
        DatasetRecord(
    ...
    # Create genai EvaluationDataset with the MLflow entity dataset
    dataset = EvaluationDataset(entity_dataset)
    
    # Test to_df method
    df = dataset.to_df()
    assert isinstance(df, pd.DataFrame)
    
    # Check that outputs column is present
    expected_columns = [
        "inputs",
        "outputs",
        "expectations",
