...
class TestBuildRequest:
    ...
    def test_with_inference_params(self):
        messages = [JudgeMessage(role="user", content="test")]
        payload = _build_request(
            model="gpt-4",
            messages=messages,
...
class TestRemoveOldestToolCallPair:
    def test_removes_tool_call_pair(self):
        messages = [
...
class TestIsContextWindowError:
    ...
    def test_context_window_errors(self, message):
...
class TestResolveProviderConfig:
    ...
    def test_extra_headers_preserved(self):
        with mock.patch.dict("os.environ", {"OPENAI_API_KEY": "sk-test"}):
            _, _, headers = _resolve_provider_config(
                "openai", "gpt-4", None, {"X-Custom": "value"}
            )
    ...
    def test_gateway_provider(self):
        ...
        with mock.patch(
            "mlflow.genai.judges.adapters.gateway_invocation.get_gateway_litellm_config",
            return_value=mock_config,
        ):
            api_base, model, headers = _resolve_provider_config(
                "gateway", "my-endpoint", None, None
            )
...
class TestInvokeViaGatewayAndHandleTools:
    def test_single_shot_no_tools(self):
    ...
    def test_tool_calling_loop(self, mock_trace):
    ...
    def test_context_window_error_triggers_pruning(self, mock_trace):
    ...
    def test_max_iterations_exceeded(self, mock_trace):
        tool_call_response = _chat_response(
            None,
            tool_calls=[
                {"id": "c1", "type": "function", "function": {"name": "f", "arguments": "{}"}},
        ...
        with (
            ...
            mock.patch.dict("os.environ", {"MLFLOW_JUDGE_MAX_ITERATIONS": "1"}),
        ):
    ...
    def test_response_format_fallback(self):
        """When a 400 error occurs with response_format, retry without it."""
    ...
    def test_non_openai_provider_without_base_url_raises(self, provider):
    ...
    def test_non_openai_provider_with_base_url_allowed(self):
        """When a custom base_url is provided, any provider is allowed."""
