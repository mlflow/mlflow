...
class TestGatewayAdapterIsApplicable:
    @pytest.mark.parametrize("provider", ["openai", "anthropic", "gemini", "mistral"])
    def test_native_providers_applicable(self, provider):
        assert GatewayAdapter.is_applicable(
            model_uri=f"{provider}:/test-model",
            prompt="test",
        )

    def test_unsupported_provider_not_applicable(self):
        assert not GatewayAdapter.is_applicable(
            model_uri="cohere:/test-model",
            prompt="test",
        )

    def test_endpoints_with_string_prompt_applicable(self):
        assert GatewayAdapter.is_applicable(
            model_uri="endpoints:/my-endpoint",
            prompt="test prompt",
        )

    def test_endpoints_with_list_prompt_not_applicable(self):
        assert not GatewayAdapter.is_applicable(
            model_uri="endpoints:/my-endpoint",
...
class TestGatewayAdapterInvokeWithTrace:
    def test_invoke_with_trace_calls_gateway_invocation(self, mock_trace):
        ...
        with mock.patch(
            "mlflow.genai.judges.adapters.gateway_adapter.invoke_via_gateway_and_handle_tools",
    ...
    def test_invoke_with_trace_string_prompt(self, mock_trace):
        mock_output = InvokeOutput(
...
class TestGatewayAdapterInvokeWithoutTrace:
    def test_invoke_without_trace_uses_gateway(self):
        adapter = GatewayAdapter()
        input_params = AdapterInvocationInput(
            model_uri="openai:/gpt-4",
            prompt=[ChatMessage(role="user", content="test")],
...
class TestGatewayAdapterEndpointRestrictions:
    def test_endpoints_rejects_base_url(self):
        adapter = GatewayAdapter()
        input_params = AdapterInvocationInput(
