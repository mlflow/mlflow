...
def test_output_equivalence_scorer_exact_match(program_outputs, expected_outputs, expected_score):
    ...
    assert (
        test_scorer.run(
            inputs={}, outputs=program_outputs, expectations={"expected_response": expected_outputs}
        )
        == expected_score
    )
...
def test_output_equivalence_scorer_llm_judge():
    ...
    with patch("mlflow.genai.judges.make_judge") as mock_make_judge:
        ...
        assert (
            test_scorer.run(
                inputs={}, outputs="output", expectations={"expected_response": "different"}
            )
            == 0.0
        )
...
def test_output_equivalence_scorer_error_handling():
    with patch("mlflow.genai.judges.make_judge") as mock_make_judge:
        ...
        assert (
            test_scorer.run(
                inputs={}, outputs="output", expectations={"expected_response": "expected"}
            )
            == 0.0
        )
