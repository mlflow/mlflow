...
def test_trace_ignore_exception_from_tracing_logic(mock_client):
    ...
    with mock.patch("mlflow.tracing.python_api.get_tracer", side_effect=ValueError("Some error")):
...
def test_start_detached_span(mock_client):
    class TestModel:
        def predict(self, x, y):
            root_span = mlflow.start_detached_span(name="root_span")
            root_span.set_inputs({"x": x, "y": y})

            z = x + y

            child_span = mlflow.start_detached_span(
                "child_span_1", span_type=SpanType.LLM, parent=root_span
            )
            child_span.set_inputs({"z": z})
            ...
            res = self.square(z, parent=root_span)
            root_span.set_outputs({"output": res})
            root_span.end()
            return res

        def square(self, t, parent: MLflowSpanWrapper = None):
            span = mlflow.start_detached_span("child_span_2", parent=parent)
            span.set_inputs({"t": t})
