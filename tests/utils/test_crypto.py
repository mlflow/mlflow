...
def test_kek_manager_from_env_var(monkeypatch):
    monkeypatch.setenv("MLFLOW_SECRETS_KEK_PASSPHRASE", "env-passphrase")
...
def test_kek_manager_no_passphrase_raises(monkeypatch):
    monkeypatch.delenv("MLFLOW_SECRETS_KEK_PASSPHRASE", raising=False)
    with pytest.raises(MlflowException, match="MLFLOW_SECRETS_KEK_PASSPHRASE"):
        KEKManager()


def test_kek_manager_empty_passphrase_raises(monkeypatch):
    monkeypatch.setenv("MLFLOW_SECRETS_KEK_PASSPHRASE", "")
    with pytest.raises(MlflowException, match="MLFLOW_SECRETS_KEK_PASSPHRASE"):
        KEKManager()
...
def test_kek_manager_version_from_env_var(monkeypatch):
    monkeypatch.setenv("MLFLOW_SECRETS_KEK_PASSPHRASE", "env-passphrase")
    monkeypatch.setenv("MLFLOW_SECRETS_KEK_VERSION", "3")
    kek_manager = KEKManager()
