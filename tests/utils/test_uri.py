...
def test_append_to_uri_path_handles_special_uri_characters_in_posixpaths():
    # NB: Certain characters are treated specially when parsing and interpreting URIs. However, in
    # the case where a URI input for `append_to_uri_path` is simply a POSIX path, these characters
    # should not receive special treatment. This test case verifies that `append_to_uri_path`
    # properly joins POSIX paths containing these characters.
