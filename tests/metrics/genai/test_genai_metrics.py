...
def test_make_custom_prompt_genai_metric_validates_input_kwargs():
    ...
    with pytest.raises(MlflowException, match="Missing variable inputs to eval_fn"):
        custom_judge_prompt_metric.evaluate(input=pd.Series(inputs))
...
def test_genai_metrics_evaluate(custom_metric):
    data = {
    ...
    with mock.patch.object(
        ...
        metric_value = custom_metric.evaluate(**data)
        scores = custom_metric.evaluate(
            **data,
    ...
    assert inspect.signature(custom_metric.evaluate).parameters.keys() == {
        "predictions",
        "inputs",
        "metrics",
        "return_only_scores",
...
def test_genai_metrics_evaluate_errors(custom_metric):
    with pytest.raises(TypeError, match=r"missing 1 required keyword-only argument: 'inputs'"):
        custom_metric.evaluate(predictions=mlflow_prediction)
    ...
    with pytest.raises(MlflowException, match=r"Missing required arguments: {'targets'}"):
        custom_metric.evaluate(**data)

    with pytest.raises(MlflowException, match=r"Unexpected arguments: {'data'}"):
        custom_metric.evaluate(**data, targets=mlflow_ground_truth, data="data")

    with pytest.raises(TypeError, match=r"Expected predictions to be a string or list"):
        custom_metric.evaluate(predictions=1, inputs="What is MLflow?", targets=mlflow_ground_truth)


def test_genai_metrics_with_llm_judge_evaluate():
    custom_judge_prompt = "This is a custom judge prompt that uses {input} and {output}"
    ...
    with mock.patch.object(
        ...
        metric_value = custom_judge_prompt_metric.evaluate(
            input=pd.Series(inputs),
            output=pd.Series(outputs),
        )
        scores = custom_judge_prompt_metric.evaluate(
            input=pd.Series(inputs),
            output=pd.Series(outputs),
            return_only_scores=True,
        )
