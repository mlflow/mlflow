...
def test_multiple_parameters_round_trip():
    ...
    def multi_param_scorer(inputs, outputs, expectations):
        return (outputs.startswith(inputs) and 
                len(outputs) > expectations.get("min_length", 0))
    ...
    assert deserialized(
        inputs="Hi", outputs="Hello world!", expectations={"min_length": 5}
    ) is False
...
def test_imports_and_feedback_round_trip():
    ...
    def feedback_scorer(outputs):
        ...
        return Feedback(
            value=len(words),
            rationale=f"Found {len(words)} words"
        )
...
def test_builtin_scorer_with_parameters_round_trip():
    ...
    with patch(
        ...
        return_value=Feedback(name="tone", value=True, rationale="Response is appropriately courteous")
    ) as mock_judge:
        result = deserialized(
    ...
    mock_judge.assert_called_once_with(
        guidelines=[tone],
        context={"response": "Thank you for bringing this to my attention. I understand your concern and will help resolve this issue promptly."},
        name="tone"
    )
