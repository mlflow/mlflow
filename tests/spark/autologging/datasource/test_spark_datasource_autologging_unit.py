...
from mlflow.spark._spark_autologging import PythonSubscriber, _get_current_listener
...
def mock_get_current_listener():
    with mock.patch(
        "mlflow.spark._spark_autologging._get_current_listener", return_value=None
...
def test_enabling_autologging_throws_for_wrong_spark_version(
    ...
    with mock.patch("mlflow.spark._spark_autologging._get_spark_major_version", return_value=2):
        with pytest.raises(
