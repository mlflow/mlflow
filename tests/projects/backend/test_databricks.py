...
from mlflow.projects.backend.databricks import DatabricksJobRunner
...
def runs_cancel_mock():
    ...
    with mock.patch("mlflow.projects.backend.databricks.DatabricksJobRunner.jobs_runs_cancel") \
            as runs_cancel_mock:
...
def runs_submit_mock():
    ...
    with mock.patch("mlflow.projects.backend.databricks.DatabricksJobRunner._jobs_runs_submit") \
            as runs_submit_mock:
...
def runs_get_mock():
    ...
    with mock.patch("mlflow.projects.backend.databricks.DatabricksJobRunner.jobs_runs_get") \
            as runs_get_mock:
        yield runs_get_mock
...
def upload_to_dbfs_mock(dbfs_root_mock):
    ...
    with mock.patch.object(
            mlflow.projects.backend.databricks.DatabricksJobRunner, "_upload_to_dbfs",
...
def dbfs_path_exists_mock(dbfs_root_mock):  # pylint: disable=unused-argument
    with mock.patch("mlflow.projects.backend.databricks.DatabricksJobRunner._dbfs_path_exists") \
            as path_exists_mock:
...
def before_run_validations_mock():  # pylint: disable=unused-argument
    with mock.patch("mlflow.projects.backend.databricks.before_run_validations"):
        yield


@pytest.fixture()
def set_tag_mock():
    with mock.patch("mlflow.projects.backend.databricks.tracking.MlflowClient") as m:
...
def test_upload_existing_project_to_dbfs(dbfs_path_exists_mock):  # pylint: disable=unused-argument
    ...
    with mock.patch("mlflow.projects.backend.databricks.DatabricksJobRunner._upload_to_dbfs") \
...
def test_run_databricks_validations(
    ...
    """
    obj_to_patch = "mlflow.projects.backend.databricks.DatabricksJobRunner._databricks_api_request"
    with mock.patch.dict(os.environ, {'DATABRICKS_HOST': 'test-host', 'DATABRICKS_TOKEN': 'foo'}),\
        mock.patch(obj_to_patch)\
            as db_api_req_mock:
