...
from mlflow.projects.yarn import (
    _validate_yarn_env,
    _generate_skein_service,
    _get_key_from_params,
    _parse_yarn_config,
)
...
def test_generate_skein_service():
    ...
    expected_files = {"test.py": skein.model.File("test.py")}

    service = _generate_skein_service(
        "1 GiB", 1, 1, {"ENV": "ENV"}, ["test.py"], "/etc/hadoop/conf", "python train.py"
    )
