...
# Build all services (COMPOSE_PARALLEL_LIMIT=1 avoids race conditions on base image)
COMPOSE_PARALLEL_LIMIT=1 ./tests/db/compose.sh build --build-arg DEPENDENCIES="$(python dev/extract_deps.py)"
```
