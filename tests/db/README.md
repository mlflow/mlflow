...
# Build all services (disable BuildKit to avoid parallel build race conditions)
DOCKER_BUILDKIT=0 ./tests/db/compose.sh build --build-arg DEPENDENCIES="$(python dev/extract_deps.py)"
```
