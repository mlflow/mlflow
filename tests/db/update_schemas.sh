...
uv run python tests/store/dump_schema.py tests/resources/db/latest_schema.sql

./tests/db/compose.sh down --volumes --remove-orphans
./tests/db/compose.sh build --build-arg DEPENDENCIES="$(uv run python dev/extract_deps.py)"
