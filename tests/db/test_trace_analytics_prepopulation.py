...
from mlflow.tracing.constant import CostKey, SpanAttributeKey, TokenUsageKey, TraceMetadataKey
...
def test_prepopulation_validates_legacy_data_before_expanding_schema(tmp_path):
    engine, _ = _prepare_database(tmp_path)
    try:
        with engine.begin() as conn:
            _seed_legacy_analytics_data(conn)
...
def test_prepopulation_conversion_semantics_match_the_frozen_migration():
    ...
    for value in numeric_values:
        assert MIGRATION_MODULE._token_count_or_none(value) == token_count_or_none(value)
        assert MIGRATION_MODULE._finite_float_or_none(value) == finite_float_or_none(value)

    assessment_values = [None, True, False, 1, 1.5, " yes ", "NO", "0.8", "text"]
    for value in assessment_values:
        assert MIGRATION_MODULE._assessment_aggregate(json.dumps(value)) == assessment_aggregate(
            value
        )
    ...
    for value in dimension_values:
        migration_dimensions = MIGRATION_MODULE._validated_dimension_attributes(
            value, ("trace-1", "span-1")
        )
        prepopulation_dimensions = prepopulation._validated_dimension_attributes(
            value, ("trace-1", "span-1")
        )
        assert migration_dimensions == prepopulation_dimensions
        assert MIGRATION_MODULE._dimension_attributes_state(
            value, True
        ) == prepopulation._dimension_attributes_state(value, True)
        ...
        for dimension_value in migration_dimensions.values():
            ...
            assert migration_value == bounded_model_dimension(dimension_value)
...
def test_prepopulation_cli(tmp_path):
    ...
    try:
        ...
        result = CliRunner().invoke(
            ...
            env={"MLFLOW_TRACKING_URI": str(engine.url)},
        )
