...
@pytest.mark.large
def test_enabling_autologging_throws_for_wrong_spark_version(spark_session,
                                                             mock_get_current_listener):
...
@pytest.mark.large
def test_enabling_autologging_throws_when_spark_hasnt_been_started(spark_session,
                                                                   mock_get_current_listener):
    # pylint: disable=unused-argument
    spark_session.stop()
    with pytest.raises(MlflowException) as exc:
