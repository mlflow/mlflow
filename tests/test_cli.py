...
def _workspace_disabled_env():
    ...
    env.pop(MLFLOW_WORKSPACE_STORE_URI.name, None)
    return env
...
def test_server_workspace_uri_sets_env_when_workspaces_enabled(tmp_path):
    ...
    try:
        with (
            ...
            assert os.environ["MLFLOW_WORKSPACE_STORE_URI"] == workspace_uri
...
def test_server_workspace_uri_warns_without_enable(tmp_path):
    ...
    try:
        with (
            ...
            assert "MLFLOW_WORKSPACE_STORE_URI" not in os.environ
...
def test_server_workspace_uri_cli_overrides_env(tmp_path):
    ...
    MLFLOW_WORKSPACE_STORE_URI.set(existing_workspace_uri)
    ...
    try:
        with (
            ...
            assert os.environ["MLFLOW_WORKSPACE_STORE_URI"] == new_workspace_uri
            assert MLFLOW_ENABLE_WORKSPACES.get() is True
    finally:
        MLFLOW_WORKSPACE_STORE_URI.unset()
        MLFLOW_ENABLE_WORKSPACES.unset()
...
def test_server_no_enable_workspaces_overrides_env(tmp_path):
    handlers._tracking_store = None
    handlers._model_registry_store = None
    backend_uri = f"sqlite:///{tmp_path / 'backend.db'}"
    artifact_root_path = tmp_path / "artifacts"
    ...
    try:
        with (
            ...
            run_server_mock.assert_called_once()
            init_backend.assert_called_once()
            assert os.environ[MLFLOW_ENABLE_WORKSPACES.name] == "false"
            assert MLFLOW_ENABLE_WORKSPACES.get() is False
    finally:
        MLFLOW_ENABLE_WORKSPACES.unset()
