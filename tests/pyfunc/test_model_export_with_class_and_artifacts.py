...
def get_model_class():
    ...
    class CustomSklearnModel(mlflux.pyfunc.PythonModel):
        ...
        def load_context(self, context):
            ...
            self.model = mlflux.sklearn.load_model(model_uri=context.artifacts["sk_model"])
...
def _conda_env():
    # NB: We need mlflux as a dependency in the environment.
...
def test_model_save_load(sklearn_knn_model, main_scoped_model_class, iris_data, tmpdir):
    ...
    mlflux.sklearn.save_model(sk_model=sklearn_knn_model, path=sklearn_model_path)
    ...
    loaded_pyfunc_model = mlflux.pyfunc.load_pyfunc(model_uri=pyfunc_model_path)
...
def test_pyfunc_model_log_load_no_active_run(sklearn_knn_model, main_scoped_model_class, iris_data):
    ...
    with mlflux.start_run():
        mlflux.sklearn.log_model(sk_model=sklearn_knn_model, artifact_path=sklearn_artifact_path)
        sklearn_model_uri = "runs:/{run_id}/{artifact_path}".format(
            run_id=mlflux.active_run().info.run_id, artifact_path=sklearn_artifact_path
        )
    ...
    loaded_pyfunc_model = mlflux.pyfunc.load_pyfunc(model_uri=pyfunc_model_uri)
    np.testing.assert_array_equal(
        loaded_pyfunc_model.predict(iris_data[0]),
        test_predict(sk_model=sklearn_knn_model, model_input=iris_data[0]),
    )
...
def test_model_load_from_remote_uri_succeeds(
    ...
    loaded_pyfunc_model = mlflux.pyfunc.load_pyfunc(model_uri=model_uri)
    np.testing.assert_array_equal(
        loaded_pyfunc_model.predict(iris_data[0]),
        test_predict(sk_model=sklearn_knn_model, model_input=iris_data[0]),
    )
...
def test_add_to_model_adds_specified_kwargs_to_mlmodel_configuration():
    ...
    assert all([item in model_config.flavors[mlflux.pyfunc.FLAVOR_NAME] for item in custom_kwargs])
...
def test_log_model_persists_specified_conda_env_in_mlflow_model_directory(
    ...
    with mlflux.start_run():
        mlflux.sklearn.log_model(sk_model=sklearn_knn_model, artifact_path=sklearn_artifact_path)
        sklearn_run_id = mlflux.active_run().info.run_id
...
def test_save_model_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    sklearn_model_path = os.path.join(str(tmpdir), "sklearn_model")
    mlflux.sklearn.save_model(sk_model=sklearn_logreg_model, path=sklearn_model_path)
...
def test_save_model_correctly_resolves_directory_artifact_with_nested_contents(
    ...
    class ArtifactValidationModel(mlflux.pyfunc.PythonModel):
        def predict(self, context, model_input):
    ...
    loaded_model = mlflux.pyfunc.load_pyfunc(model_uri=model_path)
...
def test_save_model_with_no_artifacts_does_not_produce_artifacts_dir(model_path):
    ...
    pyfunc_conf = _get_flavor_configuration(
        model_path=model_path, flavor_name=mlflux.pyfunc.FLAVOR_NAME
    )
    assert mlflux.pyfunc.model.CONFIG_KEY_ARTIFACTS not in pyfunc_conf
...
def test_save_model_with_unsupported_argument_combinations_throws_exception(model_path):
    ...
    with pytest.raises(MlflowException) as exc_info:
        mlflux.pyfunc.save_model(path=model_path, python_model=None, loader_module=None)
    assert "Either `loader_module` or `python_model` must be specified" in str(exc_info)
...
def test_log_model_with_unsupported_argument_combinations_throws_exception():
    ...
    with mlflux.start_run(), pytest.raises(MlflowException) as exc_info:
        mlflux.pyfunc.log_model(artifact_path="pyfunc_model", python_model=None, loader_module=None)
    assert "Either `loader_module` or `python_model` must be specified" in str(exc_info)
...
def test_repr_can_be_called_withtout_run_id_or_artifact_path():
    ...
    assert "flavor: someFlavour" in mlflux.pyfunc.PyFuncModel(model_meta, model_impl).__repr__()
...
def test_load_model_with_differing_cloudpickle_version_at_micro_granularity_logs_warning(
    ...
    class TestModel(mlflux.pyfunc.PythonModel):
        def predict(self, context, model_input):
            return model_input

    mlflux.pyfunc.save_model(path=model_path, python_model=TestModel())
    saver_cloudpickle_version = "0.5.8"
    model_config_path = os.path.join(model_path, "MLmodel")
    model_config = Model.load(model_config_path)
    model_config.flavors[mlflux.pyfunc.FLAVOR_NAME][
        mlflux.pyfunc.model.CONFIG_KEY_CLOUDPICKLE_VERSION
    ...
    with mock.patch("mlflux.pyfunc._logger.warning") as warn_mock, mock.patch(
        "cloudpickle.__version__"
    ) as cloudpickle_version_mock:
        cloudpickle_version_mock.__str__ = lambda *args, **kwargs: loader_cloudpickle_version
        warn_mock.side_effect = custom_warn
        mlflux.pyfunc.load_pyfunc(model_uri=model_path)
...
def test_load_model_with_missing_cloudpickle_version_logs_warning(model_path):
    ...
    with mock.patch("mlflux.pyfunc._logger.warning") as warn_mock:
        warn_mock.side_effect = custom_warn
        mlflux.pyfunc.load_pyfunc(model_uri=model_path)
...
def test_sagemaker_docker_model_scoring_with_default_conda_env(
    ...
    reloaded_pyfunc = mlflux.pyfunc.load_pyfunc(model_uri=pyfunc_model_path)
