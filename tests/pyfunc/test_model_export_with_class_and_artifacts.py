...
def _conda_env():
    ...
    return _mlflow_conda_env(
        ...
        additional_pip_deps=[
            "-e " + os.path.dirname(mlflow.__path__[0]), "cloudpickle=={}".format(
                cloudpickle.__version__), "scikit-learn=={}".format(sklearn.__version__)
        ],
...
@pytest.mark.large
def test_model_load_from_remote_uri_succeeds(sklearn_knn_model, main_scoped_model_class, tmpdir,
...
def test_pyfunc_cli_predict_command_without_conda_env_activation_succeeds(
    ...
    process = Popen([
        'mlflow', 'models', 'predict', '--model-uri', pyfunc_model_path, '-i', input_csv_path,
        "--content-type", "csv", '-o', output_json_path, '--no-conda'
    ],
                    stdout=PIPE,
                    stderr=PIPE,
...
def test_pyfunc_cli_predict_command_with_conda_env_activation_succeeds(
    ...
    process = Popen([
        'mlflow', 'models', 'predict', '--model-uri', pyfunc_model_path, '-i', input_csv_path,
        "--content-type", "csv", '-o', output_json_path
    ],
                    stderr=PIPE,
                    stdout=PIPE,
                    preexec_fn=os.setsid)
...
def test_save_model_correctly_resolves_directory_artifact_with_nested_contents(
    ...
    nested_file_relative_path = os.path.join("my", "somewhat", "heavily", "nested", "directory",
                                             "myfile.txt")
    ...
    class ArtifactValidationModel(mlflow.pyfunc.PythonModel):
        def predict(self, context, model_input):
            expected_file_path = os.path.join(context.artifacts["testdir"],
                                              nested_file_relative_path)
...
def test_load_model_with_differing_cloudpickle_version_at_micro_granularity_logs_warning(
    ...
    assert any([
        ...
        saver_cloudpickle_version in log_message and loader_cloudpickle_version in log_message
