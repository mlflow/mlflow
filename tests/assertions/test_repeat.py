"""Tests for ``@mlflow.test(repeat=, pass_threshold=)`` majority-of-N support.

Two layers:
- Pure unit tests on the decorator validation and on ``_run_repeated`` (the
  early-exit / threshold / fail-synthesis logic), driven by a fake item so they
  need no LLM and never fail the outer suite (``_run_repeated`` returns the
  error, it doesn't raise it).
- An end-to-end layer: real ``@mlflow.test(repeat=...)`` cases bundled and run by
  the plugin, with a trailing plain test asserting the recorded per-case
  outcomes. Custom ``@scorer`` stand-ins keep it judge-free.
"""
