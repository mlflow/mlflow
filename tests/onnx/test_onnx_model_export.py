...
def test_model_save_load(onnx_model, model_path):
    mlflux.onnx.save_model(onnx_model, model_path)

    # Loading ONNX model
    onnx.checker.check_model = mock.Mock()
    mlflux.onnx.load_model(model_path)
...
def test_model_save_load_evaluate_pyfunc_format(onnx_model, model_path, data, predicted):
    ...
    pyfunc_loaded = mlflux.pyfunc.load_pyfunc(model_path)
...
def test_model_save_load_multiple_inputs(onnx_model_multiple_inputs_float64, model_path):
    mlflux.onnx.save_model(onnx_model_multiple_inputs_float64, model_path)
...
# TODO: Remove test, along with explicit casting, when https://github.com/mlflux/mlflux/issues/1286
# is fixed.
@pytest.mark.large
def test_pyfunc_representation_of_float32_model_casts_and_evalutes_float64_inputs(
    ...
    """
    The ``python_function`` representation of an mlflux model with the ONNX flavor
    ...
    to the issue described in https://github.com/mlflux/mlflux/issues/1286 where
    ...
    DataFrame as JSON).
    """
    mlflux.onnx.save_model(onnx_model_multiple_inputs_float32, model_path)
...
def test_model_log(onnx_model):
    ...
    for should_start_run in [False, True]:
        try:
            ...
            mlflux.onnx.log_model(onnx_model=onnx_model, artifact_path=artifact_path)
            ...
            # Load model
            onnx.checker.check_model = mock.Mock()
            mlflux.onnx.load_model(model_uri)
            assert onnx.checker.check_model.called
        finally:
            mlflux.end_run()
...
def test_model_log_evaluate_pyfunc_format(onnx_model, data, predicted):
    ...
    with mlflux.start_run() as run:
        ...
        # Loading pyfunc model
        pyfunc_loaded = mlflux.pyfunc.load_pyfunc(model_uri=model_uri)
        assert np.allclose(
...
def test_model_save_evaluate_pyfunc_format_multi_tensor(
    ...
    with TempDir(chdr=True):
        path = "onnx_model"
        mlflux.onnx.save_model(onnx_model=multi_tensor_onnx_model, path=path)
        # Loading pyfunc model
        pyfunc_loaded = mlflux.pyfunc.load_pyfunc(model_uri=path)
        data, _ = data
        # get prediction
        feeds = {
...
def test_model_save_persists_specified_conda_env_in_mlflow_model_directory(
    ...
    mlflux.onnx.save_model(onnx_model=onnx_model, path=model_path, conda_env=onnx_custom_env)
...
def test_model_save_accepts_conda_env_as_dict(onnx_model, model_path):
    conda_env = dict(mlflux.onnx.get_default_conda_env())
    conda_env["dependencies"].append("pytest")
    mlflux.onnx.save_model(onnx_model=onnx_model, path=model_path, conda_env=conda_env)
...
def test_model_save_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    mlflux.onnx.save_model(onnx_model=onnx_model, path=model_path, conda_env=None)
    pyfunc_conf = _get_flavor_configuration(model_path=model_path, flavor_name=pyfunc.FLAVOR_NAME)
    conda_env_path = os.path.join(model_path, pyfunc_conf[pyfunc.ENV])
    with open(conda_env_path, "r") as f:
        conda_env = yaml.safe_load(f)

    assert conda_env == mlflux.onnx.get_default_conda_env()
...
def test_model_log_without_specified_conda_env_uses_default_env_with_expected_dependencies(
    ...
    artifact_path = "model"
    with mlflux.start_run():
        mlflux.onnx.log_model(onnx_model=onnx_model, artifact_path=artifact_path, conda_env=None)
        model_path = _download_artifact_from_uri(
            "runs:/{run_id}/{artifact_path}".format(
                run_id=mlflux.active_run().info.run_id, artifact_path=artifact_path
            )
...
def test_pyfunc_predict_supports_models_with_list_outputs(onnx_sklearn_model, model_path, data):
    """
    https://github.com/mlflux/mlflux/issues/2499
    User encountered issue where an sklearn model, converted to onnx, would return a list response.
    The issue resulted in an error because mlflux assumed it would be a numpy array. Therefore,
    the this test validates the service does not receive that error when using such a model.
    """
    x = data[0]
    mlflux.onnx.save_model(onnx_sklearn_model, model_path)
    wrapper = mlflux.pyfunc.load_model(model_path)
